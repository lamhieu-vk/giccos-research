	...
	if (count > 0) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				}else if (data['reason'] != undefined && data['return'] != undefined && data['return'] == false) {
					popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: transferError(data['reason']), timeout: '<?php print $_parameter->get('notification_error_timeout'); ?>'});
				}
			}
		}).fail(doneRequestFunc).done(doneRequestFunc);
...
var feed_comment_add = function (object, callback) {
	...
	var requestCommentAdd = $.ajax({
		...
		success: function (data) {
			console.log(data);
			if (data['return'] !== undefined && data['return'] === true && data['data'] !== undefined && typeof data['data'] == "object") {
...
function comment_favorite () {
	...
	button.bind('click', function() {
		var button = $(this);
		var handing = button.attr('handing-favorite');
		if (handing != 'true') {
...
function comment_load (options) {
	...
	status.each(function() {
		...
		if (object['id'] == status_id) {
			...
			if (button.attr('handing-comment-load') != "true") {
				button.attr('handing-comment-load', 'true');
				length++;
			}
		}
	});
	if (length > 0) {
		var afterDoneFunc = function() {
			status.each(function() {
				...
				if (object['id'] == status_id) {
					var button = status.find('[status-comment-load]');
					button.removeAttr('handing-comment-load');
				}
			});
			var length = 0;
		};
...
function status_comment_statistic (id, rows) {
	if (id > 0) {
		var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'action': 'stats', 'id': id, 'rows': rows};
		$.ajax({
			type: "POST",
			dataType: "json",
			url: "<?php print $_tool->links("source/ajax/feed.ajax"); ?>",
