...
Class feed {
	...
	function status_edit ($object) {
		...
		foreach ($rowsArr as $key => $rowsArrThis) {
			if ($key == "content") {
				...
				$rowsArrThis = $_tool->hash('encode', $_tool->convertDatabaseString($rowsArrThis), $_parameter->get('hash_status_content').'::'.$token);
	...
	function status_remove ($object) {
		...
		if ($statusFetch['type'] == "status") {
			...
			$mediaQuery = mysqli_query($db, "SELECT * FROM `status_photos` WHERE `status.id` = '{$status['id']}'");
			...
			$mediaQuery = mysqli_query($db, "SELECT * FROM `status_music` WHERE `status.id` = '{$status['id']}'");
			while ($mediaFetch = mysqli_fetch_assoc($mediaQuery)) {
			...
			$mediaQuery = mysqli_query($db, "SELECT * FROM `status_videos` WHERE `status.id` = '{$status['id']}'");
			while ($mediaFetch = mysqli_fetch_assoc($mediaQuery)) {
			...
			$mediaQuery = mysqli_query($db, "SELECT * FROM `status_cited` WHERE `status.id` = '{$status['id']}'");
			while ($mediaFetch = mysqli_fetch_assoc($mediaQuery)) {
	...
	function status_add ($object) {
		...
		$status['contentHashed'] = $_tool->hash('encode', $_tool->convertDatabaseString($status['content']), $_parameter->get('hash_status_content').'::'.$status['token']);
		$insertRequest = "
		INSERT INTO 
			...
			if (isset($object['media']['file']) && count($object['media']['file']) > 0) {
				...
				foreach ($object['media']['file'] as $key => $mediaFileThis) {
					...
					if (isset($uploadMediaFile['return'], $uploadMediaFile['id']) && $uploadMediaFile['return'] == true && is_numeric($uploadMediaFile['id'])) {
						mysqli_query($db, "INSERT INTO `status_photos` (`id`, `status.id`, `photos.id`) VALUES (null, '{$status['id']}', '{$uploadMediaFileCache['id']}')");
					}else {
						continue;
					}
				}
					...
					if ($usertagQuery && mysqli_num_rows($usertagQuery) == 0) {
						...
						while ($usertagFetch = mysqli_fetch_assoc($usertagQuery)) {
							if ($usertagFetch['guy.type'] == "user" || $usertagFetch['guy.type'] == "users") {
								$usertagUser = $_user->profile(array("rows" => "`id`, `fullname`, `username`, `link`", "id" => $usertagFetch['guy.id']));
								if (isset($usertagUser['return']) && $usertagUser['return'] == true) {
									$content['usertag'][] = array(
										"id" => $usertagUser['data']['id'],
										"name" => $usertagUser['data']['fullname'],
										"tag" => $usertagUser['data']['username'],
										"link" => $usertagUser['data']['link']
									);
								}else {
									continue;
								}
					...
					if (isset($status['comment']['count']) && $status['comment']['count'] == 0) {
						...
						$c_['status_comment'] = "
							<div class='statistic'>
								<div class='info'>
									<i class='icon'></i>
									<span class='count' status-statistic-comment>{$status['comment']['count']}</span>
						...
						$getCommentIdSql = "SELECT `id` FROM `status_comment` WHERE `inside.type` = 'status' AND `inside.id` = '{$status['info']['id']}' ORDER BY `id` DESC LIMIT 3";;
	...
	function status_stats ($object) {
		...
		if ($statusCount > 0) {
			...
			if ($from == "comment" || $from == "all") {
				$commentQuery = mysqli_query($db, "SELECT `id` FROM `status_comment` WHERE `status.id` = '{$statusId}'");
			}
			if ($type == "statistic" || $type == "all") {
				if (($from == "favorite" || $from == "all") && isset($favoriteQuery) && $favoriteQuery) {
					$favoriteCount = mysqli_num_rows($favoriteQuery);
				}
				if (($from == "share" || $from == "all") && isset($shareQuery) && $shareQuery) {
					$shareCount = mysqli_num_rows($shareQuery);
				}
				if (($from == "invite" || $from == "all") && isset($inviteQuery) && $inviteQuery) {
					$inviteCount = mysqli_num_rows($inviteQuery);
				}
				if (($from == "comment" || $from == "all") && isset($commentQuery) && $commentQuery) {
					$commentCount = mysqli_num_rows($commentQuery);
				}
				$statsArr = array();
				if (($from == "favorite" || $from == "all") && isset($favoriteCount)) {
					$statsArr['favorite'] = $favoriteCount;
				}
				if (($from == "share" || $from == "all") && isset($shareCount)) {
					$statsArr['share'] = $shareCount;
				}
				if (($from == "invite" || $from == "all") && isset($inviteCount)) {
					$statsArr['invite'] = $inviteCount;
				}
				if (($from == "comment" || $from == "all") && isset($commentCount)) {
		...
		if ($commentNum == 0) {
			...
			while ($comment = mysqli_fetch_assoc($commentQuery)) {
				...
				if ($commentAuthor['type'] == "user") {
					$getInfo = $_user->getInfo(array("id" => $commentAuthor['id'], "rows" => "`avatar.small`, `fullname`, `username`, `link`"));
					if (isset($getInfo['return']) && $getInfo['return'] == true) {
						$commentAuthor['avatar'] = $getInfo['data']['avatar.small'];
						$commentAuthor['name'] = $getInfo['data']['fullname'];
						$commentAuthor['tag'] = $getInfo['data']['username'];
						$commentAuthor['link'] = $getInfo['data']['link'];
					}else {
						continue;
					}
				}else {
				...
				$commentContent['text'] = $_tool->parseContent($_tool->hash('decode', $comment['content'], $_parameter->get('hash_comment_content').'::'.$comment['token']));
				...
				$commentReply = array();
				$commentReplyCountQuery = mysqli_query($db, "SELECT `id` FROM `status_comment` WHERE `comment.id` = '{$comment['id']}'");
	...
	function comment_add ($object) {
		...
		if ($insideThisCount == 0) {
			...
			$commentArr['content'] = $_tool->hash('encode', $commentContent,  $_parameter->get('hash_comment_content').'::'.$commentArr['token']);
			$insertRequest = "
			...
			if (!$insertQuery) {
				...
				//.
				$commentArrInfo['id'] = $commentArr['id'];
				$commentArrInfo['inside.type'] = $commentArr['inside.type'];
				$commentArrInfo['inside.id'] = $commentArr['inside.id'];
				$commentArrAuthor['type'] = $commentArr['author.type'];
				$commentArrAuthor['id'] = $commentArr['author.id'];
				if ($commentArrAuthor['type'] == "user") {
					$getInfo = $_user->getInfo(array("id" => $commentArrAuthor['id'], "rows" => "`avatar.small`, `fullname`, `username`, `link`"));
					if (isset($getInfo['return']) && $getInfo['return'] == true) {
						$commentArrAuthor['avatar'] = $getInfo['data']['avatar.small'];
						$commentArrAuthor['name'] = $getInfo['data']['fullname'];
						$commentArrAuthor['tag'] = $getInfo['data']['username'];
						$commentArrAuthor['link'] = $getInfo['data']['link'];
					}else {
						continue;
					}
				}else {
				...
				if ($commentArr['edit'] != null && $commentArr['edit'] != "0") {
					$commentArrEdit['status'] = true;
					$commentArrEdit['time']['stamp'] = $commentArr['edit'];
					$commentArrEdit['time']['ago'] = $_tool->agoDatetime($commentArr['edit'], 'ago');
					$commentArrEdit['time']['tip'] = $_tool->agoDatetime($commentArr['edit'], 'tip');
					$commentArrEdit['time']['json'] = json_encode(array("stamp" => $commentArrEdit['time']['stamp']));
				}else {
					$commentArrEdit['status'] = false;
				}
				$commentArrTime['stamp'] = $commentArr['time'];
				$commentArrTime['ago'] = $_tool->agoDatetime($commentArr['time'], 'ago');
				$commentArrTime['tip'] = $_tool->agoDatetime($commentArr['time'], 'tip');
				$commentArrTime['json'] = json_encode(array("stamp" => $commentArrTime['stamp']));
				$commentArrContent['text'] = $_tool->parseContent($_tool->hash('decode', $commentArr['content'], $_parameter->get('hash_comment_content').'::'.$commentArr['token']));
				$commentArrStats['favorite'] = mysqli_num_rows(mysqli_query($db, "SELECT `id` FROM `status_comment_favorite` WHERE `comment.id` = '{$commentArr['id']}'"));
				$commentArrAction['favorite']['check'] = mysqli_num_rows(mysqli_query($db, "SELECT `id` FROM `status_comment_favorite` WHERE `comment.id` = '{$commentArr['id']}' AND `guy.id` = '{$authorId}' AND `guy.type` = '{$authorType}'"));
				if ($commentArrAction['favorite']['check'] == 0) {
					$commentArrAction['favorite']['selected'] = false;
					$commentArrAction['favorite']['text'] = 'false';
					$commentArrAction['favorite']['tip'] = $_language->text('favorite', 'ucfirst');
				}else {
					$commentArrAction['favorite']['selected'] = true;
					$commentArrAction['favorite']['text'] = 'true';
					$commentArrAction['favorite']['tip'] = $_language->text('unfavorite', 'ucfirst');
				}
				$commentArrReply = array();
				$commentArrReplyCountQuery = mysqli_query($db, "SELECT `id` FROM `status_comment` WHERE `comment.id` = '{$commentArr['id']}'");
				if ($commentArrReplyCountQuery) $commentArrReply['count'] = mysqli_num_rows($commentArrReplyCountQuery); else $commentArrReply['count'] = 0;
				$commentArrData = array(
					"info" => $commentArrInfo,
					"author" => $commentArrAuthor,
					"edit" => $commentArrEdit,
					"time" => $commentArrTime,
					"content" => $commentArrContent,
					"stats" => $commentArrStats,
					"action" => $commentArrAction,
					"relpy" => $commentArrReply
				);
