			...
			if ($type == "get") {
				...
				if ($code != null) {
					...
					if ($length == 0) {
						...
						$query = mysqli_query($db, "INSERT INTO `maps_places` (`id`, `token`, `code`, `reference`, `name`, `address`, `location`, `latitude`, `longitude`, `type`, `icon`, `thumbnail`) VALUES (null, '{$places['token']}', '{$places['code']}', '{$places['code']}', '{$places['name']}', '{$places['address']}', '{$places['location']}', '{$places['latitude']}', '{$places['longitude']}', '{$places['type']}', '{$places['icon']}', '{$places['thumbnail']}')");
						if ($query == true) {
