var feed_status_embed = function () {
	...
	mediaLink.each(function(){
		...
		if (typeof thisLink.attr('media-embed') == "string" && typeof JSON.parse(thisLink.attr('media-embed')) == "object") {
			...
			if (isset(linkInfo) && typeof linkInfo == "object") {
				var hostAllow = function (host) {
					if (isset(host) && typeof host == "string" && (inArray(host, ["dailymotion", "dailymotion.com"]) || inArray(host, ["youtube", "youtube.com", "youtu.be"]) || inArray(host, ["vimeo", "vimeo.com"]) || inArray(host, ["vine", "vine.co"]) || inArray(host, ["flickr", "flickr.com"]) || inArray(host, ["myspace", "myspace.com"]) || inArray(host, ["ted", "ted.com"]) || inArray(host, ["nhaccuatui", "nhaccuatui.com"]) || inArray(host, ["mp3.zing.vn"]) || inArray(host, ["clipvn", "clip.vn"]))) {
						return true;
					}else {
						return false;
					}
				};
				if (isset(linkInfo['url']) && typeof linkInfo['url'] == "string" && (new RegExp(<?php print $_parameter->get('regex_sites_url'); ?>).test(linkInfo['url']) || new RegExp(<?php print $_parameter->get('regex_sites_giccos_direct'); ?>).test(linkInfo['url'])) && typeof linkInfo['host'] == "string" && hostAllow(linkInfo['host'])) {
					thisLink.attr('direct-disabled', 'true');
					thisLink.bind('click', function () {
						if (thisLink.attr('handing-link') != "true") {
							...
							$.ajax({
								...
								setTimeout(function() {
									...
									thisLink.removeAttr('direct-disabled');
								}, 100);
							}).done(function() {
								setTimeout(function() {
									thisLink.removeAttr('handing-link');
