...
Class feed {
	...
	function status_list ($object) {
		...
		if (isset($object['selected']) && is_array($object['selected']) && count($object['selected']) > 0) $selected = $object['selected']; else $selected = null;
		...
		if ($selected != null) {
			foreach ($selected as $labelSelected => $valueSelected) {
		...
		if ($type == "general") {
			...
			if (!isset($listQuery['tag']) || !is_string($listQuery['tag'])) {
				...
				$listQuery['source'] = "(".$listQuery['tag'].")";
			}
					...
					if (!$placesNearbyQuery) {
						...
						if (mysqli_num_rows($placesNearbyQuery) > 0) {
							...
							if (!isset($listQuery['nearby']) || $listQuery['nearby'] == null) {
								$listQuery['nearby'] = "(`status`.`places` = '1' AND `status`.`id` IN (SELECT `inside.id` FROM `status_places` WHERE `inside.type` = 'status' AND `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE (6371 * acos(cos(radians('".$placesNearbyFetch['latitude']."') ) * cos(radians(`latitude`)) * cos(radians(`longitude`) - radians('".$placesNearbyFetch['longitude']."')) + sin(radians('".$placesNearbyFetch['latitude']."')) * sin(radians(`latitude`)))) <= '".$_parameter->get('feeds_nearby_scanning_distance')."')))";
							}else {
								$listQuery['nearby'] .= " OR (`status`.`places` = '1' AND `status`.`id` IN (SELECT `inside.id` FROM `status_places` WHERE `inside.type` = 'status' AND `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE (6371 * acos(cos(radians('".$placesNearbyFetch['latitude']."') ) * cos(radians(`latitude`)) * cos(radians(`longitude`) - radians('".$placesNearbyFetch['longitude']."')) + sin(radians('".$placesNearbyFetch['latitude']."')) * sin(radians(`latitude`)))) <= '".$_parameter->get('feeds_nearby_scanning_distance')."')))";
							}
						}
					}
			...
			if (!isset($listQuery['nearby']) || !is_string($listQuery['nearby'])) {
				...
				$listQuery['source'] = "(".$listQuery['nearby'].")";
			}
		}else {
		...
		if (isset($statusArr['menu'], $statusArr['menu']['status']) && is_array($statusArr['menu']) && count($statusArr['menu']) > 0) {
			...
			foreach ($statusArr['menu']['status'] as $typeThis => $valueThis) {
				if ($typeThis == "block" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='block' status-action-block block='false'> <i></i> <span>".$_language->text('block', 'strtolower')."</span> </div>";
					...
					if ($valueThis == true) {
						$code['menu'] .= "<div class='nowrap' menu-type='follow' status-action-follow follow='true'> <i></i> <span>".$_language->text('unfollow', 'strtolower')."</span> </div>";
					}else {
						$code['menu'] .= "<div class='nowrap' menu-type='follow' status-action-follow follow='false'> <i></i> <span>".$_language->text('follow', 'strtolower')."</span> </div>";
					}
