	...
	function data ($format, $object = null) {
		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "get") {
				...
				$query = mysqli_query($db,  "SELECT `token`, `path.original`, `path.large`, `path.medium`, `path.small` FROM `photos_info` WHERE `{$label}` = '{$label}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
			...
			if ($action == "get") {
				...
				$query = mysqli_query($db, "SELECT `token`, `path.original` FROM `music_info` WHERE `id` = '{$id}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
			...
			if ($action == "get") {
				...
				$query = mysqli_query($db, "SELECT `token`, `thumbnail`, `path.hd`, `path.sd` FROM `videos_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
	...
	function cache ($format, $object = null) {
		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "add") {
				...
				$time = time();
				$request = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE (`client` = '{$_client->client(true)}' OR (`author.id` = '{$modeId}' AND `author.type` = '{$modeType}')) AND `close` != '0' AND `close` + '{$_parameter->get('media_cache_time_close')}' < '{$time}'";
				...
				$request = "SELECT `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE `close` != '0' AND `close` + '{$_parameter->get('media_cache_time_close')}' < '{$time}' LIMIT {$randLimit}";
				$query = mysqli_query($db, $request);
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_data[] = $fetch;
				}
					...
					if ($num == 0) {
						...
						if (in_array($file['ext'], $_parameter->get('photos_allow_format', true))) {
							...
							while ($fileFetch = mysqli_fetch_assoc($getFileQuery)) {
								...
								$_tool->unlink($_tool->hash('decode', $fileFetch['path'], $fileFetch['token']));
