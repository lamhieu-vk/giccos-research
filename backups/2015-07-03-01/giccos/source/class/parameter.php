...
Class parameter {
	...
	function insert ($language, $code, $value) {
		if (isset($language, $code, $value)){
			$sql = "INSERT INTO `parameter` (`id`, `code`, `value`, `language`) VALUES (NULL, '{$code}', '{$value}', '{$language}');";
	...
	function update ($language, $code, $value) {
		if (isset($language, $code, $value)){
			$sql = "UPDATE `parameter` SET `value` = '{$value}' WHERE `code` = '{$code}' AND `language` = '{$language}'";
			$query = mysqli_query($this->class['_db']->port('beta'), $sql);
			return true;
		}else {
			return false;
