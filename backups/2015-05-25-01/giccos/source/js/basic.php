...
var messagesTabPrivate = function (object) {
	...
	var actionMessagesPopup = function (popup, object) {
		...
		var typeMessagesPopup = function (object) {
			...
			inputMessages.bind('keyup', function(e) {
				var keycode = (e.keyCode ? e.keyCode : e.which);
				if (inputMessages.val() == "" && inputMessages.attr('typing') != "removed") {
					actionTypingMessagesPopup('remove', inputMessages, {'tab': tabId});
				}else if (inputMessages.val() != "" && inputMessages.attr('typing') != "added") {
					actionTypingMessagesPopup('add', inputMessages, {'tab': tabId});
				}
		...
		var actionTypingMessagesPopup = function (type, object) {
			...
			popup.attr('handling-add-typing', 'true');
		...
		var viewsMessagesPopup = function (object) {
			...
			popup.hover(function() {
				var t = setTimeout(actionRequestViews, 2000);
			    $(this).data('timeout', t);
			}, function() {
		...
		var actionViewsMessagesPopup = function (type, object) {
			...
			var failRequestFunc = function () {
				...
				popup.removeAttr('handling-add-views');
			};
			if (type == "add") {
				popup.attr('views', 'added');
			}else if (type == "remove") {
			...
			popup.attr('handling-add-views', 'true');
		...
		var getViewsMessagesPopup = function (object, loop) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (isset(data['return']) && data['return'] == true) {
						/*
						if (isset(data['data']['views']) && data['data']['views'] !== null) {
							if (countArr(data['data']['views']) === 0) {
								removeCodeTypingMessagesPopup();
							}else {
								for (var i in data['data']['views']) {
									addCodeTypingMessagesPopup(data['data']['views'][i]);
								}
							}
						}
						*/
		...
		var getNewMessagesPopup = function (object, loop) {
			if (popup.length === 0 || !isset(object['tab'])) {
				return false;
			}
			var tabId = object['tab'];
			if (isset(object['id'])) {
	...
	var messagesTabCode = "<div class='messagesTab' display='"+messagesTabDisplay+"' randkey='"+messagesTabRankey+"'> <div class='title'> <div class='name'> <i></i> <span>Name</span> </div> <div class='options'> <div class='close'><i class='icon'></i></div> </div> </div> <div class='content'> <div class='list'> </div> </div> <div class='type'> <textarea class='input' type='text' placeholder='<?php print $_language->text('type_a_messages', 'ucfirst'); ?>'></textarea> </div> </div>";
			...
			if (isset(popupPosObj) && countArr(popupPosObj) == 2) {
				...
				if (isset(tokenCookie) && tokenCookie == "<?php print $_client->client(); ?>") {
					if (isset(objCookie) && typeof objCookie == "object" && countArr(objCookie) > 0 && objCookie !== null) {
						for (var i in objCookie) {
							var popupDisplay = [];
							$(".messagesTab[tab][display]").each(function(){
							...
							if (objCookieThis === null) {
								...
								if (objCookieThis !== null && isset(objCookieThis['display']) && isset(objCookieThis['id']) && isset(objCookieThis['closed']) && isset(objCookieThis['offset']) && isset(objCookieThis['size'])) {
									if (!inArray(objCookieThis['display'], popupDisplay) && !isset(objCookieThis['removed'])) {
