		...
		if ($action == "stats") {
			...
			if (isset($existsSite, $existsSite['return'], $existsSite['exists']) && $existsSite['return'] == true) {
				if ($existsSite['exists'] == true) {
					...
					$getMentionRequest = "SELECT `id`, `time` FROM `status_link` WHERE `sites.id` = '".$siteId."' ".$getMentionRequestPush.";";
			...
			*/
			...
			if ($allowExtract == true) {
				...
				if (isset($isChanged) && !$isChanged) {
					...
					if (isset($pageDom->find("head", 0)->find("meta[property=og:tags]", 0)->content)) {
						$siteInfo['tags'] = $pageDom->find("head", 0)->find("meta[property=og:tags]", 0)->content;
					}else {
						//.
						$siteInfo['tags'] = null;
					}
					...
					if (isset($siteInfo['type']) && is_string($siteInfo['type']) && $siteInfo['type'] != null) {
						$siteInfoTypeArr = explode(" ", $siteInfo['type']);
						$siteInfoTypeShowed = array();
						$siteInfo['type'] = null;
						foreach ($siteInfoTypeArr as $labelTagThis => $valueTagThis) {
							if (in_array($valueTagThis, $siteInfoTypeShowed)) {
								continue;
							}else {
								$siteInfoTypeShowed[] = $valueTagThis;
							}
							$siteInfo['type'] .= $valueTagThis;
						}
					}else {
						$siteInfo['type'] = null;
					}
					if (isset($siteInfo['tags']) && is_string($siteInfo['tags']) && $siteInfo['tags'] != null) {
						$siteInfoTagsArr = explode(" ", $siteInfo['tags']);
						...
						foreach ($siteInfoTagsArr as $labelTagThis => $valueTagThis) {
							...
							$siteInfo['tags'] .= $valueTagThis;
						}
					}else {
						$siteInfo['tags'] = null;
					}
					if (isset($siteInfo['language']) && is_string($siteInfo['language']) && $siteInfo['language'] != null) {
					...
					if (isset($siteInfo['description']) && is_string($siteInfo['description']) && $siteInfo['description'] != null) {
					...
					if (isset($isChanged) && $isChanged) {
						...
						$insertRequest = "
						INSERT INTO 
						`sites_info` (`id`, `time`, `token`, `display`, `hash`, `scheme`, `sub`, `host`, `name`, `dot`, `path`, `mime`, `fingerprints`, `language`, `country`, `type`, `tags`, `title`, `thumbnail`, `description`, `url`, `scan`, `certificate`, `updated`) 
						VALUES 
						(NULL, '{$time}', '{$siteInfo['token']}', '{$siteInfo['display']}', '{$siteInfo['hash']}', '{$siteInfo['scheme']}', '{$siteInfo['sub']}', '{$siteInfo['host']}', '{$siteInfo['name']}', '{$siteInfo['dot']}', '{$siteInfo['path']}', '{$siteInfo['mime']}', '{$siteInfo['fingerprints']}', '{$siteInfo['language']}', '{$siteInfo['country']}', '{$siteInfo['type']}', '{$siteInfo['tags']}', '{$siteInfo['title']}', '{$siteInfo['thumbnail']}', '{$siteInfo['description']}', '{$siteInfo['url']}', '{$siteInfo['scan']}', '{$siteInfo['certificate']}', '{$time}');
						";
