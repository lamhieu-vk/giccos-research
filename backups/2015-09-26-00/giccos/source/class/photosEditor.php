...
Class photosEditor {
    ...
    function most_colors ($count = 1) {
        $colorArr = $arrangeArr = array();
        $this->execute("convert {$this->photos_tmp} -colors 256 -depth 8 -format \"%c\" histogram:info:", $analysisArr);
        foreach ($analysisArr as $key => $analysisArrThis) {
            preg_match("/([0-9]+)\:/", $analysisArrThis, $countMatches);
            preg_match("/(\#[a-zA-Z0-9]+)/", $analysisArrThis, $colorMatches);
            if (isset($colorMatches[1], $countMatches[1])) {
                $arrangeArr[$colorMatches[1]] = $countMatches[1];
            }
        }
        arsort($arrangeArr);
        foreach ($arrangeArr as $key => $arrangeArrThis) {
