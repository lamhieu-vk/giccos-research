		...
		if (statusType != boxMediaOtherOld && (statusType == "status" || statusType == "photos" || statusType == "music" || statusType == "videos" || statusType == "cited")) {
			...
			if (statusType == "status") {
					...
					if (1 + 1 == 2) {
						...
						uploadPreview.children(".demo[tag]").find(".info > .header > .action .close").bind('click', function() {
							$(this).parents(".demo[tag]").fadeOut(function() {
								...
								if (dataCache.exists('sites-notuse-url') === true) {
									dataCache.push('sites-notuse-url', url);
								}else {
									dataCache.add('sites-notuse-url', [url]);
								}
							});
						});
						if (dataCache.exists('sites-info-url') === true) {
							var infoList = dataCache.get('sites-info-url');
							var isHas = 0;
							...
							if (isHas === 0) {
								...
								newObj[url] = linkObj;
								linkObj = newObj;
								dataCache.push('sites-info-url', linkObj);
							}
						}else {
							newObj = {};
							newObj[url] = linkObj;
							linkObj = newObj;
							dataCache.add('sites-info-url', linkObj);
						}
					}
				};
				var linkScanInfo = function (url) {
					...
					var infoUrlList = dataCache.get('sites-scan-url');
					...
					$.ajax({
						...
						success: function(data) {
							if (isset(data) && isset(data['return']) && data['return'] === true) {
								console.log('xs');
								if (dataCache.exists('sites-scan-url') === true) {
									var infoList = dataCache.get('sites-scan-url');
									var isHas = 0;
									infoListLoop: for (i in infoList) {
										if (i == url) {
											isHas++;
									...
									if (isHas === 0) {
										linkObj = [];
										linkObj[url] = true;
										dataCache.push('sites-scan-url', linkObj);
									}
								}else {
									linkObj = [];
									linkObj[url] = true;
									dataCache.add('sites-scan-url', linkObj);
								}
				...
				var linkGetInfo = function (url) {
					...
					var notuseUrlList = dataCache.get('sites-notuse-url');
					...
					var infoUrlList = dataCache.get('sites-info-url');
					if (isset(infoUrlList) && typeof infoUrlList == "object") {
						if (countArr(infoUrlList) > 0) {
							for (var i in infoUrlList) {
								if (i == url) {
