...
$(document).ready(function () {
	...
	$("#infoPage > .box.stats > .content > .rows > .value.views > .statsBox").each(function () {
		if (dataCache.exists('sites_info_stats_views_dataset') === true) {
			var data = dataCache.get('sites_info_stats_views_dataset');
		}else {
			return false;
		}
	...
	$("#infoPage > .box.stats > .content > .rows > .value.mention > .statsBox").each(function () {
		if (dataCache.exists('sites_info_stats_mention_dataset') === true) {
			var data = dataCache.get('sites_info_stats_mention_dataset');
		}else {
			return false;
		}
		var lineBar = $(this).get(0).getContext("2d");
