...
Class tool {
    ...
    function getWeather ($object) {
    	...
    	if (in_array($server, ["yahoo"])) {
    		$units = (isset($object['units']) && is_string($object['units']) && in_array($object['units'], ["C", "F"])) ? strtolower($object['units']) : "f";
    		...
    		if ($locationName == null) {
    			...
    			if (!isset($reset) || (isset($reset) && $reset == false)) {
	    			$locationHash = hash('crc32', strtolower(preg_replace("/([^a-zA-Z0-9]+)/", "_", $locationName)));
