	...
	function status_list ($object) {
		...
		if (isset($except) && is_array($except) && count($except) > 0) {
			foreach ($except as $exceptThis) {
				$except[$exceptThis] = intval($_tool->convertDatabaseString($exceptThis));
			}
		...
		if (!$statusQuery) {
			...
			while ($status = mysqli_fetch_assoc($statusQuery)) {
				...
				$tips = array(
					...
					"action" => "favorite",
					"guy" => [
						array("type" => "user", "id" => "1", "name" => "Truong Tuan", "tag" => "truongtuan", "avatar" => "http://localhost/giccos/photos/raw/static/user-avatar-small.jpg", "link" => "#"),
						array("type" => "user", "id" => "2", "name" => "Tran Duong", "tag" => "tranduong", "avatar" => "http://localhost/giccos/photos/raw/static/user-avatar-small.jpg", "link" => "#")
					]
				);
				$info['id'] = $status['id'];
