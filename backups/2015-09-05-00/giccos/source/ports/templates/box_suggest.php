	...
	if (isset($boxSuggestRequire['weather'])) {
		...
		if (!isset($weatherLocation) || $weatherLocation == null) {
			...
			if (isset($getWeather, $getWeather['return'], $getWeather['data']) && $getWeather['return'] == true) {
				...
				<div class='rows time'><i></i><span>".$_language->text('updated_at', 'strtolower').":</span> <span time-text time-ago='".$dataWeather['time']."'>".$_tool->agoDatetime($dataWeather['time'], 'ago')."</span></div>
				<div class='rows wind'>
					<div class='label'>
				...
				<div class='rows astronomy'>
					...
					<div class='value'>
						<div class='td sunrise'>".$_language->text('sunrise', 'strtolower').": ".date("H:i", $dataWeather['astronomy']['sunrise'])."</div>
						<div class='td sunset'>".$_language->text('sunset', 'strtolower').": ".date("H:i", $dataWeather['astronomy']['sunset'])."</div>
					</div>
				</div>
