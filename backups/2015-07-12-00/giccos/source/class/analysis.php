...
Class analysis {
	...
	function faces ($action, $object) {
		...
		if ($action == "get") {
			...
			$numQuery = mysqli_query($db, "SELECT {$rows} FROM `analysis_faces` WHERE `{$label}` = '{$value}' {$limit}");
			if ($numQuery) $numRows = mysqli_num_rows($numQuery); else $numRows = 0;
			$data = array();
			if ($numRows == 0) {
				return array("return" => true, "data" => $data);
			}else {
				while ($faceFetch = mysqli_fetch_assoc($numQuery)) {
					$data[] = $faceFetch;
				}
				return array("return" => true, "data" => $data);
			}
			...
			if ($url != null) {
				/*
				$htmlPage = $_tool->curl($url, 5, array("cookie" => true));
				if (isset($htmlPage['return'], $htmlPage['data']) && $htmlPage['return'] == true) {
					$optionsRequest['base64'] = base64_encode($htmlPage['data']);
				}else {
			...
			if (is_array($thumbnail) && count($thumbnail) > 0) {
				...
				if (isset($getThumbnailFile['return'], $getThumbnailFile['num'], $getThumbnailFile['file']) && $getThumbnailFile['return'] == true && $getThumbnailFile['num'] > 0) {
					$thumbnailPath = $_tool->links($getThumbnailFile['file'][0]['source']);
			...
			if (isset($curlRequest['return'], $curlRequest['data']) && $curlRequest['return'] == true) {
				...
				if (isset($dataAnalysis['face_detection']) && is_array($dataAnalysis['face_detection']) && count($dataAnalysis['face_detection']) > 0) {
					...
					$checkExists = $this->faces("exists", array("label" => "faces", "value" => $faceImg));
					if (isset($checkExists['return'], $checkExists['exists']) && $checkExists['return'] == true && $checkExists['exists'] == true) {
						$is_exists = true;
					}else {
						$is_exists = false;
					}
					...
					if ($is_exists == true) {
						$is_add = false;
						$getFaceQuery = mysqli_query($db, "SELECT * FROM `analysis_faces` WHERE `faces` = '{$faceImg}' ORDER BY `id` DESC LIMIT 1");
						if (!$getFaceQuery) {
							return array("return" => false, "reason" => "");
						}else if ($getFaceQuery == true && mysqli_num_rows($getFaceQuery) == 0) {
							return array("return" => false, "reason" => "");
						}
						$obj = mysqli_fetch_assoc($getFaceQuery);
						if (!isset($obj['thumbnail'])) {
							mysqli_query($db, "DELETE FROM `analysis_faces` WHERE `id` = '{$obj['id']}'");
							$is_add = true;
						}else {
					...
					if ($is_add == true) {
						...
						VALUES 
						(NULL, '{$obj['time']}', '{$obj['updated']}', '{$obj['token']}', '{$obj['display']}', '{$obj['tag']}', '{$obj['thumbnail']}', '{$obj['faces']}', '{$obj['confidence']}', '{$obj['eye.left.x']}', '{$obj['eye.left.y']}', '{$obj['eye.right.x']}', '{$obj['eye.right.y']}', '{$obj['nose.x']}', '{$obj['nose.y']}', '{$obj['mouth.tl.x']}', '{$obj['mouth.tl.y']}', '{$obj['mouth.bl.x']}', '{$obj['mouth.bl.y']}', '{$obj['mouth.tr.x']}', '{$obj['mouth.tr.y']}', '{$obj['mouth.br.x']}', '{$obj['mouth.br.y']}', '{$obj['pose.roll']}', '{$obj['pose.yaw']}', '{$obj['pose.pitch']}', '{$obj['race.text']}', '{$obj['race.value']}', '{$obj['quality.brn']}', '{$obj['quality.shn']}', '{$obj['emotion.oe.text']}', '{$obj['emotion.oe.value']}', '{$obj['emotion.to.text']}', '{$obj['emotion.to.value']}', '{$obj['emotion.te.text']}', '{$obj['emotion.te.value']}', '{$obj['age']}', '{$obj['smile']}', '{$obj['glasses']}', '{$obj['sunglasses']}', '{$obj['eye_closed']}', '{$obj['mouth_open_wide']}', '{$obj['beauty']}', '{$obj['sex']}');
						";
						...
						if (!$insertQuery) {
							...
							$curlRequest = $_tool->curl($_parameter->get('rekognition_url'), 0, array("cookie" => true, "method" => "POST", "form" => $optionsRequest));
							return array("return" => true, "obj" => $obj);
							/*
