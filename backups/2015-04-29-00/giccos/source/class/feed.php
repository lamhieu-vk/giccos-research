				...
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							$media_fetch = $this->class['_sites']->data(array("rows" => $rows, "label" => "id", "value" => $media_data['sites.id'], "limit" => "LIMIT 1"));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								...
								if (isset($media_fetchData['thumbnail']) && $media_fetchData['thumbnail'] != null && ($media_fetchData['thumbnail'] != 0 || $media_fetchData['thumbnail'] != "0")) {
									$media_fetchData['thumbnail'] = $this->class['_tool']->links($media_fetchData['thumbnail']);
								}else {
									$media_fetchData['thumbnail'] = null;
								}
								$media_fetchData['path'] = $this->class['_tool']->links('sites/direct/'.$this->class['_tool']->hash('encode', $media_fetchData['path'], $this->class['_parameter']->get('hash_sites_direct')));
								$content['media']['data'][] = array(
									...
									"share" => array(
										"link" => $this->class['_tool']->links('link/embed/').$media_fetchData['display']
									)
								);
							}
						}
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							$media_fetch = $this->class['_media']->data("photos", array("action" => "get", "rows" => $rows, "label" => "id", "value" => $media_data['photos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id']), "limit" => "LIMIT 1"));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								$media_fetchData = $media_fetch['return'][0];
								if ($media_fetchData['file.medium'] != '0') {
									$media_thumbnail = $this->class['_tool']->links('photos/raw/').$media_fetchData['file.medium'];
								}else {
									$media_thumbnail = $this->class['_tool']->links('photos/raw/').$media_fetchData['file.original'];
								}
								if ($media_fetchData['file.large'] != '0') {
									$media_link = $this->class['_tool']->links('photos/raw/').$media_fetchData['file.large'];
								}else {
									$media_link = $this->class['_tool']->links('photos/raw/').$media_fetchData['file.original'];
								}
								$content['media']['data'][] = array(
									"display" => $media_fetchData['display'], 
									...
									"link" => $media_link,
									"share" => array(
										"link" => $this->class['_tool']->links('photos/embed/').$media_fetchData['display']
									)
								);
							}
						}
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							$media_fetch = $this->class['_media']->data("music", array("action" => "get", "rows" => $rows, "label" => "id", "value" => $media_data['music.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id']), "limit" => "LIMIT 1"));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								$media_fetchData = $media_fetch['data'][0];
								if ($media_fetchData['name'] == null) {
									$media_fetchData['name'] = $this->class['_language']->text('null:music_name', 'ucfirst');
								}
								if ($media_fetchData['singer'] == null) {
									$media_fetchData['singer'] = $this->class['_language']->text('not_updated', 'ucfirst');
								}else {
									$media_fetchData['singer'] = $this->class['_language']->text('presented_by', 'ucfirst').': '.$media_fetchData['singer'];
								}
								if ($media_fetchData['description'] == null) {
									$media_fetchData['description'] = $this->class['_language']->text('null:description_music', 'ucfirst');
								}
								$media_fetchData['link'] = $this->class['_tool']->links("music/raw/").$media_fetchData['file.original'];
								$content['media']['data'][] = array(
									"display" => $media_fetchData['display'],
									...
									"share" => array(
										"link" => $this->class['_tool']->links('music/embed/').$media_fetchData['display']
									)
								);
							}else {
								//.
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							$media_fetch = $this->class['_media']->data("videos", array("action" => "get", "rows" => $rows, "label" => "id", "value" => $media_data['videos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id']), "limit" => "LIMIT 1"));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								$media_fetchData = $media_fetch['data'][0];
								if ($media_fetchData['name'] == null) {
									$media_fetchData['name'] = $this->class['_language']->text('null:videos_name', 'ucfirst');
								}
								if ($media_fetchData['description'] == null) {
									$media_fetchData['description'] = $this->class['_language']->text('null:description_videos', 'ucfirst');
								}
								$media_fetchData['thumbnail'] = $this->class['_tool']->links("videos/raw/thumbnail/").$media_fetchData['file.thumbnail'];
								$media_fetchData['link'] = $this->class['_tool']->links('videos/raw/').$media_fetchData['file.sd'];
								$content['media']['data'][] = array(
									"display" => $media_fetchData['display'], 
									"name" => $media_fetchData['name'],
									"mime" => $media_fetchData['mime_type'],
									...
									"link" => $media_fetchData['link'],
									"share" => array(
										"link" => $this->class['_tool']->links('videos/embed/').$media_fetchData['display']
									)
								);
							}
						}
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						if ($cited_fetch['photos.id'] != '0') {
							...
							$media_fetch = $this->class['_media']->data("photos", array("action" => "get", "rows" => $rows, "label" => "id", "value" => $cited_fetch['photos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id']), "limit" => "LIMIT 1"));
						}else {
