...
Class storage {
			...
			if (!$query || !$puts) {
				...
				if ($format == "image") {
					if ($file != "none" && $compression == true) {
						...
						$storage['icc']['path'] = $_tool->hash("encode", $storage['icc']['source'], $storage['token'].'::'.$_parameter->get('hash_storage_path'));
	...
	function get ($object) {
		...
		if ($format == "image") {
			$getRequest = "SELECT {$rows} FROM `storage_image` WHERE `{$label}` = '{$value}' {$limit}";
		}else if ($format == "audio") {
			$getRequest = "SELECT {$rows} FROM `storage_audio` WHERE `{$label}` = '{$value}' {$limit}";
		}else if ($format == "video") {
			$getRequest = "SELECT {$rows} FROM `storage_video` WHERE `{$label}` = '{$value}' {$limit}";
		}else {
			return array("return" => false, "reason" => "");
		}
		if (!isset($getRequest)) {
		...
		if ($getQuery == true && mysqli_num_rows($getQuery) == 0) {
			return array("return" => true, "count" => 0, "file" => array(), "s" => $getRequest);
			...
			return array("return" => true, "count" => mysqli_num_rows($getQuery), "file" => $fileArr);
		}else {
			return array("return" => false, "reason" => "");
		}
	}
	function delete ($object) {
		...
		if ($num == 0) {
			...
			while ($fetch = mysqli_fetch_assoc($query)) {
				$file = $_tool->hash("decode", $fetch['path'], $fetch['token'].'::'.$_parameter->get('hash_storage_path'));
				if (@file_exists($file)) {
