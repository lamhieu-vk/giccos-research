...
Class tool {
	...
	function changeClassName ($name, $token = null) {
    	...
    	if ($server == 1) {
			...
			if ($curl['return'] == true) {
				...
				if ($data['query']['count'] == 0) {
					...
					return array("return" => true, "data" => $data['query']['results']['channel']);
				}
			}else {
	...
	function curl ($url, $timeout = 10, $options = null) {
		...
		if (is_array($options) && count($options)) {
			...
			if (isset($options['headers']) && is_array($options['headers']) && count($options['headers']) != null) {
				$headers = array();
				if (preg_match("/^((http|https)(\:\/\/)({$this->links('::host')}))/", $url)) {
			...
			if (preg_match("/(\:\:user)/", $links)) {
				if (preg_match("/(\:\:avatar)/", $links)) {
					if (preg_match("/(\:\:large)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_avatar_large'), false);
					}else if (preg_match("/(\:\:medium)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_avatar_medium'), false);
					}else if (preg_match("/(\:\:small)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_avatar_small'), false);
					}
				}else if (preg_match("/(\:\:cover)/", $links)) {
					if (preg_match("/(\:\:large)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_cover_large'), false);
					}else if (preg_match("/(\:\:medium)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_cover_medium'), false);
					}else if (preg_match("/(\:\:small)/", $links)) {
						$links = $this->links($this->class['_parameter']->get('default_user_cover_small'), false);
					}
				}
			}
			return $links;
	...
	function agoDatetime ($timestamp = null, $type = "ago", $length = false) { 
		if ($type == "" || $type == "ago") {
			...
			if ($seconds <= 60) {
				...
				/*
				if ($minutes == 1 && !$length) {
					$time_ago = $this->class['_language']->text('about_a_minute_ago', 'strtolower', $length);
				}else {
