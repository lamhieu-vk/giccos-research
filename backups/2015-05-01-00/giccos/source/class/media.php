			...
			if ($authorType == $guyType && $authorId == $guyId) {
				...
				if ($authorType == "user") {
					if ($guyType == "guy") {
						...
						if ($isBlocked > 0) {
							return array("return" => true, "guy" => 6);
						}else {
							if ($guyType == "user") {
			...
			if ($action == "add") {
				...
				if ($num == 0) {
					...
					while ($imgFetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($getMediaFile['return'], $getMediaFile['num'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['num'] > 0) {
							$mediaDataThis['tmp'] = $_tool->hash("decode", $getMediaFile['file'][0]['path'], $getMediaFile['file'][0]['token']);
						}else {
							continue;
						}
				...
				if ($name != null) {
					$set = $object['set'];
					$author = $this->user['mode'];
					$imgCopy_query = mysqli_query($db,"SELECT `copy`, `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$this->user['mode']['id']}' AND `author.type` = '{$this->user['mode']['type']}' AND `user.id` = '{$this->user['id']}' AND `copy` != '0'");
					if (mysqli_num_rows($imgCopy_query) > 0) {
						$imgCopy = mysqli_fetch_assoc($imgCopy_query);
						$imgOriginal_query = mysqli_query($db, "SELECT `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$imgCopy['copy']}' AND `author.id` = '{$this->user['mode']['id']}' AND `author.type` = '{$this->user['mode']['type']}' AND `user.id` = '{$this->user['id']}'");
				...
				if ($secret != null && $name != null && ($verify == true || $verify == false)) {
					...
					$sql = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE ((`secret` = '{$secret}' AND `name` = '{$name}') OR (`copy` = '{$name}')) AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}' {$pushSql}";
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							...
							$photosFacesQuery = mysqli_query($db, "SELECT `id`, `photos`, `faces` FROM `photos_faces` WHERE `photos` = '{$fetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}'");
					...
					if (isset($f_secret, $_SESSION["cache"]['photos_faces']) && is_array($f_secret) && count($f_secret) > 0 && is_array($_SESSION["cache"]['photos_faces']) && count($_SESSION["cache"]['photos_faces']) > 0) {
						...
						foreach ($cacheFaces as $key => $cacheFaceThis) {
							if (isset($cacheFaceThis['photos'], $cacheFaceThis['photos.local'], $cacheFaceThis['user.id'], $cacheFaceThis['author.type'], $cacheFaceThis['author.id']) && in_array($cacheFaceThis['photos'], $f_secret) && $cacheFaceThis['photos.local'] == "cache" && $cacheFaceThis['user.id'] == $this->user['id'] && $cacheFaceThis['author.type'] == $this->user['mode']['type'] && $cacheFaceThis['author.id'] == $this->user['mode']['id']) {
					...
					if (isset($f_secret, $_SESSION["cache"]['photos_faces_handling']) && is_array($f_secret) && count($f_secret) > 0 && is_array($_SESSION["cache"]['photos_faces_handling']) && count($_SESSION["cache"]['photos_faces_handling']) > 0) {
						...
						foreach ($cacheFacesHandling as $key => $cacheFacesHandlingThis) {
							if (isset($cacheFacesHandlingThis['photos'], $cacheFacesHandlingThis['photos.local'], $cacheFacesHandlingThis['user.id'], $cacheFacesHandlingThis['author.type'], $cacheFacesHandlingThis['author.id']) && in_array($cacheFacesHandlingThis['photos'], $f_secret) && $cacheFacesHandlingThis['photos.local'] == "cache" && $cacheFacesHandlingThis['user.id'] == $this->user['id'] && $cacheFacesHandlingThis['author.type'] == $this->user['mode']['type'] && $cacheFacesHandlingThis['author.id'] == $this->user['mode']['id']) {
				...
				$time = time();
				$request = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE (`client` = '{$_client->client(true)}' OR (`author.id` = '{$this->user['mode']['id']}' AND `author.type` = '{$this->user['mode']['type']}')) AND `close` != '0' AND `close` + '{$_parameter->get('media_cache_time_close')}' < '{$time}'";
				...
				if ($name != null && $secret != null && is_array($this->user['mode'])) {
					$client = $_client->client(true);
					$query = mysqli_query($db, "SELECT * FROM `photos_cache` WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}'");
					if ($query) $num = mysqli_num_rows($query); else $num = 0;
					if ($num == 0) {
						return array("return" => false, "reason" => "");
					}else {
						...
						$time = time();
						$close = $time + $_parameter->get('media_cache_time_close');
						$fileQuery = "UPDATE `photos_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}'";
						...
						if (in_array($file['ext'], $_parameter->get('photos_allow_format', true))) {
							...
							while ($fileFetch = mysqli_fetch_assoc($getFileQuery)) {
								...
								$_SESSION['x'][] = "UPDATE `photos_faces` SET `photos` = '{$file['display']}', `photos.local` = 'drive' WHERE `photos` = '{$fileFetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}'";
								mysqli_query($db, "UPDATE `photos_faces` SET `photos` = '{$file['display']}', `photos.local` = 'drive' WHERE `photos` = '{$fileFetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}'");
			...
			if ($action == "add") {
				...
				if ($num == 0) {
					...
					while ($musikFetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($getMediaFile['return'], $getMediaFile['num'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['num'] > 0) {
							$musikDataThis['tmp'] = $_tool->hash("decode", $getMediaFile['file'][0]['path'], $getMediaFile['file'][0]['token']);
						}else {
							continue;
						}
			...
			if ($action == "add") {
				...
				if ($num == 0) {
					...
					while ($vioFetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($getMediaFile['return'], $getMediaFile['num'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['num'] > 0) {
							$vioDataThis['tmp'] = $_tool->hash("decode", $getMediaFile['file'][0]['path'], $getMediaFile['file'][0]['token']);
						}else {
							continue;
						}
						$vioData[] = $vioDataThis;
