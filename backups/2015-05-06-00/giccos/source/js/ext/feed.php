...
function status_statistic (id, from) {
	if (id > 0) {
		$.ajax({
			...
			type: "POST",
			dataType: "json",
			data: {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'stats', 'id': id, 'from': from},
			contentType: "<?php print $_parameter->get("contentType_urlencoded.utf8"); ?>",
			error: function () {
				popupNotification({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: '<?php print $_language->text('transmission_error', 'ucfirst'); ?>. <?php print $_language->text('please_try_agian', 'ucfirst'); ?>.', timeout: '<?php print $_parameter->get('notification_error_timeout'); ?>'});
			},
	...
	function call () {
		...
		if (feed.attr('handing-newcheck') != "true") {
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'newcheck', 'feed': port, 'id': id},
				contentType: "<?php print $_parameter->get("contentType_urlencoded.utf8"); ?>",
				success: function (data) {
					if (isset(data['return']) && data['return'] == true) {
						if (data['data']['number'] != undefined && typeof data['data']['number'] == "numbder" && data['data']['number'] > 0) {
...
function status_load (options) {
	var id = options['id'],
		...
		port = options['port'],
		token = '<?php print $g_client['token']['action']['feed']; ?>',
		data = {'port' : 'feed', 'token': token, 'type': 'status', 'action': 'load', 'feed': port, 'sort': sort, 'id': id};
...
function status_block (id) {
	...
	if (count > 0) {
		...
		var id = [id],
			data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'block', 'id': id};
		$.ajax({
			url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
			type: "POST",
			dataType: "json",
...
function status_delete (id) {
	...
	if (count > 0) {
		...
		var id = [id];
		var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'delete', 'id': id};
		$.ajax({
			url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
			type: "POST",
			dataType: "json",
		...
		button.bind('click', function() {
			...
			if (button.attr('handing') != 'true') {
				...
				if (type == "delete") {
					...
					var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': type, 'id': [info['id']]};
					$.ajax({
						url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
						type: "POST",
						dataType: "json",
								...
								for (var i in data['data']) {
									...
									if (rows['return'] != undefined && rows['return'] == true) {
										...
										if (count > 0) {
											...
											popupNotification ({type: 1, title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: popupDescription, timeout: '<?php print $_parameter->get('notification_error_timeout'); ?>'});
										}
									}else {
										if (rows['reason'] != undefined && (rows['reason'] == "ERROR#FEEDS_012" || rows['reason'] == "ERROR#FEEDS_006")) {
											button.remove();
						...
						editor.find(".rows.options > .submit .send").bind('click', function() {
							...
							if (sendButton.attr('handing-edit') != 'true') {
								...
								var privateSelected = JSON.parse(editor.find(".rows.options > .private > .choosePrivate > .selected > [private]").attr('private')),
									...
									data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'edit', 'id': info['id'], 'rows': {'private->view': privateSelected['type'], 'content': content}};
								$.ajax({
									url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
									type: "POST",
									dataType: "json",
...
function status_getCodeMedia (object) {
	...
	if (object['media']['type'] == "link") {
		...
		for (var i in object['media']['data']) {
			...
			value['info'] = JSON.stringify({"type": "link", "id": value['id'], "display": value['display'], "host": value['host'], "path": value['path'], "thumbnail": value['thumbnail'], "title": value['title'], "description": value['description'], "share": value['share']});
			...
			c_['status_content_media'] = c_['status_content_media']+" <div class='link' id='"+value['id']+"' media='link' media-display='"+value['display']+"' media-info='"+value['info']+"' direct='"+value['direct']+"'> "+c_['status_content_media_thumbnail']+" <div class='info'> <div class='title rows nowrap'><span>"+value['title']+"</span></div> <div class='description rows'><span>"+value['description']+"</span></div> <div class='from rows nowrap'><span>"+value['host']+"</span></div> </div> </div> ";
		}
...
function status_favorite () {
	...
	button.bind('click', function() {
		...
		if (handing != 'true') {
			...
			var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'id': [status_id]};
...
var feed_comment_add = function (object, callback) {
	if (object['inside_type'] === undefined || object['inside_id'] === undefined || object['content'] === undefined) {
		return false;
	}
	var insideType = object['inside_type'],
		insideId = object['inside_id'],
		content = object['content'];
	var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'action': 'add', 'inside_type': insideType, 'inside_id': insideId, 'content': content};
	var requestCommentAdd = $.ajax({
		...
		success: function (data) {
			...
			if (isset(data['return']) && data['return'] === true && data['data'] !== undefined && typeof data['data'] == "object") {
				comment_push({'inside_type': insideType, 'inside_id': insideId, 'sort': '>'}, data['data']);
			}
		}
	}).fail(callback).done(callback);
}	

function feed_comment_addfromStatus () {
	...
	textarea.bind('keyup', function(e) {
		...
		if (isset(selector.attr('status')) === false || selector.attr('status') === null || typeof JSON.parse(selector.attr('status')) != "object") {
			return;
		}
		var status = JSON.parse(selector.attr('status')),
			status_id = status['id'];
		if (keycode == 13) {
			if (textarea.attr('handing-add') != 'true') {
				...
				if (value != null) {
					...
					popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: "<?php print $_language->text("comment_is_not_found", 'ucfirst'); ?>.<br> <?php print $_language->text('please_try_agian', 'ucfirst'); ?>.", timeout: '<?php print $_parameter->get('notification_error_timeout'); ?>'});
					textarea.val('').removeAttr('handing-add').attr('placeholder','<?php print $_language->text("write_a_comment", 'ucfirst'); ?>').removeAttr('disabled');
				}
			}else {
				textarea.val('').removeAttr('handing-add').attr('placeholder','<?php print $_language->text("write_a_comment", 'ucfirst'); ?>').removeAttr('disabled');
				popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: transferError('ERROR#FEEDS_008'), timeout: '<?php print $_parameter->get('notification_error_timeout'); ?>'});
			}
		}
	});
}
function feed_comment_favorite () {
	...
	button.bind('click', function() {
		...
		if (button.attr('handing-favorite') != "true") {
			...
			var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'id': [comment_id]};
...
function comment_load (options) {
	var inside = options['inside'],
		...
		sort = options['sort'],
		limit = options['limit'],
		status = $(".status.inFeeds[status][status-id='"+id+"']"),
		length = 0;
	var button = status.find("[status-comment-load]");
	button.attr("comment-load", true);
	if (status.length > 0) {
		var afterDoneFunc = function() {
			var status = $(".status.inFeeds[status][status-id='"+id+"']"),
				button = status.find("[status-comment-load]");
			button.removeAttr("comment-load");
			var length = 0;
		};
		var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'action': 'load', 'inside': inside, 'id': id, 'sort': sort, 'limit': limit};
		$.ajax({
			url: "<?php print $_tool->links("source/ajax/feed.ajax"); ?>",
			type: "POST",
			dataType: "json",
...
function comment_push (options, comment) {
	...
	insideThis.each(function(i) {
		...
		if (insideType == "status") {
			...
			var commentContent = status.find('[status-comment-content]');
			var lengthCommentLines = commentContent.find("[status-comment-lines][comment][inside-type='comment'][inside-id='"+comment['info']['id']+"']").length;
			var lengthComment = status.find("[status-comment-lines][comment-id='"+comment['info']['id']+"']").length;
			if (sort == ">" && lengthCommentLines > 0 && lengthComment === 0) {
				status.find('[status-comment-lines][comment]:first').before(c_data);
			}else if (sort == "<" && lengthCommentLines > 0 && lengthComment === 0) {
...
function feed_comment_stats (object, callback) {
	...
	var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'action': 'stats', 'obj': {'id': commentId, 'rows': commentRows, 'type': commentType}};
	var requestCommentAdd = $.ajax({
		url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
		type: "POST",
		dataType: "json",
			...
			if (isset(data['return']) && data['return'] == true) {
				if (isset(data['data']) && Object.keys(data['data']).length > 0) {
					for (var i in data['data']) {
						...
						thisComment.find("[status-comment-stats-favorite]").text(thisRow['stats']['favorite']);
						thisComment.find("[status-comment-stats-reply]").text(thisRow['stats']['reply']);
					}
				}else {
					//.
				}
		...
		if (type == "photos" && object != "" && object !== undefined && typeof JSON.parse(object) == 'object') {
			...
			if (object['type'] == 'music') {
				var box = $(this).parent('.media.MusicBox'),
					...
					c_media = '<audio id="'+object['id']+'" class="gplayer-skin video" width="98%" height="82px"> <source src="'+object['link']+'" type="'+object['mime']+'"> </audio>';
			}else if (object['type'] == 'videos') {
				var box = $(this).parent('.media.VideosBox'),
					c_info = "<div class='navBar'> <div class='info'> <div class='name nowrap'> <div class='move'><i class='fa'></i></div> <span>"+object['name']+"</span> </div> <div class='description nowrap'> <span>"+object['description']+"</span> </div> </div> </div>",
					c_media = '<video id="'+object['id']+'" class="gplayer-skin video" width="98%" height="275"> <source src="'+object['link']+'" type="'+object['mime']+'"> </video>';
			}
			var options = {
				'controls': true,
				'autoplay': false,
