				...
				for ($i = 0; $i < count($commentData); $i++ ) {
					...
					if ($comment['info']['inside.type'] == "status") {
						...
						<div class='reply'>
							...
							<span status-comment-stats-reply>{$comment['relpy']['count']}</span>
						</div>
						";
					}else {
	...
	function comment_stats ($object) {
		...
		if ($commentCount == 0) {
			...
			if ($type == "list" || $type == "all") {
				...
				if (($rows == "reply" || $rows == "all") && isset($relpyQuery) && $relpyQuery) {
					$replyList = array();
					while ($replyListFetch = mysqli_fetch_assoc($relpyQuery)) {
						$replyList[] = array(
