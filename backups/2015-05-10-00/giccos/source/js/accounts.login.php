var actionBoxLogin = function () {
	var resizeCenterSubBox = function (){
		...
		footerBox.css({
			'margin-top': (($(window).height() / 2 - loginBox.innerHeight() / 2) - 0) / 2
		});
	};
	resizeCenterSubBox();
...
var loginAccounts = function () {
	...
	var actionLoginAccounts = function () {
		...
		if (!usernameRegex(inputUsername.val()) && !emailRegex(inputUsername.val())) {
			popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: "<?php print $_language->text('username_is_invalid', 'ucfirst'); ?>, <?php print $_language->text('please_recheck', 'strtolower'); ?>.", timeout: '<?php print $_parameter->get('notification_text_action'); ?>'});
		}else if (!passwordRegex(inputPassword.val())) {
			popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: "<?php print $_language->text('password_is_invalid', 'ucfirst'); ?>, <?php print $_language->text('please_recheck', 'strtolower'); ?>.", timeout: '<?php print $_parameter->get('notification_text_action'); ?>'});
		}else {
			...
			$.ajax({
				...
				success: function(data) {
					if (isset(data['return']) && data['return'] === true) {
						...
						popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: "<?php print $_language->text('undefined', 'ucfirst'); ?>.", timeout: '<?php print $_parameter->get('notification_text_action'); ?>'});
					}
				}
