	...
	if (isset($_uri[0], $g_client['token']['source']['url']) && is_string($_uri[0]) && $_uri[0] == $g_client['token']['source']['url']) {
		...
		if (isset($_uri[1], $g_user['login']) && is_string($_uri[1]) && $g_user['login'] == false && in_array($_uri[1], array('login', 'register'), true)) {
			if (!isset($g_client['theme']['welcome'])) {
				...
				$getThemeWelcomeRequest = "SELECT `author.type`, `author.id`, `time`, `file.large` FROM `photos_info` WHERE `private.view` = '5' AND `file.large` != '0' AND `id` IN ('7') LIMIT 1";
				...
				if ($getThemeWelcomeQuery && mysqli_num_rows($getThemeWelcomeQuery) > 0) {
					...
					$themWelcomeArr['image']['background'] = $_tool->links('photos/raw/'.$getThemeWelcomeFetch['file.large']);
