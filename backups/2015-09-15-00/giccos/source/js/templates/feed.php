...
var feed_status_newcheck = function () {
	var call = function () {
		...
		if (feed.attr('handing-newcheck') != "true") {
			...
			if (isset(options['type']) && options['type'] == "status") {
				return;
			}else {
			...
			info['sort'] = ">";
			info['order'] = "new"
			//. {'type': port['type'], 'method': port['method'], 'sort': '>', 'id': id, 'logs': logs, 'except': except, 'order': 'new'}
			$.ajax({
				url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
				type: "POST",
				dataType: "json",
...
var feed_status_blockRequest = function (object) {
	...
	if (_dataCache.exists(blockHandling) === false) {
		_dataCache.add(blockHandling, true);
		var failRequestFunc = function () {
			_dataCache.remove(blockHandling);
		};
		var doneRequestFunc = function () {
			_dataCache.remove(blockHandling);
		};
		var data = {'port': 'feed', 'type': 'status', 'action': action, 'things': [{'type': thingsType, 'id': thingsId}]};
		$.ajax({
			url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
			type: "GET",
			dataType: "json",
	...
	if (isset(statusArr['menu']) && isset(statusArr['menu']['status']) && countArr(statusArr['menu']['status']) > 0) {
		...
		for (var typeThis in statusArr['menu']['status']) {
			...
			if (typeThis == "block" && valueThis === true) {
				...
				if (valueThis === true) {
					code['menu'] = code['menu'] + " <div class='nowrap' menu-type='unfollow' status-action-follow follow='true'> <i></i> <span><?php print $_language->text('unfollow', 'strtolower'); ?></span> </div> ";
				}else if (valueThis === false) {
					code['menu'] = code['menu'] + " <div class='nowrap' menu-type='follow' status-action-follow follow='false'> <i></i> <span><?php print $_language->text('follow', 'strtolower'); ?></span> </div> ";
				}
...
var feed_status_favoriteRequest = function (object) {
	...
	if (_dataCache.exists(favoriteHandling) === false) {
		_dataCache.add(favoriteHandling, true);
		var failRequestFunc = function () {
			_dataCache.remove(favoriteHandling);
		};
		var doneRequestFunc = function () {
			_dataCache.remove(favoriteHandling);
		};
		var data = {'port': 'feed', 'type': 'status', 'action': action, 'inside': [{'type': insideType, 'id': insideId}]};
		$.ajax({
			url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
			type: "GET",
			dataType: "json",
...
var feed_status_favorite = function () {
	...
	statusButtonFavorite.bind("click", favoriteStatusFunc);
};
var feed_status_followRequest = function (object) {
	...
	if (_dataCache.exists(followHandling) === false) {
		_dataCache.add(followHandling, true);
		var failRequestFunc = function () {
			_dataCache.remove(followHandling);
		};
		var doneRequestFunc = function () {
			_dataCache.remove(followHandling);
		};
