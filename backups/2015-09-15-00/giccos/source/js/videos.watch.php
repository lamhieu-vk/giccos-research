...
$(document).ready(function () {
	...
	$("#leftBox > .tab#infoTab > .box.stats > .content > .rows > .value.views > .statsBox").each(function () {
		if (_dataCache.exists('videos_info_stats_views_dataset') === true) {
			var data = _dataCache.get('videos_info_stats_views_dataset');
		}else {
			return false;
		}
	...
	$("#leftBox > .tab#infoTab > .box.stats > .content > .rows > .value.mention > .statsBox").each(function () {
		if (_dataCache.exists('videos_info_stats_mention_dataset') === true) {
			var data = _dataCache.get('videos_info_stats_mention_dataset');
		}else {
			return false;
		}
		var lineBar = $(this).get(0).getContext("2d");
