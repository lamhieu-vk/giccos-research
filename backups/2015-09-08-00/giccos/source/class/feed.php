...
Class feed {
	...
	function status_list ($object) {
		...
		$listQuery['block'] = "AND `status`.`public` = '1' AND (`status`.`id` NOT IN (SELECT `status.id` FROM `status_block` WHERE `guy.type` = '".$guyType."' AND `guy.id` = '".$guyId."'))";
		...
		if (isset($except['logs']) && is_array($except['logs']) && count($except['logs']) > 0) {
			foreach ($except['logs'] as $exceptLogsThis) {
				$except['logs'][] = intval($_tool->convertDatabaseString($exceptLogsThis));
			}
			$listQuery['except']['logs'] = "`logsId` NOT IN ('" . implode($except['logs'], "', '") . "')";
		}
		if (isset($except['status']) && is_array($except['status']) && count($except['status']) > 0) {
			foreach ($except['status'] as $exceptStatusThis) {
				$except['status'][] = intval($_tool->convertDatabaseString($exceptStatusThis));
			}
			$listQuery['except']['status'] = "`statusId` NOT IN ('" . implode($except['status'], "', '") . "')";
		}
		if ($listQuery['except']['logs'] != null && $listQuery['except']['status'] != null) {
			$listQuery['except'] = "AND (".$listQuery['except']['logs']." OR ".$listQuery['except']['status'].")";
		}else if ($listQuery['except']['logs'] != null || $listQuery['except']['status'] != null) {
			if ($listQuery['except']['logs'] != null) {
				$listQuery['except'] = "AND (".$listQuery['except']['logs'].")";
			}else if ($listQuery['except']['status'] != null) {
		...
		if ($type == "general") {
			if ($guyType == "user") {
				$listQuery['source'] = "((`status`.`author.type`, `status`.`author.id`) IN (SELECT `users_follow`.`guy.type`, `users_follow`.`guy.id` FROM `users_follow` WHERE `users_follow`.`user.id` = '".$guyId."') OR (`logs_actions`.`guy.type`, `logs_actions`.`guy.id`) IN (SELECT `users_follow`.`guy.type`, `users_follow`.`guy.id` FROM `users_follow` WHERE `users_follow`.`user.id` = '".$guyId."'))";
			}else if ($guyType == "page") {
				$listQuery['source'] = "(`status`.`author.type`, `status`.`author.id`) IN (SELECT `pages_follow`.`guy.type`, `pages_follow`.`guy.id` FROM `pages_follow` WHERE `pages_follow`.`page.id` = '".$guyId."')";
			}
			if (!isset($listQuery['source']) || !is_string($listQuery['source'])) {
				return array("return" => false, "reason" => "2");
			}else {
				$listQuery['source'] = "AND (".$listQuery['source'].")";
			}
		}else if ($type == "time") {
			...
			if ($timeValueOne != null && $timeValueTwo != null) {
				$listQuery['source'] = "(`status`.`time` BETWEEN '".$timeValueOne."' AND '".$timeValueTwo."')";
			}else {
				$listQuery['source'] = "(`status`.`time` >= '".$timeValueOne."')";
			}
			if (!isset($listQuery['source']) || !is_string($listQuery['source'])) {
				return array("return" => false, "reason" => "2");
			}else {
			...
			if (!isset($listQuery['tag']) || !is_string($listQuery['tag'])) {
				...
				$listQuery['source'] = "AND (".$listQuery['tag'].")";
			}
			...
			if (!isset($listQuery['nearby']) || !is_string($listQuery['nearby'])) {
				...
				$listQuery['source'] = "AND (".$listQuery['nearby'].")";
			}
		}else {
		...
		if (isset($listQuery['source']) && is_string($listQuery['source'])) {
			if (in_array($method, ["auto", "smart"])) {
				...
				$listQuery['code'] = "SELECT DISTINCT `status`.`id` AS `statusId`, MAX(`logs_actions`.`id`) AS `logsId` FROM `status` LEFT JOIN `logs_actions` ON `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['logs']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`type` IN ('share', 'edit') AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`things.id` = `status`.`id`) OR (`logs_actions`.`type` IN ('add') AND `logs_actions`.`obj.type` = 'status' AND `logs_actions`.`obj.id` = `status`.`id`)) ".$listQuery['source']." WHERE `status`.`id` IS NOT NULL AND `logs_actions`.`id` IS NOT NULL GROUP BY `statusId` ORDER BY `logsId` ".$listQuery['order']." ".$listQuery['limit'].";";
			}else {
	...
	function status_printcode ($object) {
		...
		if (isset($statusArr['places'], $statusArr['places']['name']) && is_array($statusArr['places']) && count($statusArr['places']) > 0) { 
			if (isset($statusArr['places']['id'], $statusArr['places']['address'])) { 
				$code['places'] = "<div class='places nowrap _tt _tt-dbl s' title='".$_language->text('checkin_at', 'strtolower')." ".$statusArr['places']['name']."' dblclick='".json_encode(array("type" => "notification", "data" => array("type" => "notification", "title" => $_language->text('information_about_places', 'ucfirst'), "description" => array("type" => "info::places", "data" => $statusArr['places']), "timeout" => 6000 )))."'> <i></i> </div>"; 
			}else {
		...
		if (isset($statusArr['menu'], $statusArr['menu']['status']) && is_array($statusArr['menu']) && count($statusArr['menu']) > 0) {
			...
			foreach ($statusArr['menu']['status'] as $typeThis => $valueThis) {
				if ($typeThis == "block" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='block'> <i></i> <span>".$_language->text('block', 'strtolower')."</span> </div>";
				}else if ($typeThis == "report" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='report'> <i></i> <span>".$_language->text('report', 'strtolower')."</span> </div>";
				}else if ($typeThis == "edit" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='edit'> <i></i> <span>".$_language->text('edit', 'strtolower')."</span> </div>";
				}else if ($typeThis == "delete" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='delete'> <i></i> <span>".$_language->text('delete', 'strtolower')."</span> </div>";
				}else if ($typeThis == "follow") {
					if ($valueThis == true) {
						$code['menu'] .= "<div class='nowrap' menu-type='unfollow'> <i></i> <span>".$_language->text('unfollow', 'ucstrtolowerfirst')."</span> </div>";
					}else {
						$code['menu'] .= "<div class='nowrap' menu-type='unfollow'> <i></i> <span>".$_language->text('unfollow', 'strtolower')."</span> </div>";
					}
				}else if ($typeThis == "popup" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='popup'> <i></i> <span>".$_language->text('popup', 'strtolower')."</span> </div>";
				}else if ($typeThis == "link" && $valueThis == true) {
					$code['menu'] .= "<div class='nowrap' menu-type='link'> <i></i> <span><a href='".$statusArr['info']['link']."'>".$_language->text('link', 'strtolower')."</a></span> </div>";
				}
			}
		...
		$code['qrcode'] = "<div class='qrcode _tt-bk e' title='".$_language->text('view_qrcode', 'normal')."' qrcode-image='".$statusArr['info']['qrcode']."'> <div class='button'><i></i></div> </div>";
		...
		<div class='".$statusClassname."' status-id='".$statusArr['info']['id']."' logs-id='".$statusArr['info']['logs']."' status='".json_encode($statusArr['infoshow'])."'>
			...
			<div class='header'>
				...
				<div class='info'> ".$code['private_view']." ".$code['private_comment']." ".$code['private_share']." ".$code['edit']." ".$code['places']." ".$code['mood']." ".$code['usertag']." </div>
			...
			foreach ($rowsArr as $key => $rowsArrThis) {
				if ($key == "content") {
					...
					if ($statusFetch['content'] == $rowsArrThis) {
						...
						//.
						mysqli_query($db, "INSERT INTO `status_edit` (`id`, `status.id`, `time`, `content`) VALUES (NULL, '".$statusId."', '".$statusFetch['time']."', '".$rowsArrThis."');");
						$isChanged++;
					}
