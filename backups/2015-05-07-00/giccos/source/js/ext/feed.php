...
function feed_status_stats (object, callback) {
	...
	var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'status', 'action': 'stats', 'obj': {'id': statusId, 'rows': statusRows, 'type': statusType}};
...
function feed_comment_showmore () {
	var buttonClickFunc = function () {
		var button = $(this),
	...
	var button = $(".status.inFeeds[status][status-id]").find("[status-comment-load]");
	button.bind('click', buttonClickFunc);
}
function feed_comment_load (options, callback) {
	if (!isset(callback)) {
		var callback = null;
	}
	...
	if (Object.keys(options).length == 0) {
		...
		var data = {'port': 'feed', 'token': '<?php print $g_client['token']['action']['feed']; ?>', 'type': 'comment', 'action': 'load', 'options': options};
						...
						for (var i in data['data']) {
							...
							if (isset(thisRow['return']) && isset(thisRow['data']) && thisRow['return'] === true) {
								for (var x in options) {
									...
									if (thisOptions['inside_type'] == thisRow['inside_type'] && thisOptions['inside_id'] == thisRow['inside_id']) {
...
function feed_comment_push (options, comment) {
	console.log(comment);
	var count = 0,
	...
	if (isset(comment['reply']['count']) && comment['reply']['count'] > 3) {
		var c_comment_showmore = "<div class='lines statistic' status-comment-lines-stats comment-id='"+comment['info']['id']+"'> <div class='showmore' status-comment-reply-load> <i class='icon'></i> <span class='text'><?php print $_language->text('show_more_reply', 'ucfirst'); ?></span> </div> <div class='info'> <i class='icon'></i> <span class='count' status-comment-stats-reply>"+comment['reply']['count']+"</span> <span class='text'><?php print $_language->text('reply(s)', 'strtolower'); ?></span> </div> </div>";
	}else {
