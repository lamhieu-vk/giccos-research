...
Class photos {
	...
	function cache ($object = array()) {
		...
		//.
		$userId = isset($object['user'], $object['user']['id']) && (is_string($object['user']['id']) || is_numeric($object['user']['id'])) ? $object['user']['id'] : null;
		...
		if (in_array($action, ["add"])) {
			...
			$fileArr['source'] = $fileArr['tmp_name'];
			$infoArr = $_tool->readFileInfo($fileArr['source'], 'image');
			if (!$infoArr) {
				return array("return" => false, "reason" => "");
			}
			$clientToken = $_client->client(true);
			$fileArr['token'] = hash('crc32', '('.$guyType.'_'.$guyId.')::'.md5_file($fileArr['source']).'::'.time().'::'.rand());
			$fileArr['secret'] = hash('crc32', $fileArr['token']);
			...
			if (isset($clientToken, $fileArr['token'], $fileArr['secret'], $fileArr['nameraw'], $fileArr['name'], $fileArr['mime'], $fileArr['size'])) {
				...
				}else if (!isset($fileArr['source']) || $fileArr['source'] == null) {
				...
				$fileUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $fileArr['source'], "mime" => $fileArr['type'], "name" => $fileArr['name'], "strip" => false));
				if (isset($fileUpload['return'], $fileUpload['data']) && $fileUpload['return'] == true) {
					$fileArr['path'] = $fileUpload['data']['display'];
					$fileArr['source'] = $fileUpload['data']['source'];
				}else {
			...
			if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['data'], $getMediaFile['data'][0]) && $getMediaFile['return'] == true) {
				$fileArr['source'] = $getMediaFile['data'][0]['source'];
			...
			if (!isset($fileArr['source'])) {
				...
				if (in_array($_tool->mime_content_type($fileArr['mime'], true), $_parameter->get('exif_read_data_allow', true))) {
					$fileArr['info'] = $this->info('read', $fileArr['source'], 'photos');
				}else {
					$fileArr['info'] = $this->info('read', null, 'photos');
					$fileArr['info']['get'] = getimagesize($fileArr['source']);
				...
				$fileArr['info'] = $_tool->readFileInfo($fileArr['source'], 'image');
				...
				$img = photosEditor::factory($fileArr['source'], true);
				...
				if ($fileArr['info']['size.width'] > $_parameter->get('photos_size_original_w')) {
					$fileArrUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $fileArr['source'], "mime" => $fileArr['mime'], "name" => $fileArr['nameraw'], "strip" => false));
				...
				if ($multiple == true && $fileArr['info']['size.width'] > $_parameter->get('photos_size_large_w')) {
					...
					$fileArrUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $fileArr['source'], "mime" => $fileArr['mime'], "name" => $fileArr['nameraw']));
					if (isset($fileArrUpload['return'], $fileArrUpload['data']) && $fileArrUpload['return'] == true) {
				...
				if ($multiple == true && $fileArr['info']['size.width'] > $_parameter->get('photos_size_small_w')) {
					...
					$fileArrUpload = $_storage->upload(array("format" => "image", "move" => "rename", "file" => $fileArr['source'], "mime" => $fileArr['mime'], "name" => $fileArr['nameraw']));
					if (isset($fileArrUpload['return'], $fileArrUpload['data']) && $fileArrUpload['return'] == true) {
