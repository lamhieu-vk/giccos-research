...
if (isset($boxSuggestRequire) && is_array($boxSuggestRequire)) {
	...
	if (isset($boxSuggestRequire['weather'])) {
		...
		if ($weatherLocation == null) {
			...
			if (1 + 1 == 2) {
				...
				foreach ($weatherLocationArr as $key => $locationThisWord) {
					if (!preg_match("/([0-9]+)/", $locationThisWord)) {
						continue;
					}
					if (!isset($locationText) || $locationText == null) {
						$locationText = $locationThisWord;
		...
		if (!isset($weatherLocation) || $weatherLocation == null) {
			$weatherContentCode = "
			<div class='null'>
				<span>".$_language->text('weather_not_available', 'strtolower')."</span>
			</div>
			";
		}else {
			...
			if (isset($getWeather, $getWeather['return'], $getWeather['data'], $getWeather['data']['forecast']) && $getWeather['return'] == true && count($getWeather['data']['forecast']) > 0) {
				...
				$weatherContentCode = "
				<div class='rows places'><i class='_tt-bk e' title='".$_language->text('location', 'strtolower').': '.$dataWeather['location']['latitude'].','.$dataWeather['location']['longitude']."'></i><a class='_tt-bk w' title='".$dataWeather['location']['country']."' href='#' target='_blank'>".$dataWeather['location']['city']."</a></div>
	...
	if (isset($boxSuggestRequire['friends'])) {
		$friendsList = isset($boxSuggestRequire['friends']['list']) ? $boxSuggestRequire['friends']['list'] : null;
		if ($friendsList == null) {
			if (isset($g_user, $g_user['suggest'], $g_user['suggest']['friends'])) {
				if (isset($g_user['suggest']['friends']['list']) && count($g_user['suggest']['friends']['list']) > 0) {
					$friendsContentCode = null;
					foreach ($g_user['suggest']['friends']['list'] as $valueUserId => $countUserId) {
						$getUserProfileOptions = array("action" => "get", "rows" => ["fullname", "username", "link", "avatar.small", "verified"], "label" => "id", "value" => $valueUserId, "limit" => "LIMIT 1");
						$getUserProfile = $_user->profile($getUserProfileOptions);
						if (isset($getUserProfile, $getUserProfile['return'], $getUserProfile['data']) && $getUserProfile['return'] == true) {
							$thisUserProfile = $getUserProfile['data'][0];
							unset($getUserProfile);
							$friendsContentCode .= "
							<div class='rows'>
								<div class='action td'>
									<i class='button add_request _tt-bk e' title='".$_language->text('send_friend_request', 'strtolower')."'></i>
								</div>
								<div class='thumbnail td'>
									<a href='#' target='_blank'><img class='img' src='".$thisUserProfile['avatar.small']."' alt='".$thisUserProfile['fullname']." (".$thisUserProfile['username'].")'></a>
								</div>
								<div class='info td'>
									<div class='name text nowrap'> <a class='_tt-bk w' title='".$thisUserProfile['fullname']."' href='#' target='_blank'>".$thisUserProfile['fullname']."</a> </div>
									<div class='tag text nowrap'> <a class='_tt-bk w' title='".$thisUserProfile['username']."' href='#' target='_blank'>".$thisUserProfile['username']."</a> </div>
								</div>
							</div>
							";
						}
		...
		if (!isset($friendsContentCode) || $friendsContentCode == null) {
			$friendsContentCode = "
			<div class='null'>
				<span>".$_language->text('people_know_notfound', 'strtolower')."</span>
			</div>
			";
		}
		$boxSuggestCode['friends'] = "
		<div class='box friends boxGrid'>
			<div class='title'>
				<div class='rows'><span>".$_language->text('you_may_know', 'ucfirst')."</span></div>
			</div>
			<div class='content'>".$friendsContentCode."</div>
		</div>
		";
	}
}
