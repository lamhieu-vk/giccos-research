...
_g['ajaxrequest'].use = function (object, callback) {
	...
	if (typeof requestDataCache === "undefined") {
		requestDataCache = {};
	}
	dataLoop: for (var label in requestData) {
		if (inArray(label.toLowerCase(), ["port", "token", "type", "action"])) {
			// requestData[label] = requestData[label];
		}else if (inArray(label.toLowerCase(), ["obj", "objrequest"])) {
			if (typeof requestData[label] !== "object") {
				continue dataLoop;
			}
			for (var sub in requestData[label]) {
				requestDataCache[sub] = requestData[label][sub];
			}
		}else {
			requestDataCache[label] = requestData[label];
			delete requestData[label];
		}
	}
	if (typeof requestDataCache !== "undefined") {
		requestData[requestVerify] = requestDataCache;
	}
	if (typeof requestData['token'] === "undefined" && typeof requestData['port'] !== "undefined") {
		<?php 
			...
			foreach ($g_client['token']['action'] as $key => $tokenActionThis) {
				if (!in_array($key, ["key"])) {
					if ($listReplaceToken == null) {
						$listReplaceToken .= "if (requestData['port'] == '".$key."') { requestData['token'] = '".$tokenActionThis."';  }";
					}else {
						$listReplaceToken .= "else if (requestData['port'] == '".$key."') { requestData['token'] = '".$tokenActionThis."';  }";
					}
				}
			}
	...
	requestDelayCallback = function (requestDelay, requestAjax, requestData) {
		setTimeout(function () {
			return _g['ajaxrequest'].send(requestAjax, requestData);
		}, requestDelay);
	};
...
_g['ajaxify'].request = function (url, cb) {
	...
	var requestAction = _g['ajaxrequest']({
		'type': 'POST',
		'url': '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
		'data': {'port': 'ajaxify', 'url': url},
		'datatype': 'json',
		'done': requestDoneFunc,
