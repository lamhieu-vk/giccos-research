...
var feed_status_blockRequest = function (object) {
	...
	if (_g['cache'].exists(blockHandling) === false) {
		_g['cache'].add(blockHandling, true);
		var failRequestFunc = function () {
			_g['cache'].remove(blockHandling);
		};
		var doneRequestFunc = function () {
			_g['cache'].remove(blockHandling);
		};
		...
		buttonMenu.bind('click', function() {
			...
			if (button.attr('handing') != "true") {
				...
				if (type == "delete") {
					...
					data = {'port': 'feed', 'type': 'status', 'action': type, 'id': [info['id']]};
					_g['xhr']({
						type: 'GET',
						url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
						data: data,
...
var feed_status_favoriteRequest = function (object) {
	...
	if (_g['cache'].exists(favoriteHandling) === false) {
		_g['cache'].add(favoriteHandling, true);
		errorRequestFunc = doneRequestFunc = function () {
			_g['cache'].remove(favoriteHandling);
		};
...
var feed_status_followRequest = function (object) {
	...
	if (_g['cache'].exists(followHandling) === false) {
		_g['cache'].add(followHandling, true);
		errorRequestFunc = doneRequestFunc = function () {
			_g['cache'].remove(followHandling);
		};
		data = {'port': 'feed', 'type': 'status', 'action': action, 'inside': [{'type': insideType, 'id': insideId}]};
		_g['xhr']({
			type: 'GET',
...
var feed_comment_addfromComment = function () {
	...
	buttonCommentReply.bind('click', function(){
		...
		if (commentReplyThis.length === 0) {
			var c_inputType = "<div class='lines reply typing _h' status-comment-lines-reply comment-id='"+commentThis.attr('comment-id')+"'> <i></i> <textarea type='text' placeholder='<?php print $_language->text('write_a_reply', 'strtolower'); ?>' status-comment-action-reply></textarea> </div>";
...
var feed_comment_showmore = function () {
	...
	var buttonLoadComment = $(".status.inFeeds[status][status-id]").find("[status-comment-showmore]");
	...
	insideThis.each(function(i) {
		...
		if (insideType == "status") {
			...
			if (sort == ">" && lengthCommentLines > 0 && lengthComment === 0) {
				...
				commentContent.find("[status-comment-null]").fadeOut(function() {
					$(this).remove();
				});
			}
			var thisComment = status.find("[comment-id='"+commentArr['info']['id']+"']");
