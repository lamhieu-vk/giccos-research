...
Class image {
	...
	function comment ($object = array()) {
		...
		if (in_array($action, ["add"])) {
			...
			if ($listOrder == "id") {
				$listCommentRequest = "SELECT `id` FROM `image_comment` WHERE `removed` = '0' AND `id` ".$listSort." '".$_tool->parseDBString($commentId)."' AND (`inside.type` = '".$_tool->parseDBString($insideArr['type'])."' AND `inside.id` = '".$_tool->parseDBString($insideArr['id'])."') ".(preg_match("/\>/", $listSort) ? "ORDER BY `id` DESC" : "ORDER BY `id` ASC")." ".$listNumber." ;";
