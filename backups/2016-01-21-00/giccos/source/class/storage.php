...
Class storage {
	...
	function upload ($object = array()) {
		...
		$storage['nameraw'] = basename($storage['source']);
		...
		if (!$insertStorageQuery) {
			...
			if (!$puts) {
				...
				if ($format == "image") {
					...
					$updateFileHashRequest = "UPDATE `storage_image` SET `updated` = '".$storage['updated']."', `hash.difference` = '".$storage['hash.difference']."', `hash.perceptual` = '".$storage['hash.perceptual']."', `hash.average` = '".$storage['hash.average']."', `hash.profile` = '".$storage['hash.profile']."' WHERE `id` = '".$storage['id']."' ;";
