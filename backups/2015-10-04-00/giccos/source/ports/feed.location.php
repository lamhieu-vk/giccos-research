			...
			if (isset($mapsPlaces['return'], $mapsPlaces['data']) && $mapsPlaces['return'] == true) {
				...
				if (isset($mapsPlacesInfo) && is_array($mapsPlacesInfo) && count($mapsPlacesInfo) > 0) {
					...
					<div class='li'> 
						...
						<div class='thumbnail' style='background-image: url(".str_replace("480x480", "580x580", $mapsPlacesInfo['thumbnail']).");'>
							<img src='".str_replace("480x480", "580x580", $mapsPlacesInfo['thumbnail'])."' alt='".$_language->text('maps_static_at')." ".$mapsPlacesInfo['name']." - ".$mapsPlacesInfo['address']." (".$mapsPlacesInfo['location'].")'>
						</div> 
					</div>
					";
				}
