<?php
/**
 * (internal use only, this is unofficial api)
 *
...
class translator {
	 ...
	 */
	public function translateText($text, $toLanguage = "vi", $fromLanguage = null, $translit = false) {
		if (empty($this->_errors)) {
			...
			for ($i = 0; $i < strlen($text); $i += 1000) {
				...
				$result .= $this->_parseGoogleResponse($response, $translit);
			}
			return $result;
		}
		return false;
	 ...
	 */
	public function translateArray($array, $fromLanguage = "en",
			$toLanguage = "ru", $translit = false) {
		if (empty($this->_errors)) {
			$text = implode("[<#>]", $array);
	...
	const UserAgent = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/534.24 (KHTML, like Gecko) Chrome/11.0.696.71 Safari/534.24";
	...
	private function _parseGoogleResponse($response, $translit = false) {
		if (empty($this->_errors)) {
			$result = "";
