...
Class language {
	...
	function getCode ($code = "en") {
		$getCodeRequest = "SELECT `code` FROM `languages_codes` WHERE `country` = '".$code."' LIMIT 1;";
		$getCodeQuery = mysqli_query($this->class['_db']->port('beta'), $getCodeRequest);
		if (!$getCodeQuery) {
			return array("return" => false, "reason" => "");
		}
		if (mysqli_num_rows($getCodeQuery) == 0) {
			return array("return" => true, "code" => "en");
		}else {
			$languageCode = mysqli_fetch_assoc($getCodeQuery)['code'];
			return array("return" => true, "code" => $languageCode);
		}
	}
	...
	function text ($code = null, $type = "normal", $short = false) {
		//. $this->data = $_SESSION["client"]['language']['data'];
		if (isset($this->data[$this->language]) && is_array($this->data[$this->language]) && count($this->data[$this->language]) > 0) {
			foreach ($this->data[$this->language] as $key => $value) {
				if (strtolower($value['code']) == strtolower($code) && $value['short'] == $short) {
					$fetch = $value;
					break;
				}else {
					continue;
			...
			$sql_code = strtolower($code);
			if($short == true){
				$sql_port = "AND `short` = 'true'";
			}else {
				$sql_port = "AND `short` = 'false'";
			}
			$sql = "SELECT * FROM `languages_values` WHERE `language` = '{$this->language}' AND `code` = '{$sql_code}' {$sql_port} LIMIT 1;";
			$query = mysqli_query($this->class['_db']->port('beta'), $sql);
			if (!$query) {
				return null;
			}
			if (mysqli_num_rows($query) > 0) {
				...
				$sql = "SELECT * FROM `languages_values` WHERE `language` = 'en' AND `code` = '{$sql_code}' {$sql_port} LIMIT 1;";
				$query = mysqli_query($this->class['_db']->port('beta'), $sql);
				$fetch = mysqli_fetch_assoc($query);
			}
		}
		if (isset($fetch) && is_array($fetch)) {
			...
			if($type == "normal"){
				...
				$text = ucfirst($text);
				$text = preg_replace_callback('/((\.|[\n]+|\\n)([\s]+)?)([a-zA-Z]+)/', function ($matchs) { return $matchs[2].$matchs[3].ucfirst($matchs[4]); }, $text);
