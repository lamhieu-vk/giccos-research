...
Class tool {
	...
	function ratioHammingDistance ($firstString = null, $secondString = null) {
	...
	function hamdist ($firstString, $secondString) {
		if (extension_loaded('gmp')) {
			//. $dh = gmp_hamdist(sprintf("0x%x", $firstString), sprintf("0x%x", $secondString));
			$dh = gmp_hamdist(sprintf($firstString), sprintf($secondString));
		}else {
			$dh = 0;
			for ($i = 0; $i < 64; $i++) {
				$k = (1 << $i);
	...
	function convertGTMTimezone ($gtm) {
		$timezoneArr = array(
		...
		if (isset($timezoneArr[$gtm])) {
			return $timezoneArr[$gtm];
		}
		return null;
	}
	function isCachedPage ($url = null) {
		...
		//.
		$urlToken = isset($url) && is_string($url) && $url != null ? hash('crc32', $url) : hash('crc32', $_client->uri());
		if (isset($_COOKIE[$urlToken]) && is_string($_COOKIE[$urlToken])) {
			$pageCached = $_COOKIE[$urlToken];
			$pageCached = json_decode($this->hash('decode', $pageCached, $urlToken), true);
			if (isset($pageCached, $pageCached['type'], $pageCached['length'], $pageCached['etag'], $pageCached['modified'])) {
				header("Content-Type: ". $pageCached['type']);
				header("Cache-Control: max-age=". $pageCached['expire']);
				header("Content-Length: ". $pageCached['length']);
				header("Last-Modified: ".gmdate('D, d M Y H:i:s', $pageCached['modified'])." GMT");
				header("ETag: ". $pageCached['etag']);
    ...
    function getWeather ($object) {
    	...
    	$server = isset($object['server']) && (is_string($object['server']) || is_numeric($object['server'])) ? $object['server'] : "yahoo";
    	$reset = isset($object['reset']) && is_bool($object['reset']) ? $object['reset'] : false;
    	if (in_array($server, ["yahoo"])) {
    		$units = (isset($object['units']) && is_string($object['units']) && preg_match("/(c|f)/i", $object['units'])) ? strtolower($object['units']) : "f";
    		$locationName = (isset($object['location']) && is_string($object['location'])) ? $object['location'] : null;
    		if ($locationName == null) {
    			return array("return" => false, "reason" => "");
    		}else {
    			$locationHash = strtolower(preg_replace("/([^a-zA-Z0-9]+)/", "_", $locationName).'::'.$units); // $locationHash = hash('crc32', strtolower(preg_replace("/([^a-zA-Z0-9]+)/", "_", $locationName)));
    			$busyListArr = array();
    			if (isset($_SESSION["cache"], $_SESSION["cache"]['weather'], $_SESSION["cache"]['weather']['busylist']) && is_array($_SESSION["cache"]['weather']['busylist'])) {
    				$busyListArr = $_SESSION["cache"]['weather']['busylist'];
    			}
    			foreach ($busyListArr as $keyBusyList => $valueBusyList) {
    				if ($keyBusyList == $locationHash) {
    					if ($valueBusyList > time()) {
    						return array("return" => false, "reason" => "");
    						break;
    					}else {
    						if (isset($_SESSION["cache"]['weather']['busylist'][$keyBusyList])) {
    							unset($_SESSION["cache"]['weather']['busylist'][$keyBusyList]);
    						}
    					}
    				}
    			}
    			if (!isset($reset) || (isset($reset) && $reset == false)) {
	    			if (isset($_SESSION["cache"]['weather'][$locationHash])) {
	    				$useCaching = true;
	    				$weatherCache = $_SESSION["cache"]['weather'][$locationHash];
	    				if (is_array($weatherCache) && count($weatherCache) > 0) {
	    					if (isset($weatherCache['time']) && is_numeric($weatherCache['time'])) {
	    						if ($weatherCache['time'] + $_parameter->get('weather_cache_timeout') <= time()) {
	    							$useCaching = false;
	    							unset($_SESSION["cache"]['weather'][$locationHash]);
	    						}
	    					}else {
	    						$useCaching = false;
	    						unset($_SESSION["cache"]['weather'][$locationHash]);
	    				...
	    				if (isset($useCaching, $weatherCache) && $useCaching == true) {
	    					return array("return" => true, "data" => $weatherCache);
	    				}
	    			}
	    		}
    		}
    		$urlRequest = "SELECT * FROM weather.forecast WHERE u = '".$units."' AND woeid IN (SELECT woeid FROM geo.places(1) WHERE text = '".$locationName."')";
			$urlRequest = "http://query.yahooapis.com/v1/public/yql". "?q=" .urlencode($urlRequest). "&format=json&diagnostics=true";
			$curlAction = $this->curl($urlRequest, 0);
			if (isset($curlAction['return'], $curlAction['data']) && $curlAction['return'] == true) {
				$data = $curlAction['data'];
				...
				if (isset($data['query']['count']) && $data['query']['count'] == 0) {
					...
					$dataArr = array();
					if (isset($data['query'], $data['query']['results'], $data['query']['results']['channel']) && count($data['query']['results']['channel']) > 0) {
						$dataArr = $data['query']['results']['channel'];
						$dataArr['hash'] = hash('crc32', strtolower($dataArr['language'].'::'.serialize($dataArr['location']['city'])));
						$dataArr['time'] = time(); 
						if (isset($dataArr['item'])) {
							$dataArr['forecast'] = $dataArr['item'];
							unset($dataArr['item']);
						}
						if (isset($dataArr['forecast'], $dataArr['forecast']['lat'])) {
							$dataArr['location']['latitude'] = $dataArr['forecast']['lat'];
							unset($dataArr['forecast']['lat']);
						}
						if (isset($dataArr['forecast'], $dataArr['forecast']['long'])) {
							$dataArr['location']['longitude'] = $dataArr['forecast']['long'];
							unset($dataArr['forecast']['long']);
						}
						if (isset($dataArr['astronomy'], $dataArr['astronomy']['sunrise'])) {
							$dataArr['astronomy']['sunrise'] = strtotime($dataArr['astronomy']['sunrise']);
						}
						if (isset($dataArr['astronomy'], $dataArr['astronomy']['sunset'])) {
							$dataArr['astronomy']['sunset'] = strtotime($dataArr['astronomy']['sunset']);
						}
						if (isset($dataArr['forecast'], $dataArr['forecast']['forecast'])) {
							$dataArr['forecast'] = $dataArr['forecast']['forecast'];
						}
						if (isset($dataArr['forecast']) && is_array($dataArr['forecast']) && count($dataArr) > 0) {
							foreach ($dataArr['forecast'] as $i => $dataThis) {
								if (isset($dataThis['code']) && (is_string($dataThis['code']) || is_numeric($dataThis['code']))) {
									$dataArr['forecast'][$i]['code'] = $dataThis['code'] = strtolower(preg_replace("/(\s)/", "_", preg_replace("/(\)|\()/", "", $this->statusWeatherCode($dataThis['code']))));
									$dataArr['forecast'][$i]['text'] = $dataThis['text'] = $_language->text($dataThis['code'], 'strtolower');
								}
								if (isset($dataThis['day']) && is_string($dataThis['day'])) {
									$dataArr['forecast'][$i]['day'] = $dataThis['day'] = $_language->text($dataThis['day'], 'strtolower');
								}
								if (isset($dataThis['date']) && is_string($dataThis['date'])) {
									$dataArr['forecast'][$i]['date'] = $dataThis['date'] = strtotime($dataThis['date']);
								}
							}
						}
						$dataArr['request'] = $urlRequest;
						$_SESSION["cache"]['weather'][$locationHash] = $dataArr;
						return array("return" => true, "data" => $dataArr);
					}else {
						if (!isset($_SESSION["cache"]['weather']['busylist'])) {
							$_SESSION["cache"]['weather']['busylist'] = array();
						}
						$_SESSION["cache"]['weather']['busylist'][$locationName] = time() + $_parameter->get('weather_busy_timeout');
						return array("return" => false, "reason" => "");
					}
				}else {
					return array("return" => false, "reason" => "");
	...
	function curl ($url, $timeout = 10, $options = null) {
		...
		if (isset($options['referer']) && $options['referer'] == true) {
			curl_setopt($ch, CURLOPT_REFERER, $_client->fullurl());
	...
	function links ($links = null, $local = false) {
		$_client = $this->class['_client'];
		//.
		$home = $_client->protocol()."://localhost/giccos/";
		if ($links == "::host") {
			return "localhost";
	...
	function parseLink ($text = null) {
		...
		$text = preg_replace($_parameter->get('regex_usertag'), '<a class="gUsertag" usertag="$2" href="'.$this->links('wall/$2').'" target="_blank">@$2</a>', $text);
	    $text = preg_replace($_parameter->get('regex_hashtag'), '<a class="gHashtag" hashtag="$2" href="'.$this->links('feed/tags/$2').'">#$2</a>', $text);
	    return $text;
	}
	...
	function agoDatetime ($timestamp = null, $type = "ago", $length = false) { 
		...
		if ($type == "" || $type == "ago") {
			...
			if ($seconds <= 60) {
				$timeText = isset($timeType) && $timeType == "to" ? $_language->text('a_few_seconds', 'strtolower', $length) : $_language->text('a_few_seconds', 'strtolower', $length);
			}else if ($minutes < 60) {
				$timeText = $minutes.$stringSpace;
				$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('minutes_left', 'strtolower', $length) : $_language->text('minutes_ago', 'strtolower', $length);
			}else if ($hours < 24 ) {
				$timeText = $hours.$stringSpace;
				$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('hours_left', 'strtolower', $length) : $_language->text('hours_ago', 'strtolower', $length);
			}else if ($days < 7) {
				if ($days == 1 && !$length) {
					$timeText = date("H:i", $timestamp).','.$stringSpace;
					$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('tomorrow', 'strtolower', $length) : $_language->text('yesterday', 'strtolower', $length);
				}else {
					$timeText = $days.$stringSpace;
					$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('days_left', 'strtolower', $length) : $_language->text('days_ago', 'strtolower', $length);
				}
			}else if ($weeks < 4) {
				if ($weeks == 1 && !$length) {
					$timeText = date("H:i", $timestamp).','.$stringSpace.strtolower($this->convertWeekday(date("w", $timestamp), $length));
				}else {
					$timeText = $weeks.$stringSpace;
					$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('weeks_left', 'strtolower', $length) : $_language->text('weeks_ago', 'strtolower', $length);
				}
			}else if ($months < 12) {
				$timeText = $months.$stringSpace;
				$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('months_left', 'strtolower', $length) : $_language->text('months_ago', 'strtolower', $length);
			}else {
				$timeText = $years.$stringSpace;
				$timeText .= isset($timeType) && $timeType == "to" ? $_language->text('years_left', 'strtolower', $length) : $_language->text('years_ago', 'strtolower', $length);
			}
		}else if ($type == "tip") {
			$timeDifference = abs(time() - $timestamp);
			$years = round($timeDifference / 29030400); 
			if ($years > 0) {
				$timeText = date("H:i, d/n/Y", $timestamp);
			}else {
				$timeText = date("H:i, d/n", $timestamp);
			}
		}
		return $timeText;
	}
    ...
    function readFileInfo ($file, $format) {
    	if ($file == null) {
    		return false;
    	}
		if (in_array($format, ["image", "photos"])) {
			...
			$data['tmp'] = isset($file) ? $file : null;
			$data['aperture.f'] = isset($info['COMPUTED']['ApertureFNumber']) ? $info['COMPUTED']['ApertureFNumber'] : null;
			$data['b.o.m'] = isset($info['COMPUTED']['ByteOrderMotorola']) ? $info['COMPUTED']['ByteOrderMotorola'] : null;
			$data['size.height'] = isset($info['COMPUTED']['Height']) ? $info['COMPUTED']['Height'] : null;
			$data['size.width'] = isset($info['COMPUTED']['width']) ? $info['COMPUTED']['width'] : null;
			$data['file.datetime'] = isset($info['FILE']['FileDateTime']) ? $info['FILE']['FileDateTime'] : null;
			$data['file.size'] = isset($info['FILE']['FileSize']) ? $info['FILE']['FileSize'] : null;
			$data['file.type'] = isset($info['FILE']['FileType']) ? $info['FILE']['FileType'] : null;
			$data['file.source'] = isset($info['FILE']['FileSource']) ? $info['FILE']['FileSource'] : null;
			$data['flash'] = isset($info['FILE']['Flash']) ? $info['FILE']['Flash'] : null;
			$data['focal.length'] = isset($info['FILE']['FocalLength']) ? $info['FILE']['FocalLength'] : null;
			$data['iso'] = isset($info['FILE']['ISOSpeedRatings']) ? $info['FILE']['ISOSpeedRatings'] : null;
			$data['make'] = isset($info['IFD0']['Make']) ? $info['IFD0']['Make'] : null;
			$data['metering.mode'] = isset($info['EXIF']['MeteringMode']) ? $info['EXIF']['MeteringMode'] : null;
			$data['mime_type'] = isset($info['FILE']['MimeType']) ? $info['FILE']['MimeType'] : null;
			$data['model'] = isset($info['IFD0']['Model']) ? $info['IFD0']['Model'] : null;
			$data['software'] = isset($info['IFD0']['Software']) ? $info['IFD0']['Software'] : null;
			$data['ccd.width'] = isset($info['FILE']['CCDWidth']) ? $info['FILE']['CCDWidth'] : null;
			$data['exposure.time'] = isset($info['EXIF']['ExposureTime']) ? $info['EXIF']['ExposureTime'] : null;
			$data['exposure.mode'] = isset($info['EXIF']['ExposureMode']) ? $info['EXIF']['ExposureMode'] : null;
			$data['f_number'] = isset($info['EXIF']['FNumber']) ? $info['EXIF']['FNumber'] : null;
			$data['aperture.max_value'] = isset($info['EXIF']['MaxApertureValue']) ? $info['EXIF']['MaxApertureValue'] : null;
			$data['sensing.method'] = isset($info['EXIF']['SensingMethod']) ? $info['EXIF']['SensingMethod'] : null;
			$data['shutter.speed'] = isset($info['EXIF']['ShutterSpeedValue']) ? $info['EXIF']['ShutterSpeedValue'] : null;
			$data['exposure.bias_value'] = isset($info['EXIF']['ExposureBiasValue']) ? $info['EXIF']['ExposureBiasValue'] : null;
			$data['digital_zoom.ratio'] = isset($info['EXIF']['DigitalZoomRatio']) ? $info['EXIF']['DigitalZoomRatio'] : null;
			return $data;
		}else if (in_array($format, ["audio", "music"])) {
			...
			$data['tmp'] = isset($file) ? $file : null;
			$data['codec_name'] = isset($info['streams'][0]['codec_name']) ? $info['streams'][0]['codec_name'] : null;
			$data['codec_long_name'] = isset($info['streams'][0]['codec_long_name']) ? $info['streams'][0]['codec_long_name'] : null;
			$data['bitrate'] = isset($info['format']['bit_rate']) ? $info['format']['bit_rate'] : null;
			$data['duration'] = isset($info['format']['duration']) ? $info['format']['duration'] : null;
			$data['file.size'] = isset($info['format']['size']) ? $info['format']['size'] : null;
			return $data;
		}else if (in_array($format, ["video", "videos"])) {
			...
			$data['codec_name'] = isset($info['streams'][0]['codec_name']) ? $info['streams'][0]['codec_name'] : null;
			$data['codec_long_name'] = isset($info['streams'][0]['codec_long_name']) ? $info['streams'][0]['codec_long_name'] : null;
			$data['dimension.height'] = isset($info['streams'][0]['height']) ? $info['streams'][0]['height'] : null;
			$data['dimension.width'] = isset($info['streams'][0]['width']) ? $info['streams'][0]['width'] : null;
			$data['display.ratio'] = isset($info['streams'][0]['display_aspect_ratio']) ? $info['streams'][0]['display_aspect_ratio'] : null;
			$data['bit_rate'] = $data['bitrate'] = isset($info['format']['bit_rate']) ? $info['format']['bit_rate'] : null;
			$data['framerate'] = isset($info['streams'][0]['r_frame_rate']) ? $info['streams'][0]['r_frame_rate'] : null;
			$data['duration'] = isset($info['format']['duration']) ? $info['format']['duration'] : null;
			$data['file.size'] = isset($info['format']['size']) ? $info['format']['size'] : null;
			$data['audio.codec_name'] = isset($info['streams'][1]['codec_name']) ? $info['streams'][1]['codec_name'] : null;
			$data['audio.codec_long_name'] = isset($info['streams'][1]['codec_long_name']) ? $info['streams'][1]['codec_long_name'] : null;
			$data['audio.profile'] = isset($info['streams'][1]['profile']) ? $info['streams'][1]['profile'] : null;
			$data['audio.codec_type'] = isset($info['streams'][1]['codec_type']) ? $info['streams'][1]['codec_type'] : null;
			$data['audio.codec_time_base'] = isset($info['streams'][1]['codec_time_base']) ? $info['streams'][1]['codec_time_base'] : null;
			$data['audio.codec_tag_string'] = isset($info['streams'][1]['codec_tag_string']) ? $info['streams'][1]['codec_tag_string'] : null;
			$data['audio.channels'] = isset($info['streams'][1]['channels']) ? $info['streams'][1]['channels'] : null;
			$data['audio.channel_layout'] = isset($info['streams'][1]['channel_layout']) ? $info['streams'][1]['channel_layout'] : null;
			$data['audio.duration'] = isset($info['streams'][1]['duration']) ? $info['streams'][1]['duration'] : null;
			$data['audio.bit_rate'] = $data['audio.bitrate'] = isset($info['streams'][1]['bit_rate']) ? $info['streams'][1]['bit_rate'] : null;
			return $data;
		}else {
			return false;
		}
