...
_g['cache'].exists = function (name) {
	dataCacheHash = _g['cache'].setup();
	objCache = {};
...
_g['cache'].add = function (name, data) {
	dataCacheHash = _g['cache'].setup();
	objCache = {};
	if (isset(window) && isset(window[dataCacheHash])) {
		objCache = window[dataCacheHash];
...
_g['cache'].push = function (name, data) {
	dataCacheHash = _g['cache'].setup();
	objCache = {};
	if (isset(window) && isset(window[dataCacheHash])) {
		objCache = window[dataCacheHash];
...
_g['ajaxrequest'].use = function (object, callback) {
	...
	if (requestType.toLowerCase() == "get") {
		requestAjax.open(requestType, requestURL + '?' + requestData, requestAsync, requestUsername, requestPassword);
	}else {
	...
	if (!requestCrossdomain) {
		requestAjax.setRequestHeader("X-Requested-With", "XMLHttpRequest");
	}
	...
	requestDoneCallback = function (requestAjax) {
		_g['ajaxrequest'].event(requestAjax, 'load', function () {
			requestResponse = _g['ajaxrequest'].response(requestAjax);
			requestDone(_g['ajaxrequest'].responseparse(requestDataType, requestResponse['data']), requestResponse);
		});
	};
	requestDoneCallback(requestAjax);
	delete requestDoneCallback;
	callback(requestAjax);
	if (typeof requestDelayCallback !== "undefined") delete requestDelayCallback;
	requestDelayCallback = function (requestDelay, requestAjax, requestData) {
		setTimeout(function () {
			requestSend = _g['ajaxrequest'].send(requestAjax, requestData);
			requestDelay = requestAjax = requestData = undefined;
			return requestSend;
		}, requestDelay);
	};
	requestDelayCallback(requestDelay, requestAjax, requestData);
	delete requestDelayCallback;
};
_g['ajaxrequest'].send = function(request, data, cb) {
...
_g['ajaxify'].ready = function (cb) {
	...
	if (_g['cache'].exists('client_ajaxify_ready') === true)  {
		cb(_g['cache'].get('client_ajaxify_ready'));
	}else {
...
_g['ajaxify'].setup = function (cb) {
	...
	var setAjaxifyStatus = function (value) {
		_g['cache'].add('client_ajaxify_ready', value);
		cb(value);
	};
	_g['ajaxify'].autoload();
...
_g['ajaxify'].request = function (url, cb) {
	...
	if (_g['cache'].exists('client_ajaxify_request_('+url+')') === true) {
		console.log(_g['cache'].remove('client_ajaxify_request_('+url+')'));
		console.log(url);
		return false;
	}else {
		_g['cache'].new('client_ajaxify_request_('+url+')');
	}
	...
	abortRequestFunc = function () {
		...
		_g['cache'].remove('client_ajaxify_request_('+url+')');
	};
			...
			wrapperLoadRemoveOps['callback'] = function () {
				if (isset(returnObj) && isset(returnObj['return']) && returnObj['return'] === true) {
					if (isset(returnObj['data']) && typeof returnObj['data'] === "object" && countArr(returnObj['data']) > 0) {
						...
						_g['ajaxify'].reload(returnObj['data'], function (resultArr) {
...
_g['ajaxify'].loadwrapper = function (t, cb) {
	...
	if (t === true) {
		...
		_g['cache'].add((thisWrapper['selector']).toString(), dataCaching);
		...
		thisWrapper.fadeOut(500, function() {
			if (_g['cache'].exists((thisWrapper['selector']).toString()) === true) {
				dataCaching = _g['cache'].get((thisWrapper['selector']).toString());
				_g['cache'].remove((thisWrapper['selector']).toString());
			}else {
				dataCaching = {};
			}
...
var checkFileLoaded = function () {
	fileLoadedArr = {};
	if (_g['cache'].exists('client_source_file') === true) {
		fileLoadedArr = _g['cache'].get('client_source_file');
	}else {
		_g['cache'].add('client_source_file', {});
	}
	$("script").each(function() {
		...
		if (isset(fileSrc) && !inArray(fileSrc, fileLoadedArr)) {
			...
			_g['cache'].push('client_source_file', fileThisArr);
		}
	});
	$("link").each(function() {
...
var loadFileSource = function (type, cg) {
	...
	if (isset(src)) {
		...
		if (_g['cache'].exists('client_source_loading_('+src+')') === true) {
			return false;
		}else {
			_g['cache'].new('client_source_loading_('+src+')');
		}
		var getFileContentFailFunc = function () {
			_g['cache'].remove('client_source_loading_('+src+')');
		};
...
var isAudioNotifyReady = function (cb) {
	...
	var allowValue;
	if (_g['cache'].exists('client_audionotify_ready') === true)  {
		allowValue = _g['cache'].get('client_audionotify_ready');
		cb(allowValue);
	}else {
		var cbSetupFunc = function (value) {
			allowValue = value;
...
var setupAudioNotify = function (cb) {
	...
	var setMediaPlayerStatus = function (value) {
		_g['cache'].add('client_audionotify_ready', value);
		cb(value);
	};
	var gSource = $("#gSource");
	if (gSource.length === 0) {
...
var setupRecorderConfig = function (cb) {
	...
	var setRecorderStats = function (value) {
		_g['cache'].add('client_recorder_ready', value);
		cb(value);
	};
...
var isRecorderReady = function (cb) {
	...
	var allowValue;
	if (_g['cache'].exists('client_recorder_ready') === true)  {
		allowValue = _g['cache'].get('client_recorder_ready');
		cb(allowValue);
	}else {
		var cbSetupFunc = function (value) {
			allowValue = value;
...
var setupMediaPlayer = function (cb) {
	...
	var setMediaPlayerStatus = function (value) {
		_g['cache'].add('client_mediaplayer_ready', value);
		cb(value);
	};
...
var isMediaPlayerReady = function (cb) {
	...
	if (_g['cache'].exists('client_mediaplayer_ready') === true)  {
		cb(_g['cache'].get('client_mediaplayer_ready'));
	}else {
		var cbSetupFunc = function (value) {
			cb(value);
		};
...
var callbackAllFunc = function () {
	...
	_g['ajaxify'].ready(ajaxifyReadyFunc);
	...
	setTimeout(function() {
		...
		/*
		console.log(_g['cache'].add('sdsd', {'sdsd': 'sdfsdfsdf', 'sdfsdfas': 'sdfsaf'}));
		setTimeout(function () {
			console.log(_g['cache'].get('sdsd'));
		}, 5000);
		*/
		...
		console.log(_g['cache'].get('client_source_file'));
		*/
		/*
		f = function (x) {
			console.log(x);
