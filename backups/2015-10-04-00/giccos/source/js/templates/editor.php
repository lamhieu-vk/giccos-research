...
var editor_tagPlaces = function (selector) {
	...
	function requestFind (input) {
		...
		if (_g['cache'].exists(placesCacheName) == true) {
			var placesDataCache = _g['cache'].get(placesCacheName);
			...
			success: function(data) {
				if (isset(data['return']) && data['return'] === true) {
					...
					if (isset(data['data'])) {
						...
						boxSuggest(input, boxSuggestOptions);
						_g['cache'].add(placesCacheName, placesFoundedArr);
					}
					setTimeout(function() {
						input.removeAttr('val');
					}, 1000);
		...
		if (statusType != boxMediaOtherOld && (statusType == "status" || statusType == "photos" || statusType == "music" || statusType == "videos" || statusType == "cited")) {
			...
			if (statusType == "status") {
					...
					if (1 + 1 == 2) {
						...
						uploadPreview.children(".demo[tag]").find(".info > .header > .action .close").bind('click', function() {
							$(this).parents(".demo[tag]").fadeOut(function() {
								...
								if (_g['cache'].exists('sites-notuse-url') === true) {
									_g['cache'].push('sites-notuse-url', url);
								}else {
									_g['cache'].add('sites-notuse-url', [url]);
								}
							});
						});
						if (_g['cache'].exists('sites-info-url') === true) {
							var infoList = _g['cache'].get('sites-info-url');
							var isHas = 0;
							...
							if (isHas === 0) {
								...
								newObj[url] = linkObj;
								linkObj = newObj;
								_g['cache'].push('sites-info-url', linkObj);
							}
						}else {
							newObj = {};
							newObj[url] = linkObj;
							linkObj = newObj;
							_g['cache'].add('sites-info-url', linkObj);
						}
					}
				};
				var linkScanInfo = function (url) {
					...
					var infoUrlList = _g['cache'].get('sites-scan-url');
					...
					$.ajax({
						...
						success: function(data) {
							if (isset(data) && isset(data['return']) && data['return'] === true) {
								console.log('xs');
								if (_g['cache'].exists('sites-scan-url') === true) {
									var infoList = _g['cache'].get('sites-scan-url');
									var isHas = 0;
									infoListLoop: for (i in infoList) {
										if (i == url) {
											isHas++;
									...
									if (isHas === 0) {
										linkObj = [];
										linkObj[url] = true;
										_g['cache'].push('sites-scan-url', linkObj);
									}
								}else {
									linkObj = [];
									linkObj[url] = true;
									_g['cache'].add('sites-scan-url', linkObj);
								}
				...
				var linkGetInfo = function (url) {
					...
					if (_g['cache'].exists('sites-notuse-url') === true) {
						var notuseUrlList = _g['cache'].get('sites-notuse-url');
					}else {
						var notuseUrlList = {};
					}
					...
					if (_g['cache'].exists('sites-info-url') === true) {
						var infoUrlList = _g['cache'].get('sites-info-url');
					}else {
						var infoUrlList = {};
					}
					if (isset(infoUrlList) && typeof infoUrlList == "object") {
