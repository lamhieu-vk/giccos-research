	...
	if ($port == "accounts" && $token == $g_client['token']['action']['accounts']) {
		...
		*/
		...
		if (isset($htmlPage['return'], $htmlPage['data']) && $htmlPage['return'] == true) {
			...
			if (!isset($pageDom->find("body", 0)->find("#gMain", 0)->find("#gBox", 0)->innertext)) {
				$pageMain['box'] = null;
			}else {
				$pageMain['box'] = $pageDom->find("body", 0)->find("#gMain", 0)->find("#gBox", 0)->innertext;
