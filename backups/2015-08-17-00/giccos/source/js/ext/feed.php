					...
					if (isset(data['return']) && data['return'] == true && isset(data['stats'])) {
						if (isset(data['stats']['count']) && typeof data['stats']['count'] == "number" && data['stats']['count'] > 0) {
							...
							popupNotification ({
								...
								'callback': function() {
									popupNotification ({type: 'notify', title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('please_wait_load_data', 'ucfirst'); ?>', timeout: 2000});
									//.
									var feed = $(".thisFeeds[feed]"),
										options = {};
									if (isset(feed.attr('feed')) && typeof feed.attr('feed') == "string" && typeof JSON.parse(feed.attr('feed')) == "object") {
...
var feed_status_load = function (options) {
	...
	var id = isset(options['id']) && (typeof options['id'] === "string" || typeof options['id'] === "number") ? options['id'] : null;
	...
	var data = {'port' : 'feed', 'type': 'status', 'action': 'load', 'obj': {'type': 'general', 'method': method, 'sort': sort, 'id': id, 'except': except, 'order': order}};
	...
	status.each(function() {
		...
		buttonQRcode.bind('click', function() {
			...
			if (button.attr('handing') != "true") {
				...
				popupQR.find(".body").find("img").hide().after("<span class='loading'><?php print $_language->text('loading', 'ucfirst'); ?></span>");
				popupQR.find(".body").find("img").bind('load', function() {
...
var feed_feed_status_loadMedia = function () {
	...
	media.bind('click', function (e) {
		...
		if (type == "photos" && object != "" && object !== undefined && typeof JSON.parse(object) == "object") {
			...
			if (media.attr('handing-media') != "true") {
				$("#fullscreen").remove();
				...
				$("body").append(code);
				setTimeout(function() {
					media.removeAttr('handing-media');
				},100);
			...
			if (object['type'] == "music") {
				var box = $(this).parent('.media.MusicBox'),
					...
					c_media = '<audio id="'+object['id']+'" class="gplayer-skin v-simple audio" width="98%" height="175"> </audio>';
			}else if (object['type'] == "videos") {
				var box = $(this).parent('.media.VideosBox'),
					c_info = "<div class='navBar'> <div class='info'> <div class='name nowrap'> <div class='move'><i class='fa'></i></div> <span>"+object['name']+"</span> </div> <div class='description nowrap'> <span>"+object['description']+"</span> </div> </div> </div>",
					c_media = '<video id="'+object['id']+'" class="gplayer-skin v-simple video" width="98%" height="275"> </video>';
			}
			...
			for (var t in object['link']) {
				if (!isset(object['src']) || typeof object['src'] !== "object") {
					object['src'] = {};
				}
				if (typeof object['link'][t] === "string") {
					object['src'][countArr(object['src'])] = {'text': t, 'source': object['link'][t]};
				}else if (typeof object['link'][t] === "object") {
					var objectThis = {'text': t};
					if (isset(object['link'][t]['source']) && typeof object['link'][t]['source'] === "string") {
						objectThis['source'] = object['link'][t]['source'];
					}else if (isset(object['link'][t]['src']) && typeof object['link'][t]['src'] === "string") {
						objectThis['source'] = object['link'][t]['src'];
					}else if (isset(object['link'][t]['type']) && typeof object['link'][t]['type'] === "string") {
						objectThis['type'] = object['link'][t]['type'];
					}else if (isset(object['link'][t]['mime']) && typeof object['link'][t]['mime'] === "string") {
						objectThis['type'] = object['link'][t]['mime'];
					}
					object['src'][countArr(object['src'])] = objectThis;
				}
			}
