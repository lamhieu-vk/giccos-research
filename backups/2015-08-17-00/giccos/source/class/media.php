		...
		if ($format == "photos") {
			...
			if ($action == "add") {
				...
				if ($limit == false || (mysqli_num_rows($getOriginalQuery) > 0 && mysqli_num_rows($getCopyQuery) == 0)) {
					...
					$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $original['path'], "limit" => "LIMIT 1"));
				...
				$getPhotosExpiresRequest = "SELECT * FROM `photos_cache` WHERE (`client` = '".$_client->client(true)."' OR `user.id` = '".$userId."' OR (`author.id` = '".$guyId."' AND `author.type` = '".$guyType."')) AND `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."';";
				...
				$getPhotosOtherRequest = "SELECT * FROM `photos_cache` WHERE `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."' LIMIT ".rand($_parameter->get('cacheClean_user_limit_min'), $_parameter->get('cacheClean_user_limit_max')).";";
			...
			if ($action == "add") {
				...
				if ($limit == false || (mysqli_num_rows($getOriginalQuery) > 0 && mysqli_num_rows($getCopyQuery) == 0)) {
					$originalFetch = mysqli_fetch_assoc($getOriginalQuery);
					$getMediaFile = $_storage->get(array("format" => "audio", "label" => "display", "value" => $original['path'], "limit" => "LIMIT 1"));
					if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['data'], $getMediaFile['data'][0]) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
						$originalFetch['tmp'] = $getMediaFile['data'][0]['source'];
					}else {
						return array("return" => false, "reason" => "");
				...
				$expiresArr = $removedArr = $nameArr = $secretArr = array();
				$getMusicExpiresRequest = "SELECT * FROM `music_cache` WHERE (`client` = '".$_client->client(true)."' OR `user.id` = '".$userId."' OR (`author.id` = '".$guyId."' AND `author.type` = '".$guyType."')) AND `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."';";
				...
				$getMusicOtherRequest = "SELECT * FROM `music_cache` WHERE `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."' LIMIT ".rand($_parameter->get('cacheClean_user_limit_min'), $_parameter->get('cacheClean_user_limit_max')).";";
				...
				if (isset($file['tmp'])) {
					...
					if ($multiple == true && ($file['info']['bitrate'] / 1000 >= 320)) {
						...
						if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true) {
							...
							exec($_SESSION['as'] = "ffmpeg -y -i {$_client->root()}{$file['tmp']} -codec:a libmp3lame -b:a 320K -ac 2 -map_metadata -1 {$_client->root()}{$file['source']['320kbps']} 2>&1 &", $outputContent320kbps);
					...
					if ($multiple == true && ($file['info']['bitrate'] / 1000 >= 128)) {
						...
						if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true) {
							...
							exec($_SESSION['as'] = "ffmpeg -y -i {$_client->root()}{$file['tmp']} -codec:a libmp3lame -b:a 128K -ac 2 -map_metadata -1 {$_client->root()}{$file['source']['128kbps']} 2>&1 &", $outputContent128kbps);
			...
			if ($action == "add") {
				...
				if (isset($client, $token, $secret, $nameraw, $name, $mime, $size)) {
					...
					if (!$addVideosCacheQuery) {
						...
						$thumbnailPosition = intval($duration / 2);
				...
				$expiresArr = $removedArr = $nameArr = $secretArr = array();
				$getVideosExpiresRequest = "SELECT * FROM `videos_cache` WHERE (`client` = '".$_client->client(true)."' OR `user.id` = '".$userId."' OR (`author.id` = '".$guyId."' AND `author.type` = '".$guyType."')) AND `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."';";
				...
				$getVideosOtherRequest = "SELECT * FROM `videos_cache` WHERE `close` != '0' AND `close` - '".time()."' >= '".$_parameter->get('media_cache_time_close')."' LIMIT ".rand($_parameter->get('cacheClean_user_limit_min'), $_parameter->get('cacheClean_user_limit_max')).";";
