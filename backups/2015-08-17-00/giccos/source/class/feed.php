...
Class feed {
			...
			while ($status = mysqli_fetch_assoc($statusQuery)) {
				...
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
					...
					if (mysqli_num_rows($mediaQuery) == 0) {
						...
						while ($mediaData = mysqli_fetch_assoc($mediaQuery)) {
							$mediaFetch = $_media->data("photos", array("guy" => array("type" => $author['type'], "id" => $author['id']), "action" => "get", "rows" => ["token", "display", "mime_type", "file.original", "file.large", "file.medium"], "label" => "id", "value" => $mediaData['photos.id'], "limit" => "LIMIT 1"));
							if (isset($mediaFetch['return'], $mediaFetch['data'], $mediaFetch['data'][0]) && $mediaFetch['return'] == true) {
								$mediaFetchData = $mediaFetch['data'][0];
	...
	function status_add ($object) {
		...
		VALUES 
		(null, '{$status['token']}', '{$status['display']}', '{$status['time']}', '{$status['date']}', '{$status['edit']}', '{$userId}', '{$guyType}', '{$guyId}', '{$status['private']['view']}', '{$status['private']['comment']}', '{$status['private']['share']}', '{$status['type']}', '{$status['contentHashed']}', '{$status['usertag']}', '{$status['hashtag']}', '{$status['places']}', '{$status['mood']}', '{$status['link']}', '{$status['share']}');
		";
