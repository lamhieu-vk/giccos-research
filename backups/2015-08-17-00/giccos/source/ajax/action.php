	...
	if ($port == "accounts" && $token == $g_client['token']['action']['accounts']) {
		...
		if ($type == "status" && in_array($action, ["xxx", "newcheck", "load", "get"]) && !function_exists("getCodeQuery")) {
			function getCodeQuery ($ObjRequest = null, $getFor = null) {
				...
				if (isset($ObjRequest['except']) && is_array($ObjRequest['except']) && count($ObjRequest['except']) > 0) $except = $ObjRequest['except']; else $except = null;
				...
				if ($id != null && $port != null) {
					...
					if (isset($port['type']) && $port['type'] == "general") {
					...
					}else if (isset($port['type']) && ($port['type'] == "nearby" || $port['type'] == "places")) {
						if (isset($port['require']) && is_array($port['require'])) {
							if (isset($port['require']['value']) && is_numeric($port['require']['value']) &&$port['require']['value'] != null) {
								$getPositionNearbyRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".$_tool->convertDatabaseString($port['require']['value'])."' LIMIT 1";
							}else if (isset($port['require']['value']) && is_string($port['require']['value']) &&$port['require']['value'] != null) {
								if (preg_match("/([a-zA-Z0-9]{32})/", $port['require']['value'])) {
									$getPositionNearbyRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `display` = '".$_tool->convertDatabaseString($port['require']['value'])."' OR `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}else {
									$getPositionNearbyRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}
							}
							if (isset($getPositionNearbyRequest) && is_string($getPositionNearbyRequest) && $getPositionNearbyRequest != null) {
								...
								if ($getPositionNearbyQuery == true && mysqli_num_rows($getPositionNearbyQuery) == 0) {
						...
						if (isset($port['require']) && is_array($port['require'])) {
							if (isset($port['require']['value']) && is_numeric($port['require']['value']) &&$port['require']['value'] != null) {
								$getPositionCountryRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".$_tool->convertDatabaseString($port['require']['value'])."' LIMIT 1";
							}else if (isset($port['require']['value']) && is_string($port['require']['value']) &&$port['require']['value'] != null) {
								if (preg_match("/([a-zA-Z0-9]{32})/", $port['require']['value'])) {
									$getPositionCountryRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `display` = '".$_tool->convertDatabaseString($port['require']['value'])."' OR `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}else {
									$getPositionCountryRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}
							}
							if (isset($getPositionCountryRequest) && is_string($getPositionCountryRequest) && $getPositionCountryRequest != null) {
								...
								if ($getPositionCountryQuery == true && mysqli_num_rows($getPositionCountryQuery) == 0) {
						...
						if (isset($port['require']) && is_array($port['require'])) {
							if (isset($port['require']['value']) && is_numeric($port['require']['value']) &&$port['require']['value'] != null) {
								$getPositionLiveRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".$_tool->convertDatabaseString($port['require']['value'])."' LIMIT 1";
							}else if (isset($port['require']['value']) && is_string($port['require']['value']) &&$port['require']['value'] != null) {
								if (preg_match("/([a-zA-Z0-9]{32})/", $port['require']['value'])) {
									$getPositionLiveRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `display` = '".$_tool->convertDatabaseString($port['require']['value'])."' OR `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}else {
									$getPositionLiveRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".$_tool->convertDatabaseString($port['require']['value'])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
								}
							}
							if (isset($getPositionLiveRequest) && is_string($getPositionLiveRequest) && $getPositionLiveRequest != null) {
								...
								if ($getPositionLiveQuery == true && mysqli_num_rows($getPositionLiveQuery) == 0) {
					...
					}else if (isset($port['type']) && in_array($port['type'], ["hashtag", "tags", "tag"])) {
							...
							if (isset($feed_['content']['require'][$i][0], $feed_['content']['require'][$i][1])) {
								...
								for ($x = 0; $x < count($feed_['content']['require'][$i][1]); $x++) {
									...
									if (preg_match("/^(\@)/", $feed_['content']['require'][$i][1][$x]) == true) {
										...
										$getAuthorIdQuery = mysqli_query($class['_db']->port('beta'), "SELECT `id` FROM `users` WHERE `username` = '{$cache_hashtag_['username']}' LIMIT 1");
										if (!$getAuthorIdQuery) {
											continue;
										}
						...
						if ($feed_['content']['require'] == null) {
							...
							if (isset($port['require']['label'])) $feed_['content']['require']['label'] = $port['require']['label']; else $feed_['content']['require']['label'] = null;
							if (isset($port['require']['value'])) $feed_['content']['require']['value'] = $port['require']['value']; else $feed_['content']['require']['value'] = null;
							if (!in_array($port['require']['label'], ["stamp", "string"]) || $port['require']['value'] == null) {
								...
								if ($port['require']['label'] == "stamp") {
									//.
								}else if ($port['require']['label'] == "string") {
					...
					}else if (isset($port['type']) && $port['type'] == "wall") {
						if (isset($port['require']['label'], $port['require']['value']) && is_string($port['require']['label']) && is_string($port['require']['value'])) {
							if (in_array($port['require']['label'], ["id", "username"])) {
								//.
							}else {
								return array("return" => false, "reason" => "");
							}
						...
						if (isset($port['require']['type']) && $port['require']['type'] == 0) {
							$feed_['query']['source'] = "`author.type` = 'user' AND `author.id` IN (SELECT `id` FROM `users` WHERE `{$port['require']['label']}` = '{$port['require']['value']}')";
						}else if (isset($port['require']['type']) && $port['require']['type'] == 1) {
							$feed_['query']['source'] = "`author.type` = 'user' AND `author.id` IN (SELECT `id` FROM `users` WHERE `{$port['require']['label']}` = '{$port['require']['value']}')";
						}else {
							$feed_['query']['source'] = false;
						}
						if (isset($feed_['query']['source']) && is_string($feed_['query']['source'])) {
							$feed_['query']['source'] = "AND ({$feed_['query']['source']})";
						}else {
							$feed_['query']['source'] = false;
						}
					...
					if (isset($except) && is_array($except) && count($except) > 0) {
						foreach ($except as $key => $value) {
							$except[$key] = intval($class['_tool']->convertDatabaseString($value));
						}
						$feed_['query']['except'] = "AND `id` NOT IN ('" . implode($except, "', '") . "')";
					}else {
						$feed_['query']['except'] = "";
					}
					if ($g_user['mode']['type'] == "user") {
					...
					$feed_['query']['code'] = "SELECT {$feed_['query']['select']} FROM `status` WHERE `id` {$feed_['follow']['sort']} '{$id}' {$feed_['query']['block']} {$feed_['query']['source']} {$feed_['query']['private']} {$feed_['query']['except']} {$feed_['query']['order']} {$feed_['query']['limit']}";
		...
		if ($type == "status") {
			if ($action == "add" || $action == "upload") {
				...
				if (isset($ObjRequest['type']) && is_string($ObjRequest['type'])) $feedOptions['type'] = $ObjRequest['type']; else $feedOptions['type'] = null;
				if (isset($ObjRequest['method']) && is_string($ObjRequest['method'])) $feedOptions['method'] = $ObjRequest['method']; else $feedOptions['method'] = null;
				if (isset($ObjRequest['sort']) && is_string($ObjRequest['sort'])) $feedOptions['sort'] = ">"; else $feedOptions['sort'] = null;
				if (isset($ObjRequest['id']) && is_numeric($ObjRequest['id'])) $feedOptions['id'] = 0; else $feedOptions['id'] = null;
				//if (isset($ObjRequest['except']) && is_array($ObjRequest['except']) && count($ObjRequest['except']) > 0) $feedOptions['except']  = $ObjRequest['except']; else $feedOptions['except'] = null;
				if (isset($ObjRequest['order']) && is_string($ObjRequest['order'])) $feedOptions['order']  = $ObjRequest['order']; else $feedOptions['order'] = null;
				if (isset($ObjRequest['limit']) && is_numeric($ObjRequest['limit'])) $feedOptions['limit']  = $ObjRequest['limit']; else $feedOptions['limit'] = null;
				if (!isset($feedOptions['type']) || !isset($feedOptions['method']) || !isset($feedOptions['sort']) || !isset($feedOptions['id']) || !isset($feedOptions['order'])) {
					die(print json_encode(array("return" => false, "reason" => "")));
				}
				$feedOptions['guy'] = $g_user['mode'];
				$statusList = $_feed->status_list($feedOptions);
				if (isset($statusList['return'], $statusList['count'], $statusList['data']) && $statusList['return'] == true) {
					if ($statusList['count'] == 0) {
						ie(print json_encode(array("return" => true, "count" => 0, "data" => array())));
					}
					$listStatusId = array();
					...
					if (isset($statusFetch['return'], $statusFetch['count'], $statusFetch['data']) && $statusFetch['return'] == true) {
						foreach ($statusFetch['data'] as $key => $statusThis) {
							if (isset($statusThis['info'], $statusThis['info']['token'])) {
								unset($statusFetch['data'][$key]['info']['token']);
							}
						}
				...
				}else if (isset($statusList['return'], $statusList['reason']) && $statusList['return'] == false) {
					die(print json_encode(array("return" => false, "reason" => $statusList['reason'])));
				}else {
					die(print json_encode(array("return" => false, "reason" => "")));
				}
				...
				if (isset($actionGetCodeQuery['return'], $actionGetCodeQuery['code']) && $actionGetCodeQuery['return'] == true && $actionGetCodeQuery['code'] != null) {
					$statusGet = $_feed->status_get(array("return" => "json", "query" => $actionGetCodeQuery['code'], "guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])));
				...
				foreach ($idArr as $key => $idArrThis) {
					if ((is_string($idArrThis) || is_numeric($idArrThis)) && $idArrThis > 0) {
						...
						$blockRequest = $_feed->status_block(array_merge(array("user" => array("id" => $g_user['id']), "guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusBlockOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					if ((is_string($idArrThis) || is_numeric($idArrThis)) && $idArrThis > 0) {
						...
						$deleteRequest = $_feed->status_remove(array_merge(array("user" => array("id" => $g_user['id']),"author" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusDeleteOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$favoriteRequest = $_feed->status_favorite(array_merge(array("guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusfavoriteOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$unfavoriteRequest = $_feed->status_favorite(array_merge(array("guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusunfavoriteOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$followRequest = $_feed->status_follow(array_merge(array("user" => array("id" => $g_user['id']), "guy" => $g_user['mode']), $statusFollowOptions));
					if (isset($followRequest['return']) && $followRequest['return'] == true) {
						$followArr[] = array("id" => $idArrThis, "return" => true, "action" => $action);
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$unfollowRequest = $_feed->status_follow(array_merge(array("user" => array("id" => $g_user['id']), "guy" => $g_user['mode']), $statusUnfollowOptions));
					if (isset($unfollowRequest['return']) && $unfollowRequest['return'] == true) {
						$unfollowArr[] = array("id" => $idArrThis, "return" => true, "action" => $action);
