	...
	rows.find("[friend-request]").bind("click", function(){
		...
		if (!isset(button.attr('friend-request'))) {
			...
			if (isset(object['action']) && object['id'] !== 0) {
				if (inArray(object['action'], ['add_request', 'remove_request', 'exists_request', 'comfirm_request', 'remove_friend'])) {
				...
				if (isset(action)) {
					var data = {'port': 'user', 'type': 'friend', 'action': action, 'id': id};
				}else {
					return;
				}
						...
						success: function (data) {
							if (isset(data['return'])&& data['return'] == true) {
								...
								if (action == "add_request") {
									...
									button.attr('friend-request', JSON.stringify({'action': 'remove_request', 'id': id})).attr('title', '<?php print $_language->text('cancel_request', 'ucfirst'); ?>').find(".add").removeClass("add").addClass("cancel");
								}else if (action == "remove_request") {
									var popupDescription = "<?php print $_language->text('cancel_request_success', 'ucfirst'); ?>";
									button.attr('friend-request', JSON.stringify({'action': 'add_request', 'id': id})).attr('title', '<?php print $_language->text('send_request', 'ucfirst'); ?>').find(".cancel").removeClass("cancel").addClass("add");
								}else if (action == "remove_friend") {
									var popupDescription = "<?php print $_language->text('remove_friends_success', 'ucfirst'); ?>";
									button.attr('friend-request', JSON.stringify({'action': 'add_request', 'id': id})).attr('title', '<?php print $_language->text('send_request', 'ucfirst'); ?>').find(".remove").removeClass("remove").addClass("add");
								}else if (action == "comfirm_request") {
									var popupDescription = "<?php print $_language->text('add_friends_success', 'ucfirst'); ?>";
									button.attr('friend-request', JSON.stringify({'action': 'remove_friend', 'id': id})).attr('title', '<?php print $_language->text('remove_friends', 'ucfirst'); ?>').find(".accept").removeClass("accept").addClass("remove");
								}else {
									//.
								}
...
var wall_friendsPush = function (order, object) {
	...
	if (isset(object['is_you']) && object['is_you'] === true) {
		...
		if (isset(object['is_friend']) && object['is_friend'] === true) {
			var buttonAction = "<div class='action _tt-bk s' friend-request='"+JSON.stringify({'action': 'remove_friend', 'id': object['id']})+"' title='<?php print $_language->text('remove_friends', 'ucfirst'); ?>'> <i class='remove'></i> </div>";
			var textReason = "<?php print $_language->text('friends', 'ucfirst'); ?>";
		}else {
			if (isset(object['send_request']) && object['send_request'] === true) {
				var buttonAction = "<div class='action _tt-bk s' friend-request='"+JSON.stringify({'action': 'remove_request', 'id': object['id']})+"' title='<?php print $_language->text('cancel_request', 'ucfirst'); ?>'> <i class='cancel'></i> </div>";
			}else {
				if (isset(object['waiting_request']) && object['waiting_request'] === true) {
					var buttonAction = "<div class='action _tt-bk s' friend-request='"+JSON.stringify({'action': 'comfirm_request', 'id': object['id']})+"' title='<?php print $_language->text('accept_request', 'ucfirst'); ?>'> <i class='accept'></i> </div>";
				}else {
					var buttonAction = "<div class='action _tt-bk s' friend-request='"+JSON.stringify({'action': 'add_request', 'id': object['id']})+"' title='<?php print $_language->text('send_request', 'ucfirst'); ?>'> <i class='add'></i> </div>";
				}
			}
