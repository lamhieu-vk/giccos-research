...
consoleWarning();
/*
var dataCache = function (object) {
	var action = isset(object) && isset(object['action']) && typeof object['action'] === "string" ? object['action'] : null;
	if (action == "read") {
		var objCache = {};
		if (isset(window['dataCache'])) {
			objCache = window['dataCache'];
		}else {
			objCache = {};
		}
		return objCache;
...
dataCache.push = function (name, data) {
	...
	cacheLoop: for (var i in objCache) {
		thisCache = objCache[i];
		if (isset(thisCache) && isset(thisCache['name']) && thisCache['name'] == name) {
			objCache[i]['data'] = thisCache.concat(data);
			break cacheLoop;
		}
	}
	window['dataCaching'] = objCache;
