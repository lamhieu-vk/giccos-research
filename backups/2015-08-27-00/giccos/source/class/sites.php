...
Class sites {
	...
	function scan ($object) {
		...
		if ($action == "extract") {
			...
			if (1 + 1 == 2) {
				$addScanRequest = "INSERT INTO `sites_scan` (`id`, `time`, `token`, `display`, `hash`, `total`, `data`, `status`, `updated`) VALUES (NULL, '".$infoArr['time'] = time()."', '".$infoArr['token']."', '".$infoArr['display']."', '".$infoArr['hash']."', '".$infoArr['total']."', '".$infoArr['data']."', '".$infoArr['status']."', '".$infoArr['updated']."');";
		...
		if (!isset($htmlPage) || !is_string($htmlPage) || $htmlPage == null) {
			...
			if ($allowExtract == true) {
				...
				if (isset($isChanged) && !$isChanged) {
						...
						if (isset($getCertificate, $getCertificate['return'], $getCertificate['data'], $getCertificate['data']['hash']) && $getCertificate['return'] == true) {
							...
							if (isset($existsCertificate, $existsCertificate['return'], $existsCertificate['exists']) && $existsCertificate['return'] == true) {
								if ($existsCertificate['exists'] == true) {
									...
									if (isset($addCertificate, $addCertificate['return']) && $addCertificate['return'] == true) {
					...
					if (isset($isChanged) && $isChanged) {
						mysqli_query($db, "UPDATE `sites_info` SET `fingerprints` = '{$siteInfo['fingerprints']}', `thumbnail` = '{$siteInfo['thumbnail']}', `title` = '{$siteInfo['title']}', `description` = '{$siteInfo['description']}', `scan` = '{$siteInfo['scan']}', `certificate` = '{$siteInfo['certificate']}', `updated` = '{$time}' WHERE `id` = '{$siteInfoSaved['id']}'");
						$infoId = $siteInfoSaved['id'];
					}else {
						$insertRequest = "
						INSERT INTO 
						`sites_info` (`id`, `time`, `token`, `display`, `scheme`, `sub`, `host`, `name`, `dot`, `path`, `mime`, `fingerprints`, `title`, `thumbnail`, `description`, `url`, `scan`, `certificate`, `updated`) 
						VALUES 
						(NULL, '{$time}', '{$siteInfo['token']}', '{$siteInfo['display']}', '{$siteInfo['scheme']}', '{$siteInfo['sub']}', '{$siteInfo['host']}', '{$siteInfo['name']}', '{$siteInfo['dot']}', '{$siteInfo['path']}', '{$siteInfo['mime']}', '{$siteInfo['fingerprints']}', '{$siteInfo['title']}', '{$siteInfo['thumbnail']}', '{$siteInfo['description']}', '{$siteInfo['url']}', '{$siteInfo['scan']}', '{$siteInfo['certificate']}', '{$time}');
						";
