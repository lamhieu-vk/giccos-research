	...
	if ($port == "photos" && $token == $g_client['token']['action']['photos']) {
		...
		if ($type == "editor") {
					...
					foreach ($file as $i => $thisFile) {
						...
						if ($secret != null && ($verify == true || $verify == false)) {
							...
							if (isset($deleteFile['return']) && $deleteFile['return'] == true) {
								if (isset($_SESSION["cache"]['photos_usertag_notfound'])) {
									$cachePhotosUsertagNotfound = $_SESSION["cache"]['photos_usertag_notfound'];
									...
									$_SESSION["cache"]['photos_usertag_notfound'] = $cachePhotosUsertagNotfound;
								}
								if (isset($_SESSION["cache"]['photos_usertag_handling'])) {
									$cachePhotosUsertagHandling = $_SESSION["cache"]['photos_usertag_handling'];
									...
									$_SESSION["cache"]['photos_usertag_handling'] = $cachePhotosUsertagHandling;
								}
								if (isset($_SESSION["cache"]['photos_usertag'])) {
									$cachePhotosUsertag = $_SESSION["cache"]['photos_usertag'];
									...
									$_SESSION["cache"]['photos_usertag'] = $cachePhotosUsertag;
								}
			...
			if ($action == "getFaces") {
				...
				$getMediaCache = $_media->cache('photos', array("action" => "get", "label" => "secret", "value" => $secret));
				if (isset($getMediaCache['return']) && $getMediaCache['return'] == true && is_array($getMediaCache['data']) && count($getMediaCache['data']) > 0) {
					$fileCache = $getMediaCache['data'];
					if (isset($fileCache['path']) && $fileCache['path'] != null) {
						$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $fileCache['path'], "rows" => "*", "limit" => "LIMIT 1"));
					...
					$imageData = array(
						"link" => $_tool->links($_parameter->get('pathVirtual_cache_photos').'/'.$fileCache['name']),
						"secret" => $fileCache['secret'],
						"mime" => $fileCache['mime'],
						"name" => $fileCache['name']
					);
					if (!isset($_SESSION["cache"]['photos_usertag_notfound'])) {
						$cachePhotosUsertagNotfound = array();
					}else {
						$cachePhotosUsertagNotfound = $_SESSION["cache"]['photos_usertag_notfound'];
					}
					foreach ($cachePhotosUsertagNotfound as $key => $cachePhotosUsertagNotfoundThis) {
						if (isset($cachePhotosUsertagNotfoundThis['photos'], $cachePhotosUsertagNotfoundThis['user.id'], $cachePhotosUsertagNotfoundThis['author.type'], $cachePhotosUsertagNotfoundThis['author.id']) && is_string($cachePhotosUsertagNotfoundThis['photos'])) {
							if ($cachePhotosUsertagNotfoundThis['user.id'] == $g_user['id'] && $cachePhotosUsertagNotfoundThis['author.type'] == $g_user['mode']['type'] && $cachePhotosUsertagNotfoundThis['author.id'] == $g_user['mode']['id'] && $cachePhotosUsertagNotfoundThis['photos'] == $fileCache['secret']) {
								$_SESSION["cache"]['photos_usertag_notfound'] = $cachePhotosUsertagNotfound;
					...
					if (!isset($_SESSION["cache"]['photos_usertag_handling'])) {
						$cachePhotosUsertagHandling = array();
					}else {
						$cachePhotosUsertagHandling = $_SESSION["cache"]['photos_usertag_handling'];
					}
					foreach ($cachePhotosUsertagHandling as $key => $cachePhotosUsertagHandlingThis) {
						if (isset($cachePhotosUsertagHandlingThis['photos'], $cachePhotosUsertagHandlingThis['user.id'], $cachePhotosUsertagHandlingThis['author.type'], $cachePhotosUsertagHandlingThis['author.id']) && is_string($cachePhotosUsertagHandlingThis['photos'])) {
							if ($cachePhotosUsertagHandlingThis['user.id'] == $g_user['id'] && $cachePhotosUsertagHandlingThis['author.type'] == $g_user['mode']['type'] && $cachePhotosUsertagHandlingThis['author.id'] == $g_user['mode']['id'] && $cachePhotosUsertagHandlingThis['photos'] == $fileCache['secret']) {
								$_SESSION["cache"]['photos_usertag_handling'] = $cachePhotosUsertagHandling;
								if ($robotRequest == $g_client['token']['robot']) {
					...
					foreach ($cachePhotosUsertag as $key => $cachePhotosUsertagThis) {
						if (isset($cachePhotosUsertagThis['photos'])) {
							if (isset($cachePhotosUsertagThis['user.id'], $cachePhotosUsertagThis['author.type'], $cachePhotosUsertagThis['author.id'])) {
								if ($cachePhotosUsertagThis['photos'] == $fileCache['secret']) {
									if (isset($cachePhotosUsertagThis['faces'], $cachePhotosUsertagThis['thumbnail'], $cachePhotosUsertagThis['display'], $cachePhotosUsertagThis['position'], $cachePhotosUsertagThis['size'], $cachePhotosUsertagThis['ratio'])) {
										if ($cachePhotosUsertagThis['user.id'] == $g_user['id'] && $cachePhotosUsertagThis['author.type'] == $g_user['mode']['type'] && $cachePhotosUsertagThis['author.id'] == $g_user['mode']['id']) {
											$facesDataCache[] = $cachePhotosUsertagThis;
										}else {
											continue;
										}
									}else {
					...
					$photosUsertag_query = mysqli_query($_db->port('beta'), "SELECT * FROM `photos_cache_usertag` WHERE `photos` = '{$fileCache['secret']}' AND `author.type` = '{$g_user['mode']['type']}' AND `author.id` = '{$g_user['mode']['id']}' AND `user.id` = '{$g_user['id']}'");
					if ($photosUsertag_query) $photosUsertag_num = mysqli_num_rows($photosUsertag_query); else $photosUsertag_num = 0;
					if ($photosUsertag_num > 0) {
						$facesData = array();
						while ($photosUsertag_fetch = mysqli_fetch_assoc($photosUsertag_query)) {
							$checkExists = $_analysis->faces("exists", array("label" => "display", "value" => $photosUsertag_fetch['faces']));
							if (isset($checkExists['return'], $checkExists['exists']) && $checkExists['return'] == true && $checkExists['exists'] == false) {
								mysqli_query($_db->port('beta'), "DELETE FROM `photos_cache_usertag` WHERE `id` = '{$photosUsertag_fetch['id']}'");
								continue;
							}
							$facesData[] = $facesDataThis = array(
								"photos" => $photosUsertag_fetch['photos'],
								"faces" => $photosUsertag_fetch['faces'],
								"thumbnail" => $_tool->links($_parameter->get('pathVirtual_cache_photos').'/'.$fileCache['name'].'/'.$photosUsertag_fetch['display']),
								"display" => $photosUsertag_fetch['display'],
								"position" => array(
									"x" => $photosUsertag_fetch['boundingbox.tl.x'], 
									"y" => $photosUsertag_fetch['boundingbox.tl.y']
								),
								"size" => array(
									"height" => $photosUsertag_fetch['boundingbox.size.height'],
									"width" => $photosUsertag_fetch['boundingbox.size.width']
								),
								"ratio" => array(
									"height" => $photosUsertag_fetch['boundingbox.ratio.height'],
									"width" => $photosUsertag_fetch['boundingbox.ratio.width']
								)
							);
							$facesDataThis['user.id'] = $g_user['id'];
							$facesDataThis['author.type'] = $g_user['mode']['type'];
							$facesDataThis['author.id'] = $g_user['mode']['id'];
							$_SESSION["cache"]['photos_usertag'][] = $facesDataThis;
						}
						if ($robotRequest == $g_client['token']['robot']) {
							die(print json_encode(array("return" => true)));
						}
						...
						$imgUrl = $_tool->links($_parameter->get('pathVirtual_cache_photos').'/'.$fileCache['name']);
						$getAnalysis = $_analysis->faces("check", array("url" => $imgUrl));
						if (isset($getAnalysis['return'], $getAnalysis['faces']) && $getAnalysis['return'] == true) {
							if (count($getAnalysis['faces']) == 0) {
								$_SESSION["cache"]['photos_usertag_notfound'][] = array(
									"user.id" => $g_user['id'],
									"author.type" => $g_user['mode']['type'],
									"author.id" => $g_user['mode']['id'],
									"photos" => $fileCache['secret']
								);
							}
							...
							foreach ($faceAnalysis as $key => $faceAnalysisThis) {
								if ($faceAnalysisThis['confidence'] < $_parameter->get('rekognition_rate_allow')) {
									...
									$faceAnalysis[$key]['photos'] = $fileCache['secret'];
								}
							}
							foreach ($faceAnalysis as $key => $faceAnalysisThis) {
								...
								if (isset($addAnalysisFaces['return'], $addAnalysisFaces['obj']) && $addAnalysisFaces['return'] == true && is_array($addAnalysisFaces['obj']) && count($addAnalysisFaces['obj']) > 0) {
									$cacheTaguser = array();
									$cacheTaguser['photos'] = $fileCache['secret'];
									...
									$insertCacheTaguser = mysqli_query($_db->port('beta'), "INSERT INTO `photos_cache_usertag` (`id`, `user.id`, `author.type`, `author.id`, `photos`, `faces`, `faces.image`, `token`, `display`, `guy.type`, `guy.id`, `boundingbox.tl.y`, `boundingbox.tl.x`, `boundingbox.size.height`, `boundingbox.size.width`, `boundingbox.ratio.height`, `boundingbox.ratio.width`) VALUES (NULL, '{$g_user['id']}', '{$g_user['mode']['type']}', '{$g_user['mode']['id']}', '{$cacheTaguser['photos']}', '{$cacheTaguser['faces']}', '{$cacheTaguser['faces.image']}', '{$cacheTaguser['token']}', '{$cacheTaguser['display']}', null, null, '{$faceAnalysisThis['boundingbox.tl.y']}', '{$faceAnalysisThis['boundingbox.tl.x']}', '{$faceAnalysisThis['boundingbox.size.height']}', '{$faceAnalysisThis['boundingbox.size.width']}', '{$faceAnalysisThis['boundingbox.ratio.height']}', '{$faceAnalysisThis['boundingbox.ratio.width']}');");
									if (!$insertCacheTaguser) {
										...
										$facesData[] = $facesDataThis = array(
											...
											"thumbnail" => $_tool->links($_parameter->get('pathVirtual_cache_photos').'/'.$fileCache['name'].'/'.$cacheTaguser['display']),
