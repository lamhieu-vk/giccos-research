					...
					<div id="messagesData" class="boxGrid">
						...
						<div class="content">
							...
							if ($messagesIdCount == 0) {
								...
								//.
								$codeLinesTab = null;
								$messagesDataLastReuqest = "SELECT DISTINCT  `tab` FROM `messages_data` WHERE `tab` IN (SELECT `tab` FROM `messages_members` WHERE `guy.type` = '{$g_user['mode']['type']}' AND `guy.id` = '{$g_user['mode']['id']}') GROUP BY `tab` ORDER BY MAX(`id`) DESC";
