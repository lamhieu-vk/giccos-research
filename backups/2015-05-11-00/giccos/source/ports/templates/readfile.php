...
if (!file_exists($readFile['path']) && preg_match("/^(http|https|ftp|ftps)/i", $readFile['path'])) {
	if (isset($g_client['fileOtherCache']) && count($g_client['fileOtherCache']) > 0) {
		$fileOtherCache = $g_client['fileOtherCache'];
	}else {
	...
	$_SESSION['client']['fileOtherCache'] = $fileOtherCache;
	$g_client['fileOtherCache'] = $fileOtherCache;
}else {
