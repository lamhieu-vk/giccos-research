...
if (in_array($readFile['mime_type'], $readFilePhotos) && isset($_SESSION['client']['cache']['other']['image']) && is_array($_SESSION['client']['cache']['other']['image']) && count($_SESSION['client']['cache']['other']['image']) > 0) {
	$fileOtherCache = $_SESSION['client']['cache']['other']['image'];
	foreach ($fileOtherCache as $key => $fileOtherCacheValue) {
		if (isset($fileOtherCacheValue['path'], $fileOtherCacheValue['source']) && $fileOtherCacheValue['path'] == $readFile['path']) {
			$readFile['path'] = $fileOtherCacheValue['source'];
		}
	}
}
ob_start();
...
if (!file_exists($readFile['path']) && preg_match("/^(http|https|ftp|ftps)/i", $readFile['path'])) {
	if (isset($_SESSION['client']['cache']['other']['image']) && is_array($_SESSION['client']['cache']['other']['image']) && count($_SESSION['client']['cache']['other']['image']) > 0) {
		$fileOtherCache = $_SESSION['client']['cache']['other']['image'];
	}else {
		$fileOtherCache = array();
	}
	$thisFileisCached = $fileOtherCacheChanged = 0;
	if (in_array($readFile['mime_type'], $readFilePhotos)) {
		$pathFile = $_parameter->get('storage_cache_folder_path').'/image/'.$fileID.'.'.$_tool->mime_content_type($readFile['mime_type'], true);
	}
	if (isset($pathFile) && $pathFile != null) {
		if (file_exists($pathFile)) {
			//.
		}else {
			$fopenPathFile = fopen($pathFile, "wb");
			fwrite($fopenPathFile, $_tool->contentMediaFile($readFile['path'])); 
	    	fclose($fopenPathFile);
	    	$expireTimeTotal = $_tool->timeNow() + $expireTime;
	    	$thisFileHash = $fileID;
	    	$thisFilePath = $_tool->hash('encode', $pathFile, $thisFileToken.'::'.$_parameter->get('hash_cache_image_path'));
	    	mysqli_query($_db->port('beta'), "INSERT INTO `storage_cache_image` (`id`, `time`, `expires`, `hash`, `path`) VALUES (NULL, '{$_tool->timeNow()}', '{$expireTimeTotal}', '{$thisFileHash}', '{$thisFilePath}');");
		}
	}
	if ($thisFileisCached == 0) {
		$lastChange = time();
		$fileOtherCache[] = array("mime" => $readFile['mime'], "path" => $readFile['path'], "source" => $pathFile, "filemtime" => $lastChange);
		$fileOtherCacheChanged++;
	}
	if ($fileOtherCacheChanged > 0) {
		$_SESSION['client']['cache']['other']['image'] = $fileOtherCache;
	}
	if (isset($pathFile)) {
