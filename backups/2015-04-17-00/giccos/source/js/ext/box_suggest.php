					...
					$.ajax({
						...
						success: function (data) {
							if (data['return'] !== undefined && data['return'] == true) {
								...
								popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst', false); ?>', description: transferError(data['reason']), from: '<?php print $_language->text('system', 'ucfirst', false); ?>', timeout: <?php print $_parameter->get('notification_error_timeout'); ?>});
							}
						}
					}).done(function(){
						buttonRequest.each(function(){
