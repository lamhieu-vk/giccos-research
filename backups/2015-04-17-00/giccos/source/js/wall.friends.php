function friendsRequest () {
	...
	rows.find("[friend-request]").bind("click", function(){
		var button = $(this);
		if (button.attr('handling-request') !== undefined) {
			return false;
		}
		if (button.attr('friend-request') !== undefined && typeof JSON.parse(button.attr('friend-request')) == "object") {
			var object = JSON.parse(button.attr('friend-request'));
			if (object['action'] !== undefined && object['id'] !== 0) {
				...
				if (action !== undefined) {
					var data = {'token': '<?php print $g_client['token']['action']['user']; ?>', 'type': 'friends', 'action': action, 'id': id};
				}
...
function friendsLoad (object) {
	...
	if (content.attr("handling-load") != "true") {
		...
		$.ajax({
			type: "POST",
			dataType: "json",
			url: "<?php print $_tool->links("source/ajax/wall.ajax"); ?>",
			data: {"token": "<?php print $g_client['token']['wall']['friends']; ?>", "wall_id": wall_id, "port": "friends", "type": object['type'], "friends_id": object['id'], "order": object['order'], "limit": object['limit']},
			contentType: "<?php print $_parameter->get("contentType_urlencoded.utf8"); ?>",
			success: function (data) {
				console.log(data);
