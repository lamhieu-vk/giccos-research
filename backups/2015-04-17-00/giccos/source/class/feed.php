...
Class feed {
				...
				if ($length == 0) {
					...
					if ($status['type'] == "status") {
						...
						while ($media_fetch = mysqli_fetch_assoc($media_query)) {
							$media_remove = $this->class['_media']->data("photos", array("action" => "delete", "id" => $media_fetch['photos.id'], "private" => true, "author" => array("type" => $status['author.type'], "id" => $status['author.id'])));
						}
						...
						while ($media_fetch = mysqli_fetch_assoc($media_query)) {
							$media_remove = $this->class['_media']->data("music", array("action" => "delete", "id" => $media_fetch['music.id'], "private" => true, "author" => array("type" => $status['author.type'], "id" => $status['author.id'])));
						}
						...
						while ($media_fetch = mysqli_fetch_assoc($media_query)) {
							$media_remove = $this->class['_media']->data("videos", array("action" => "delete", "id" => $media_fetch['videos.id'], "private" => true, "author" => array("type" => $status['author.type'], "id" => $status['author.id'])));
						}
						...
						while ($media_fetch = mysqli_fetch_assoc($media_query)) {
							...
							if ($media_this_length > 0) {
								...
								if ($media_this['photos.id'] != '0') {
									$media_remove = $this->class['_media']->data("photos", array("action" => "delete", "id" => $media_this['photos.id'], "private" => true, "author" => array("type" => $status['author.type'], "id" => $status['author.id'])));
								}
	...
	function status_add ($object) {
		if (isset($object) && is_array($object)) {
			...
			$token = hash('md5', time().'::('.$author['type'].'_'.$author['id'].')'.'::'.rand());
			$time = time();
			$edit = false;
			...
			$link = $share = 0;
			$content = $this->class['_tool']->hash('encode', $this->class['_tool']->convertDatabaseString($content), $this->class['_parameter']->get('hash_status_content').'::'.$token);
			$insert = mysqli_query($db,"INSERT INTO `status` (`id`, `token`, `time`, `edit`, `user.id`, `author.type`, `author.id`, `private.view`, `private.comment`, `private.share`, `type`, `content`, `usertag`, `hashtag`, `places`, `mood`, `link`, `share`) VALUES (null, '{$token}', '{$time}', '{$edit}', '{$this->user['id']}', '{$author['type']}', '{$author['id']}', '{$private['view']}', '{$private['comment']}', '{$private['share']}', '{$type}', '{$content}', '{$this->usertag['type']}', '{$hashtag_value}', '{$places}', '{$mood}', '{$link}', '{$share}')");
			...
			mysqli_query($db,"INSERT INTO `status_follow` (`id`, `time`, `status.id`, `guy.type`, `guy.id`) VALUES (null, '{$time}', '{$status['id']}', '{$author['type']}', '{$author['id']}')");
			if ($type == 1 || $type == "status") {
				if (isset($object['link']) && $object['link'] != false && isset($object['link']['path']) && $object['link']['path'] != null) {
					...
					if (isset($this_id) && $this_id > 0) {
						...
						mysqli_query($db,"INSERT INTO `status_share` (`id`, `time`, `status.id`, `this.id`, `guy.type`, `guy.id`) VALUES (null, '{$this->class['_tool']->timeNow()}', '{$this_id}', '{$status['id']}', '{$author['type']}', '{$author['id']}')");
						$is_share = true;
				...
				if (isset($object['media']['file']) && count($object['media']['file']) > 0) {
					if (is_array($object['media']['scrapbook']) && isset($object['media']['scrapbook']['value'])) {
						if ($object['media']['scrapbook']['value'] == true) {
							if (isset($object['media']['scrapbook']['data']['name']['token']) && $object['media']['scrapbook']['data']['name']['token'] != null) {
								...
								$scrapbook_query = mysqli_query($db,"SELECT `id` FROM `photos_scrapbook` WHERE `token` = '{$scrapbook_token}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
								...
								$scrapbook_name = $this->class['_tool']->convertDatabaseString($object['media']['scrapbook']['data']['name']);
								$scrapbook_query = mysqli_query($db,"SELECT `id` FROM `photos_scrapbook` WHERE `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}' AND `name` = '{$scrapbook_name}'");
								if (mysqli_num_rows($scrapbook_query) > 0) {
									$scrapbook = mysqli_fetch_assoc($scrapbook_query)['id'];
								}else {
									if (isset($object['media']['scrapbook']['data']['places']) && is_array($object['media']['scrapbook']['data']['places']) && $object['media']['scrapbook']['data']['places'] != null) {
										if (isset($object['media']['scrapbook']['data']['places']['location']) && $object['media']['scrapbook']['data']['places']['location'] != null) {
											$object['media']['scrapbook']['data']['places']['location'] = $this->class['_tool']->stringFloat("10,6", $object['media']['scrapbook']['data']['places']['location'], 2);
									...
									if (isset($object['media']['scrapbook']['data']['description']) && $object['media']['scrapbook']['data']['description'] != null) {
										$scrapbook_description = $this->class['_tool']->convertDatabaseString($object['media']['scrapbook']['data']['description']);
									}else {
										$scrapbook_description = null;
									}
									$scrapbook_token = hash('crc32', time().'::'.rand().'::('.$author['type'].'_'.$author['id'].')');
									mysqli_query($db, "INSERT INTO `photos_scrapbook` (`id`, `author.type`, `author.id`, `token`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$scrapbook_token}', '{$time}', '{$scrapbook_name}', '{$scrapbook_places['type']}', '{$scrapbook_places['value']}', '{$scrapbook_description}')");
					...
					foreach ($object['media']['file'] as $key => $value) {
						$upload_file = $this->class['_media']->cache("photos", array("action" => "upload", "secret" => $value['secret'], "name" => $value['name'], "scrapbook" => $scrapbook, "multiple" => true, "attachments" => $attachments, "public" => $public));
				...
				if (isset($object['media']['file']) && count($object['media']['file']) > 0) {
					if (isset($object['media']['info']['data']['album'])) {
						...
						if (is_array($object_album) && isset($object_album['token']) && $object_album['token'] != null) {
							...
							$album_query = mysqli_query($db,"SELECT `id` FROM `music_album` WHERE `token` = '{$album_token}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
							...
							$album_name = $this->class['_tool']->convertDatabaseString($object_album);
							$album_query = mysqli_query($db,"SELECT `id` FROM `music_album` WHERE `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}' AND `name` = '{$album_name}'");
							if (mysqli_num_rows($album_query) > 0) {
								$album = mysqli_fetch_assoc($album_query)['id'];
							}else {
								$album_places['type'] = 0;
								...
								$album_token = hash('crc32', time().'::'.rand().'::('.$author['type'].'_'.$author['id'].')');
								mysqli_query($db,"INSERT INTO `music_album` (`id`, `author.type`, `author.id`, `token`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$album_token}', '{$time}', '{$album_name}', '{$album_places['type']}', '{$album_places['id']}', '{$album_description}')");
					...
					foreach ($object['media']['file'] as $key => $value) {
						...
						$upload_file = $this->class['_media']->cache("music", array("action" => "upload", "secret" => $value['secret'], "name" => $value['name'], "album" => $album, "info" => $info));
						if (isset($upload_file['return']) && $upload_file['return'] == true) {
				...
				if (isset($object['media']['file']) && count($object['media']['file']) > 0) {
					if (isset($object['media']['info']['data']['playlist'])) {
						...
						if (is_array($object_playlist) && isset($object_playlist['token']) && $object_playlist['token'] != null) {
							...
							$playlist_query = mysqli_query($db,"SELECT `id` FROM `videos_playlist` WHERE `token` = '{$playlist_token}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
							...
							$playlist_name = $this->class['_tool']->convertDatabaseString($object_playlist);
							$playlist_query = mysqli_query($db,"SELECT `id` FROM `videos_playlist` WHERE `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}' AND `name` = '{$playlist_name}'");
							if (mysqli_num_rows($playlist_query) > 0) {
								$playlist = mysqli_fetch_assoc($playlist_query)['id'];
							}else {
								...
								$playlist_token = hash('crc32', time().'::'.rand().'::('.$author['type'].'_'.$author['id'].')');
								mysqli_query($db,"INSERT INTO `videos_playlist` (`id`, `author.type`, `author.id`, `token`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$playlist_token}', '{$time}', '{$playlist_name}', '{$playlist_places['type']}', '{$playlist_places['id']}', '{$playlist_description}')");
					...
					foreach ($object['media']['file'] as $key => $value) {
						$info = $object['media']['info']['data'];
						$upload_file = $this->class['_media']->cache("videos", array("action" => "upload", "secret" => $value['secret'], "name" => $value['name'], "playlist" => $playlist, "info" => $info));
						if (isset($upload_file['return']) && $upload_file['return'] == true) {
				...
				if (isset($object['media']['info']['data']) && is_array($object['media']['info']['data'])) {
					...
					$cited_token = hash('crc32', md5(time().'::'.rand().'::'.base64_encode($media_data['author'].'::'.$media_data['quote'])));
					mysqli_query($db, "INSERT INTO `cited` (`id`, `token`, `author.type`, `author.id`, `time`, `author`, `quote`, `photos.id`) VALUES (null, '{$cited_token}', '{$author['type']}', '{$author['id']}', '{$time}', '{$media_data['author']}', '{$media_data['quote']}', '{$media_data['photos.id']}')");
					...
					if (isset($object['media']['file']) && count($object['media']['file']) > 0) {
						...
						//$upload_file = $this->class['_media']->cache("photos", array("action" => "upload", "secret" => $value['secret'], "name" => $value['name'], "scrapbook" => $scrapbook, "multiple" => false));
						$upload_file = $this->class['_media']->cache("photos", array("action" => "upload", "secret" => $value['secret'], "name" => $value['name'], "scrapbook" => $scrapbook, "multiple" => false, "attachments" => false, "public" => false, "maxsize" => array("width" => $_parameter->get('cited_photosSize_length'))));
						if (isset($upload_file['return']) && $upload_file['return'] == true) {
			...
			if (isset($is_share, $is_share_statusID) && $is_share == true) {
				$this->class['_logs']->actions("add", array("action" => "share", "things" => "status", "things_id" => $is_share_statusID, "obj" => "status", "obj_id" => $status['id']));
			}else {
				$this->class['_logs']->actions("add", array("action" => "add", "things" => null, "things_id" => null, "obj" => "status", "obj_id" => $status['id']));
			}
			return array("return" => true);
		}else {
			return array("return" => false, "reason" => "");
	...
	function status_get ($object) {
		...
		if (mysqli_num_rows($query) == 0) {
			...
			while ($status = mysqli_fetch_assoc($query)) {
				...
				$info['link'] = $this->class['_tool']->links('feed/status/').$info['token'];
				...
				if ($status['edit'] == 0) {
					...
					$edit['time']['ago'] = $this->class['_tool']->agoDatetime($status['edit'], 'ago');
					$edit['time']['tip'] = $this->class['_tool']->agoDatetime($status['edit'], 'tip');
					$edit['time']['json'] = json_encode(array("stamp" => $edit['time']['stamp'], "length" => "1"));
				}
				if ($status['places'] == 0) {
					$places = null;
				}else if ($status['places'] == 1) {
					$places_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `type`, `value` FROM `status_places` WHERE `status.id` = '{$status['id']}'");
					if (mysqli_num_rows($places_query) == 0) {
						$places = null;
					}else {
						$places = mysqli_fetch_assoc($places_query);
						if ($places['type'] == 1) {
							$places_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `address`, `location`, `name`, `thumbnail`, `type` FROM `maps_places` WHERE `id` = '{$places['value']}'");
							$places = mysqli_fetch_assoc($places_query);
							$places['by'] = 1;
				...
				$date['ago'] = $this->class['_tool']->agoDatetime($status['date'], 'ago');
				$date['tip'] = $this->class['_tool']->agoDatetime($status['date'], 'tip');
				$date['json'] = json_encode(array("stamp" => $date['stamp'], "length" => "1"));
				$private['view']['text'] = $this->class['_language']->text("private_{$status['private.view']}_text", "ucfirst");
				$private['view']['tip'] = $this->class['_language']->text("private_{$status['private.view']}_tip", "ucfirst");
				$private['view']['stamp'] = $status['private.view'];
				$private['view']['json'] = json_encode(array("stamp" => $private['view']['stamp']));
				$private['comment']['text'] = $this->class['_language']->text("private_{$status['private.comment']}_text", "ucfirst");
				$private['comment']['tip'] = $this->class['_language']->text("private_{$status['private.comment']}_tip", "ucfirst");
				$private['comment']['stamp'] = $status['private.comment'];
				$private['comment']['json'] = json_encode(array("stamp" => $private['comment']['stamp']));
				$private['share']['text'] = $this->class['_language']->text("private_{$status['private.share']}_text", "ucfirst");
				$private['share']['tip'] = $this->class['_language']->text("private_{$status['private.share']}_tip", "ucfirst");
				$private['share']['stamp'] = $status['private.share'];
				$private['share']['json'] = json_encode(array("stamp" => $private['share']['stamp']));
				if ($status['mood'] == 0) {
					$content['mood'] = null;
				}else {
					$mood_query = mysqli_query($this->class['_db']->port('beta'),"SELECT * FROM `status_mood` WHERE `status.id` = '{$status['id']}'");
					if (mysqli_num_rows($mood_query) == 0) {
						...
						if ($mood['type'] == 1) {
							if (preg_match("/^(\[feel\])/", $mood['value'])) {
								$content['mood']['push'] = $this->class['_language']->text('feeling');
							}else {
								$content['mood']['push'] = null;
							}
							$content['mood']['text'] = $content['mood']['push'].' '.$this->class['_language']->text($mood['value']).'.';
				...
				if ($status['usertag'] == 0) {
					...
					$usertag_query = mysqli_query($this->class['_db']->port('beta'),"SELECT * FROM `status_usertag` WHERE `status.id` = '{$status['id']}'");
					if (mysqli_num_rows($usertag_query) == 0) {
						...
						while ($usertag = mysqli_fetch_assoc($usertag_query)) {
							if ($usertag['guy.type'] == "user" || $usertag['guy.type'] == "users") {
								$usertag_user = mysqli_fetch_assoc(mysqli_query($this->class['_db']->port('beta'),"SELECT `id`, `fullname`, `username`, `link` FROM `users` WHERE `id` = '{$usertag['guy.id']}'"));
				...
				$content['string'] = $this->class['_tool']->hash('decode', $status['content'], $this->class['_parameter']->get('hash_status_content').'::'.$status['token']);
				$content['text'] = $this->class['_tool']->parseContent($content['string']);
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
					$media_query = mysqli_query($this->class['_db']->port('beta'),"SELECT * FROM `status_link` WHERE `status.id` = '{$status['id']}' LIMIT 1");
					if (mysqli_num_rows($media_query) == 0) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							$rows = "`token`, `host`, `path`, `title`, `thumbnail`, `description`";
							$media_fetch = $this->class['_sites']->data(array("rows" => $rows, "id" => $media_data['sites.id']));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								if (isset($media_fetch['data']['thumbnail']) && $media_fetch['data']['thumbnail'] != null && ($media_fetch['data']['thumbnail'] != 0 || $media_fetch['data']['thumbnail'] != "0")) {
									$media_fetch['data']['thumbnail'] = $this->class['_tool']->links($media_fetch['data']['thumbnail']);
								}else {
									$media_fetch['data']['thumbnail'] = null;
								}
								$media_fetch['data']['path'] = $this->class['_tool']->links('sites/direct/'.$this->class['_tool']->hash('encode', $media_fetch['data']['path'], $this->class['_parameter']->get('hash_sites_direct')));
								$content['media']['data'][] = array(
									"token" => $media_fetch['data']['token'],
									"host" => $media_fetch['data']['host'],
									"title" => $media_fetch['data']['title'],
									"description" => $media_fetch['data']['description'],
									"thumbnail" => $media_fetch['data']['thumbnail'],
									"path" => $media_fetch['data']['path']
								);
							}
						}
					}
				}else if ($getMedia == true && $info['type'] == "status" && $status['share'] == 1) {
					$media_sql = "SELECT * FROM `status` WHERE `id` = (SELECT `status.id` FROM `status_share` WHERE `this.id` = '{$status['id']}' LIMIT 1) LIMIT 1";
					$media_query = mysqli_query($this->class['_db']->port('beta'), $media_sql);
					if (mysqli_num_rows($media_query) == 0) {
						$content['media'] = null;
					}else {
						$content['media'] = array();
					...
					$media_query = mysqli_query($this->class['_db']->port('beta'),"SELECT DISTINCT `status.id`, `photos.id` FROM `status_photos` WHERE `status.id` = '{$status['id']}'");
					if (mysqli_num_rows($media_query) == 0) {
						$content['media'] = null;
					}else {
						$content['media'] = array();
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							$rows = "`token`, `mime_type`, `file.original`, `file.large`, `file.medium`";
							$media_fetch = $this->class['_media']->data("photos", array("action" => "get", "rows" => $rows, "id" => $media_data['photos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id'])));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								if ($media_fetch['data']['file.medium'] != '0') {
									$media_thumbnail = $this->class['_tool']->links("photos/raw/").$media_fetch['data']['file.medium'];
								}else {
									$media_thumbnail = $this->class['_tool']->links("photos/raw/").$media_fetch['data']['file.original'];
								}
								if ($media_fetch['data']['file.large'] != '0') {
									$media_link = $this->class['_tool']->links("photos/raw/").$media_fetch['data']['file.large'];
								}else {
									$media_link = $this->class['_tool']->links("photos/raw/").$media_fetch['data']['file.original'];
								}
								$content['media']['data'][] = array(
									"token" => $media_fetch['data']['token'], 
									"mime" => $media_fetch['data']['mime_type'], 
					...
					$media_query = mysqli_query($this->class['_db']->port('beta'), "SELECT DISTINCT `status.id`, `music.id` FROM `status_music` WHERE `status.id` = '{$status['id']}' LIMIT 1");
					if (mysqli_num_rows($media_query) == 0) {
						$content['media'] = null;
					}else {
						$content['media'] = array();
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							$rows = "`token`, `name`, `mime_type`, `singer`, `description`, `file.original`";
							$media_fetch = $this->class['_media']->data("music", array("action" => "get", "rows" => $rows, "id" => $media_data['music.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id'])));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								if ($media_fetch['data']['name'] == null) {
									$media_fetch['data']['name'] = $this->class['_language']->text('null:music_name', 'ucfirst');
								}
								if ($media_fetch['data']['singer'] == null) {
									$media_fetch['data']['singer'] = $this->class['_language']->text('not_updated', 'ucfirst');
								}else {
									$media_fetch['data']['singer'] = $this->class['_language']->text('presented_by', 'ucfirst').': '.$media_fetch['data']['singer'];
								}
								if ($media_fetch['data']['description'] == null) {
									$media_fetch['data']['description'] = $this->class['_language']->text('null:description_music', 'ucfirst');
								}
								$media_fetch['data']['link'] = $this->class['_tool']->links("music/raw/").$media_fetch['data']['file.original'];
								$content['media']['data'][] = array(
									"token" => $media_fetch['data']['token'], 
									"name" => $media_fetch['data']['name'],
									"mime" => $media_fetch['data']['mime_type'], 
									"singer" => $media_fetch['data']['singer'], 
									"description" => $media_fetch['data']['description'],
									"link" => $media_fetch['data']['link']
								);
							}else {
								//.
							}
					...
					$media_query = mysqli_query($this->class['_db']->port('beta'),"SELECT DISTINCT `status.id`, `videos.id` FROM `status_videos` WHERE `status.id` = '{$status['id']}' LIMIT 1");
					if (mysqli_num_rows($media_query) == 0) {
						$content['media'] = null;
					}else {
						$content['media'] = array();
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							$rows = "`token`, `name`, `mime_type`, `description`, `file.thumbnail`, `file.sd`";
							$media_fetch = $this->class['_media']->data("videos", array("action" => "get", "rows" => $rows, "id" => $media_data['videos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id'])));
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								if ($media_fetch['data']['name'] == null) {
									$media_fetch['data']['name'] = $this->class['_language']->text('null:videos_name', 'ucfirst');
								}
								if ($media_fetch['data']['description'] == null) {
									$media_fetch['data']['description'] = $this->class['_language']->text('null:description_videos', 'ucfirst');
								}
								$media_fetch['data']['thumbnail'] = $this->class['_tool']->links("videos/raw/thumbnail/").$media_fetch['data']['file.thumbnail'];
								$media_fetch['data']['link'] = $this->class['_tool']->links("videos/raw/").$media_fetch['data']['file.sd'];
								$content['media']['data'][] = array(
									"token" => $media_fetch['data']['token'], 
									"name" => $media_fetch['data']['name'],
									"mime" => $media_fetch['data']['mime_type'],
					...
					$media_query = mysqli_query($this->class['_db']->port('beta'),"SELECT * FROM `status_cited` WHERE `status.id` = '{$status['id']}' LIMIT 1");
					if (mysqli_num_rows($media_query) == 0) {
						$content['media'] = null;
					}else {
						$content['media'] = array();
						...
						$cited_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `token`, `quote`, `author`, `photos.id` FROM `cited` WHERE `id` = '{$cited_id}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}' LIMIT 1");
						$cited_fetch = mysqli_fetch_assoc($cited_query);
						if ($cited_fetch['photos.id'] != '0') {
							$rows = "`token`, `file.original`, `file.medium`";
							$media_fetch = $this->class['_media']->data("photos", array("action" => "get", "rows" => $rows, "id" => $cited_fetch['photos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id'])));						}else {
							$media_fetch['return'] = false;
						}
						if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
							if ($media_fetch['data']['file.medium'] != '0') {
								$media_path = $media_fetch['data']['file.medium'];
							}else {
								$media_path = $media_fetch['data']['file.original'];
							}
							$cited_fetch['thumbnail'] = $this->class['_tool']->links("photos/raw/").$media_path;
						}else {
							$cited_fetch['thumbnail'] = null;
						}
						$content['media']['data'][] = array(
							"token" => $cited_fetch['token'],
				...
				if (mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_follow` WHERE `status.id` = '{$info['id']}' AND `guy.id` = '{$this->user['mode']['id']}' AND `guy.type` = '{$this->user['mode']['type']}'")) > 0) {
				...
				$statistic['follow'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_follow` WHERE `status.id` = '{$info['id']}'"));
				$statistic['favorite'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_favorite` WHERE `status.id` = '{$info['id']}'"));
				$statistic['share'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_share` WHERE `status.id` = '{$info['id']}'"));
				$statistic['invite'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_invite` WHERE `status.id` = '{$info['id']}'"));
				...
				if ($author['type'] == $this->user['mode']['type'] && $author['id'] == $this->user['mode']['id']) {
					...
					$action['comment']['check'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `friends` WHERE `user.id` = '{$this->user['id']}' AND `guy.id` = '{$author['id']}' LIMIT 1"));
				}else if ($private['comment']['stamp'] == 3) {
					if ($author['type'] == "user") {
						$action['comment']['check'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `friends` WHERE (`user.id` = '{$this->user['id']}' AND `guy.id` = '{$author['id']}') OR (`guy.id` = '{$author['id']}' AND `user.id` IN (SELECT `guy.id` FROM `friends` WHERE `user.id` = '{$this->user['id']}')) LIMIT 1"));
					}else {
						$action['comment']['check'] = 0;
					}
				...
				if ($action['favorite']['check'] > 0) {
					...
					$action['favorite']['selected'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_favorite` WHERE `status.id` = '{$info['id']}' AND `guy.id` = '{$this->user['mode']['id']}' AND `guy.type` = '{$this->user['mode']['type']}'"));
				...
				$comment_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_comment` WHERE `status.id` = '{$status['id']}'");
			...
			if ($return == "json") {
				for ($i = 0; $i < count($statusData); $i++ ) {
					$query = "SELECT * FROM `status_comment` WHERE `status.id` = '{$statusData[$i]['info']['id']}' ORDER BY `id` DESC LIMIT 3";
					$comment = $this->comment_get(array("return" => "json", "query" => $query));
					if ($comment['return'] == true) {
						$returnData[$i]['comment']['data'] = $comment['data'];
					}
					$query = $comment = null;
				}
				...
				for ($i = 0; $i < count($statusData); $i++ ) {
					$status = null;
					$status = $statusData[$i];
					$c_['status_info'] = json_encode($status['info']);
					if (isset($status['author']['verified']) && $status['author']['verified'] == 1) {
						$c_['status_author_verified'] = "
							<div class='verified _tt e' title='{$this->class['_language']->text('verified', 'ucfirst')}'>
								<i></i>
							</div>
						";
					}else {
					...
					if (isset($status['date']) && $status['date'] != null) {
						$c_['status_date'] = "
							<div class='date nowrap _tt s' time='{$status['date']['json']}' title='{$this->class['_language']->text('posted_at', 'ucfirst')} {$status['date']['tip']}'>
								<i></i>
								<a href='{$this->class['_tool']->links('feed/time')}/{$status['date']['stamp']}' date-show>{$status['date']['ago']}</a>
							</div>
						";
					}else {
						$c_['status_date'] = null;
					...
					if (isset($status['edit']['status']) && $status['edit']['status'] == true) {
						$c_['status_edit'] = "
							<div class='edited nowrap _tt s' time='{$status['edit']['time']['json']}' title='{$this->class['_language']->text('edited_at', 'ucfirst')} {$status['edit']['time']['tip']}'>
								<i></i>
							</div>
						";
					}else {
					...
					if (isset($status['places']) && $status['places'] != null) {
						if ($status['places']['by'] == 1) {
							...
							$c_['status_places_dbl_data'] = array(
								"type" => 1, 
								"title" => $this->class['_language']->text('information_about_places', 'ucfirst'),
								"description" => array("type" => "info::places", "data" => $status['places']),
								"from" => $this->class['_language']->text('status', 'ucfirst'),
								"timeout" => $this->class['_parameter']->get('notification_info_places')
							);
							...
							$c_['status_places'] = "
								<div class='places nowrap _tt _tt-dbl s' title='{$this->class['_language']->text('at', 'ucfirst')} {$status['places']['name']}' dblclick='{$c_['status_places_dbl']}'>
									<i></i>
								</div>
							";
						}else if ($status['places']['by'] == 2) {
							$c_['status_places'] = "
								<div class='places nowrap _tt _tt-dbl s' title='{$this->class['_language']->text('at', 'ucfirst')} {$status['places']['name']}'>
									<i></i>
								</div>
							";
						}
									...
									if ($text == "block" && $menu_text == true) {
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='block'>
												<i></i>
												<span>{$this->class['_language']->text('block', 'ucfirst')}</span>
											</div>
										";
										...
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='report'>
												<i></i>
												<span>{$this->class['_language']->text('report', 'ucfirst')}</span>
											</div>
										";
										...
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='edit'>
												<i></i>
												<span>{$this->class['_language']->text('edit', 'ucfirst')}</span>
											</div>
										";
										...
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='delete'>
												<i></i>
												<span>{$this->class['_language']->text('delete', 'ucfirst')}</span>
											</div>
										";
									}else if ($text == "follow") {
										if ($menu_text == true) {
											$c_['status_menu_tab'] .= "
												<div class='nowrap' menu-type='unfollow'>
													<i></i>
													<span>{$this->class['_language']->text('unfollow', 'ucfirst')}</span>
												</div>
											";
										}else {
											$c_['status_menu_tab'] .= "
												<div class='nowrap' menu-type='follow'>
													<i></i>
													<span>{$this->class['_language']->text('follow', 'ucfirst')}</span>
												</div>
											";
										}
										...
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='popup'>
												<i></i>
												<span>{$this->class['_language']->text('popup', 'ucfirst')}</span>
											</div>
										";
										...
										$c_['status_menu_tab'] .= "
											<div class='nowrap' menu-type='link'>
												<i></i>
												<span><a href='{$this->class['_tool']->links("feed/status/")}{$status['info']['token']}' target='_blank'>{$this->class['_language']->text('link', 'ucfirst')}</a></span>
											</div>
										";
									}
								}else if ($type == "author") {
					...
					if (isset($status['content']['usertag']) && $status['content']['usertag'] != null) {
						...
						$c_['status_content_usertag'] = "
							<div class='usertag'>
								<div class='with'><span>- {$this->class['_language']->text('with', 'normal')}</span></div>
					...
					if (isset($status['action']['favorite']['status']) && $status['action']['favorite']['status'] == true) {
						if ($status['action']['favorite']['selected'] == true) {
							$c_['status_action_favorite'] = "
								<div class='favorite'>
									<span status-action-favorite favorite='true'>{$this->class['_language']->text("unfavorite", "ucfirst")}</span>
								</div>
							";
						}else {
							$c_['status_action_favorite'] = "
								<div class='favorite'>
									<span status-action-favorite favorite='false'>{$this->class['_language']->text("favorite", "ucfirst")}</span>
								</div>
							";
						}
					}else {
					...
					if (isset($status['action']['share']['status']) && $status['action']['share']['status'] == true) {
						$c_['status_action_share'] = "
							<div class='share'>
								<span status-action-share>{$this->class['_language']->text("share", "ucfirst")}</span>
							</div>
						";
					}else {
					...
					if (isset($status['action']['invite']['status']) && $status['action']['invite']['status'] == true) {
						$c_['status_action_invite'] = "
							<div class='invite'>
								<span status-action-invite>{$this->class['_language']->text("invite", "ucfirst")}</span>
							</div>
						";
					}else {
					...
					if (isset($status['action']['comment']['status']) && $status['action']['comment']['status'] == true) {
						...
						$c_['private_comment'] = "
							<textarea type='text' placeholder='{$this->class['_language']->text('write_a_comment', 'ucfirst')}' status-action-comment handing='false'></textarea>
						";
					}else {
						$c_['private_comment_class'] = "null";
						$c_['private_comment'] = "
							<span>{$this->class['_language']->text('status_unlimited_comment', 'ucfirst')}</span>
						";
					}
					if (isset($status['comment']['number']) && $status['comment']['number'] == 0) {
						$c_['status_comment_null'] = "
							<div class='null' status-comment-null>
								<span status-comment-null>{$this->class['_language']->text('status_null_comment', 'ucfirst')}</span>
							</div>
						";
						...
						if ($status['comment']['number'] > 3) {
							$c_['status_comment_showmore'] = "
								<div class='showmore' status-comment-load>
									<i class='icon'></i>
									<span class='text'>{$this->class['_language']->text('show_more_comments', 'ucfirst')}</span>
								</div>
							";
						}else {
						...
						$c_['status_comment'] = "
							<div class='statistic'>
								<div class='info'>
									...
									<span class='text'>{$this->class['_language']->text('comment(s)', 'strtolower')}</span>
								</div>
								{$c_['status_comment_showmore']}
							</div>
						";
	...
	function status_getCodeMedia ($object) {
		...
		if (isset($object['media']['type']) && $object['media']['type'] == "link") {
			...
			foreach ($object['media']['data'] as $key => $value) {
				$value['id'] = "media_link_".$value['token']."_".rand();
				$value['info'] = json_encode(array("type" => "link", "id" => $value['id'], "host" => $value['host'], "path" => $value['path'], "thumbnail" => $value['thumbnail'], "title" => $value['title'], "description" => $value['description']));
				...
				$c_['status_content_media'] .= "
				<div class='link' id='{$value['id']}' media='link' media-token='{$value['token']}' media-info='{$value['info']}' direct='{$value['direct']}'>
			...
			foreach ($object['media']['data'] as $key => $value) {
				$value['id'] = "media_photos_".$value['token']."_".rand();
				$value['info'] = json_encode(array("type" => "photos", "id" => $value['id'], "mime" => $value['mime'], "thumbnail" => $value['thumbnail'], "link" => $value['link']));
				$c_['status_content_media'] .= "
					<div class='photos' id='{$value['id']}' media='photos' media-token='{$value['token']}' media-info='{$value['info']}' style='background-image: url({$value['thumbnail']})'>
			...
			$c_['status_content_media'] = "<div class='media MusicBox'>";
				foreach ($object['media']['data'] as $key => $value) {
					$value['id'] = "media_music_".$value['token']."_".rand();
					$value['info'] = json_encode(array("type" => "music", "id" => $value['id'], "link" => $value['link'], "name" => $value['name'], "mime" => $value['mime'], "singer" => $value['singer'], "description" => $value['description']));
					$c_['status_content_media'] .= "
					<div class='music' id='{$value['id']}' media='music' media-token='{$value['token']}' media-info='{$value['info']}'>
						<div class='icon'>
			...
			$c_['status_content_media'] = "<div class='media VideosBox'>";
				foreach ($object['media']['data'] as $key => $value) {
					$value['id'] = "media_videos_".$value['token']."_".rand();
					$value['info'] = json_encode(array("type" => "videos", "id" => $value['id'], "link" => $value['link'], "thumbnail" => $value['thumbnail'], "name" => $value['name'], "mime" => $value['mime'], "description" => $value['description']));
					$c_['status_content_media'] .= "
					<div class='videos' id={$value['id']}' media='videos' media-token='{$value['token']}' media-info='{$value['info']}' style='background-image: url({$value['thumbnail']});'>
						<div class='wrap'></div>
						<div class='icon'>
							<i class='fa fa-play-circle'></i>
						</div>
			...
			$value['info'] = json_encode(array("type" => "cited", "id" => $value['id'], "author" => $value['author'], "quote" => $value['quote']));
			...
			if (isset($object['media']['data'][0]['exists']) && $object['media']['data'][0]['exists'] == true) {
				...
				$c_['status_content_media'] .= "
					...
					<div class='share' media='share'>
						<div class='author'><span>{$this->class['_language']->text('from', 'strtolower', false)} <a href='{$info['link']}' target='_blank'>{$this->class['_language']->text('status', 'strtolower', false)}</a> {$this->class['_language']->text('by', 'strtolower', false)} <a href='{$author['link']}' target='_blank'>{$author['name']} ({$author['tag']})</a></span></div>
						<div class='text'>
	...
	function status_statistic ($object) {
		...
		if (isset($object['from']) && is_string($object['from'])) $from = $object['from']; else $from = "all";
		if ($status_id > 0) {
			$db = $this->class['_db']->port('beta');
			if ($status_id > 0) {
				...
				if ($statusNum > 0) {
					...
					return array("return" => true, "statistic" => $statistic, "list" => $list);
				}else {
					return array("return" => false, "reason" => "FEEDS_004");
				}
			}else {
				return array("return" => false, "reason" => "FEEDS_001");
			}
		}else {
			return array("return" => false, "reason" => "");
		}
	...
	function status_follow ($object) {
		...
		if ($status_id > 0 && ($action == "add" || $action == "remove")) {
			...
			if ($user['login'] && $user['id'] > 0) {
				$status_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `id`, `user.id`, `author.id`, `author.type`, `private.comment` FROM `status` WHERE `id` = '{$status_id}'");
				if (mysqli_num_rows($status_query) > 0) {
					$status = mysqli_fetch_assoc($status_query);
					$follow_num = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_follow` WHERE `status.id` = '{$status['id']}' AND `guy.id` = '{$user['mode']['id']}' AND `guy.type` = '{$user['mode']['type']}'"));
					if ($follow_num == 0 && $action == "add") {
						$relationship_num = 0;
						$status_private = $status['private.comment'];
						if ($status['author.type'] == 'user' && $user['mode']['type'] == 'user') {
							if ($user['id'] == $status['user.id']) {
								$relationship_num = 1;
							}
							if ($relationship_num = 0) {
								$num = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `friends` WHERE `user.id` = '{$user['id']}' AND `guy.id` = '{$status['user.id']}'"));
								if ($num > 0) {
									$relationship_num = 2;
								}
							}
							if ($relationship_num = 0) {
								$num = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `friends` WHERE `user.id` = '{$user['id']}' AND (`guy.id` = '{$status['user.id']}' OR `guy.id` IN (SELECT `user.id` FROM `friends` WHERE `guy.id` = '{$status['user.id']}'))"));
								if ($num > 0) {
									$relationship_num = 3;
								}
							}
							...
							if ($user['mode']['id'] == $status['author.id'] && $user['mode']['type'] == $status['author.type']) {
								$relationship_num = 1;
							}
							if ($relationship_num = 0) {
								$relationship_num = 4;
						...
						if ($relationship_num < $status_private) {
							$time = time();
							$action = mysqli_query($this->class['_db']->port('beta'),"INSERT INTO `status_follow` (`id`, `time`, `status.id`, `guy.id`, `guy.type`) VALUES (null, '{$time}', '{$status['id']}', '{$user['mode']['id']}', '{$user['mode']['type']}')");
							if ($action) {
								return array("return" => true);
							}else {
					...
					}else if ($follow_num > 0 && $action == "remove") {
						$action = mysqli_query($this->class['_db']->port('beta'),"DELETE FROM `status_follow` WHERE `status.id` = '{$status['id']}' AND `guy.id` = '{$user['mode']['id']}' AND `guy.type` = '{$user['mode']['type']}'");
						if ($action) {
							return array("return" => true);
						}else {
							return array("return" => false, "reason" => "ERROR#FEEDS_007");
					...
					return array("return" => false, "reason" => "ERROR#FEEDS_004");
				}
			}else {
			...
			if ($status_id == 0) {
				return array("return" => false, "reason" => "ERROR#FEEDS_001");
			}else {
				return array("return" => false, "reason" => "ERROR#FEEDS_002");
			}
		}
	...
	function status_favorite ($object) {
		...
		if ($status_id > 0 && ($action == "add" || $action == "remove")) {
			$db = $this->class['_db']->port('beta');
			$this->callUserInfo();
			if (isset($this->user['login'], $this->user['mode']['id'], $this->user['mode']['type']) && $this->user['login'] == true) {
				$status_query = mysqli_query($db, "SELECT `id`, `user.id`, `author.id`, `author.type`, `private.comment` FROM `status` WHERE `id` = '{$status_id}'");
				if (mysqli_num_rows($status_query) > 0) {
					$status = mysqli_fetch_assoc($status_query);
					$favorite_num = mysqli_num_rows(mysqli_query($db, "SELECT `id` FROM `status_favorite` WHERE `status.id` = '{$status['id']}' AND `guy.id` = '{$this->user['mode']['id']}' AND `guy.type` = '{$this->user['mode']['type']}'"));
					if ($favorite_num == 0 && $action == "add") {
						...
						if ($status['author.type'] == "user" && $this->user['mode']['type'] == "user") {
							...
							if ($relationship_num = 0) {
								$num = mysqli_num_rows(mysqli_query($db, "SELECT `id` FROM `friends` WHERE `user.id` = '{$user['id']}' AND `guy.id` = '{$status['user.id']}'"));
								if ($num > 0) {
									$relationship_num = 2;
								}
							}
							if ($relationship_num = 0) {
								$num = mysqli_num_rows(mysqli_query($db, "SELECT `id` FROM `friends` WHERE `user.id` = '{$user['id']}' AND (`guy.id` = '{$status['user.id']}' OR `guy.id` IN (SELECT `user.id` FROM `friends` WHERE `guy.id` = '{$status['user.id']}'))"));
								if ($num > 0) {
									$relationship_num = 3;
								}
							}
						...
						if ($relationship_num < $status_private) {
							$time = time();
							$action = mysqli_query($db, "INSERT INTO `status_favorite` (`id`, `time`, `status.id`, `guy.id`, `guy.type`) VALUES (null, '{$time}', '{$status['id']}', '{$this->user['mode']['id']}', '{$this->user['mode']['type']}')");
							if (!$action) {
								return array("return" => false, "reason" => "ERROR#FEEDS_007");
							}else {
								$this->class['_logs']->actions("add", array("action" => "favorite", "things" => "status", "things_id" => $status['id']));
								return array("return" => true);
							}
						}else {
							return array("return" => false, "reason" => "ERROR#FEEDS_006");
						...
						$action = mysqli_query($db, "DELETE FROM `status_favorite` WHERE `status.id` = '{$status['id']}' AND `guy.id` = '{$this->user['mode']['id']}' AND `guy.type` = '{$this->user['mode']['type']}'");
						if (!$action) {
							return array("return" => false, "reason" => "ERROR#FEEDS_007");
						}else {
							$this->class['_logs']->actions("add", array("action" => "unfavorite", "things" => "status", "things_id" => $status['id']));
							return array("return" => true);
						}
					}else {
						if ($action != "add" && $action != "remove") {
	...
	function comment_get ($object = array("return" => "json", "query" => null, "class" => "gFeeds-status-comment")) {
		...
		$this->callUserInfo();
		$comment_query = mysqli_query($this->class['_db']->port('beta'), $query);
		if (mysqli_num_rows($comment_query) == 0) {
			...
			while ($comment = mysqli_fetch_assoc($comment_query)) {
				...
				if ($comment_author['type'] == "user") {
					$getInfo = $this->class['_user']->getInfo(array("id" => $comment_author['id'], "rows" => "`avatar.small`, `fullname`, `username`, `link`"));
					if (isset($getInfo['return']) && $getInfo['return'] == true) {
				...
				if ($comment['edit'] != "") {
					...
					$comment_edit['time']['ago'] = $this->class['_tool']->agoDatetime($comment['edit'], 'ago');
					$comment_edit['time']['tip'] = $this->class['_tool']->agoDatetime($comment['edit'], 'tip');
				...
				$comment_time['ago'] = $this->class['_tool']->agoDatetime($comment['time'], 'ago');
				$comment_time['tip'] = $this->class['_tool']->agoDatetime($comment['time'], 'tip');
				$comment_time['json'] = json_encode(array("stamp" => $comment_time['stamp']));
				$comment_content['text'] = $this->class['_tool']->parseContent($this->class['_tool']->hash('decode', $comment['content'], $key));
				$comment_statistic['favorite'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_comment_favorite` WHERE `comment.id` = '{$comment['id']}'"));
				$comment_action['favorite']['check'] = mysqli_num_rows(mysqli_query($this->class['_db']->port('beta'),"SELECT `id` FROM `status_comment_favorite` WHERE `comment.id` = '{$comment['id']}' AND `guy.id` = '{$this->user['mode']['id']}' AND `guy.type` = '{$this->user['mode']['type']}'"));
				if ($comment_action['favorite']['check'] == 0) {
					$comment_action['favorite']['selected'] = false;
					$comment_action['favorite']['text'] = 'false';
					$comment_action['favorite']['tip'] = $this->class['_language']->text('favorite', 'ucfirst');
				}else {
					$comment_action['favorite']['selected'] = true;
					$comment_action['favorite']['text'] = 'true';
					$comment_action['favorite']['tip'] = $this->class['_language']->text('unfavorite', 'ucfirst');
				}
				$commentData[] = array (
					'info' => $comment_info,
			...
			if ($return == 'json') {
				...
				for ($i = 0; $i < count($commentData); $i++ ) {
					...
					if ($comment['edit']['status']) {
						$c_comment_edit = "
							<div class='edit'>
								<i class='fa fa-pencil _tt s' time='{$comment['edit']['time']['json']}' title='{$comment['edit']['time']['tip']}'></i>
	...
	function comment_add ($object = array("status_id" => 0, "value" => null)) {
		if (isset($object['status_id']) && is_string($object['status_id'])) $status_id = $object['status_id']; else $status_id = 0;
		if (isset($object['value']) && is_string($object['value'])) $value = $this->class['_tool']->convertDatabaseString($object['value']); else $value = null;
			...
			if (isset($this->user['login'], $this->user['mode']['id'], $this->user['mode']['type']) && $this->user['login'] == true) {
				...
				if (mysqli_num_rows($status_query) > 0) {
					...
					if ($relationship_num < $status_private) {
						$time = time();
						$key = $time."::".$status_id;
						$value = $this->class['_tool']->hash('encode', $this->class['_tool']->convertDatabaseString($value), $key);
						$action = mysqli_query($db, "INSERT INTO `status_comment` (`id`, `time`, `status.id`, `author.id`, `author.type`, `edit`, `content`) VALUES (null, '{$time}', '{$status['id']}', '{$this->user['mode']['id']}', '{$this->user['mode']['type']}', null, '{$value}')");
						if ($action) {
