		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					$file = $object['file'];
					$time = time();
					$close = $time + $this->class['_parameter']->get('media_cache_time_close');
					$client = $this->class['_client']->client(true);
					$token = $_SESSION["client"]['token']['cache']['photos'];
					$secret = hash('crc32', md5_file($file['tmp_name']));
					$nameraw = $file['name'];
					$ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
					$name = hash('crc32', md5($token.'::'.$secret.'::'.rand())).'.'.$ext;
					$copy = 0;
					$mime = $file['type'];
					$size = $file['size'];//$this->class['_tool']->getFileSize($tmp, true);
					$file['size'] = array();
					$file['size']['file'] = $size;
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $nameraw != null && $mime != null) {
						...
						$insertQuery = "INSERT INTO `photos_cache` (`id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `copy`, `mime`, `size`, `path`, `user.id`, `author.id`, `author.type`) VALUES (null, '{$this->class['_tool']->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$copy}', '{$mime}', '{$size}', '{$path}', '{$this->user['id']}', '{$author['id']}', '{$author['type']}')";
						$insert = mysqli_query($this->class['_db']->port('beta'), $insertQuery);
						if (!$insert) {
							return array("return" => false, "reason" => "");
						}else {
							$tmp = $fileUpload['storage']['source'];
							if ($file['size']['file'] > $this->class['_parameter']->get('photos_allow_size')) {
								list($file['size']['width'], $file['size']['height']) = getimagesize($tmp);
								$size_limit = $this->class['_parameter']->get('photos_allow_size');
				...
				$original_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `photos_cache` WHERE `secret` = '{$secret}' AND `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				$copy_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `photos_cache` WHERE `secret` = '{$secret}' AND `copy` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				...
				$_storage = $this->class['_storage'];
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `photos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				if (mysqli_num_rows($query) > 0) {
				...
				if (isset($object['name']) && $object['name'] != null) {
					...
					if (isset($object['name']) && is_string($object['name'])) $name = $object['name']; else $name = null;
					$author = $this->user['mode'];
					$imgCopy_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `copy`, `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' AND `copy` != '0'");
					if (mysqli_num_rows($imgCopy_query) > 0) {
						$imgCopy = mysqli_fetch_assoc($imgCopy_query);
						$imgOriginal_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$imgCopy['copy']}'");
				...
				if ($key != null && ($verify == true || $verify == false)) {
					...
					$_storage = $this->class['_storage'];
					$sql = "SELECT `id`, `name`, `path`, `token` FROM `photos_cache` WHERE `name` = '{$key}' OR `copy` = '{$key}' {$pushSql}";
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							mysqli_query($this->class['_db']->port('beta'), "DELETE FROM `photos_cache` WHERE `id` = '{$fetch['id']}'");
						}else {
							continue;
						}
					}
				...
				$client = $this->class['_client']->client(true);
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `id`, `name`, `path`, `token` FROM `photos_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}')) AND `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}'");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "image",
				...
				$cacheServerQuery = mysqli_query($this->class['_db']->port('beta'), "SELECT `name`, `path`, `token` FROM `photos_cache` WHERE `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}' LIMIT {$randLimit}");
				...
				$f_name = array();
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `name` FROM `photos_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'))");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
				}
				if (isset($f_name, $_SESSION["cache"]['photosHandling']) && count($f_name) > 0) {
				...
				if ($name != null && $secret != null) {
					$close = $this->class['_tool']->timeNow() + $this->class['_parameter']->get('media_cache_time_close');
					$client = $this->class['_client']->client(true);
					$author = $this->user['mode'];
					$query = mysqli_query($this->class['_db']->port('beta'), "UPDATE `photos_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
					if (!$query) {
						return array("return" => false, "reason" => "");
					}else {
						return array("return" => true);
				...
				if ($name != null && $secret != null && is_array($author)) {
					$client = $this->class['_client']->client(true);
					$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `photos_cache` WHERE `client` = '{$client}' AND `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
					if (mysqli_num_rows($query) == 0) {
						...
						mysqli_query($this->class['_db']->port('beta'), $fileQuery);
						...
						if (in_array($file['ext'], $this->class['_parameter']->get('photos_allow_format', true))) {
							$file['token'] = hash('crc32', time().'::'.rand().'::'.$file['name']);
							if (in_array($file['ext'], $this->class['_parameter']->get('exif_read_data_allow', true))) {
							...
							if ($file['info']['file.size'] > $this->class['_parameter']->get('photos_allow_size')) {
								$size_limit = $this->class['_parameter']->get('photos_allow_size');
							...
							if ($file['info']['size.width'] > $this->class['_parameter']->get('photos_size_original_w')) {
							...
							if ($multiple == true && $file['info']['size.width'] > $this->class['_parameter']->get('photos_size_large_w')) {
								$img->resize($this->class['_parameter']->get('photos_size_large_w'), 0);
								$fileUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $file['tmp'], "mime" => $file['mime'], "name" => $file['nameraw']));
								if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true && is_array($fileUpload['storage'])) {
							...
							if ($multiple == true && $file['info']['size.width'] > $this->class['_parameter']->get('photos_size_medium_w')) {
								$img->resize($this->class['_parameter']->get('photos_size_medium_w'), 0);
								$fileUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $file['tmp'], "mime" => $file['mime'], "name" => $file['nameraw']));
								if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true && is_array($fileUpload['storage'])) {
							...
							if ($multiple == true && $file['info']['size.width'] > $this->class['_parameter']->get('photos_size_small_w')) {
								$img->resize($this->class['_parameter']->get('photos_size_small_w'), 0);
								$fileUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $file['tmp'], "mime" => $file['mime'], "name" => $file['nameraw']));
								if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true && is_array($fileUpload['storage'])) {
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					...
					$ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
					$name = hash('crc32', md5($token.'::'.$secret.'::'.rand())).'.'.$ext;
					$nameraw = $this->class['_tool']->convertDatabaseString($file['name']);
					$copy = 0;
					$mime = $file['type'];
					$tmp = $this->class['_parameter']->get('music_cache_forder').'/'.$name;
					$info = $this->info('read', $file['tmp_name'], 'audio');
					$duration = $info['duration'];
					$size = $file['size'];
					if ($size > $this->class['_parameter']->get('music_allow_size')) {
						return array("return" => false, "reason" => "");
					}
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $mime != null && $tmp != null && $path != null) {
						$insertSql =  "INSERT INTO `music_cache` (`id`, `user.id`, `author.type`, `author.id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `copy`, `mime`, `size`, `duration`, `path`) VALUES (null, '{$this->user['id']}', '{$author['type']}', '{$author['id']}', '{$this->class['_tool']->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$copy}', '{$mime}', '{$size}', '{$duration}', '{$path}')";
						$insert = mysqli_query($this->class['_db']->port('beta'),$insertSql);
						if ($insert == true) {
							return array(
								"return" => true, 
								"data" => array(
				...
				$original_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `music_cache` WHERE `secret` = '{$secret}' AND `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				$copy_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `music_cache` WHERE `secret` = '{$secret}' AND `copy` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				if (mysqli_num_rows($original_query) > 0 && mysqli_num_rows($copy_query) == 0) {
					$original = mysqli_fetch_assoc($original_query);
				...
				$_storage = $this->class['_storage'];
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `music_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				if (mysqli_num_rows($query) > 0) {
				...
				if (isset($object['name']) && $object['name'] != null) {
					...
					if (isset($object['name']) && is_string($object['name'])) $name = $object['name']; else $name = null;
					$author = $this->user['mode'];
					$musikCopy_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `copy`, `path`, `token`, `secret`, `name`, `nameraw` FROM `music_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' AND `copy` != '0'");
					if (mysqli_num_rows($musikCopy_query) > 0) {
						$musikCopy = mysqli_fetch_assoc($musikCopy_query);
						$musikOriginal_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `path`, `token`, `secret`, `name`, `nameraw` FROM `music_cache` WHERE `name` = '{$musikCopy['copy']}'");
				...
				if ($key != null && ($verify == true || $verify == false)) {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						$delMediaFile = $_storage->delete($delMediaFileOpts);
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							mysqli_query($this->class['_db']->port('beta'), "DELETE FROM `music_cache` WHERE `id` = '{$fetch['id']}'");
						}else {
							continue;
						}
					}
				...
				$client = $this->class['_client']->client(true);
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `id`, `name`, `path`, `token` FROM `music_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}')) AND `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}'");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "audio",
				...
				$randLimit = rand($_parameter->get('cacheClean_by_users_limit_rand_min'), $_parameter->get('cacheClean_by_users_limit_rand_max'));
				$cacheServerQuery = mysqli_query($this->class['_db']->port('beta'), "SELECT `name`, `path`, `token` FROM `music_cache` WHERE `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}' LIMIT {$randLimit}");
				while ($fetch = mysqli_fetch_assoc($cacheServerQuery)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "audio",
				...
				$f_name = array();
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `name` FROM `music_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'))");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
				}
				if (isset($f_name, $_SESSION["cache"]['musicHandling']) && count($f_name) > 0) {
				...
				if ($name != null && $secret != null) {
					...
					$client = $this->class['_client']->client(true);
					$author = $this->user['mode'];
					$query = mysqli_query($this->class['_db']->port('beta'), "UPDATE `music_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
					if (!$query) {
						return array("return" => false, "reason" => "");
					}else {
						return array("return" => true);
				...
				if ($name != null && $secret != null && is_array($author)) {
					...
					$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `music_cache` WHERE `client` = '{$client}' AND `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
					if (mysqli_num_rows($query) == 0) {
						//.
					}else {
						...
						$time = time();
						$close = $time + $this->class['_parameter']->get('media_cache_time_close');
						mysqli_query($this->class['_db']->port('beta'), "UPDATE `music_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
						$file = mysqli_fetch_assoc($query);
						$file['name'] = basename($file['name']);
						...
						if (in_array($file['ext'], $this->class['_parameter']->get('music_allow_format', true))) {
							$file['token'] = hash('crc32', time().'::'.rand().'::'.$file['name']);
							...
							if (isset($object['info']['name']) && $object['info']['name'] != null) {
								$file['name'] = $this->class['_tool']->convertDatabaseString($object['info']['name']);
							}else {
							...
							if (isset($object['info']['description']) && $object['info']['description'] != null) {
								$file['description'] = $this->class['_tool']->convertDatabaseString($object['info']['description']);
							}else {
								$file['description'] = null;
							}
							if (isset($object['info']['singer']) && $object['info']['singer'] != null) {
								$file['singer'] = $this->class['_tool']->convertDatabaseString($object['info']['singer']);
							}else {
								$file['singer'] = null;
							}
							$file['private.view'] = $file['private.comment'] = $file['private.share'] = 4;
							$sql = "INSERT INTO `music_info` (`id`, `user.id`, `author.type`, `author.id`, `album`, `time`, `token`, `name`, `nameraw`, `singer`, `tags`, `description`, `private.view`, `private.comment`, `private.share`, `file.original`, `secret.original`, `path.original`, `file.datetime`, `file.size`, `bitrate`, `codec`, `duration`, `mime_type`) VALUES (null, '{$this->user['id']}', '{$author['type']}', '{$author['id']}', '{$album}', '{$this->class['_tool']->timeNow()}', '{$file['token']}', '{$file['name']}', '{$file['nameraw']}', '{$file['singer']}', '{$file['tags']}', '{$file['description']}', '{$file['private.view']}', '{$file['private.comment']}', '{$file['private.share']}', '{$file['file']['original']}', '{$file['secret']['original']}', '{$file['path']['original']}', '{$file['info']['file.datetime']}', '{$file['info']['file.size']}', '{$file['info']['bitrate']}', '{$file['info']['codec']}', '{$file['info']['duration']}', '{$file['info']['mime_type']}')";
							$insert = mysqli_query($this->class['_db']->port('beta'),$sql);
							$music_id = mysqli_insert_id($this->class['_db']->port('beta'));
							$file_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `id`, `path`, `token` FROM `music_cache` WHERE (`name`= '{$name}' OR `copy`= '{$name}') AND `secret` = '{$secret}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
							while ($file_fetch = mysqli_fetch_assoc($file_query)) {
								$this->class['_tool']->unlink($this->class['_tool']->hash('decode', $file_fetch['path'], $file_fetch['token']));
								mysqli_query($this->class['_db']->port('beta'),"DELETE FROM `music_cache` WHERE `id` = '{$file_fetch['id']}'");
							}
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					...
					$duration = $info['duration'];
					$size = $file['size'];
					if ($size > $this->class['_parameter']->get('videos_allow_size')) {
						return array("return" => false, "reason" => "");
					}
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $mime != null && $path != null) {
						$sql = "INSERT INTO `videos_cache` (`id`, `user.id`, `author.type`, `author.id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `thumbnail`, `thumbnail.path`, `copy`, `mime`, `size`, `duration`, `path`) VALUES (null, '{$this->user['id']}', '{$author['type']}', '{$author['id']}', '{$this->class['_tool']->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$thumbnail}', null, '{$copy}', '{$mime}', '{$size}', '{$duration}', '{$path}')";
						$insert = mysqli_query($this->class['_db']->port('beta'), $sql);
						if (!$insert) {
							return array("return" => false, "reason" => "");
						}else {
							$idInserted = mysqli_insert_id($this->class['_db']->port('beta'));
							...
							if (isset($fileUpload['return'], $fileUpload['storage']) && $fileUpload['return'] == true && is_array($fileUpload['storage'])) {
								...
								mysqli_query($this->class['_db']->port('beta'), "UPDATE `videos_cache` SET `thumbnail.path` = '{$fileUpload['storage']['display']}' WHERE `id` = '{$idInserted}'");
							}else {
								//.
							}
							$at = intval($duration / 2);
							exec("ffmpeg -itsoffset -{$at} -i {$this->class['_client']->root()}{$tmp} -vcodec mjpeg -vframes 1 -an -f rawvideo -s 720x480 {$this->class['_client']->root()}{$thumbnailPath} 2>&1");
				...
				$original_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `videos_cache` WHERE `secret` = '{$secret}' AND `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				$copy_query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `videos_cache` WHERE `secret` = '{$secret}' AND `copy` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				if (mysqli_num_rows($original_query) > 0 && mysqli_num_rows($copy_query) == 0) {
					$original = mysqli_fetch_assoc($original_query);
				...
				$_storage = $this->class['_storage'];
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `videos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
				if (mysqli_num_rows($query) > 0) {
				...
				if (isset($object['name']) && $object['name'] != null) {
					...
					if (isset($object['name']) && is_string($object['name'])) $name = $object['name']; else $name = null;
					$author = $this->user['mode'];
					$vioCopy_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `copy`, `path`, `token`, `secret`, `name`, `nameraw`, `thumbnail` FROM `photos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' AND `copy` != '0'");
					if (mysqli_num_rows($vioCopy_query) > 0) {
						$vioCopy = mysqli_fetch_assoc($vioCopy_query);
						$vioOriginal_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `path`, `token`, `secret`, `name`, `nameraw`, `thumbnail` FROM `videos_cache` WHERE `name` = '{$vioCopy['copy']}'");
				...
				if ($key != null && ($verify == true || $verify == false)) {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						$delMediaFile = $_storage->delete($delMediaFileOpts);
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							mysqli_query($this->class['_db']->port('beta'), "DELETE FROM `videos_cache` WHERE `id` = '{$fetch['id']}'");
						}else {
							continue;
						}
					}
				...
				$client = $this->class['_client']->client(true);
				$author = $this->user['mode'];
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `id`, `name`, `path`, `token` FROM `videos_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}')) AND `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}'");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "video",
				...
				$randLimit = rand($_parameter->get('cacheClean_by_users_limit_rand_min'), $_parameter->get('cacheClean_by_users_limit_rand_max'));
				$cacheServerQuery = mysqli_query($this->class['_db']->port('beta'), "SELECT `name`, `path`, `token` FROM `videos_cache` WHERE `close` != '0' AND `close` + '{$this->class['_parameter']->get('media_cache_time_close')}' < '{$time}' LIMIT {$randLimit}");
				while ($fetch = mysqli_fetch_assoc($cacheServerQuery)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "video",
				...
				$f_name = array();
				$query = mysqli_query($this->class['_db']->port('beta'), "SELECT `name` FROM `videos_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'))");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
				}
				if (isset($f_name, $_SESSION["cache"]['videosHandling']) && count($f_name) > 0) {
				...
				if ($name != null && $secret != null) {
					...
					$client = $this->class['_client']->client(true);
					$author = $this->user['mode'];
					$query = mysqli_query($this->class['_db']->port('beta'),"UPDATE `videos_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}'");
					if (!$query) {
						return array("return" => false, "reason" => "");
					}else {
						return array("return" => true);
				...
				if ($name != null && $secret != null && is_array($author)) {
					...
					$_client = $this->class['_client'];
					$client = $_client->client(true);
					$query = mysqli_query($this->class['_db']->port('beta'), "SELECT * FROM `videos_cache` WHERE `client` = '{$client}' AND `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
					if (mysqli_num_rows($query) == 0) {
						//.
					}else {
						...
						$time = time();
						$close = $time + $this->class['_parameter']->get('media_cache_time_close');
						mysqli_query($this->class['_db']->port('beta'), "UPDATE `videos_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
						$file = mysqli_fetch_assoc($query);
						$file['name'] = basename($file['name']);
						$file['ext'] = pathinfo($file['name'], PATHINFO_EXTENSION);
						...
						if (in_array($file['ext'], $this->class['_parameter']->get('videos_allow_format', true))) {
							$file['token'] = hash('crc32', time().'::'.rand().'::'.$file['name']);
							...
							if (!$insert) {
								...
								if (isset($fileUploadThumbnail['return'], $fileUploadThumbnail['storage']) && $fileUploadThumbnail['return'] == true && is_array($fileUploadThumbnail['storage'])) {
									...
									mysqli_query($this->class['_db']->port('beta'), "UPDATE `videos_info` SET `secret.thumbnail` = '{$fileUploadThumbnail['storage']['secret']}', `path.thumbnail` = '{$fileUploadThumbnail['storage']['display']}' WHERE `id` = '{$videos_id}'");
								}else {
									//. error.
								}
								$timeCropThumbnail = intval($file['info']['duration'] / 2);
								exec("ffmpeg -itsoffset -{$timeCropThumbnail} -i {$this->class['_client']->root()}{$file['source']['sd']} -vcodec mjpeg -vframes 1 -an -f rawvideo -s 720x480 {$this->class['_client']->root()}{$thumbnailPath} 2>&1");
								$_storage->recheck(array("format" => "image", "label" => "display", "value" => $thumbnailDisplay));

								$file_query = mysqli_query($this->class['_db']->port('beta'),"SELECT `id`, `path`, `token`, `thumbnail` FROM `videos_cache` WHERE (`name`= '{$name}' OR `copy`= '{$name}') AND `secret` = '{$secret}' AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'");
								while ($file_fetch = mysqli_fetch_assoc($file_query)) {
									$this->class['_tool']->unlink($this->class['_tool']->hash('decode', $file_fetch['path'], $file_fetch['token']));
									$this->class['_tool']->unlink($this->class['_parameter']->get('videos_cache_forder').'/thumbnail/'.$file_fetch['thumbnail']);
									mysqli_query($this->class['_db']->port('beta'), "DELETE FROM `videos_cache` WHERE `id` = '{$file_fetch['id']}'");
								}
