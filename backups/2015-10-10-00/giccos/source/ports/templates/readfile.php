...
if (isset($readFile['time']) && is_numeric($readFile['time'])) {
	...
	if (isset($readFile['path']) && file_exists($readFile['path'])) {
		...
		if ((!isset($lastChange) || !is_numeric($lastChange))) {
			setcookie("gReadfile", hashString("encode", $lastChange = time()), 0, $_parameter->get('cookie.host.path'), $g_client['http']['secure']);
		}
	}
}
...
if (!$pageUpdated && $checkID) {
	...
	$urlToken = hash('crc32', $_client->uri());
	//. setcookie($urlToken, $_tool->hash('encode', json_encode($pageCached), $urlToken), 0, $_client->uri(), $g_client['http']['secure']);
