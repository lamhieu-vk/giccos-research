...
if (isset($wall_['key'], $wall_['tab']) && $wall_['key'] !=  null && $wall_['tab'] !=  null) {
	$wall_['key'] = $_tool->convertDatabaseString($wall_['key']);
	if (is_numeric($wall_['key'])) {
		$getWallIdRequest = "SELECT `id` FROM `users` WHERE `id` = '".$wall_['key']."' LIMIT 1;";
	}else if (is_string($wall_['key'])) {
		$getWallIdRequest = "SELECT `id` FROM `users` WHERE `username` = '".$wall_['key']."' LIMIT 1;";
	}else {
		//.
		die(header("Location: {$_tool->links('::redirect::oops')}"));
	}
	$getWallIdQuery = mysqli_query($_db->port('beta'), $getWallIdRequest);
	if (!$getWallIdQuery || mysqli_num_rows($getWallIdQuery) == 0) {
		//.
		die(header("Location: {$_tool->links('::redirect::oops')}"));
	}
	$wall_['id'] = mysqli_fetch_assoc($getWallIdQuery)['id'];
	$getInfo = $_user->profile(array("action" => "get", "rows" => ["*"], "label" => "id", "value" => $wall_['id'], "limit" => "LIMIT 1"));
	if (isset($getInfo['return'], $getInfo['data'][0]) && $getInfo['return'] == true) {
		$getInfo['data'] = $getInfo['data'][0];
		...
		$wall_['title'] = $wall_['titleraw'] = $wall_['about']['fullname']." (".$wall_['about']['username'].")";
		if (isset($wall_['tab']) && $wall_['tab'] == "about") {
			$wall_['title'] .= " - ".$_language->text('about', 'ucwords');
		}else if (isset($wall_['tab']) && $wall_['tab'] == "timeline") {
			$wall_['title'] .= " - ".$_language->text('timeline', 'ucwords');
		}else if (isset($wall_['tab']) && $wall_['tab'] == "friends") {
		...
		}else if (isset($wall_['tab']) && $wall_['tab'] == "photos") {
			$wall_['title'] .= " - ".$_language->text('photos', 'ucwords');
		}else if (isset($wall_['tab']) && $wall_['tab'] == "music") {
			$wall_['title'] .= " - ".$_language->text('music', 'ucwords');
		}else if (isset($wall_['tab']) && $wall_['tab'] == "videos") {
			$wall_['title'] .= " - ".$_language->text('videos', 'ucwords');
		}else if (isset($wall_['tab']) && $wall_['tab'] == "places") {
		...
		<div id="gMain">
			...
			<div id="gBox">
				...
				<div id="centerBox">
					...
					<div id="navBox" class="boxGrid<?php if (isset($wall_['about']['dead']) && $wall_['about']['dead'] == 1) { print " dead"; } ?>">
						<div class="coverBox">
							<div class="wrap"></div>
							<div class="img" style="background-image: url(<?php print $wall_['about']['cover.large']; ?>);">
								<img src="<?php print $wall_['about']['cover.large']; ?>" alt="<?php print $_language->text('cover_of', 'ucfirst').' '.$wall_['about']['fullname'].' ('.$wall_['about']['username'].')'; ?>">
							</div>
						</div>
						<div class="avatarBox">
							<div class="border"></div>
							<div class="img" style="background-image: url(<?php print $wall_['about']['avatar.large']; ?>);">
								<img src="<?php print $wall_['about']['avatar.large']; ?>" alt="<?php print $_language->text('avatar_of', 'ucfirst').' '.$wall_['about']['fullname'].' ('.$wall_['about']['username'].')'; ?>">
							</div>
						</div>
						<div class="infoBox">
							<div class="text">
						...
						<div class="tagsInfo">
						<?php
						if (isset($wall_['about']['verified']) && $wall_['about']['verified'] == 1) {
							print "<div class='tags verified _tt-we s' title='{$_language->text('verified', 'ucfirst')}'><i></i></div>";
						}
						if (isset($wall_['about']['dead']) && $wall_['about']['dead'] == 1) {
					...
					<div id="mainBox">
						...
						<div id="wallContent">
							...
							if (isset($wall_['tab']) && $wall_['tab'] == "about") {
								...
								if (isset($wall_['about']['quote']) && $wall_['about']['quote'] != null) {
									$codeInfoTab['introduction'] .= "
									<div class='value oe'>
										<div class='texttitle'><span>{$_language->text('quote', 'ucfirst')}</span></div>
										<div class='textmain'><span>{$wall_['about']['quote']}</span></div>
									</div>
									";
								}
								...
								$getContact = $_user->contact(array("action" => "get", "id" => $wall_['id']));
								if (isset($getContact['return'], $getContact['data']) && $getContact['return'] == true) {
									$contactEmailInfo = $contactPhoneInfo = array();
									foreach ($getContact['data'] as $contactId => $contactData) {
										if (isset($contactData['type']) && $contactData['type'] == "email") {
											$contactEmailInfo[] = $contactData;
										}else if (isset($contactData['type']) && $contactData['type'] == "phone") {
											$contactPhoneInfo[] = $contactData;
										}
									}
								}else {
									//.
								}
								if (isset($contactEmailInfo) && is_array($contactEmailInfo)) {
									$contactEmailInfoDataThis_s = 0;
									foreach ($contactEmailInfo as $key => $contactEmailInfoDataThis) {
								...
								if (isset($contactPhoneInfo) && is_array($contactPhoneInfo)) {
									$contactPhoneInfoDataThis_s = 0;
									foreach ($contactPhoneInfo as $key => $contactPhoneInfoDataThis) {
								...
								$educationInfoGet = $_user->education(array("action" => "get", "id" => $wall_['id'], "rows" => "*", "limit" => "ORDER BY `id` DESC"));
								...
								$workplacesInfoGet = $_user->workplaces(array("action" => "get", "id" => $wall_['id'], "rows" => "*", "limit" => "ORDER BY `id` DESC"));
								...
								if (isset($wall_['about']['live']) && $wall_['about']['live'] != null) {
									if (isset($wall_['about']['live.description']) && is_string($wall_['about']['live.description'])) {
										$codeInfoTab['places.live.description'] = "<div class='texttitle'></div> <div class='textmain _tt-bk e' title='{$_language->text('description_about_country', 'strtolower')}'><span>{$wall_['about']['live.description']}</span></div>";
									}else {
										$codeInfoTab['places.live.description'] = null;
									}
									if (is_numeric($wall_['about']['live'])) {
										$placesLiveMapsDataRequest = $_maps->places(array("action" => "get", "label" => "id", "value" => $wall_['about']['live'], "limit" => "LIMIT 1"));
										if (isset($placesLiveMapsDataRequest['return'], $placesLiveMapsDataRequest['data'], $placesLiveMapsDataRequest['data'][0]) && $placesLiveMapsDataRequest['return'] == true) {
											$placesLiveMapsData = $placesLiveMapsDataRequest['data'][0];
											$codeInfoTab['places'] .= "
											<div class='value to'>
												<div class='texttitle'><span>{$_language->text('live', 'ucfirst')}</span></div>
												<div class='textmain'><span>{$placesLiveMapsData['name']}</span></div>
												<div class='texttitle'></div>
												<div class='textmain'><small>{$placesLiveMapsData['address']}</small></div>
												{$codeInfoTab['places.live.description']}
												<div class='imgMaps'><div class='img' style='background-image: url(".preg_replace("/(markers\=color\:red)/", "markers=color:gray", preg_replace("/(zoom=15)/", "zoom=14", preg_replace("/(400x400)/", "324x248", $placesLiveMapsData['thumbnail']))).");'><img src='".preg_replace("/(markers\=color\:red)/", "markers=color:gray", preg_replace("/(zoom=15)/", "zoom=14", preg_replace("/(400x400)/", "324x248", $placesLiveMapsData['thumbnail'])))."' alt='{$placesLiveMapsData['name']} - {$placesLiveMapsData['address']}'></div></div>
											</div>
											";
										}
									}else if (is_string($wall_['about']['live'])) {
										$codeInfoTab['places'] .= "
										<div class='value to'>
											<div class='texttitle'><span>{$_language->text('live', 'ucfirst')}</span></div>
									...
									if (isset($wall_['about']['live.description']) && is_string($wall_['about']['live.description'])) {
										$codeInfoTab['places.live.description'] = "<div class='textmain _tt-bk e' title='{$_language->text('description_about_live', 'strtolower')}'><span>{$wall_['about']['live.description']}</span></div>";
										$codeInfoTab['places'] .= "
										<div class='value to'>
											<div class='texttitle'><span>{$_language->text('live', 'ucfirst')}</span></div>
											{$codeInfoTab['places.live.description']}
								...
								if (isset($wall_['about']['country']) && $wall_['about']['country'] != null) {
									if (isset($wall_['about']['country.description']) && is_string($wall_['about']['country.description'])) {
										$codeInfoTab['places.country.description'] = "<div class='texttitle'></div> <div class='textmain _tt-bk e' title='{$_language->text('description_about_country', 'strtolower')}'><span>{$wall_['about']['country.description']}</span></div>";
									}else {
										$codeInfoTab['places.country.description'] = null;
									}
									if (is_numeric($wall_['about']['country'])) {
										$placesCountryMapsDataRequest = $_maps->places(array("action" => "get", "label" => "id", "value" => $wall_['about']['country'], "limit" => "LIMIT 1"));
										if (isset($placesCountryMapsDataRequest['return'], $placesCountryMapsDataRequest['data'], $placesCountryMapsDataRequest['data'][0]) && $placesCountryMapsDataRequest['return'] == true) {
											$placesCountryMapsData = $placesCountryMapsDataRequest['data'][0];
											if (isset($placesCountryMapsData['id'], $placesLiveMapsData['id']) && $placesCountryMapsData['id'] == $placesLiveMapsData['id']) {
												$codeInfoTab['places'] .= "
												<div class='value to'>
												...
												<div class='value to'>
													<div class='texttitle'><span>{$_language->text('country', 'ucfirst')}</span></div>
													<div class='textmain'><span>{$placesCountryMapsData['name']}</span></div>
													<div class='texttitle'></div>
													<div class='textmain'><small>{$placesCountryMapsData['address']}</small></div>
													{$codeInfoTab['places.country.description']}
													<div class='imgMaps'><div class='img' style='background-image: url(".preg_replace("/(markers\=color\:red)/", "markers=color:gray", preg_replace("/(zoom=15)/", "zoom=14", preg_replace("/(400x400)/", "324x248", $placesCountryMapsData['thumbnail']))).");'><img src='".preg_replace("/(markers\=color\:red)/", "markers=color:gray", preg_replace("/(zoom=15)/", "zoom=14", preg_replace("/(400x400)/", "324x248", $placesCountryMapsData['thumbnail'])))."' alt='{$placesCountryMapsData['name']} - {$placesCountryMapsData['address']}'></div></div>
												</div>
												";
											}
										}
									}else if (is_string($wall_['about']['country'])) {
										$codeInfoTab['places'] .= "
										<div class='value to'>
											<div class='texttitle'><span>{$_language->text('country', 'ucfirst')}</span></div>
									...
									if (isset($wall_['about']['country.description']) && is_string($wall_['about']['country.description'])) {
										$codeInfoTab['places.country.description'] = "<div class='textmain _tt-bk e' title='{$_language->text('description_about_country', 'strtolower')}'><span>{$wall_['about']['country.description']}</span></div>";
										$codeInfoTab['places'] .= "
										<div class='value to'>
											<div class='texttitle'><span>{$_language->text('country', 'ucfirst')}</span></div>
											{$codeInfoTab['places.country.description']}
								...
								if ($typeGetTimeline == 0) {
									$sqlGetTimeline = "SELECT * FROM `status` WHERE `author.type` = 'user' AND `author.id` IN (SELECT `id` FROM `users` WHERE `id` = '{$wall_['id']}') ORDER BY `id` DESC LIMIT 10";
								...
								$queryStatusTimeline = mysqli_query($_db->port('beta'), $sqlGetTimeline);
								if ($queryStatusTimeline == true) {
									$numStatusTimeline = mysqli_num_rows($queryStatusTimeline);
									if ($numStatusTimeline == 0) {
										...
										$getStatusTimeline = $_feed->status_get(array("return" => "html", "class" => "status", "query" => $sqlGetTimeline, "media" => true, "guy" => $g_user['mode']));
										if (isset($getStatusTimeline['return'], $getStatusTimeline['data']) && $getStatusTimeline['return'] == true) {
											$codeStatusTimeline = $getStatusTimeline['data'];
										}else {
											$codeStatusTimeline = null;
										}
									}
								}else {
									$codeStatusTimeline = null;
								}
								$feedRequire = json_encode(array("port" => array("type" => "wall", "require" => array("label" => "id", "value" => $wall_['id'], "type" => $typeGetTimeline))));
								print "
								<div id='tabTimeline'>
									<div id='gFeeds' class='thisFeeds feed timeline' feed='{$feedRequire}'>
										<div class='info inFeeds boxGrid' class='thisTotal'>
											<div class='title'>
												<span>{$_language->text('timeline', 'ucfirst', false)}: {$wall_['titleraw']}</span>
											</div>
								...
								$workplacesInfoGet = $_user->workplaces(array("action" => "get", "id" => $wall_['id'], "rows" => "`id`", "limit" => "LIMIT 1"));
								...
								$educationInfoGet = $_user->education(array("action" => "get", "id" => $wall_['id'], "rows" => "`id`", "limit" => "LIMIT 1"));
