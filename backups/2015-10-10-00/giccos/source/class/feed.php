...
Class feed {
	...
	function status_printcode ($object = array()) {
		...
		if (isset($statusArr['action'], $statusArr['action']['comment'], $statusArr['action']['comment']['status']) && $statusArr['action']['comment']['status'] == true) {
			$code['comment_type'] = "<div class='type' comment-type> <textarea type='text' placeholder='".$_language->text('write_a_comment', 'strtolower')."' status-action-comment></textarea> </div>";
		}else {
			$code['comment_type'] = "<div class='type null' comment-type> <span>".$_language->text('status_unlimited_comment', 'strtolower')."</span> </div>";
		}
		if (isset($statusArr['comment'], $statusArr['comment']['count'])) {
			...
			if ($statusArr['comment']['count'] == 0) {
				...
				$code['comment_content'] = "<div class='null' comment-null> <span comment-null>".$_language->text('status_null_comment', 'strtolower')."</span> </div>";
			}else if ($statusArr['comment']['count'] > 0) {
				if ($statusArr['comment']['count'] >= 3) {
					$code['comment_stats'] = "<div class='showmore' comment-showmore> <i class='icon'></i> <span class='text'>".$_language->text('show_more_comments', 'strtolower')."</span> </div>";
				}else {
					$code['comment_stats'] = null;
				}
				$code['comment_content'] = null;
			...
			if (isset($statusArr['comment']['count']) && $statusArr['comment']['count'] > 0) {
				$code['comment_stats'] = "<div class='statistic'> <div class='info'> <i class='icon'></i> <span class='count' comment-stats>".$statusArr['comment']['count']."</span> <span class='text'>".$_language->text('comment(s)', 'strtolower')."</span> </div> ".$code['comment_stats']." </div>";
			}
			//.
		}else {
			...
			while ($status = mysqli_fetch_assoc($statusQuery)) {
				...
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
					...
					if (mysqli_num_rows($mediaQuery) == 0) {
						...
						while ($mediaData = mysqli_fetch_assoc($mediaQuery)) {
							$mediaFetch = $_videos->fetch(array("guy" => array("type" => $guyType, "id" => $guyId), "rows" => ["display", "mime_type", "name", "tags", "description", "file.hd", "file.vga", "file.qvga"], "label" => "id", "value" => $mediaData['videos.id']));
							if (isset($mediaFetch['return'], $mediaFetch['data'], $mediaFetch['data'][0]) && $mediaFetch['return'] == true) {
								$mediaFetchData = $mediaFetch['data'][0];
								$content['media']['data'][] = $mediaFetchData;
							}
						}
					}
			...
			while ($status = mysqli_fetch_assoc($query)) {
				...
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
					...
					if ($mediaQuery && mysqli_num_rows($mediaQuery) == 0) {
						...
						$content['media']['type'] = "videos";
						while ($mediaData = mysqli_fetch_assoc($mediaQuery)) {
							$mediaFetch = $_media->data("videos", array("action" => "get", "rows" => ['token', 'display', 'name', 'tags', 'mime_type', 'description', 'file.thumbnail', 'file.sd'], "label" => "id", "value" => $mediaData['videos.id'], "private" => true, "author" => array("type" => $author['type'], "id" => $author['id']), "limit" => "LIMIT 1"));
							if (isset($mediaFetch['return'], $mediaFetch['data'], $mediaFetch['data'][0]) && $mediaFetch['return'] == true) {
								$mediaFetchData = $mediaFetch['data'][0];
								if ($mediaFetchData['name'] == null) {
				...
				for ( $i = 0; $i < count($statusData); $i++ ) {
					...
					if (isset($status['comment']['count']) && $status['comment']['count'] == 0) {
						$c_['status_comment_null'] = "
							<div class='null' comment-null>
								<span comment-null>{$_language->text('status_null_comment', 'ucfirst')}</span>
							</div>
						";
						...
						if ($status['comment']['count'] > 3) {
							$c_['status_comment_showmore'] = "
								<div class='showmore' comment-load>
									<i class='icon'></i>
									...
									if (isset($getComment['return'], $getComment['data']) && $getComment['return'] == true && $getComment['data'] != null) {
										...
										if ($commentReplyCount == 0) {
											...
											if ($commentReplyCount > 3) {
												...
												<div class='lines statistic' comment-lines-stats comment-id='{$getCommentIdFetch['id']}'>
													<div class='info'>
														<i class='icon'></i>
														<span class='count' comment-stats-reply>{$commentReplyCount}</span>
														<span class='text'>{$_language->text('reply(s)', 'strtolower')}</span>
													</div>
													<div class='showmore' comment-reply-load>
														<i class='icon'></i>
					...
					<div class='{$className} inFeeds boxGrid' status-id='{$status['info']['id']}' status='{$c_['status_info']}'>
						...
						<div class='footer'>
							...
							<div class='comment'>
								<div class='type {$c_['private_comment_class']}' comment-type>
									{$c_['private_comment']}
								</div>
								{$c_['status_comment']}
		...
		if (isset($object['media']['type']) && $object['media']['type'] == "link") {
			...
			$c_['status_content_media'] = "<div class='media VideosBox'>";
				foreach ($object['media']['data'] as $key => $value) {
					...
					/*
					$c_['status_content_media'] .= "
					<div class='videos' id='{$value['id']}' media='videos' media-display='{$value['display']}' media-info='".json_encode($value)."' style='background-image: url(".$value['source']['thumbnail'].");'>
						<div class='wrap'></div>
						<div class='icon'>
							<i></i>
						</div>
					...
					";
					*/
					$c_['status_content_media_tag'] = null;
					foreach (explode(" ", $value['tags']) as $keyTag => $valueTag) {
						$c_['status_content_media_tag'] .= "<div class='li'> <a href='".$_tool->links('videos/tag/'.$valueTag)."' target='_blank'>".$valueTag."</a> </div>";
					}
					$c_['status_content_media'] .= "
					<div id='".$value['id']."' class='videos' media='videos' media-display='".$value['display']."'>
						<div class='thumbnail' style='background-image: url(".$value['source']['thumbnail'].");'></div>
						<div class='wrap'></div>
						<div class='context'>
							<div class='name text'> <a href='".$value['link']."' target='_blank'>".$value['name']."</span> </div>
							<div class='tag text'> ".$c_['status_content_media_tag']." </div>
						</div>
					</div>
					";
				}
	...
	function comment_printcode ($object = array()) {
		...
		$code['action_reply'] = (isset($commentArr['info'], $commentArr['info']['inside'], $commentArr['info']['inside']['type'], $commentArr['reply']['count']) && $commentArr['info']['inside']['type'] == "status") ? "<div class='reply'> <i class='_tt s' comment-action-reply title='".$_language->text('reply_comment', 'strtolower(str)')."'></i> <span comment-stats-reply>".$commentArr['reply']['count']."</span> </div>" : "";
		if (isset($commentArr['action'], $commentArr['action']['favorite'], $commentArr['stats'], $commentArr['stats']['favorite']) && is_bool($commentArr['action']['favorite']) && is_numeric($commentArr['stats']['favorite'])) {
			...
			if ($commentArr['action']['favorite'] == true) {
				$code['action_favorite'] = "<i class='_tt s' comment-action-favorite favorite='true' title='".$_language->text('unfavorite', 'strtolower')."'></i>";
			}else if ($commentArr['action']['favorite'] == false) {
				$code['action_favorite'] = "<i class='_tt s' comment-action-favorite favorite='false' title='".$_language->text('favorite', 'strtolower')."'></i>";
			}
	...
	function comment_fetch ($object = array()) {
		...
		if (!$commentQuery) {
			...
			while ($comment = mysqli_fetch_assoc($commentQuery)) {
				...
				$commentStats['favorite'] = mysqli_num_rows($commentStatsFavoriteCountQuery);
				...
				$commentActionFavoriteExistsNum = mysqli_num_rows($commentActionFavoriteExistsQuery);
			...
			if ($returnType == "json") {
				...
				for ($i = 0; $i < count($commentData); $i++ ) {
					...
					if ($comment['info']['inside.type'] == "status") {
						$c_comment_reply = "
						<div class='reply'>
							<i class='_tt s' comment-action-reply title='{$_language->text('reply_comment', 'ucfirst')}'></i>
							<span comment-stats-reply>{$comment['reply']['count']}</span>
						</div>
						";
					}else {
					...
					$c_data .= "
						<div class='{$className}' comment-lines comment-id='{$comment['info']['id']}' inside-type='{$comment['info']['inside.type']}' inside-id='{$comment['info']['inside.id']}' comment='{$c_comment_info}'>
							<div class='bL'>
							...
							<div class='bR'>
								...
								<div class='footer'>
									<div class='favorite'>
										<i class='_tt s' comment-action-favorite favorite='{$comment['action']['favorite']['text']}' title='{$comment['action']['favorite']['tip']}'></i>
										<span comment-stats-favorite>{$comment['stats']['favorite']}</span>
									</div>
									{$c_comment_reply}
									{$c_comment_edit}
								</div>
	...
	function comment_stats ($object = array()) {
		...
		if ($commentCount == 0) {
			...
			if ($rows == "favorite" || $rows == "all") {
				$favoriteQuery = mysqli_query($db, "SELECT `id` FROM `status_favorite` WHERE `inside.type` = 'comment' AND `inside.id` = '".$commentId."' ;");
			}
			if ($rows == "reply" || $rows == "all") {
				$replyQuery = mysqli_query($db, "SELECT `id` FROM `status_comment` WHERE `inside.type` = 'comment' AND `inside.id` = '".$commentId."' ;");
			}
			if ($type == "stats" || $type == "all") {
				if (($rows == "favorite" || $rows == "all") && isset($favoriteQuery) && $favoriteQuery) {
					$favoriteCount = mysqli_num_rows($favoriteQuery);
