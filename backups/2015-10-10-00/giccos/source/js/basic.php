...
_g['global'].append = function (code, cb) {
	cb = typeof cb === "function" ? cb : function () {};
...
_g['session'].enable = function (cb) {
	cb = typeof cb === "function" ? cb : function () {};
	cb(true);
	return true;
};
_g['session'].ready = function (cb) {
...
_g['session'].exists = function (name) {
...
_g['session'].add = function (name, data) {
	dataCacheHash = _g['session'].key();
	objCache = {};
	if (isset(window) && isset(window[dataCacheHash])) {
		objCache = window[dataCacheHash];
...
_g['session'].push = function (name, data) {
	dataCacheHash = _g['session'].key();
	objCache = {};
	if (isset(window) && isset(window[dataCacheHash])) {
		objCache = window[dataCacheHash];
...
_g['session'].remove = function (name) {
	dataCacheHash = _g['session'].key();
	objCache = {};
	if (isset(window) && isset(window[dataCacheHash])) {
		objCache = window[dataCacheHash];
...
_g['cache'].ready = function (cb) {
	...
	if (_g['session'].exists('client_cache_ready') === true)  {
		...
		_g['cache'].setup(cbSetupFunc);
	}
};
_g['cache'].setup = function (cb) {
	if (typeof cb !== "function") {
		return false;
	}
	var setCacheStatus = function (value) {
...
_g['cache'].clean = function () {
	
};
_g['cache'].delete = function () {
	
};
_g['cache'].exists = function () {
	
};
_g['cache'].set = function () {
	
};
_g['cache'].get = function () {
	
};
_g['cache'].autoload = function () {
...
_g['source'].push = function (obj, cb) {
	obj = typeof obj === "object" ? obj : {};
	cb = typeof cb === "function" ? cb : function () {};
	_g['source'].append();
	sourceBox = $("#gSource");
	_g['source'].recheck (function (listLoaded) {
		if (!isset(obj['type']) || !isset(obj['src']) || !isset(obj['data'])) {
			cb(false);
			return false;
		}else {
		...
		isLoaded = false;
		if (obj['type'] == "js") {
			sourceBox.append("<script type='text/javascript'>" + obj['data'] + "</script>");
			document.window.onload = eval(obj['data']);
			isLoaded = true;
		}else if (obj['type'] == "css") {
			sourceBox.append("<style type='text/css' rel='stylesheet'>" + obj['data'] + "</style>");
			isLoaded = true;
		}
		if (isLoaded) {
...
_g['source'].load = function (obj, cb) {
	obj = typeof obj === "object" ? obj : {};
	cb = typeof cb === "function" ? cb : function () {};
	if (isset(obj['type']) && typeof obj['type'] === "string" && isset(obj['src']) && typeof obj['src'] === "string") {
		//.
		fileType = obj['type'];
		fileSrc = obj['src'];
	}else {
		...
		_g['xhr']({
			...
			'done': function (fileContent) {
				if (typeof fileContent !== "string") {
					...
					_g['source']({'type': fileType, 'src': fileSrc, 'data': fileContent}, function (returnValue) {
						cb(returnValue);
					});
				}
			}
...
_g['source'].autoload = function () {
	_g['source'].ready(function (x) {
		console.log('source - enable: ' + x);
	});
};
_g['source'].autoload();
_g['ticktock'] = function () {

};
_g['ticktock'].ready = function (cb) {
	if (typeof cb !== "function") {
		return false;
	}
...
_g['loader'].add = function (object) {
	...
	if (isset(object['close']) && object['close'] === true) {
		codeClose = "<div class='close' close></div>";
	}else {
		codeClose = "";
	}
	codeWrapper = "<div id='wrapperLoading' randkey='"+randkey+"'> "+codeClose+" <div class='loading'></div> </div>";
	$("body").css({'overflow-x': 'hidden', 'overflow-y': 'hidden'});
	console.log('asdasd');
	_g['global'].append(codeWrapper);
	thisWrapper = $("#wrapperLoading[randkey='"+randkey+"']");
...
_g['loader'].remove = function (object) {
	if (!isset(object)) {
		return false;
	}
...
_g['player'].use = function () {

};
_g['player'].setup = function (cb) {
	if (typeof cb !== "function") {
		return false;
	}
	...
	loadFileSource('js', {'src': '<?php print $_tool->links("source/js/templates/mediaplayer.js"); ?>'}, function (loadedJS) {
	...
	loadFileSource('css', {'src': '<?php print $_tool->links("source/css/templates/mediaplayer.css"); ?>'}, function (loadedCSS) {
...
_g['player'].autoload = function () {
	_g['player'].ready(function (x) {
		console.log('player - enable: ' + x);
	});
};
_g['ajaxrequest'] = function (object, callback) {
	_g['ajaxrequest'].use(object, callback);
};
_g['ajaxrequest'].new = function (object, callback) {
	_g['ajaxrequest'].use(object, callback);
};
_g['ajaxrequest'].use = function (object, callback) {
	if (typeof object !== "object") {
		return;
	}else {
		if (typeof callback !== "undefined") delete callback;
		if (typeof requestType !== "undefined") delete requestType;
		if (typeof requestURL !== "undefined") delete requestURL;
		if (typeof requestUsername !== "undefined") delete requestUsername;
		if (typeof requestPassword !== "undefined") delete requestPassword;
		if (typeof requestAsync !== "undefined") delete requestAsync;
		if (typeof requestHeaders !== "undefined") delete requestHeaders;
		if (typeof requestDataProcess !== "undefined") delete requestDataProcess;
		if (typeof requestDelay !== "undefined") delete requestDelay;
		if (typeof requestDelayCallback !== "undefined") delete requestDelayCallback;
		if (typeof requestDataType !== "undefined") delete requestDataType;
		if (typeof requestData !== "undefined") delete requestData;
		if (typeof requestEvent !== "undefined") delete requestEvent;
		if (typeof requestCrossdomain !== "undefined") delete requestCrossdomain;
		if (typeof requestUpload !== "undefined") delete requestUpload;
		if (typeof requestProgress !== "undefined") delete requestProgress;
		if (typeof requestError !== "undefined") delete requestError;
		if (typeof requestAbort !== "undefined") delete requestAbort;
		if (typeof requestTimelimit !== "undefined") delete requestTimelimit;
		if (typeof requestTimeout !== "undefined") delete requestTimeout;
		if (typeof requestLoad !== "undefined") delete requestLoad;
		if (typeof requestBefore !== "undefined") delete requestBefore;
		if (typeof requestAfter !== "undefined") delete requestAfter;
		if (typeof requestDone !== "undefined") delete requestDone;
		if (typeof requestDoneCallback !== "undefined") delete requestDoneCallback;
		if (typeof requestCallback !== "undefined") delete requestCallback;
		if (typeof requestRandkey !== "undefined") delete requestRandkey;
		if (typeof requestVerify !== "undefined") delete requestVerify;
	}
	callback = typeof callback === "function" ? callback : function () {};
	requestType = typeof object['type'] === "string" ? object['type'] : 'POST';
	requestURL = typeof object['url'] === "string" ? object['url'] : '';
	requestUsername = typeof object['username'] === "string" ? object['username'] : null;
	requestPassword = typeof object['password'] === "string" ? object['password'] : null;
	requestAsync = typeof object['async'] === "boolean" ? object['async'] : true;
	requestHeaders = typeof object['headers'] === "object" ? object['headers'] : {};
	requestDataProcess = typeof object['dataprocess'] === "boolean" ? object['dataprocess'] : true;
	requestDelay = typeof object['delay'] === "number" ? object['delay'] : 0;
	requestDataType = typeof object['datatype'] === "string" ? object['datatype'] : '*';
	requestData = typeof object['data'] !== "undefined" ? object['data'] : null;
	requestEvent = typeof object['event'] === "object" ? object['event'] : {};
	requestCrossdomain = typeof object['crossdomain'] === "boolean" ? object['crossdomain'] : false;
	requestUpload = typeof object['upload'] === "object" ? object['upload'] : {};
	requestProgress = typeof object['progress'] === "function" ? object['progress'] : function () {};
	requestError = typeof object['error'] === "function" ? object['error'] : function () {};
	requestAbort = typeof object['abort'] === "function" ? object['abort'] : function () {};
	requestTimelimit = typeof object['timelimit'] === "number" ? object['timelimit'] : 0;
	requestTimeout = typeof object['timeout'] === "function" ? object['timeout'] : function () {};
	requestLoad = typeof object['load'] === "function" ? object['load'] : function () {};
	requestBefore = typeof object['before'] === "function" ? object['before'] : function () {};
	requestAfter = typeof object['after'] === "function" ? object['after'] : function () {};
	requestDone = typeof object['done'] === "function" ? object['done'] : function () {};
	requestCallback = typeof object['callback'] === "function" ? object['callback'] : function () {};
	...
	requestObj[requestRandkey] = _g['ajaxrequest'].create();
	requestAjax = requestObj[requestRandkey];
	//.
	eventLoop: for (var eventType in requestEvent) {
		eventFunction = requestEvent[eventType];
	...
	uploadLoop: for (var uploadType in requestUpload) {
		uploadFunction = requestUpload[uploadType];
	...
	if (hasContentType === 0 && requestDataProcess && typeof requestData !== "string") {
		requestHeaders['content-type'] = "<?php print $_parameter->get('contentType_urlencoded.utf8'); ?>";
	}
	if (requestDataProcess === true && typeof requestData !== "string") {
		requestData = _g['ajaxrequest'].dataparse(requestData);
	}
	if (requestType.toLowerCase() == "get") {
		requestAjax.open(requestType, typeof requestData === "string" ? requestURL + '?' + requestData : requestURL, requestAsync, requestUsername, requestPassword);
	}else {
		requestAjax.open(requestType, requestURL, requestAsync, requestUsername, requestPassword);
	}
	if (!requestCrossdomain) {
...
_g['ajaxrequest'].done = function(request, cb) {
	...
	try {
		_g['ajaxrequest'].statechange(request, function () {
			if (typeof requestReady !== "undefined") delete requestReady;
			_g['ajaxrequest'].ready(request, function (requestReady) {
				if (requestReady['state'] === 4) {
					requestResponse = _g['ajaxrequest'].response(request);
					cb(_g['ajaxrequest'].responseparse(requestDataType, requestResponse['data']), requestResponse, requestReady);
				}
			});
		});
		return true;
...
_g['ajaxrequest'].send = function(request, timeout, data, cb) {
	if (typeof request !== "object") {
		return false;
	}
	cb = typeof cb === "function" ? cb : function () {};
	try {
		setTimeout(function () {
			request.send(typeof data !== "undefined" ? data : null);
...
_g['ajaxrequest'].id = function(key) {
	//.
};
_g['ajaxrequest'].create = function() {
	objXHR = false;
...
_g['ajaxrequest'].serialize = function(obj, prefix) {
  var str = [];
  for (var p in obj) {
    if (obj.hasOwnProperty(p)) {
      var k = prefix ? prefix + "[" + p + "]" : p, v = obj[p];
      str.push(typeof v == "object" ?
        _g['ajaxrequest'].serialize(v, k) :
        encodeURIComponent(k) + "=" + encodeURIComponent(v));
    }
  }
  return str.join("&");
};
_g['ajaxrequest'].dataparse = function (data) {
	if (typeof data === "object" && countArr(data) > 0) {
		dataCaching = {};
    ...
    string = _g['ajaxrequest'].serialize(data);
	return string;
};
_g['ajaxrequest'].responseparse = function (type, data) {
	if (data === null) {
		return null;
	}
...
_g['ajaxrequest'].statechange = function (request, cb) {
	if (typeof request !== "object") {
		return false;
	}
	cb = typeof cb === "function" ? cb : function () {};
...
_g['ajaxrequest'].ready = function (request, cb) {
	if (typeof request !== "object") {
		return false;
	}
...
_g['ajaxrequest'].response = function (request, type, cb) {
	if (typeof request !== "object") {
		return false;
	}
	...
	if (type == "data") {
		...
		responseObj = {};
		responseObj['data'] = request.response ? request.response : null;
		responseObj['text'] = request.responseText ? request.responseText : null;
		responseObj['type'] = request.responseType ? request.responseType : null;
		responseObj['url'] = request.responseURL ? request.responseURL : null;
		responseObj['xml'] = request.responseXML ? request.responseXML : null;
	}
	if (typeof cb === "function") {
		cb(responseObj);
	}
...
_g['ajaxrequest'].upload = function (request, type, fn, sync) {
	if (typeof request !== "object") {
		return false;
	}
	type = typeof type === "string" ? type : null;
...
_g['ajaxrequest'].event = function (request, type, fn, sync) {
	if (typeof request !== "object") {
		return false;
	}
	type = typeof type === "string" ? type : null;
...
_g['ajaxify'].direct = function (url, target) {
	if (!isset(url)) {
		return false;
	}
	if (!isset(target)) {
...
_g['ajaxify'].request = function (url, cb) {
	...
	abortRequestFunc = errorRequestFunc = doneRequestFunc = requestAction = undefined;
	abortRequestFunc = function () {
		_g['loader'].remove({'randkey': keyEventLoad});
		_g['session'].remove('client_ajaxify_request_('+url+')');
		cb(false);
	};
	errorRequestFunc = function () {
		_g['loader'].remove({'randkey': keyEventLoad});
		_g['session'].remove('client_ajaxify_request_('+url+')');
		cb(false);
	};
	doneRequestFunc = function (returnObj) {
...
_g['ajaxify'].loadwrapper = function (t, cb) {
...
_g['ajaxify'].reload = function (obj, cb) {
	if (!isset(obj)) {
		return false;
	}
	cb = isset(cb) && typeof cb === "function" ? cb : function () {};
						...
						loopPushCss: for (var cssKey in obj['include']['css']) {
							...
							if (isset(cssThis) && isset(cssThis['src']) && typeof cssThis['src'] === "string" && cssThis['src'] != "") {
								loadFileSource('css', {'src': cssThis['src']}, function (loadFileThis) {
									if (isset(loadFileThis) && isset(loadFileThis['return']) && loadFileThis['return'] === true) {
...
var resizeTemplates = function () {
	...
	if (!$("html#giccos").hasClass("blank")) {
		...
		var loopResizeBox = function (count) {
	...
	if (isset(object['description']) && typeof object['description'] === "object") {
		if (isset(object['description']['type']) && typeof object['description']['type'] === "string" && inArray(object['description']['type'], ["info::places"])) {
			if (isset(object['description']['data']) && typeof object['description']['data'] === "object") {
				...
				var codeDescriptionBodyDescription = (isset(object['description']['data']['description']) && typeof object['description']['data']['description'] === "string") ? '<br> <span><?php print $_language->text('description', 'ucfirst'); ?>: '+object['description']['data']['description'] + '</span>' : '<br> <span><?php print $_language->text('not_found_info_places', 'strtolower'); ?></span>';
...
var ExplorerPopup = function (object) {
	...
	loadData = function (object) {
		if (tab.attr("handling-data") != "true") {
			...
			configData = {'type': object['port'], 'local': object['type'], 'class': 'list'};
			data = {'port': 'explorer', 'type': object['port'], 'local': object['type'], 'action': 'get', 'class': 'list'};
			_g['xhr']({
				type: 'POST',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
	...
	pushData = function (config, object) {
...
var messagesTabPrivate = function (object) {
	...
	var actionMessagesPopup = function (popup, object) {
		...
		var getInfoPopup = function (reset) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'tab', 'action': 'get', 'label': 'display', 'value': messagesTabDisplay, 'limit': '1'};
			_g['xhr']({
				type: 'GET',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
				...
				var actionSettingsLanguage = function () {
					var popupSettingsLanguage = function (language) {
						...
						var changeLanguageRequest = function () {
							...
							data = {'port': 'messages', 'type': 'private', 'manage': 'members', 'action': 'change', 'id': tabId, 'label': 'translate', 'value': tagLanguage, 'guy': {'type': 'mode', 'id': 'auto'}};
							_g['xhr']({
								type: 'POST',
								url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
								data: data,
					...
					data = {'port': 'messages', 'type': 'private', 'manage': 'members', 'action': 'get', 'id': tabId, 'rows': ['translate'], 'guy': {'type': 'mode', 'id': 'auto'}, 'limit': 1};
					_g['xhr']({
						type: 'POST',
						url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
						data: data,
				...
				var actionSettingsName = function () {
					var popupSettingsName = function (name) {
						...
						var changeNameRequest = function () {
							...
							data = {'port': 'messages', 'type': 'private', 'manage': 'tab', 'action': 'change', 'id': tabId, 'label': 'name', 'value': newName, 'guy': {'type': 'mode', 'id': 'auto'}};
							_g['xhr']({
								type: 'POST',
								url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
								data: data,
		...
		var getTypingMessagesPopup = function (object, loop) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'typing', 'action': 'get', 'id': tabId};
			_g['xhr']({
				type: 'GET',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
				datatype: 'json',
					...
					if (isset(data['return']) && data['return'] === true) {
						if (isset(data['data']) && isset(data['data']['typing'])) {
							var isFounded = 0;
		...
		var addCodeTypingMessagesPopup = function (object) {
			...
			var codeMessagesRow = "<div class='rows "+pushClass+" typing _h' typing='"+JSON.stringify(infoObj)+"'> <div class='avatar'> <a href='"+object['guy']['link']+"'><img class='img' src='"+object['guy']['avatar']+"'></a> </div> <div class='text'> <div class='infotext'> <div class='icon status _tt-bk s' title='"+object['guy']['name'] +" ("+object['guy']['tag']+") - <?php print $_language->text('typing', 'strtolower'); ?>'></div> </div> <div class='context'> <i class='arrow'></i> <div class='maintext'> <span>"+contentText+"</span></div> </div> </div> </div>";
		...
		var actionViewsMessagesPopup = function (type, object) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'views', 'action': type, 'id': tabId};
			_g['xhr']({
				type: 'POST',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
		...
		var getViewsMessagesPopup = function (object, loop) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'views', 'action': 'get', 'id': tabId, 'messages': messagesId};
			_g['xhr']({
				type: 'GET',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
		...
		var addCodeViewsMessagesPopup = function(object) {
			...
			var reloadTimeFunc = function () {
				...
				var titleCode = ""+object['guy']['name']+" ("+object['guy']['tag']+") - <?php print $_language->text('seen_at', 'strtolower'); ?> "+agoDatetime(object['time'], 'ago')+"";
				boxViews.attr('tooltip', titleCode);
			};
			var infoObj = {'time': object['time'], 'guy': object['guy']};
			var codeViewsBox = "<div class='box _tt-bk s _h' views='"+JSON.stringify(infoObj)+"' randkey='"+randkey+"' title='"+object['guy']['name']+" ("+object['guy']['tag']+") - <?php print $_language->text('seen_at', 'strtolower'); ?> "+agoDatetime(object['time'], 'tip')+"'> <img class='img' src='"+object['guy']['avatar']+"'> </div>";
		...
		var addMessagesPopup = function (input, object) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'data', 'action': 'add', 'id': tabId, 'content': messagesContent};
			_g['xhr']({
				type: 'GET',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
		...
		var getNewMessagesPopup = function (object, loop) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'data', 'action': 'get', 'id': tabId, 'sort': '>', 'label': 'id', 'value': messagesId, 'limit': null};
			_g['xhr']({
				type: 'POST',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
		...
		var getOldMessagesPopup = function (object) {
			...
			data = {'port': 'messages', 'type': 'private', 'manage': 'data', 'action': 'get', 'id': tabId, 'sort': '<', 'label': 'id', 'value': messagesId, 'limit': '10'};
			_g['xhr']({
				type: 'POST',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
		...
		var getLastMessagesPopup = function (object) {
			...
			popup.attr('handling-get-messages', 'true');
			data = {'port': 'messages', 'type': 'private', 'manage': 'data', 'action': 'get', 'id': tabId, 'sort': '>=<', 'limit': '10'};
			_g['xhr']({
				type: 'POST',
				url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
				data: data,
	...
	_g['global'].append(messagesTabCode);
	var messagesTabThis = $(".messagesTab[display='"+messagesTabDisplay+"']");
	...
	var requestCreateMessages = function () {
		...
		var createMessagesTab = function (object) {
			...
			if (popupCreateMessages.attr('handling-create') != "true") {
				...
				data = {'port': 'messages', 'type': 'private', 'manage': 'tab', 'action': 'add', 'name': object['name'], 'private': object['private'], 'guy': object['guy'], 'language': object['language']};
				_g['xhr']({
					type: 'POST',
					url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
					data: data,
		...
		var findMessagesGuy = function (input, value) {
			...
			if (popupCreateMessages.attr('handling-find-guy') != "true") {
				...
				data = {'port': 'messages', 'type': 'private', 'manage': 'guy', 'action': 'search', 'from': 'all', 'value': value};
				_g['xhr']({
					type: 'POST',
					url: '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
					data: data,
...
var user_InfoEditor = function (object) {
			...
			_g['xhr']({
				...
				datatype: 'json',
					...
					if (isset(data['return']) && data['return'] === true) {
						...
						tab.children(".boxContent").find(".body").html('').append("<div class='not_allowed'> <i></i> <br> <span><?php print $_language->text('not_allowed', 'strtolower'); ?></span> </div>");
	...
	var pushData = function (infoObj) {
		...
		if (infoObj['type'] == "introduction") {
			...
			var linesImportant = "<div class='lines important'> <label><?php print $_language->text('important', 'ucfirst'); ?></label> <div class='email'><input class='input email' type='text' placeholder='<?php print $_language->text('type_email', 'upperlower'); ?>'><div class='private _tt-bk s' title='<?php print $_language->text('change_private', 'ucfirst'); ?>'></div></div> <div class='phone'><input class='input phone' type='text' placeholder='<?php print $_language->text('type_phonenumber', 'upperlower'); ?>'> <div class='private _tt-bk s' title='<?php print $_language->text('change_private', 'ucfirst'); ?>'></div> </div> </div>";
			...
			var addPhoneContact = function () {
				...
				addButton.find(".addbutton").bind('click', function () {
					...
					var randkey = randomKeyString(10);
					mainBox.find(".lines.phone > .add").before('<div class="textinfo" randkey="'+randkey+'"> <input class="input phone" type="text" placeholder="<?php print $_language->text('type_phonenumber', 'upperlower'); ?>"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div>');
					mainBox.find(".lines.phone > [randkey='"+randkey+"']").removeAttr('randkey').hide().fadeIn().children(".private").choosePrivate({'type': 'info', 'id': 4});
					setupTooltip();
					autoPostionBox();
			...
			var addEmailContact = function () {
				...
				addButton.find(".addbutton").bind('click', function () {
					addButton.stop(true, true).hide();
					var randkey = randomKeyString(10);
					mainBox.find(".lines.email > .add").before('<div class="textinfo" randkey="'+randkey+'"> <input class="input email _tt-bk s" type="text" placeholder="<?php print $_language->text('type_email', 'upperlower'); ?>"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div>');
					mainBox.find(".lines.email > [randkey='"+randkey+"']").removeAttr('randkey').hide().fadeIn().children(".private").choosePrivate({'type': 'info', 'id': 4});
					setupTooltip();
					autoPostionBox();
			...
			if (isset(infoObj['data']['contact']) && countArr(infoObj['data']['contact']) > 0) {
				contactLoop: for (var i in infoObj['data']['contact']) {
					...
					if (isset(thisContact['type']) && thisContact['type'] == "email") {
						...
						mainBox.find(".lines.email > .add").before('<div class="textinfo" contact-id="'+thisContact['id']+'"> <input class="input email" type="text" placeholder="<?php print $_language->text('type_email', 'upperlower'); ?>" value="'+thisContact['value']+'"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div> </div>');
						...
						mainBox.find(".lines.phone > .add").before('<div class="textinfo" contact-id="'+thisContact['id']+'"> <input class="input phone" type="text" placeholder="<?php print $_language->text('type_phonenumber', 'upperlower'); ?>" value="'+thisContact['value']+'"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div>');
			...
			if (isset(infoObj['data']['education']) && countArr(infoObj['data']['education']) > 0) {
				educationLoop: for (var i in infoObj['data']['education']) {
					...
					mainBox.find(".lines.add").after('<div class="lines education" education-id="'+thisEducation['id']+'"> <label>'+thisEducation['school']+'</label> <div class="textinfo"> <div class="li school"><input class="input school" type="text" placeholder="<?php print $_language->text('type_schoolname', 'upperlower'); ?>" value="'+thisEducation['school']+'"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div> <div class="li special"><input class="input special" type="text" placeholder="<?php print $_language->text('type_school_specialized', 'upperlower'); ?>" value="'+thisEducation['special']+'"></div> <div class="li description"><textarea class="input description as" type="text" placeholder="<?php print $_language->text('type_description', 'upperlower'); ?>" value="'+thisEducation['description']+'"></textarea></div> <div class="li time"> <div class="start _tt-bk s" title="<?php print $_language->text('type_starttime', 'upperlower'); ?>"> <label><?php print $_language->text('start_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> <div class="stop _tt-bk s" title="<?php print $_language->text('type_endtime', 'upperlower'); ?> - <?php print $_language->text('not_type_if_now', 'upperlower'); ?>"> <label><?php print $_language->text('end_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> </div> </div> </div>');
			...
			var addWorkplacesTab = function () {
				...
				addButton.find(".addbutton").bind('click', function () {
					var randkey = randomKeyString(10);
					mainBox.find(".lines.add").after('<div class="lines workplaces" randkey="'+randkey+'"> <label>-</label> <div class="textinfo"> <div class="li company"><input class="input company" type="text" placeholder="<?php print $_language->text('type_company_name', 'upperlower'); ?>"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div> <div class="li work awards"><input class="input work" type="text" placeholder="<?php print $_language->text('type_company_work', 'upperlower'); ?>"> <input class="input awards" type="text" placeholder="<?php print $_language->text('type_company_awards', 'upperlower'); ?>"> </div> <div class="li address"><input class="input address as" type="text" placeholder="<?php print $_language->text('type_company_address', 'upperlower'); ?>"></div> <div class="li description"><textarea class="input description as" type="text" placeholder="<?php print $_language->text('type_description', 'upperlower'); ?>"></textarea></div> <div class="li time"> <div class="start _tt-bk s" title="<?php print $_language->text('type_starttime', 'upperlower'); ?>"> <label><?php print $_language->text('start_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> <div class="stop _tt-bk s" title="<?php print $_language->text('type_endtime', 'upperlower'); ?> - <?php print $_language->text('not_type_if_now', 'upperlower'); ?>"> <label><?php print $_language->text('end_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> </div> </div> </div>');
			...
			if (isset(infoObj['data']['workplaces']) && countArr(infoObj['data']['workplaces']) > 0) {
				workplacesLoop: for (var i in infoObj['data']['workplaces']) {
					...
					mainBox.find(".lines.add").after('<div class="lines workplaces" workplaces-id="'+thisWorkplaces['id']+'"> <label>'+thisWorkplaces['company']+'</label> <div class="textinfo"> <div class="li company"><input class="input company" type="text" placeholder="<?php print $_language->text('type_company_name', 'upperlower'); ?>" value="'+thisWorkplaces['company']+'"> <div class="private" title="<?php print $_language->text('change_private', 'ucfirst'); ?>"></div></div> <div class="li work awards"><input class="input work" type="text" placeholder="<?php print $_language->text('type_company_work', 'upperlower'); ?>" value="'+thisWorkplaces['work']+'"> <input class="input awards" type="text" placeholder="<?php print $_language->text('type_company_awards', 'upperlower'); ?>" value="'+thisWorkplaces['awards']+'"> </div> <div class="li address"><input class="input address as" type="text" placeholder="<?php print $_language->text('type_company_address', 'upperlower'); ?>" value="'+thisWorkplaces['address']+'"></div> <div class="li description"><textarea class="input description as" type="text" placeholder="<?php print $_language->text('type_description', 'upperlower'); ?>" value="'+thisWorkplaces['description']+'"></textarea></div> <div class="li time"> <div class="start _tt-bk s" title="<?php print $_language->text('type_starttime', 'upperlower'); ?>"> <label><?php print $_language->text('start_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> <div class="stop _tt-bk s" title="<?php print $_language->text('type_endtime', 'upperlower'); ?> - <?php print $_language->text('not_type_if_now', 'upperlower'); ?>"> <label><?php print $_language->text('end_time', 'ucfirst'); ?></label> <div> <input class="input day" date-type="day" type="text" placeholder="<?php print $_language->text('day', 'strtolower'); ?>"> <input class="input month" date-type="month" type="text" placeholder="<?php print $_language->text('month', 'strtolower'); ?>"> <input class="input year" date-type="year" type="text" placeholder="<?php print $_language->text('year', 'strtolower'); ?>"> </div> </div> </div> </div> </div>');
			...
			var linesLive = "<div class='lines live'> <label><?php print $_language->text('live', 'ucfirst'); ?></label> <div class='textinfo'> <div class='li live'><div class='private' title='<?php print $_language->text('change_private', 'ucfirst'); ?>'></div> <input class='input live value' type='text' placeholder='<?php print $_language->text('where_do_you_live', 'upperlower'); ?>'></div> <div class='li description'><textarea class='input live description as' type='text' placeholder='<?php print $_language->text('write_something_about_live', 'strtolower'); ?>'></textarea></div></div> </div>";
			var linesCountry = "<div class='lines country'> <label><?php print $_language->text('country', 'ucfirst'); ?></label> <div class='textinfo'> <div class='li country'><div class='private' title='<?php print $_language->text('change_private', 'ucfirst'); ?>'></div> <input class='input country value' type='text' placeholder='<?php print $_language->text('where_do_you_from', 'upperlower'); ?>'></div> <div class='li description'><textarea class='input country description as' type='text' placeholder='<?php print $_language->text('write_something_about_country', 'strtolower'); ?>'></textarea></div></div> </div>";
...
var checkFileLoaded = function () {
	fileLoadedArr = {};
	if (_g['session'].exists('client_source_file') === true) {
		fileLoadedArr = _g['session'].get('client_source_file');
	}else {
		_g['session'].add('client_source_file', {});
	}
	$("script").each(function() {
		...
		if (isset(fileSrc) && !inArray(fileSrc, fileLoadedArr)) {
			...
			_g['session'].push('client_source_file', fileThisArr);
		}
	});
	$("link").each(function() {
...
var loadFileSource = function (type, cg, cb) {
	cb = typeof cb === "function" ? cb : function () {};
	...
	if (isset(src)) {
		...
		if (_g['session'].exists('cl+ient_source_loading_('+src+')') === true) {
			cb({'return': false});
		}else {
			_g['session'].new('client_source_loading_('+src+')');
		}
		errorRequestFunc = function () {
			console.log('xs');
			_g['session'].remove('client_source_loading_('+src+')');
			cb({'return': false});
		};
		doneRequestFunc = function () {
			_g['session'].remove('client_source_loading_('+src+')');
		...
		x = _g['xhr']({
			...
			'error': errorRequestFunc
		}, function (requestFile, requestReturn) {
...
var setupMediaPlayer = function (cb) {
	...
	var setMediaPlayerStatus = function (value) {
		_g['session'].add('client_mediaplayer_ready', value);
		cb(value);
	};
...
var isMediaPlayerReady = function (cb) {
	...
	if (_g['session'].exists('client_mediaplayer_ready') === true)  {
		cb(_g['session'].get('client_mediaplayer_ready'));
	}else {
		var cbSetupFunc = function (value) {
			cb(value);
		};
...
var callbackAllFunc = function () {
	...
	setTimeout(function() {
		...
		// user_InfoEditor({'id': 1, 'setup': 'about'});
		/*
		console.log(_g['session'].add('sdsd', {'sdsd': 'sdfsdfsdf', 'sdfsdfas': 'sdfsaf'}));
		setTimeout(function () {
			console.log(_g['session'].get('sdsd'));
		}, 5000);
		*/
		...
		console.log(_g['session'].get('client_source_file'));
		*/
		/*
		f = function (x) {
			console.log(x);
