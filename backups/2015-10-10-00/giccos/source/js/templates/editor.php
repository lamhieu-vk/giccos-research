		...
		for (var i in faces) {
			...
			if (thisFace['guy.type'] === null || thisFace['guy.id'] === null || thisFace['guy.name'] === null) {
				...
				typeNameThis.find("[tag-obj] .close").bind('click', function() {
					typeNameThis.html("<input class='ip_s1 ip_as' type='text' placeholder='<?php print $_language->text('who_is_it', 'strtolower'); ?>' input>");
...
var editor_pushAddons = function (callback) {
	...
	button.bind('click', function() {
		...
		if (push == "mood") {
			...
			if (input.length === 0) {
				div.children("div:last").after('<div class="push" push="mood" value="true"> <input class="ip_s1 ip_as" type="text" placeholder="<?php print $_language->text('what_do_you_doing', 'strtolower'); ?>" input /> </div>');
			}else {
				input.slideToggle();
				var toggle = true;
			}
			...
			if (input.length === 0) {
				div.children("div:last").after('<div class="push" push="usertag" value="true"> <input class="ip_s1 ip_as" type="text" placeholder="<?php print $_language->text('who_with_you', 'strtolower'); ?>" input /> </div>');
			}else {
				input.slideToggle();
				var toggle = true;
			}
			...
			if (input.length === 0) {
				div.children("div:last").after('<div class="push" push="places" value="true"> <input class="ip_s1 ip_as" type="text" placeholder="<?php print $_language->text('where_are_you', 'strtolower'); ?>" input /> </div>');
			}else {
				input.slideToggle();
				var toggle = true;
			}
...
var editor_tagPlaces = function (selector) {
	...
	function requestFind (input) {
		...
		var placesHash = (input.val().replace("/([^a-zA-Z0-9]+)/", "")).toLowerCase();
		var placesCacheName = "editor_places_data["+placesHash+"]";
		if (_g['session'].exists(placesCacheName) == true) {
			var placesDataCache = _g['session'].get(placesCacheName);
			if (isset(placesDataCache) && typeof placesDataCache === "object") {
				var boxSuggestOptions = {
					'multiple': false,
					'output': true,
					'title': '<?php print $_language->text('you_may_find', 'ucfirst'); ?>',
					'content': {
						'<?php print $_language->text('places', 'ucfirst'); ?>': placesDataCache
					}
				};
				boxSuggest(input, boxSuggestOptions);
				return true;
			...
			success: function(data) {
				if (isset(data['return']) && data['return'] === true) {
					...
					if (isset(data['data'])) {
						...
						boxSuggest(input, boxSuggestOptions);
						_g['session'].add(placesCacheName, placesFoundedArr);
					}
					setTimeout(function() {
						input.removeAttr('val');
					}, 1000);
...
var editor_mediaUpload = function (object) {
    ...
    if (object['format'] == "photos" && object['type'] == "preview") {
        ...
        for (var i in object['data']) {
            ...
            if (count === 0) {
                var fileInfo = {
                    ...
                    'mime': rows['mine'],
                    'link': rows['link'],
                    'randkey': rows['randkey']
                };
                var code = " <div class='preview' photos-preview file='" + JSON.stringify(fileInfo) + "'> <span class='_tt s' title='<?php print $_language->text('tag_photos', 'strtolower'); ?>' tag></span> <span class='_tt s' title='<?php print $_language->text('edit_photos', 'strtolower'); ?>' edit></span> <span class='_tt s' title='<?php print $_language->text('remove_from_list', 'strtolower'); ?>' remove></span> <span class='_tt s' title='<?php print $_language->text('remove_photos', 'strtolower'); ?>' close></span> <div style='background-image: url(" + rows['link'] + ");' img></div> <div class='filter'></div> </div> ";
                var thisPreview = upload.find("[photos-uploading='"+rows['randkey']+"']");
                if (thisPreview.length === 0) {
                	upload.find(".list").append(code);
                }else {
        ...
        if (upload.find("[photos-remove]").length === 0) {
            var buttonRemove = ' <div class="button remove" photos-remove remove><i></i></div> ';
...
var editor_mediaPreview = function () {
	...
	if (editor.attr('type') == "photos") {
		...
		var uploadMediaPreview = function () {
			...
			if (upload.length === 0) {
				var previewBox = "<div class='upload photos' upload preview photos> <div class='list'> </div> <div class='action'> <div class='button add upload' photos-add upload><i></i></div> <div class='button add select' photos-add select><i></i></div> </div> </div>";
			...
			var requestUploadFunc = function (files) {
				...
				var code = " <div class='preview uploading' photos-uploading='"+randkey+"'> <div img></div> <div uploading> <div class='navProcess-buffering'></div> </div> <div class='filter'></div> </div> ";
				var upload = editor.find("[upload][preview][photos]");
		        upload.find(".list").append(code);
		        var thisPreview = upload.find("[photos-uploading='"+randkey+"']");
		        thisPreview.hide().fadeIn();
				fileToBlob(files, function (fileSrc) {
					fileObj = {'src': fileSrc, 'min': {'w': 100, 'h': 100}};
					imgResizeBlob(fileObj, function (urlPreview) {
						thisPreview.find("[img]").css({'background-image': 'url('+urlPreview+')'});
					});
				});
				files['randkey'] = randkey;
				var data = new FormData();
				data.append('port', 'photos');
				data.append('token', '<?php print $g_client['token']['action']['photos']; ?>');
				data.append('type', 'cache');
				data.append('action', 'add');
				data.append('randkey[]', randkey);
				data.append('file[]', files);
				requestUpload = _g['xhr']({
					'type': 'POST',
					'url': '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
					'data': data,
					'dataprocess': false,
					...
					'done': function (data) {
						console.log(data);
						if (isset(data) && isset(data['return']) && data['return'] === true && isset(data['data']) && countArr(data['data']) > 0) {
							mediaFile = [];
							for (var i in data['data']) {
								rows = data['data'][i];
								if (isset(rows['return']) && rows['return'] === true) {
									mediaFile[countArr(mediaFile)] = rows['data'];
								}else if (isset(rows['return']) && rows['return'] === false) {
									if (isset(rows['randkey']) && typeof rows['randkey'] === "string") {
										var fileRandkey = rows['randkey'];
										...
										upload.find("[photos-uploading='"+fileRandkey+"']").addClass('error');
										setTimeout(function () {
											upload.find("[photos-uploading='"+fileRandkey+"']").fadeOut(function () {
											 	$(this).remove();
											});
										}, 1000);
									}
		...
		uploadFormInput.change(function() {
			console.log(window.URL.createObjectURL($(this)[0].files[0]));return;
			if ($(this).val() != "") {
				uploadMediaPreview();
				uploadForm.submit();
			}
		...
		*/
		...
		var input = contentBox.children("[input][classify='cited']"),
			uploadForm = input.find("[form][data='cited']"),
			uploadFormInput = uploadForm.children("input"),
			uploadButtonUpload = input.find("[button][upload]"),
			uploadButtonSelect = input.find("[button][select]"),
			uploadPreview = contentBox.children("[upload][preview][cited]");
		uploadButtonUpload.bind('click', function() {
			var uploadButtonUpload = $(this);
			if (uploadButtonUpload.attr('handling-click') != "true") {
				uploadButtonUpload.attr('handling-click', 'true');
	...
	buttonChooseType.bind('click', function() {
		...
		if (statusType != boxMediaOtherOld && (statusType == "status" || statusType == "photos" || statusType == "music" || statusType == "videos" || statusType == "cited")) {
			...
			if (statusType == "status") {
				...
				var linkPushCode = function (url, linkObj) {
					if (!isset(linkObj)) {
						return false;
					}
					...
					if (1 + 1 == 2) {
						...
						if (isset(linkObj['thumbnail']) && typeof linkObj['thumbnail'] == "string") {
							var code_thumbnail = "<div class='thumbnail'> <div style='background-image: url("+linkObj['thumbnail']+");'></div> </div>";
						}else {
							var code_thumbnail = "";
						}
						var code = "<div class='demo boxLink' tag='link' tag-info='"+JSON.stringify(linkObj)+"' tag-display='"+linkObj['display']+"' tag-url='"+linkObj['url']+"'> "+code_thumbnail+" <div class='info'> <div class='header'> <div class='action'> <i class='close'></i> </div> <div class='title nowrap'><span>"+linkObj['title']+"</span></div> </div> <div class='body'> <div class='description'><span>"+linkObj['description']+"</span></div> </div> <div class='footer'> <div class='information'><i></i><span><?php print $_language->text('information', 'strtolower'); ?></span></div> <div class='host'><i></i><span>"+linkObj['host']+"</span></div> </div> </div> </div>";
						uploadPreview.attr('show', 'true').html(code).hide().stop(true, true).fadeIn();
						uploadPreview.children(".demo[tag]").find(".info > .header > .action .close").bind('click', function() {
							$(this).parents(".demo[tag]").fadeOut(function() {
								uploadPreview.attr('show', 'false').remove();
								if (_g['session'].exists('sites-notuse-url') === true) {
									_g['session'].push('sites-notuse-url', url);
								}else {
									_g['session'].add('sites-notuse-url', [url]);
								}
							});
						});
						if (_g['session'].exists('sites-info-url') === true) {
							var infoList = _g['session'].get('sites-info-url');
							var isHas = 0;
							infoListLoop: for (i in infoList) {
								if (i == url) {
									isHas++;
									break infoListLoop;
								}
							...
							if (isHas === 0) {
								...
								newObj[url] = linkObj;
								linkObj = newObj;
								_g['session'].push('sites-info-url', linkObj);
							}
						}else {
							newObj = {};
							newObj[url] = linkObj;
							linkObj = newObj;
							_g['session'].add('sites-info-url', linkObj);
						}
					}
				};
				var linkScanInfo = function (url) {
					var tagUrl = uploadPreview.children(".demo").attr('tag-url');
					if (isset(tagUrl) && typeof tagUrl === "string" && tagUrl == url) {
						//.
					}
					var infoUrlList = _g['session'].get('sites-scan-url');
					if (isset(infoUrlList) && typeof infoUrlList == "object") {
						if (countArr(infoUrlList) > 0) {
							for (var i in infoUrlList) {
								if (i == url && infoUrlList[i] === true) {
									console.log(i);
									return false;
								}
							}
					...
					if (textarea.attr('handing-link-scan-check') == "true") {
						return false;
					}else {
						textarea.attr('handing-link-scan-check', 'true');
					}
					$.ajax({
						url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
						type: "POST",
						data: {'port': 'sites', 'type': 'scan', 'action': 'check', 'url': url},
						dataType: "json",
						success: function(data) {
							if (isset(data) && isset(data['return']) && data['return'] === true) {
								console.log('xs');
								if (_g['session'].exists('sites-scan-url') === true) {
									var infoList = _g['session'].get('sites-scan-url');
									var isHas = 0;
									infoListLoop: for (i in infoList) {
										if (i == url) {
											isHas++;
									...
									if (isHas === 0) {
										linkObj = [];
										linkObj[url] = true;
										_g['session'].push('sites-scan-url', linkObj);
									}
								}else {
									linkObj = [];
									linkObj[url] = true;
									_g['session'].add('sites-scan-url', linkObj);
								}
						...
						setTimeout(function() {
							textarea.removeAttr('handing-link-scan-check');
						}, 100);
					}).done(function() {
						setTimeout(function() {
							textarea.removeAttr('handing-link-scan-check');
				...
				var linkGetInfo = function (url) {
					var tagUrl = contentBox.children("[upload][preview][link]").find(".demo").attr('tag-url');
					if (isset(tagUrl) && typeof tagUrl === "string" && tagUrl == url) {
						return false;
					}
					if (_g['session'].exists('sites-notuse-url') === true) {
						var notuseUrlList = _g['session'].get('sites-notuse-url');
					}else {
						var notuseUrlList = {};
					}
					if (isset(notuseUrlList) && typeof notuseUrlList == "object") {
						if (countArr(notuseUrlList) > 0) {
							for (var i in notuseUrlList) {
								if (notuseUrlList[i] == url) {
									return false;
								}
							}
						}
					...
					if (_g['session'].exists('sites-info-url') === true) {
						var infoUrlList = _g['session'].get('sites-info-url');
					}else {
						var infoUrlList = {};
					}
					if (isset(infoUrlList) && typeof infoUrlList == "object") {
						if (countArr(infoUrlList) > 0) {
							for (var i in infoUrlList) {
								if (i == url) {
									console.log(infoUrlList[i]);
									linkPushCode(url, infoUrlList[i]);
									return false;
								}
							}
						}
					}
					if (textarea.attr('handing-link-info-extract') == "true") {
						return false;
					}else {
						textarea.attr('handing-link-info-extract', 'true');
					}
					$.ajax({
						url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
						type: "POST",
						...
						setTimeout(function() {
							textarea.removeAttr('handing-link-info-extract');
						}, 100);
					}).done(function() {
						setTimeout(function() {
							textarea.removeAttr('handing-link-info-extract');
