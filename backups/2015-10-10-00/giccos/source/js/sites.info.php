var sites_rateBox_stats = function (elem, options) {
...
$(document).ready(function () {
	$("#infoPage > .box.rate > .content > .rows > .value > .rateBox").each(function () {
		...
		sites_rateBox_stats($(this), options);
	});
	$("#infoPage > .box.stats > .content > .rows > .value.views > .statsBox").each(function () {
		if (_g['session'].exists('sites_info_stats_views_dataset') === true) {
			var data = _g['session'].get('sites_info_stats_views_dataset');
		}else {
			return false;
		}
	...
	$("#infoPage > .box.stats > .content > .rows > .value.mention > .statsBox").each(function () {
		if (_g['session'].exists('sites_info_stats_mention_dataset') === true) {
			var data = _g['session'].get('sites_info_stats_mention_dataset');
		}else {
			return false;
		}
		var lineBar = $(this).get(0).getContext("2d");
