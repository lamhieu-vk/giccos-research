				...
				if ($secret != null && $name != null && ($verify == true || $verify == false)) {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							...
							$photosFacesQuery = mysqli_query($db, "SELECT `id`, `photos`, `faces`, `faces.image` FROM `photos_faces` WHERE `photos` = '{$fetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$this->user['mode']['type']}' AND `author.id` = '{$this->user['mode']['id']}' AND `user.id` = '{$this->user['id']}'");
							if ($photosFacesQuery && $photosLikeThisNum == 0) {
								while ($photosFacesFetch = mysqli_fetch_assoc($photosFacesQuery)) {
									$delphotosFaces = $_storage->delete(array("format" => "image", "label" => "display", "value" => $photosFacesFetch['faces.image']));
									$delFacesUsertag = $_analysis->faces("delete", array("label" => "display", "value" => $photosFacesFetch['faces']));
									if (isset($delphotosFaces['return'], $delFacesUsertag['return']) && $delphotosFaces['return'] == true && $delFacesUsertag['return'] == true) {
