...
if (isset($g_client['token']['ajax'], $_SERVER['HTTP_TOKEN'], $_SERVER['HTTP_X_REQUESTED_WITH'], $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_SERVER['HTTP_TOKEN'] == $g_client['token']['ajax'] && $_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest" && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")) {
	...
	if (($port == "user" || $port == "users") && $token == $g_client['token']['action']['user']) {
		...
		if ($type == "info") {
			...
			if ($action == "get") {
				if (isset($_POST['id']) && is_string($_POST['id'])) $id = $_POST['id']; else $id = null;
				...
				$rowsAllows = ["username", "email", "phone", "fullname", "firstname", "middlename", "lastname", "nickname", "gender", "language"];
				$rowsGet = null;
				foreach ($rows as $rowsValue) {
					if (in_array($rowsValue, $rowsAllows)) {
		...
		if ($type == "editor") {
			...
			if ($action == "add") {
				...
				if (isset($_POST['file']) && is_array($_POST['file']) && count($_POST['file']) > 0) $file = $_POST['file']; else $file = null;
				if ($file == null) {
					die(print json_encode(array("return" => false, "reason" => "")));
				}else {
					$_media = new media;
					foreach ($file as $i => $thisFile) {
						...
						if ($secret != null && $name != null && ($verify == true || $verify == false)) {
							$deleteFile = $_media->cache("photos", array("action" => "delete", "secret" => $secret, "name" => $name, "verify" => $verify));
			...
			if ($action == "write") {
				...
				if (isset($_POST['boundingbox']['tl']['y']) && is_string($_POST['boundingbox']['tl']['y'])) $boundingbox_tl_y = $_POST['boundingbox']['tl']['y']; else $boundingbox_tl_y = null;
				if (isset($_POST['boundingbox']['tl']['x']) && is_string($_POST['boundingbox']['tl']['x'])) $boundingbox_tl_x = $_POST['boundingbox']['tl']['x']; else $boundingbox_tl_x = null;
				if (isset($_POST['boundingbox']['size']['height']) && is_string($_POST['boundingbox']['size']['height'])) $boundingbox_size_height = $_POST['boundingbox']['size']['height']; else $boundingbox_size_height = null;
				if (isset($_POST['boundingbox']['size']['width']) && is_string($_POST['boundingbox']['size']['width'])) $boundingbox_size_width = $_POST['boundingbox']['size']['width']; else $boundingbox_size_width = null;
				if (isset($_POST['guy']['type']) && is_string($_POST['guy']['type'])) $guy_type = $_POST['guy']['type']; else $guy_type = 0;
				if (isset($_POST['guy']['id']) && is_string($_POST['guy']['id'])) $guy_id = $_POST['guy']['id']; else $guy_id = null;
				...
				if (isset($getExists['return'], $getExists['exists']) && $getExists['return'] == true && $getExists['exists'] == true) {
					...
					if ($photos_local == "cache") {
						...
						$photosQuerySql = "SELECT `display` FROM {$dbName} WHERE `id` = '{$photos_id}' ORDER BY `id` DESC LIMIT 1";
					}
					...
					$numTagQuerySql = "SELECT `id` FROM `photos_faces` WHERE `photos` = '{$photos_display}' AND `photos.local` = '{$photos_local}' AND `boundingbox.tl.y` = '{$boundingbox_tl_y}' AND `boundingbox.tl.x` = '{$boundingbox_tl_x}' AND `boundingbox.size.height` = '{$boundingbox_size_height}' AND `boundingbox.size.width` = '{$boundingbox_size_width}' LIMIT 1";
					...
					if ($numTag == 0) {
						...
						//. check private guy.
						$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $photos_display, "rows" => "`token`, `path`", "limit" => "LIMIT 1"));
						...
						$cacheFacestag = array();
						$cacheFacestag['photos'] = $photos_display;
						$cacheFacestag['photos.local'] = $photos_local;
						...
						VALUES 
						(NULL, '{$cacheFacestag['token']}', '{$cacheFacestag['display']}', '{$g_user['id']}', '{$g_user['mode']['type']}', '{$g_user['mode']['id']}', '{$cacheFacestag['photos']}', '{$cacheFacestag['photos.local']}', '{$cacheFacestag['faces']}', '{$cacheFacestag['faces.image']}', {$guy_type}, {$guy_id}, '{$boundingbox_tl_y}', '{$boundingbox_tl_x}', '{$boundingbox_size_height}', '{$boundingbox_size_width}', '{$boundingbox_ratio_height}', '{$boundingbox_ratio_width}');
						";
						...
						if (isset($getAnalysis['return'], $getAnalysis['faces']) && $getAnalysis['return'] == true && is_array($getAnalysis['faces'])) {
							...
							foreach ($faceAnalysis as $key => $faceAnalysisThis) {
								...
								if (isset($addAnalysisFaces['return'], $addAnalysisFaces['obj']) && $addAnalysisFaces['return'] == true && is_array($addAnalysisFaces['obj']) && count($addAnalysisFaces['obj']) > 0) {
									...
									$cacheFacestag['display'] = hash('md5', $cacheFacestag['token']);
									$insertCacheTaguserSql = "INSERT INTO `photos_faces` (`id`, `token`, `display`, `user.id`, `author.type`, `author.id`, `photos`, `photos.local`, `faces`, `faces.image`, `guy.type`, `guy.id`, `boundingbox.tl.y`, `boundingbox.tl.x`, `boundingbox.size.height`, `boundingbox.size.width`, `boundingbox.ratio.height`, `boundingbox.ratio.width`) VALUES (NULL, '{$cacheFacestag['token']}', '{$cacheFacestag['display']}', '{$g_user['id']}', '{$g_user['mode']['type']}', '{$g_user['mode']['id']}', '{$cacheFacestag['photos']}', '{$cacheFacestag['photos.local']}', '{$cacheFacestag['faces']}', '{$cacheFacestag['faces.image']}', null, null, '{$faceAnalysisThis['boundingbox.tl.y']}', '{$faceAnalysisThis['boundingbox.tl.x']}', '{$faceAnalysisThis['boundingbox.size.height']}', '{$faceAnalysisThis['boundingbox.size.width']}', '{$faceAnalysisThis['boundingbox.ratio.height']}', '{$faceAnalysisThis['boundingbox.ratio.width']}');";
				...
				if (isset($_POST['tag']['label']) && is_string($_POST['tag']['label'])) $tag_label = $_POST['tag']['label']; else $tag_label = null;
				if (isset($_POST['tag']['value']) && is_string($_POST['tag']['value'])) $tag_value = $_POST['tag']['value']; else $tag_value = null;
				if (isset($_POST['guy']['type']) && is_string($_POST['guy']['type'])) $guy_type = $_POST['guy']['type']; else $guy_type = 0;
				if (isset($_POST['guy']['id']) && is_string($_POST['guy']['id'])) $guy_id = $_POST['guy']['id']; else $guy_id = null;
				if ($tag_label == null || $tag_value == null) {
					die(print json_encode(array("return" => false, "reason" => "")));
				}
				if ($guy_type == null && $guy_id == null) {
					$guy_set = false;
				}else if ($guy_type != null && $guy_id != null) {
					$guy_set = true;
				}else {
					die(print json_encode(array("return" => false, "reason" => "")));
				}
				$numTagQuerySql = "SELECT * FROM `photos_faces` WHERE `{$tag_label}` = '{$tag_value}' AND `author.type` = '{$g_user['mode']['type']}' AND `author.id` = '{$g_user['mode']['id']}' AND `user.id` = '{$g_user['id']}'";
				$numTagQuery = mysqli_query($_db->port('beta'), $numTagQuerySql);
				if ($numTagQuery) $numTag = mysqli_num_rows($numTagQuery); else $numTag = 0;
				if ($numTag == 0) {
					die(print json_encode(array("return" => false, "reason" => "")));
					...
					//. check guy private.
					if ($guy_set == true) {
						if ($guy_type == "user" || $guy_type == "users") {
							$getGuyName = $_user->profile(array("id" => $guy_id, "rows" => "`fullname`"));
							if (isset($getGuyName['return'], $getGuyName['data']) && $getGuyName['return'] == true) {
					...
					if (!$guy_set) {
						...
						$updateSqlPush = "`guy.type` = '{$guy_type}', `guy.id` = '{$guy_id}'";
					}
					$updateSql = "UPDATE `photos_faces` SET {$updateSqlPush} WHERE `{$tag_label}` = '{$tag_value}' AND `author.type` = '{$g_user['mode']['type']}' AND `author.id` = '{$g_user['mode']['id']}' AND `user.id` = '{$g_user['id']}'";
								...
								if (isset($cachePhotosGetFacesThis['photos'])) {
									if (isset($cachePhotosGetFacesThis['user.id'], $cachePhotosGetFacesThis['author.type'], $cachePhotosGetFacesThis['author.id'])) {
										if ($cachePhotosGetFacesThis['photos'] == $tagFetch['photos'] && $cachePhotosGetFacesThis['photos.local'] == $tagFetch['photos.local'] && $cachePhotosGetFacesThis['display'] == $tagFetch['display']) {
											if (isset($cachePhotosGetFacesThis['faces'], $cachePhotosGetFacesThis['thumbnail'], $cachePhotosGetFacesThis['display'], $cachePhotosGetFacesThis['position'], $cachePhotosGetFacesThis['size'], $cachePhotosGetFacesThis['ratio'])) {
												if ($cachePhotosGetFacesThis['user.id'] == $g_user['id'] && $cachePhotosGetFacesThis['author.type'] == $g_user['mode']['type'] && $cachePhotosGetFacesThis['author.id'] == $g_user['mode']['id']) {
													$cachePhotosGetFaces[$key]['guy.type'] = $guy_type;
													$cachePhotosGetFaces[$key]['guy.id'] = $guy_id;
