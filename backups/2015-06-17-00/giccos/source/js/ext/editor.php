				...
				if (isset(data['return']) && data['return'] === true) {
					...
					if (isset(data['data'])) {
						...
						placesLoop: for (var i in data['data']) {
							...
							thisPlaces['tag']['code'] = thisPlaces['place_id'];
	...
	if (editor.attr('type') == "photos") {
		...
		var uploadMediaPreview = function () {
			...
			var sendRequestFunc = function () {
				...
				var requestUpload = form.ajaxForm({
					url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
					type: "POST",
					data: data,
					dataType: "json",
					beforeSend: function(e) {
						function readImageThumbnail (input) {
						...
						var code = " <div class='preview' photos-uploading='"+key+"' photos-name> <div uploading> <div></div> </div> <div class='filter'></div> </div> ";
						var upload = editor.find("[upload][preview][photos]");
		                upload.find("[photos-add]").first().before(code);
		                readImageThumbnail(form.find("input[type='file']")[0]);
					},
					uploadProgress: function(event, position, total, percentComplete) {
					    console.log(percentComplete);
					    var thisPreview = upload.find("[photos-uploading='"+key+"']");
					    thisPreview.find("[uploading]").children().width(percentComplete + '%');
					    thisPreview.find(".filter").css({'opacity': '1'});
					},
					success: function(data) {
					   // console.log(data);
			...
			if (statusType == "status") {
				...
				if (setup_inputVideos === 0) {
					var requestPlaylistFind = function (input) {
						...
						var data = {'port': 'user', 'token': '<?php print $g_client['token']['action']['user']; ?>', 'type': 'autocomplete', 'path': 'media', 'from': 'playlist', 'value': input.val()};
						$.ajax({
							url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
							type: "POST",
							dataType: "json",
...
var editor_upload = function (object) {
	...
	if (error !== true && editor.attr('handing-upload') !== "true") {
		...
		$.ajax({
			...
			success: function(data) {
				if (isset(data['return']) && data['return'] === true) {
					popupNotification ({type: 1, title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('status_has_been_posted','ucfirst'); ?>. <br /> <?php print $_language->text('pull_up_for_updates','ucfirst'); ?>.', from: '<?php print $_language->text('system', 'ucfirst'); ?>'});
					editor_resetForm();
