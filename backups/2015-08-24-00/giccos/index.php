		...
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			if (is_string($_uri[2]) && preg_match("/^(static)/i", $_uri[2])) {
				...
				if (!$_tool->siteDie($photos_url)) {
					$readFile = array("type" => "other", "path" => $photos_url, "mime" => 'image/'.$photos_ext, "expire" => $_parameter->get('http_cache_expire_static'));
					die(require_once ("source/ports/templates/readfile.php"));
				}else {
		...
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			if (is_string($_uri[2]) && preg_match("/^(static)/i", $_uri[2])) {
				...
				if (mysqli_num_rows($fileQuery) == 0) {
					...
					$fileFetch = mysqli_fetch_assoc($fileQuery);
					if (isset($fileFetch['path.320kbps'], $fileFetch['file.original']) && is_string($fileFetch['path.320kbps']) && is_string($fileFetch['file.320kbps']) && $fileFetch['file.320kbps'] == $fileName) {
						$filePath = $fileFetch['path.320kbps'];
					}else if (isset($fileFetch['path.128kbps'], $fileFetch['file.original']) && is_string($fileFetch['path.128kbps']) && is_string($fileFetch['file.320kbps']) && $fileFetch['file.128kbps'] == $fileName) {
						$filePath = $fileFetch['path.128kbps'];
					}else if (isset($fileFetch['path.original'], $fileFetch['file.original']) && is_string($fileFetch['path.original']) && is_string($fileFetch['file.320kbps']) && $fileFetch['file.original'] == $fileName) {
