					...
					if (isset(feed['port']['type']) && isset(feed['port']['require']) && typeof feed['port']['type'] == "string") {
						if (feed['port']['type'] == "hashtag" || feed['port']['type'] == "tags") {
							if (typeof feed['port']['require'] == "object") {
								...
								var allowPush = 0;
								var tagsFounded = content.match(/(^|\s)?\#([a-zA-Z0-9\_]+)($|\s)?/ig);
								...
								if (allowPush === 0) {
									...
									for (var s in hashtagAllowObj) {
										if (contentPushed === 0 && !content.match(/(\n)$/) && content.match(/^(\S+)/)) {
											content = content + "\n#" + hashtagAllowObj[s];
										}else {
											if (!content.match(/([^\s])$/)) {
												content = content + "#" + hashtagAllowObj[s];
											}else {
												content = content + " #" + hashtagAllowObj[s];
											}
										}
