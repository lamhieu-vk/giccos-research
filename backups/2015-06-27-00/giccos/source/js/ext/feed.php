var feed_status_embed = function () {
	...
	mediaLink.each(function(){
		var thisLink = $(this);
		if (typeof thisLink.attr('media-info') == "string" && typeof JSON.parse(thisLink.attr('media-info')) == "object") {
			var linkInfo = JSON.parse(thisLink.attr('media-info'));
			if (isset(linkInfo['embed']) && typeof linkInfo['embed'] == "object") {
				if (isset(linkInfo['embed']['code']) && typeof linkInfo['embed']['code'] == "string") {
					thisLink.removeAttr('direct');
					thisLink.bind('click', function(){
						thisLink.replaceWith(linkInfo['embed']['code']);
					});
				}
			}
		}
...
var status_newcheck = function () {
	var call = function () {
		...
		if (feed.attr('handing-newcheck') != "true") {
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: {'port': 'feed', 'type': 'status', 'action': 'newcheck', 'feed': port, 'id': id},
				contentType: "<?php print $_parameter->get("contentType_urlencoded.utf8"); ?>",
				success: function (data) {
	...
	var wait = function () {
		setTimeout(function() {
			call();
		}, <?php print $_parameter->get('status_timeout_newcheck'); ?> / 10);
	}
	wait();
}
...
var status_load = function (options) {
	var id = options['id'],
		sort = options['sort'],
		port = options['port'],
		data = {'port' : 'feed', 'type': 'status', 'action': 'load', 'feed': port, 'sort': sort, 'id': id};
...
var status_getCodeMedia = function (object) {
	...
	if (object['media']['type'] == "link") {
		...
		for (var i in object['media']['data']) {
			...
			var infoShowObj = {"type": "link", "id": value['id'], "display": value['display'], "host": value['host'], "path": value['path'], "thumbnail": value['thumbnail'], "title": value['title'], "description": value['description'], "share": value['share']};
			if (isset(value['embed'])) {
				infoShowObj['embed'] = value['embed'];
				value['direct'] = "";
			}else {
...
var feed_status_push = function (options, status) {
	...
	if (isset(status['comment']) && isset(status['comment']['count']) && status['comment']['number'] === 0) {
		...
		if (isset(status['comment']) && isset(status['comment']['count']) && status['comment']['count'] > 0) {
			c_['status_comment'] = " <div class='statistic'> <div class='info'> <i class='icon'> </i> <span class='number' status-statistic-comment>"+status['comment']['count']+"</span> <span class='text'> <?php print $_language->text('comment(s)', 'strtolower'); ?> </span> </div>"+c_['status_comment_showmore']+"</div> ";
		}else {
			c_['status_comment'] = "";
		}
...
var status_toggleMenu = function () {
	var status = $(".status.inFeeds[status]");
	status.each(function() {
		var status = $(this),
	...
	media.bind('click', function() {
		...
		if (type == "photos" && object != "" && object !== undefined && typeof JSON.parse(object) == 'object') {
			...
			var options = {
				...
				'autoplay': false,
				'preload': 'auto',
				'poster': 'http://www.fmwconcepts.com/misc_tests/foot_process/foot_rainbow_spots.png',
				'plugins': {
    				'replay': {
      					'active': true,
