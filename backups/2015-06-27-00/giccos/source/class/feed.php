...
Class feed {
	...
	function status_quickinfo ($object) {
		...
		if (mysqli_num_rows($getInfoQuery) > 0) {
			$infoFetch = mysqli_fetch_assoc($query);
			$infoArr = array(
				"id" => $infoFetch['id'], 
	...
	function status_edit ($object) {
		...
		if ($isChanged == 0) {
			...
			$updateRequest = "UPDATE `status` SET {$setUpdate}, `edit` = '{$_tool->timeNow()}' WHERE `id` = '{$statusFetch['id']}'";
	...
	function status_add ($object) {
		...
		if (isset($status['places']) && $status['places'] > 0) {
			$placesArr = array();
			if (isset($object['places']['display']) && is_string($object['places']['display'])) {
	...
	function status_getCodeMedia ($object) {
		...
		if (isset($object['media']['type']) && $object['media']['type'] == "link") {
			...
			foreach ($object['media']['data'] as $key => $value) {
				...
				$infoShowArr = array("type" => "link", "id" => $value['id'], "display" => $value['display'], "host" => $value['host'], "url" => $value['url'], "thumbnail" => $value['thumbnail'], "title" => $value['title'], "description" => $value['description'], "share" => $value['share']);
				if (isset($value['embed'])) {
					$infoShowArr['embed'] = $value['embed'];
					$value['direct'] = "";
				}else {
					$value['direct'] = json_encode(array("by" => "click", "to" => $value['url'], "target" => "_blank"));
				}
