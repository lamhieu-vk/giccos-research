...
if (!isset($g_user['login']) || !$g_user['login']) {
	foreach ($cookieArr as $cookieThisKey => $cookieThisValue) {
		if (in_array($cookieThisKey, ["messagesTabPostition"])) setcookie($cookieThisKey, null, time() - 31536000, $_parameter->get('cookie.host.path'), $g_client['http']['secure'], false);
	}
}
foreach ($cookieArr as $cookieThisKey => $cookieThisValue) {
...
if (isset($g_client['token']['source']['url']) && (in_array($_uri[0], $_uri_0, true) || $_uri[0] == $g_client['token']['source']['url'])) {
	...
	if (isset($_uri[0], $g_client['token']['source']['url']) && is_string($_uri[0]) && $_uri[0] == $g_client['token']['source']['url']) {
		...
		if (isset($g_user['mode']) && (!isset($_uri[1]) || (isset($_uri[1]) && is_string($_uri[1]) && in_array($_uri[1], ["", "general"])))) {
		...
		}else if (isset($_uri[1], $g_user['mode']) && is_string($_uri[1]) && $_uri[1] == "nearby") {
			$feed_['content']['title'] = "";
			$feed_['content']['this'] = json_encode(array("port" => array("type" => "nearby", "require" => "all")));
			$feed_['content']['from'] = $_language->text('nearby', 'strtolower', false);
			if (isset($_uri[2]) && $_uri[2] != null && !in_array(strtolower($_uri[2]), ["country", "live"])) {
				if (is_numeric($_uri[2]) && $_uri[2] != null) {
					...
					$getPositionNearbyRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".addslashes($_uri[2])."' LIMI 1";
					...
					$getPositionNearbyRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".addslashes($_uri[2])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
				}else {
				...
				if (isset($getPositionNearbyRequest) && is_string($getPositionNearbyRequest) && $getPositionNearbyRequest != null) {
					...
					if (!$getPositionNearbyQuery || mysqli_num_rows($getPositionNearbyQuery) == 0) {
						...
						$feed_['content']['from'] = $positionNearbyQuery['name'];
			...
			}else if (isset($_uri[2], $g_user['mode']['type']) && in_array(strtolower($_uri[2]), ["country", "live"]) && $g_user['mode']['type'] == "user") {
				$feed_['exists']['nearby'] = true;
				$feed_['content']['from'] = $_language->text('nearby', 'strtolower', false);
				if (strtolower($_uri[2]) == "live" && isset($g_user['live']) && is_numeric($g_user['live']) && $g_user['live'] != null) {
					$getPositionNearbyQuery = mysqli_query($_db->port('beta'), "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '{$g_user['live']}' LIMIT 1");
					if (!$getPositionNearbyQuery || mysqli_num_rows($getPositionNearbyQuery) == 0) {
						$feed_['exists']['nearby'] = false;
					}else {
						$feed_['content']['this'] = json_encode(array("port" => array("type" => "nearby", "require" => array("label" => "live", "label" => "auto"))));
						$positionNearbyQuery = mysqli_fetch_assoc($getPositionNearbyQuery);
						$feed_['content']['from'] = $positionNearbyQuery['name'];
						$feed_['query']['source'] = "(`places` = '1' AND `id` IN (SELECT `status.id` FROM `status_places` WHERE `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE ( 6371 * acos( cos( radians('{$positionNearbyQuery['latitude']}') ) * cos( radians( `latitude` ) ) * cos( radians( `longitude` ) - radians('{$positionNearbyQuery['longitude']}') ) + sin( radians('{$positionNearbyQuery['latitude']}') ) * sin( radians( `latitude` ) ) ) ) <= '{$_parameter->get('feeds_nearby_scanning_distance')}')))";
					}
				}else if (strtolower($_uri[2]) == "country" && isset($g_user['country']) && is_numeric($g_user['country']) && $g_user['country'] != null) {
					$getPositionNearbyQuery = mysqli_query($_db->port('beta'), "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '{$g_user['country']}' LIMIT 1");
					if (!$getPositionNearbyQuery || mysqli_num_rows($getPositionNearbyQuery) == 0) {
						$feed_['exists']['nearby'] = false;
					}else {
						$feed_['content']['this'] = json_encode(array("port" => array("type" => "nearby", "require" => array("label" => "country", "label" => "auto"))));
						$positionNearbyQuery = mysqli_fetch_assoc($getPositionNearbyQuery);
						$feed_['content']['from'] = $positionNearbyQuery['name'];
						$feed_['query']['source'] = "(`places` = '1' AND `id` IN (SELECT `status.id` FROM `status_places` WHERE `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE ( 6371 * acos( cos( radians('{$positionNearbyQuery['latitude']}') ) * cos( radians( `latitude` ) ) * cos( radians( `longitude` ) - radians('{$positionNearbyQuery['longitude']}') ) + sin( radians('{$positionNearbyQuery['latitude']}') ) * sin( radians( `latitude` ) ) ) ) <= '{$_parameter->get('feeds_nearby_scanning_distance')}')))";
					}
			...
			if (isset($_uri[2]) && $_uri[2] != null) {
				if (is_numeric($_uri[2]) && $_uri[2] != null) {
					...
					$getPositionCountryRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".addslashes($_uri[2])."' LIMI 1";
				}else if (is_string($_uri[2]) && $_uri[2] != null) {
					$feed_['content']['this'] = json_encode(array("port" => array("type" => "country", "require" => array("label" => "address", "value" => $_uri[2]))));
					$getPositionCountryRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".addslashes($_uri[2])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
				}else {
				...
				if (isset($getPositionCountryRequest) && is_string($getPositionCountryRequest) && $getPositionCountryRequest != null) {
					...
					if (!$getPositionCountryQuery || mysqli_num_rows($getPositionCountryQuery) == 0) {
						...
						$feed_['content']['from'] = $positionCountryQuery['name'];
						$feed_['query']['source'] = "(`places` = '1' AND `id` IN (SELECT `status.id` FROM `status_places` WHERE `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE ( 6371 * acos( cos( radians('{$positionCountryQuery['latitude']}') ) * cos( radians( `latitude` ) ) * cos( radians( `longitude` ) - radians('{$positionCountryQuery['longitude']}') ) + sin( radians('{$positionCountryQuery['latitude']}') ) * sin( radians( `latitude` ) ) ) ) <= '{$_parameter->get('feeds_country_scanning_distance')}')))";
					}
				}else {
					$feed_['exists']['country'] = false;
				}
			}else if (isset($g_user['mode']['type']) && $g_user['mode']['type'] == "user") {
				$feed_['content']['this'] = json_encode(array("port" => array("type" => "country", "require" => "auto")));
			...
			if (isset($_uri[2]) && $_uri[2] != null) {
				if (is_numeric($_uri[2]) && $_uri[2] != null) {
					...
					$getPositionLiveRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `id` = '".addslashes($_uri[2])."' LIMI 1";
				}else if (is_string($_uri[2]) && $_uri[2] != null) {
					$feed_['content']['this'] = json_encode(array("port" => array("type" => "live", "require" => array("label" => "address", "value" => $_uri[2]))));
					$getPositionLiveRequest = "SELECT `latitude`, `longitude`, `name` FROM `maps_places` WHERE `address` LIKE '%".addslashes($_uri[2])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1";
				}else {
				...
				if (isset($getPositionLiveRequest) && is_string($getPositionLiveRequest) && $getPositionLiveRequest != null) {
					...
					if (!$getPositionLiveQuery || mysqli_num_rows($getPositionLiveQuery) == 0) {
						...
						$feed_['content']['from'] = $positionLiveQuery['name'];
						$feed_['query']['source'] = "(`places` = '1' AND `id` IN (SELECT `status.id` FROM `status_places` WHERE `value` REGEXP '^[0-9]+$' AND `value` IN (SELECT `id` FROM `maps_places` WHERE ( 6371 * acos( cos( radians('{$positionLiveQuery['latitude']}') ) * cos( radians( `latitude` ) ) * cos( radians( `longitude` ) - radians('{$positionLiveQuery['longitude']}') ) + sin( radians('{$positionLiveQuery['latitude']}') ) * sin( radians( `latitude` ) ) ) ) <= '{$_parameter->get('feeds_live_scanning_distance')}')))";
					}
				}else {
					$feed_['exists']['live'] = false;
				}
			}else if (isset($g_user['mode']['type']) && $g_user['mode']['type'] == "user") {
				$feed_['content']['this'] = json_encode(array("port" => array("type" => "live", "require" => "auto")));
		...
		*/
		...
		if (isset($_uri[1], $g_user['login']) && is_string($_uri[1]) && $g_user['login'] == false && in_array($_uri[1], array("login", "register", "getpassword", "forgotpassword", "active"), true)) {
			if ($_uri[1] == "login") {
				...
				if (isset($_uri[2]) && is_string($_uri[2])) $keyGetPassword = addslashes($_uri[2]); else $keyGetPassword = null;
