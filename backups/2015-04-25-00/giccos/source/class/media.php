		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "add") {
				...
				if ($key != null && ($verify == true || $verify == false)) {
					...
					$_storage = $this->class['_storage'];
					$sql = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE `name` = '{$key}' OR `copy` = '{$key}' {$pushSql}";
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							...
							$photosUsertagQuery = mysqli_query($db, "SELECT `id`, `photos`, `faces`, `faces.image` FROM `photos_cache_usertag` WHERE `photos` = '{$fetch['secret']}'");
							while ($photosUsertagFetch = mysqli_fetch_assoc($photosUsertagQuery)) {
								...
								if (isset($delPhotosUsertag['return']) && $delPhotosUsertag['return'] == true) {
									mysqli_query($db, "DELETE FROM `photos_cache_usertag` WHERE `id` = '{$photosUsertagFetch['id']}'");
									mysqli_query($db, "DELETE FROM `analysis_faces` WHERE `display` = '{$photosUsertagFetch['faces']}'");
								}else {
									continue;
								}
							}
