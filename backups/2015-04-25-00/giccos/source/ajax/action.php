		...
		if ($type == "editor") {
			...
			if ($action == "getFaces") {
				...
				if (isset($getMediaCache['return']) && $getMediaCache['return'] == true && is_array($getMediaCache['data']) && count($getMediaCache['data']) > 0) {
					...
					$photosUsertag_query = mysqli_query($_db->port('beta'), "SELECT * FROM `photos_cache_usertag` WHERE `photos` = '{$fileCache['secret']}' AND `user.id` = '{$g_user['id']}'");
					...
					if ($photosUsertag_num > 0) {
						...
						if (isset($getAnalysis['return'], $getAnalysis['faces']) && $getAnalysis['return'] == true && count($getAnalysis['faces']) > 0) {
							$faceAnalysis = $getAnalysis['faces'];
							$facesData = array();
							foreach ($faceAnalysis as $key => $faceAnalysisThis) {
								if ($faceAnalysisThis['confidence'] < 0.5) {
							...
							foreach ($faceAnalysis as $key => $faceAnalysisThis) {
								...
								$addAnalysisFaces = $_analysis->faces("addObj", array("face" => $faceAnalysisThis, "thumbnail" => $thumbnail));
								if (isset($addAnalysisFaces['return'], $addAnalysisFaces['obj']) && $addAnalysisFaces['return'] == true && is_array($addAnalysisFaces['obj']) && count($addAnalysisFaces['obj']) > 0) {
									...
									$insertCacheTaguser = mysqli_query($_db->port('beta'), "INSERT INTO `photos_cache_usertag` (`id`, `user.id`, `photos`, `faces`, `faces.image`, `token`, `display`, `guy.type`, `guy.id`, `boundingbox.tl.x`, `boundingbox.tl.y`, `boundingbox.ratio.height`, `boundingbox.ratio.width`) VALUES (NULL, '{$g_user['id']}', '{$cacheTaguser['photos']}', '{$cacheTaguser['faces']}', '{$cacheTaguser['faces.image']}', '{$cacheTaguser['token']}', '{$cacheTaguser['display']}', null, null, '{$faceAnalysisThis['boundingbox.tl.x']}', '{$faceAnalysisThis['boundingbox.tl.y']}', '{$faceAnalysisThis['boundingbox.ratio.height']}', '{$faceAnalysisThis['boundingbox.ratio.width']}');");
