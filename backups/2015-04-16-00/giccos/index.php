...
if (isset($g_client['token']['source']['url']) && (in_array($_uri[0], $_uri_0, true) || $_uri[0] == $g_client['token']['source']['url'])) {
	...
	if (isset($_uri[0], $g_client['token']['source']['url']) && $_uri[0] == $g_client['token']['source']['url']) {
		if (isset($_uri[1], $g_client['token']['source']['js']) && $_uri[1] == $g_client['token']['source']['js']) {
			// Port: javascripts.
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")){
				...
				if (file_exists($scripts_file)) {
					$readFile = array("path" => $scripts_file, "mime" => $_parameter->get('contentType_js.utf8'));
		...
		}else if (isset($_uri[1], $g_client['token']['source']['css']) && $_uri[1] == $g_client['token']['source']['css']) {
			// Port: css.
			for ($i = 2; $i < count($_uri); $i++) {
				if ($i > 2) {
					$css_file .= "/{$_uri[$i]}";
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")){
				if (preg_match("/(\.css)$/", $css_file)) {
					...
					if (file_exists($css_file)) {
						$readFile = array("path" => $css_file, "mime" => $_parameter->get('contentType_css.utf8'));
						die(require_once ("source/ports/templates/readfile.php"));
					}else {
						die(header("HTTP/1.1 503 Service Unavailable"));
					}
					...
					if (file_exists($css_file)) {
						$readFile = array("path" => $css_file, "mime" => 'image/'.$ext_format);
						die(require_once ("source/ports/templates/readfile.php"));
					}else {
						die(header("HTTP/1.1 503 Service Unavailable"));
					}
		...
		}else if (isset($_uri[1], $g_client['token']['source']['ajax']) && $_uri[1] == $g_client['token']['source']['ajax']) {
			// Port: ajax.
			for ($i = 2; $i < count($_uri); $i++) {
				if ($i > 2) {
					$ajax_file .= "/{$_uri[$i]}";
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")) {
				...
				if (file_exists($ajax_file)) {
					$readFile = array("path" => $ajax_file, "mime" => $_parameter->get('contentType_json.utf8'));
					die(require_once ("source/ports/templates/readfile.php"));
				}else {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}
		...
		}else if (isset($_uri[1], $g_client['token']['source']['xml']) && $_uri[1] == $g_client['token']['source']['xml']) {
			// Port: xml.
			for ($i = 2; $i < count($_uri); $i++) {
				if ($i > 2) {
					$xml_file .= "/{$_uri[$i]}";
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")){
				...
				if (file_exists($xml_file)) {
					$readFile = array("path" => $xml_file, "mime" => $_parameter->get('contentType_xml.utf8'));
					die(require_once ("source/ports/templates/readfile.php"));
				}else {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}
		...
		}else if (isset($_uri[1], $g_client['token']['source']['font']) && $_uri[1] == $g_client['token']['source']['font']) {
			// Port: font.
			for ($i = 2; $i < count($_uri); $i++) {
				if ($i > 2) {
					$font_file .= "/{$_uri[$i]}";
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")){
				...
				if (file_exists($font_file)) {
					...
					$readFile = array("path" => $font_file, "mime" => $file_mine.'; charset: utf-8');
					die(require_once ("source/ports/templates/readfile.php"));
				}else {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}
	...
	}else if ((isset($_uri[0]) && $_uri[0] == "feed") && (isset($g_user['login']) && $g_user['login'] == true)) {
		// Port: feed.
		if ((isset($_SERVER['HTTP_REFERER']) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) == false) || $_SERVER['HTTP_HOST'] != $_tool->links("::host")) {
			die(header("HTTP/1.1 503 Service Unavailable"));
		}
		if (isset($g_user['mode']) && (!isset($_uri[1]) || (isset($_uri[1]) && in_array($_uri[1], ["", "general"])))) {
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "time") {
			if (isset($_uri[2], $g_user['mode']) && $_uri[2] != null && is_string($_uri[2])) {
				$feed_['value'] = true;
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "family") {
			$feed_['content']['title'] = "";
			$feed_['content']['this'] = json_encode(array("port" => array("type" => "feed", "require" => "none")));
			$feed_['content']['from'] = $_language->text('family', 'strtolower', false);
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "friends") {
			$feed_['content']['title'] = "";
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "country") {
			$feed_['content']['title'] = "";
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "live") {
			$feed_['content']['title'] = "";
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "pages") {
			$feed_['content']['title'] = "";
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "groups") {
			$feed_['content']['title'] = "";
		...
		}else if (isset($_uri[1], $g_user['mode']) && in_array($_uri[1], ["tag", "tags", "hashtag", "hashtags"])) {
			if (isset($_uri[2], $g_user['mode']) && $_uri[2] != null && is_string($_uri[2])) {
				$feed_['value'] = true;
				$feed_['content']['uri'] = strtolower($_uri[2]);
				$feed_['content']['uri'] = array_filter(explode("&", $_uri[2]));
		...
		}else if (isset($_uri[1], $g_user['mode']) && $_uri[1] == "status") {
			if (isset($_uri[2], $g_user['mode']) && $_uri[2] != null && is_string($_uri[2])) {
				$feed_['value'] = true;
				$feed_['content']['uri'] = strtolower($_uri[2]);
				$feed_['content']['uri'] = array_filter(explode("&", $_uri[2]));
	...
	}else if ((isset($_uri[0]) && $_uri[0] == "groups") && (isset($g_user['login']) && $g_user['login'] == true)) {
		// Port: groups.
	}else if ((isset($_uri[0]) && $_uri[0] == "wall") && (isset($g_user['login']) && $g_user['login'] == true)) {
		// Port: wall.
	...
	}else if ((isset($_uri[0]) && $_uri[0] == "messages") && (isset($g_user['login']) && $g_user['login'] == true)) {
		// Port: messages.
		$messages_ = array();
	...
	}else if ((isset($_uri[0]) && $_uri[0] == "accounts")) {
		// Port: accounts.
		if (isset($_uri[1], $g_user['login']) && $g_user['login'] == false && in_array($_uri[1], array('login', 'register'), true)) {
			if ($_uri[1] == "login") {
	...
	}else if (isset($_uri[0]) && ($_uri[0] == "photos" || $_uri[0] == "fotos")) {
		// Port: photos.
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			...
			}else if (is_string($_uri[2]) && preg_match("/^(other)/i", $_uri[2])) {
				for ($i = 3; $i < count($_uri); $i++) {
					if ($i > 3) {
						$photos_file .= "/{$_uri[$i]}";
					}else {
			...
			$fileQuery = mysqli_query($_db->port('beta'),"SELECT `path` FROM `photos_cache` WHERE `name` = '{$fileName}' AND `author.type` = '{$g_user['mode']['type']}' AND `author.id` = '{$g_user['mode']['id']}' LIMIT 1");
			if (mysqli_num_rows($fileQuery) > 0) {
				$file = mysqli_fetch_assoc($fileQuery);
				$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $file['path'], "rows" => "`token`, `path`, `mime`", "limit" => "LIMIT 1"));
				if (isset($getMediaFile['return'], $getMediaFile['num'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['num'] > 0) {
					$file_mime =  $getMediaFile['file'][0]['mime'];
					$file_url = $_tool->hash("decode", $getMediaFile['file'][0]['path'], $getMediaFile['file'][0]['token']);
					if (!$_tool->siteDie($file_url)) {
	...
	}else if (isset($_uri[0]) && ($_uri[0] == "music" || $_uri[0] == "musik")) {
		// Port: music.
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			if (is_string($_uri[2]) && preg_match("/^(static)/i", $_uri[2])) {
				//.
	...
	}else if (isset($_uri[0]) && ($_uri[0] == "videos" || $_uri[0] == "vio")) {
		// Port: videos.
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			if (is_string($_uri[2]) && preg_match("/^(static)/i", $_uri[2])) {
				for ($i = 3; $i < count($_uri); $i++) {
				...
				$videos_url = $_parameter->get('path_video_static_default').'/'.$videos_file;
				if (!$_tool->siteDie($videos_url) && preg_match("/(\.swf)$/", $videos_url)) {
					$readFile = array("path" => $videos_url, "mime" => $_parameter->get('contentType_swf.utf8'), "expire" => $_parameter->get('http_cache_expire_static'));
					die(require_once ("source/ports/templates/readfile.php"));
				}else {
					die(require_once("source/ports/oops.php"));
				}
			...
			if (isset($_uri[2]) && is_string($_uri[2])) $token = $_uri[2]; else $token = null;
			if ($token == null) {
				die(require_once("source/ports/oops.php"));
			}
			$videos_query = mysqli_query($_db->port('beta'),"SELECT `id`, `token`, `folder`, `mime_type`, `thumbnail`, `path.hd`, `path.sd` FROM `videos_info` WHERE `token` = '{$token}'");
			if (mysqli_num_rows($videos_query) > 0) {
				$videos = mysqli_fetch_assoc($videos_query);
				$videos_folder = $videos['folder'];
				$videos_mime = $videos['mime_type'];
				$videos_thumbnail = $_tool->links($_tool->hash('decode', $videos['thumbnail'], $videos['token']));
				if ($videos['path.hd']) {
					$videos_url['hd'] = $_tool->links($_tool->hash('decode', $videos['path.hd'], $videos['token']));
				}else {
					$videos_url['hd'] = null;
				}
				$videos_url['sd'] = $_tool->links($_tool->hash('decode', $videos['path.sd'], $videos['token']));
				if (!$_tool->siteDie($videos_url['sd'])) {
	...
	}else if ((isset($_uri[0]) && $_uri[0] == "maps")) {
		// Port: maps.
		if (isset($_uri[1]) && $_uri[1] == "raw") {
			if (isset($_uri[2]) && $_uri[2] == "places") {
				for ($i = 3; $i < count($_uri); $i++) {
					if ($i > 3) {
						$places_image .= "/{$_uri[$i]}";
					}else {
		...
		}else if (isset($_uri[1]) && $_uri[1] == "places") {
			die(print_r("updating.."));
		}
	}else if ((isset($_uri[0]) && $_uri[0] == "oops")) {
		// Port: oops.
	}else if ((isset($_uri[0]) && $_uri[0] == "sites")) {
		// Port: sites.
		if (isset($_uri[1]) && $_uri[1] == "direct") {
			for ($i = 2; $i < count($_uri); $i++) {
				if ($i > 2) {
					$sites_url .= "/{$_uri[$i]}";
				}else {
