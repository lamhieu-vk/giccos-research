	...
	function action (selector, output, type, multiple) {
		...
		function select (type, input, multiple, output) {
		    ...
		    if (type == 'places') {
		        ...
		        var tag_code = '<span class="tag" tag="places" tag tag-address="'+places+'" tag-code="'+code+'"><a tag-name>'+places+'</a><i class="fa fa-times" tag-close></i></span>';
		        ...
		        var tag_code = '<span class="tag" tag="info" tag-type="'+type+'" tag-id="'+id+'"><a tag-name>'+name+'</a><i class="fa fa-times" tag-close></i></span>';
		        ...
		        var tag_code = '<span class="tag" tag="mood" tag-type="'+type+'" tag-code="'+code+'" tag-text="'+text+'"><a tag-name>'+text+'</a><i class="fa fa-times" tag-close></i></span>';
		        var tag = parent.find("span.tag[tag][tag-type='"+type+"'][tag-id='"+id+"']");
		        if(multiple == true && tag.length == 0){
				    input.before(tag_code);
				    boxSuggest.remove();
		        ...
		        var tag_code = '<span class="tag" tag="media" tag-type="'+type+'" tag-token="'+token+'" tag-thumbnail="'+thumbnail+'" tag-name="'+name+'"><a tag-name>'+name+'</a><i class="fa fa-times" tag-close></i></span>';
		        var tag = parent.find("span.tag[tag][tag-type='"+type+"'][tag-id='"+id+"']");
		        if(multiple == true && tag.length == 0){
				    input.before(tag_code);
				    boxSuggest.remove();
...
function ExplorerPopup (object) {
	...
	if (tab.length == 0) {
		var code = ' <div id="ExplorerPopup"> <div class="wrapFilter"></div> <div class="boxContent"> <div class="header"> <span>'+titleText+'</span> <i close> </i> </div>  <div class="body"> <div class="nav"> <div class="list"> <div class="rows active" type="cache"> <span>Cache</span> </div> <div class="rows" type="sync"> <span>Sync</span> </div> </div> <div class="arrow"></div> </div> <div class="data"> <div class="main"> </div> </div> </div> <div class="footer"> <div class="copyright"> <span> <?php print $_parameter->get('PhotosEditorPopup_footer_text'); ?> </span> </div> </div> </div> </div> ';
		$("body").append(code);
		$("body").css('overflow-y', 'hidden');
		var tab = $("#ExplorerPopup");
		tab.hide().fadeIn();
