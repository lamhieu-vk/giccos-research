		...
		if (!isset($htmlPage) || !is_string($htmlPage) || $htmlPage == null) {
			...
			if ($allowExtract == true) {
				...
				if (isset($isChanged) && !$isChanged) {
					...
					if (isset($siteInfo['title']) && is_string($siteInfo['title'])) {
						$siteInfo['title'] = $_tool->convertDatabaseString($siteInfo['title']);
					}else {
						$siteInfo['title'] = null;
					}
					if (isset($siteInfo['thumbnail']) && is_string($siteInfo['thumbnail'])) {
					...
					$siteInfo['secure'] = 0;
					if (isset($isChanged) && $isChanged) {
						mysqli_query($db, "UPDATE `sites_info` SET `fingerprints` = '{$siteInfo['fingerprints']}', `thumbnail` = '{$siteInfo['thumbnail']}', `title` = '{$siteInfo['title']}', `description` = '{$siteInfo['description']}', `updated` = '{$time}' WHERE `id` = '{$siteInfoSaved['id']}'");
						$infoId = $siteInfoSaved['id'];
					}else {
						$insertRequest = "
						INSERT INTO 
						`sites_info` (`id`, `time`, `token`, `display`, `scheme`, `sub`, `host`, `name`, `dot`, `path`, `mime`, `fingerprints`, `title`, `thumbnail`, `description`, `url`, `secure`, `updated`) 
						VALUES 
						(NULL, '{$time}', '{$siteInfo['token']}', '{$siteInfo['display']}', '{$siteInfo['scheme']}', '{$siteInfo['sub']}', '{$siteInfo['host']}', '{$siteInfo['name']}', '{$siteInfo['dot']}', '{$siteInfo['path']}', '{$siteInfo['mime']}', '{$siteInfo['fingerprints']}', '{$siteInfo['title']}', '{$siteInfo['thumbnail']}', '{$siteInfo['description']}', '{$siteInfo['url']}', '{$siteInfo['secure']}', '{$time}');
						";
