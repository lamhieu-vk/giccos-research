...
Class tool {
	...
	function isCachedPage () {
		if (isset($_SESSION["cache"]['page'])) {
			...
			$urlNow = $this->class['_client']->fullurl();
			if (is_array($pageCached) && isset($pageCached[$urlNow]) && $pageCached[$urlNow] == true) {
	...
	function parseLink ($text = null) {
		$text = preg_replace($this->class['_parameter']->get('regex_sites_giccos_host'), "<a class='gLink' target='_blank' href='$1'>$1</a>", $text);
		$string = $text;
		preg_match_all("/((http|https|ftp|ftps):\/\/[\S*]+[\S*])/", $string, $matches);
		foreach ($matches[0] as $key => $links) {
			if (preg_match($this->class['_parameter']->get('regex_sites_giccos_host'), $links)) {
				continue;
			}
			$oldLinks = $links;
		...
		$text = preg_replace("/(^|\s)@([a-zA-Z0-9\_]+)/", '<a class="gUsertag" usertag="$2" href="'.$this->links('wall/$2').'" target="_blank">@$2</a>', $text);
	    $text = preg_replace("/(^|\s)#([a-zA-Z0-9\_]+)/", '<a class="gHashtag" hashtag="$2" href="'.$this->links('feed/tags/$2').'">#$2</a>', $text);
	    return $text;
	}
