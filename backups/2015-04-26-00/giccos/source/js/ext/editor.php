...
function PhotosTagPopup (object) {
	console.log(object);
	...
	if (object['data'] != undefined && object['data'] != null) {
		if (object['data']['faces'] != undefined && typeof object['data']['faces'] == "object" && Object.keys(object['data']['faces']).length > 0) {
			var tagbarActive = true;
		}else {
			var tagbarActive = false;
		}
		if (object['data']['image'] != undefined && object['data']['image'] != null) {
	...
	if (tagbarActive != undefined && tagbarActive == true) {
	...
	var popupCode = '<div id="PhotosTagPopup" secret="" randkey="'+randkey+'"> <div class="wrapFilter"> </div> <div class="boxContent"> <div class="header"> <span><?php print $_language->text('photos_tag', 'ucfirst'); ?></span> <i close> </i> </div> <div class="body"> <div class="preview" preview>'+imgTag+'</div> '+tagbar+' </div> <div class="footer"> <div class="copyright"> <span> <?php print $_language->text('PhotosTagPopup_footer_text'); ?> </span> </div> </div> </div> </div>';
	...
	tagsBox.children(".wrapper").css({'height': imageBox.find(".img").height(), 'width': imageBox.find(".img").width()});
	if (tagbarActive == true && tagbar != '') {
		...
		for (var i in object['data']['faces']) {
			var thisFace = object['data']['faces'][i];
			var tagClass = "tag_"+thisFace['photos']+"_"+thisFace['faces'];
			var tagHeight = imgHeight / thisFace['ratio']['height'];
			var tagWidth = imgWidth / thisFace['ratio']['width'];
			var positionX = thisFace['position']['x'];
			var positionY = thisFace['position']['y'];
			var codeTags = " <div class='thisTag "+tagClass+"' tag-name='"+tagClass+"' tag-height='"+tagHeight+"' tag-width='"+tagWidth+"' ratio-height='"+thisFace['ratio']['height']+"' ratio-width='"+thisFace['ratio']['width']+"' position-x='"+positionX+"' position-y='"+positionY+"' info='"+JSON.stringify(thisFace)+"'></div> ";
			wrapperTag.append(codeTags);
	...
	function pushTypeName () {
		...
		var actionWhenClick = function() {
			...
			if (thisTag.attr('handling-typing') != "true" && typeNameThis.length == 0) {
				...
				thisNameType.children("input").AutoComplete({'path': 'info', 'from': 'friends', 'output': true, 'multiple': false, callback: callbackFunc});
        ...
        imgPreview.find("[tag]").bind('click', function() {
            ...
            var file = JSON.parse(img.parents("[photos-preview]").attr("file"));
           	if (img.attr('handling-tag') != "true" && file['secret'] !== undefined && file['name'] !== undefined) {
           		...
           		popupNotification ({type: 1, title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('data_processing','ucfirst'); ?>, <?php print $_language->text('please_wait','strtolower'); ?>...', timeout: '<?php print $_parameter->get('notification_text_action'); ?>'});
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data['return'] !== undefined && data['return'] == true) {
                            ...
                            if (onPopup !== undefined && onPopup == true) {
	                            var PhotosTagPopupOptions = {
								...
								PhotosTagPopup(PhotosTagPopupOptions);
                            }
                        }else {
                        	//.
                        }
