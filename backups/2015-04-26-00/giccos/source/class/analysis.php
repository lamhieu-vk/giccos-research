...
Class analysis {
		...
		if ($action == "check") {
			...
			if (is_array($thumbnail) && count($thumbnail) > 0) {
				...
				if (isset($fileUploadThumbnail['return'], $fileUploadThumbnail['storage']) && $fileUploadThumbnail['return'] == true && is_array($fileUploadThumbnail['storage'])) {
					...
					$fileUploadedEditor->resize(200, 0);
			...
			if (isset($curlRequest['return'], $curlRequest['data']) && $curlRequest['return'] == true) {
				...
				if (isset($dataAnalysis['face_detection']) && is_array($dataAnalysis['face_detection']) && count($dataAnalysis['face_detection']) > 0) {
					...
					if (!$insertQuery) {
						...
						$changeFaceAnalysis = $this->faces("rename", array("label" => "display", "value" => $obj['display'], "tag" => $_parameter->get('rekognition_tagdefault'), "newtag" => $obj['tag']));
			...
			if (isset($object['value']) && is_string($object['value'])) $value = $object['value']; else $value = null;
			if (isset($object['tag']) && is_string($object['tag'])) $tag = $object['tag']; else $tag = null;
			if (isset($object['newtag']) && is_string($object['newtag'])) $newtag = $object['newtag']; else $newtag = null;
			if ($newtag == null) {
				return array("return" => false, "reason" => "");
			}
			if ($tag == null || $tag == $_parameter->get('rekognition_tagdefault')) {
				$tag = $_parameter->get('rekognition_tagdefault');
				$querySql = "SELECT `id`, `tag`, `img_index` FROM `analysis_faces` WHERE `{$label}` = '{$value}' LIMIT 1";
			}else {
				$querySql = "SELECT `id`, `tag`, `img_index` FROM `analysis_faces` WHERE `{$label}` = '{$value}' AND `tag` = '{$tag}' LIMIT 1";
			...
			if ($analysisQuery && mysqli_num_rows($analysisQuery) > 0) {
				...
				$curlRequest = $_tool->curl($_parameter->get('rekognition_url'), 5, array("cookie" => false, "method" => "POST", "form" => $optionsRequest));
				if (isset($curlRequest['return'], $curlRequest['data']) && $curlRequest['return'] == true) {
					$changeQuery = mysqli_query($db, "UPDATE `analysis_faces` SET `updated` = '{$time}', `tag` = '{$newtag}' WHERE `id` = '{$analysisFetch['id']}'");
					if (!$changeQuery) {
						return array("return" => false, "reason" => "");
					}else {
						return array("return" => true);
	...
	/*$obj = $face;
			...
			$insertSql = "
			INSERT INTO `analysis_faces` 
			(`id`, `time`, `updated`, `token`, `display`, `uploader.type`, `uploader.id`, `taged`, `thumbnail`, `img_index`, `confidence`, `eye.left.x`, `eye.left.y`, `eye.right.x`, `eye.right.y`, `nose.x`, `nose.y`, `mouth.tl.x`, `mouth.tl.y`, `mouth.bl.x`, `mouth.bl.y`, `mouth.tr.x`, `mouth.tr.y`, `mouth.br.x`, `mouth.br.y`, `pose.roll`, `pose.yaw`, `pose.pitch`, `race.text`, `race.value`, `quality.brn`, `quality.shn`, `emotion.oe.text`, `emotion.oe.value`, `emotion.to.text`, `emotion.to.value`, `emotion.te.text`, `emotion.te.value`, `age`, `smile`, `glasses`, `sunglasses`, `eye_closed`, `mouth_open_wide`, `beauty`, `sex`) 
			VALUES 
			(NULL, '{$obj['time']}', '{$obj['updated']}', '{$obj['token']}', '{$obj['display']}', '{$obj['uploader.type']}', '{$obj['uploader.id']}', '{$obj['taged']}', '{$obj['thumbnail']}', '{$obj['img_index']}', '{$obj['confidence']}', '{$obj['eye.left.x']}', '{$obj['eye.left.y']}', '{$obj['eye.right.x']}', '{$obj['eye.right.y']}', '{$obj['nose.x']}', '{$obj['nose.y']}', '{$obj['mouth.tl.x']}', '{$obj['mouth.tl.y']}', '{$obj['mouth.bl.x']}', '{$obj['mouth.bl.y']}', '{$obj['mouth.tr.x']}', '{$obj['mouth.tr.y']}', '{$obj['mouth.br.x']}', '{$obj['mouth.br.y']}', '{$obj['pose.roll']}', '{$obj['pose.yaw']}', '{$obj['pose.pitch']}', '{$obj['race.text']}', '{$obj['race.value']}', '{$obj['quality.brn']}', '{$obj['quality.shn']}', '{$obj['emotion.oe.text']}', '{$obj['emotion.oe.value']}', '{$obj['emotion.to.text']}', '{$obj['emotion.to.value']}', '{$obj['emotion.te.text']}', '{$obj['emotion.te.value']}', '{$obj['age']}', '{$obj['smile']}', '{$obj['glasses']}', '{$obj['sunglasses']}', '{$obj['eye_closed']}', '{$obj['mouth_open_wide']}', '{$obj['beauty']}', '{$obj['sex']}');
			";*/
