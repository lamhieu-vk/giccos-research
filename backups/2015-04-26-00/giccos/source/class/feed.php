...
Class feed {
	...
	function comment_get ($object = array("return" => "json", "query" => null, "class" => "gFeeds-status-comment")) {
		...
		if (mysqli_num_rows($comment_query) == 0) {
			...
			while ($comment = mysqli_fetch_assoc($comment_query)) {
				...
				$comment_reply = array();
				$comment_reply_query = mysqli_query($this->class['_db']->port('beta'), "SELECT `id` FROM `status_comment` WHERE `comment.id` = '{$comment['id']}'");
	...
	function comment_add ($object) {
		if (isset($object['status_id']) && (is_string($object['status_id']) || is_numeric($object['status_id']))) $status_id = $object['status_id']; else $status_id = 0;
		if (isset($object['comment_id']) && (is_string($object['comment_id']) || is_numeric($object['comment_id']))) $comment_id = $object['comment_id']; else $comment_id = 0;
		if (isset($object['value']) && is_string($object['value'])) $value = $this->class['_tool']->convertDatabaseString($object['value']); else $value = null;
		if (($status_id > 0 || $comment_id > 0) && $value != null) {
			$this->user = $_SESSION["user"];
			$user = $this->user;
			$db = $this->class['_db']->port('beta');
			if (isset($user['login'], $user['mode']['id'], $user['mode']['type']) && $user['login'] == true) {
				if ($status_id > 0) {
					$status_query = mysqli_query($db, "SELECT `id`, `user.id`, `author.id`, `author.type`, `private.comment` FROM `status` WHERE `id` = '{$status_id}'");
					if ($status_query == true && mysqli_num_rows($status_query) > 0) {
						$status = mysqli_fetch_assoc($status_query);
						$relationship_num = 0;
						$status_private = $status['private.comment'];
						if ($status['author.type'] == 'user' && $user['mode']['type'] == 'user') {
					...
					$comment_query = mysqli_query($db, "SELECT `id` FROM `status_comment` WHERE `id` = '{$comment_id}'");
					if ($comment_query == true && mysqli_num_rows($comment_query) > 0) {
