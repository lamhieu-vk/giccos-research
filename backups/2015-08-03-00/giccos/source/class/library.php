				...
				if (isset($requestAction['return'], $requestAction['data']) && $requestAction['return'] == true) {
					...
					if (is_array($dataArrOriginal) && count($dataArrOriginal) > 0) {
						foreach ($dataArrOriginal as $thisKey => $thisValue) {
							$dataArr[$thisKey]['word'] = $thisValue['word'];
							$dataArr[$thisKey]['source'] = $thisValue['sourceDictionary'];
							$dataArr[$thisKey]['example'] = $thisValue['exampleUses'];
							$dataArr[$thisKey]['related'] = $thisValue['relatedWords'];
							$dataArr[$thisKey]['type'] = $thisValue['partOfSpeech'];
							$dataArr[$thisKey]['attribution'] = $thisValue['attributionText'];
							$dataArr[$thisKey]['content'] = $thisValue['text'];
							$dataArr[$thisKey]['cited'] = $thisValue['citations'];
							//.
							...
							if (isset($dataArr[$thisKey]['related'][0], $dataArr[$thisKey]['related'][0]['words'])) {
								...
								/*
								$relatedText = null;
								foreach ($dataArr[$thisKey]['related'] as $relatedValue) {
									if ($relatedText == null) {
										$relatedText = $relatedValue;
									}else {
										$relatedText .= " ".$relatedValue;
									}
								}
								$dataArr[$thisKey]['related'] = $relatedText;
								*/
							}
		...
		if ($action == "extract") {
					...
					if ($extractAllow == true) {
						...
						if (isset($extractLibrary['return'], $extractLibrary['data']) && $extractLibrary['return'] == true) {
							...
							foreach ($extractLibrary['data'] as $thisData) {
								...
								if ($addAllow == true) {
									$addWord = $this->words(array("action" => "add", "language" => $language, "word" => $word, "type" => $thisData['type'], "content" => $thisData['content'], "example" => $thisData['example'], "related" => $thisData['related'], "attribution" => $thisData['attribution'], "source" => $thisData['source'], "host" => "wordnik", "primary" => false));
			...
			if (!$getWordQuery) {
				...
				while ($wordFetch = mysqli_fetch_assoc($getWordQuery)) {
					if (isset($wordFetch['related']) && is_string($wordFetch['related'])) {
