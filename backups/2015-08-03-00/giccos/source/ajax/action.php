...
if (isset($g_client['token']['ajax'], $_SERVER['HTTP_TOKEN'], $_SERVER['HTTP_X_REQUESTED_WITH'], $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_SERVER['HTTP_TOKEN'] == $g_client['token']['ajax'] && $_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest" && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")) {
	...
	if ($port == "accounts" && $token == $g_client['token']['action']['accounts']) {
		...
		if ($type == "reload") {
			...
			if (isset($ObjRequest['id']) && is_string($ObjRequest['id'])) $id = $ObjRequest['id']; else $id = null;
			if ($id == null) {
				die(print json_encode(array("return" => false, "reason" => "")));
			}
			if ($g_user['id'] == $id) {
				...
				if ($from == "user" || $from == "friends" || $from == "all") {
					...
					if ($searchQuery == true && mysqli_num_rows($searchQuery) > 0) {
						while ($searchFetch = mysqli_fetch_assoc($searchQuery)) {
							$getUserAction = $_user->profile(array("action" => "get", "rows" => ["id", "username", "fullname", "avatar.small", "cover.small"], "id" => $searchFetch['id']));
			...
			if ($action == "get") {
				...
				$rowsStr = $rowsKey = $rowsValue = null;
				if (count($rowsArr) > 0) {
					foreach ($rowsArr as $rowsKey => $rowsValue) {
						if (!isset($rowsStr) || $rowsStr == null) {
							$rowsStr .= "`{$rowsValue}`";
						}else {
							$rowsStr .= ", `{$rowsValue}`";
						}
					...
					$getInfo = $_user->getInfo($getInfoOptions);
				...
				if (isset($getContact) && $getContact == true) {
					$getContact = $_user->getContact(array("id" => $userId));
				...
				if (isset($getEducation) && $getEducation == true) {
					$getEducation = $_user->getEducation(array("id" => $userId));
				...
				if (isset($getWorkplaces) && $getWorkplaces == true) {
					$getWorkplaces = $_user->getWorkplaces(array("id" => $userId));
				...
				if (isset($getLive) && $getLive == true) {
					$getLive = $_user->getInfo(array("rows" => "`live`", "label" => "id", "value" => $userId));
				...
				if (isset($getCountry) && $getCountry == true) {
					$getCountry = $_user->getInfo(array("rows" => "`country`", "label" => "id", "value" => $userId));
		...
		if ($type == "status") {
			if ($action == "add" || $action == "upload") {
				...
				$statusRequest = $_feed->status_add(array_merge(array("author" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusAddOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$followRequest = $_feed->status_follow(array_merge(array("guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusfollowOptions));
				...
				foreach ($idArr as $key => $idArrThis) {
					...
					$unfollowRequest = $_feed->status_unfollow(array_merge(array("guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id'])), $statusunfollowOptions));
						...
						if (isset($getWallFriends['return'], $getWallFriends['data'], $getWallFriends['count']) && $getWallFriends['return'] == true) {
							if (isset($getProfile) && $getProfile == true) {
								foreach ($getWallFriends['data'] as $key => $thisUserId) {
									$getUserInfo = $_user->profile(array("rows" => "`id`, `username`, `avatar.small`, `fullname`, `link`", "label" => "id", "value" => $thisUserId['user.id'], "limit" => "LIMIT 1"));
