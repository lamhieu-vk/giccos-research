	...
	function status_list ($object) {
		...
		if (isset($except) && is_array($except) && count($except) > 0) {
			foreach ($except as $exceptThis) {
				$except[$key] = intval($class['_tool']->convertDatabaseString($exceptThis));
			}
		...
		if ($type == "general") {
			if ($method == "auto") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND ((`logs_actions`.`guy.type`, `logs_actions`.`guy.id`) IN (SELECT `guy.type`, `guy.id` FROM `users_follow` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND ((`logs_actions`.`guy.type`, `logs_actions`.`guy.id`) IN (SELECT `guy.type`, `guy.id` FROM `pages_follow` WHERE `page.id` = '".$guyId."')) ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "new") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`status`.`author.type`, `status`.`author.id`) IN (SELECT `guy.type`, `guy.id` FROM `users_follow` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`status`.`author.type`, `status`.`author.id`) IN (SELECT `guy.type`, `guy.id` FROM `pages_follow` WHERE `page.id` = '".$guyId."')) ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}
			...
			if ($timeValueOne != null && $timeValueTwo != null) {
				$listQuery['time'] = "AND (`status`.`time` BETWEEN '".$timeValueOne."' AND '".$timeValueTwo."')";
			}else {
				$listQuery['time'] = "AND (`status`.`time` >= '".$timeValueOne."')";
			}
			if ($method == "auto") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['time']." ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['time']." ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "new") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." ".$listQuery['time']." ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." ".$listQuery['time']." ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "action") {
			...
			if ($method == "auto") {
				if ($guyType == "user" && $type == "family") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND (`logs_actions`.`guy.type` = 'user' AND `logs_actions`.`guy.id` IN (SELECT `guy.id` FROM `users_family` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "user" && $type == "friends") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND (`logs_actions`.`guy.type` = 'user' AND `logs_actions`.`guy.id` IN (SELECT `guy.id` FROM `friends` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page" && $type == "related") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND (`logs_actions`.`guy.type` = 'page' AND `logs_actions`.`guy.id` IN (SELECT `guy.id` FROM `pages_related` WHERE `page.id` = '".$guyId."')) ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "new") {
				if ($guyType == "user" && $type == "family") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND (`status`.`author.type` = 'user' AND `status`.`author.id` IN (SELECT `guy.id` FROM `users_family` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "user" && $type == "friends") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND (`status`.`author.type` = 'user' AND `status`.`author.id` IN (SELECT `guy.id` FROM `friends` WHERE `user.id` = '".$guyId."')) ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page" && $type == "related") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND (`status`.`author.type` = 'page' AND `status`.`author.id` IN (SELECT `guy.id` FROM `pages_related` WHERE `page.id` = '".$guyId."')) ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "action") {
				if ($guyType == "user" && $type == "family") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) AND (`logs_actions`.`guy.type` = 'user' AND `logs_actions`.`guy.id` IN (SELECT `guy.id` FROM `users_family` WHERE `user.id` = '".$guyId."')) ORDER BY `logs_actions`.`id` ".$listQuery['order']." ".$listQuery['limit'];
			...
			foreach ($placesArr as $placesThis) {
				if (is_numeric($placesThis)) {
					...
					$placesNearbyRequest = "SELECT `latitude`, `longitude` FROM `maps_places` WHERE `address` LIKE '%".$_tool->convertDatabaseString($_uri[2])."%' ORDER BY CHAR_LENGTH(`address`) ASC LIMIT 1;";
					$placesNearbyQuery = mysqli_query($db, $placesNearbyRequest);
					if (!$placesNearbyQuery) {
						continue;
					}else {
			...
			if (!isset($listQuery['nearby']) || !is_string($listQuery['nearby'])) {
				...
				$listQuery['nearby'] = "AND (".$listQuery['nearby'].")";
			}
			if ($method == "auto") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['nearby']." ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['nearby']." ORDER BY `status`.`time`, `logs_actions`.`time` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "new") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." ".$listQuery['nearby']." ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT `status`.`id` FROM `status` WHERE `status`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." ".$listQuery['nearby']." ORDER BY `status`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}else if ($method == "action") {
				if ($guyType == "user") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['nearby']." ORDER BY `logs_actions`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}else if ($guyType == "page") {
					$listQuery['code'] = "SELECT DISTINCT `status`.`id` FROM `status` INNER JOIN `logs_actions` WHERE `logs_actions`.`id` ".$listQuery['sort'] ." '".$listQuery['id']."' ".$listQuery['except']." ".$listQuery['private']." ".$listQuery['hide']." ".$listQuery['block']." AND ((`logs_actions`.`things.id` = `status`.`id` AND `logs_actions`.`things.type` = 'status' AND `logs_actions`.`type` IN ('favorite', 'follow', 'share')) OR (`logs_actions`.`obj.id` = `status`.`id` AND `log_actions`.`obj.type` = 'status' AND `logs_actions`.`type` IN ('add')')) ".$listQuery['nearby']." ORDER BY `logs_actions`.`id` ".$listQuery['order']." ".$listQuery['limit'];
				}
			}
		...
		if (isset($listQuery['source']) && is_string($listQuery['source'])) {
			$getListQuery = mysqli_query($db, $listQuery['source']);
			if (!$getListQuery) {
				return array("return" => false, "reason" => "");
			}else {
				$listArr = array();
				...
				if (isset($mediaObjThis['exists']) && $mediaObjThis['exists'] == true) {
					...
					$c_['status_content_media'] .= "
						...
						<div class='share' media='share'>
							<div class='author'><span>".$_language->text('from', 'strtolower')." <a href='".$statusInfo['link']."' target='_blank'>".$_language->text('status', 'strtolower')."</a> ".$_language->text('posted_at', 'strtolower')." <a href='".$_tool->links('feed/time/'.$statusDate['stamp'])."' target='_blank'>".$_tool->agoDatetime($statusDate['stamp'], 'ago')."</a> ".$_language->text('by', 'strtolower')." <a href='".$statusAuthor['link']."' target='_blank'>".$statusAuthor['name']." (".$statusAuthor['tag'].")</a></span></div>
							<div class='text'>
