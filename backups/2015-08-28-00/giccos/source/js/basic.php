...
var resizeTemplates = function () {
	var setSizeBox = function () {
		...
		sidebar.css({
			...
			height: ($(window).innerHeight() - navBar.innerHeight()) / 2
		});
...
dataCache.exists = function (name) {
	var objCache = {};
	if (isset(window) && isset(window['dataCaching'])) {
		objCache = window['dataCaching'];
	}
	added = 0;
	cacheLoop: for (var i in objCache) {
		thisCache = objCache[i];
		if (isset(thisCache) && isset(thisCache['name']) && thisCache['name'] == name) {
			added++;
			break cacheLoop;
		}
	}
...
dataCache.new = function (name) {
	...
	if (added === 0) {
		objCache[countArr(objCache)] = {'name': name, 'data': null};
	}
	return true;
};
dataCache.add = function (name, data) {
	var objCache = {};
	if (isset(window) && isset(window['dataCaching'])) {
		objCache = window['dataCaching'];
	}
	...
	cacheLoop: for (var i in objCache) {
		thisCache = objCache[i];
		if (isset(thisCache) && isset(thisCache['name']) && thisCache['name'] == name) {
			objCache[i]['data'] = data;
			added++;
			break cacheLoop;
		}
	}
	if (added === 0) {
		objCache[countArr(objCache)] = {'name': name, 'data': data};
	}
	window['dataCaching'] = objCache;
	return true;
};
dataCache.push = function (name, data) {
	var objCache = {};
	if (isset(window) && isset(window['dataCaching'])) {
		objCache = window['dataCaching'];
	}
	cacheLoop: for (var i in objCache) {
		thisCache = objCache[i];
		if (isset(thisCache) && isset(thisCache['name']) && thisCache['name'] == name) {
			if (isset(thisCache['data']) && typeof thisCache['data'] === "object") {
				for (x in data) {
					thisCache['data'][x] = data[x];
				}
				objCache[i]['data'] = thisCache['data'];
			}else {
				thisCache['data'] += data;
				objCache[i]['data'] = thisCache['data'];
			}
...
$(document).ready(function() {
	...
	setTimeout(function() {
		...
		*/
		/*	
		console.log(dataCache.add('sdsd', {'sdsd': 'sdfsdfsdf', 'sdfsdfas': 'sdfsaf'}));
		setTimeout(function () {
			console.log(dataCache.get('sdsd'));
		}, 5000);
		*/
