	...
	function suggest ($object) {
		...
		if ($type == "user") {
			...
			if (in_array("places", $rowsArr)) {
				...
				//.
				/*$usersPlacesId = array_unique($usersPlacesId);
				if (count($usersPlacesId) > 0) {
					$usersNearbyRequest = "SELECT `id` FROM `users` WHERE `live` REGEXP '^[0-9]+$' AND `live` IN ('".implode($usersPlacesId,"', '")."');";
					$usersNearbyQuery = mysqli_query($db, $usersNearbyRequest);
					if (!$usersNearbyQuery) {
						//.
					}else {
