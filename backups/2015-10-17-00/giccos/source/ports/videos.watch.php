			...
			<div id="gMain">
				...
				<div id="gInclude">
					...
					$(document).ready(function(){
						resizeCallbackFunc = function () {
							button = this;
							player = button['player_'];
							playerElement = $('#' + player['id_']);
							isActivated = button.hasClass('gplayer-control-active');
							if (isActivated == true) {
						...
						thumbnailLink = "<?php print $videoInfoArr['source']['thumbnail']; ?>";
						if (isset(thumbnailLink) && thumbnailLink != "") {
							videoPlayerOptions['poster'] = thumbnailLink;
						}
						<?php
						...
						print "videoPlayerOptions['src'] = [".$videoPlayerSrc.'];';
						?>
						_g['player'].use("videoPlayer", videoPlayerOptions);
					});
					</script>
				</div>
			</div>
