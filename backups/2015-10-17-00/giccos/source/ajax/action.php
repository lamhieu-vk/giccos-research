...
if (isset($g_client['token']['ajax'], $_SERVER['HTTP_TOKEN'], $_SERVER['HTTP_X_REQUESTED_WITH'], $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_SERVER['HTTP_TOKEN'] == $g_client['token']['ajax'] && $_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest" && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")) {
	if (isset($_SERVER['HTTP_RANDKEY'])) {
		setcookie('gXHR_'.$_SERVER['HTTP_RANDKEY'], null, $_tool->timeNow() - 31536000, $_parameter->get('cookie.host.path'), $g_client['http']['secure']);
	...
	//. die(print json_encode(array("return" => false, "reason" => $_REQUEST)));
	...
	if ($port == "accounts" && $token == $g_client['token']['action']['accounts']) {
		...
		if ($type == "reload") {
			...
			if (!$photosMediaCacheClean || !$musicMediaCacheClean || !$videosMediaCacheClean) {
				...
				//.
				if (isset($g_cache['weather']) && is_array($g_cache['weather']) && count($g_cache['weather']) > 0) {
					$locationDataArr = $g_cache['weather'];
					foreach ($locationDataArr as $key => $locationDataThis) {
						if (isset($locationDataThis['time'])) {
							if ($locationDataThis['time'] + $_parameter->get('weather_cache_timeout') < time()) {
								unset($_SESSION["cache"]['weather'][$key]);
								unset($g_cache['weather'][$key]);
							}
						}else {
							unset($_SESSION["cache"]['weather'][$key]);
							unset($g_cache['weather'][$key]);
		...
		if ($type == "editor") {
			...
			if ($action == "add") {
				...
				foreach ($fileArr as $key => $fileArrThis) {
					$fileArrThis['randkey'] = isset($ObjRequest['randkey'][$key]) ? $ObjRequest['randkey'][$key] : null;
					$addCacheFileOpts = array("action" => "add", "file" => $fileArrThis);
					$addCacheFileOpts = array_merge($addCacheFileOpts, array(
						"guy" => $g_user['mode'], 
						"user" => array(
							"id" => $g_user['id']
						)
					));
					$addCacheFile = $_photos->cache($addCacheFileOpts);
					if (isset($addCacheFile, $addCacheFile['return'], $addCacheFile['data']) && $addCacheFile['return'] == true) {
						$dataCacheFile = $addCacheFile['data'];
						if (isset($dataCacheFile['source'])) {
							unset($dataCacheFile['source']);
						}
						if (isset($dataCacheFile['name']) && is_string($dataCacheFile['name'])) {
							$dataCacheFile['link'] = $_tool->links('photos/cache/'.$dataCacheFile['name']);
						}
						$dataCacheFile['randkey'] = $fileArrThis['randkey'];
						$dataArr[] = array("return" => true, "data" => $dataCacheFile);
					}else if (isset($addCacheFile, $addCacheFile['return'], $addCacheFile['reason']) && $addCacheFile['return'] == false) {
						$dataArr[] = array("return" => false, "reason" => $addCacheFile['reason'], "file" => $fileArrThis);
					}else {
						$dataArr[] = array("return" => false, "reason" => "", "file" => $fileArrThis);
					}
				}
				$dataCount = count($dataArr);
				die(print json_encode(array("return" => true, "count" => $dataCount, "data" => $dataArr)));
			}else if ($action == "copy") {
		...
		if ($type == "cache") {
			...
			if ($action == "get") {
				$videoLabel = isset($ObjRequest['label']) && is_string($ObjRequest['label']) ? $ObjRequest['label'] : null;
				$videoValue = isset($ObjRequest['value']) && (is_string($ObjRequest['value']) || is_numeric($ObjRequest['value'])) ? $ObjRequest['value'] : null;
				if ($videoLabel == null || $videoValue == null) {
					die(print json_encode(array("return" => false, "reason" => "sdfs")));
				}
				$getVideoPreviewOptions = array(
					"guy" => array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id']),
					"action" => "get",
					"label" => $videoLabel,
					"value" => $videoValue
				);
				$getVideoPreview = $_videos->preview($getVideoPreviewOptions);
				if (isset($getVideoPreview, $getVideoPreview['return'], $getVideoPreview['exists']) && $getVideoPreview['return'] == true) {
					if (isset($getVideoPreview['data']) && $getVideoPreview['exists'] == true) {
						die(print json_encode(array("return" => true, "exists" => true, "data" => $getVideoPreview['data'])));
					}else {
						die(print json_encode(array("return" => true, "exists" => false, "x" => $getVideoPreview['x'])));
					}
				}else if (isset($getVideoPreview, $getVideoPreview['return'], $getVideoPreview['reason']) && $getVideoPreview['return'] == true) {
					die(print json_encode(array("return" => false, "reason" => $getVideoPreview['reason'])));
				}else {
					die(print json_encode(array("return" => false, "reason" => "")));
				}
			}else {
		...
		if ($type == "status") {
			if (in_array($action, ["add", "upload"])) {
			...
			}else if (in_array($action, ["newcheck", "get", "load"])) {
				...
				$listOptions['except']['id'] = isset($ObjRequest['except'], $ObjRequest['except']['id']) && is_array($ObjRequest['except']['id']) && count($ObjRequest['except']['id']) > 0 ? $ObjRequest['except']['id'] : null;
				...
				if (!in_array($listOptions['type'], ["country", "family", "friends", "general", "hashtag", "near", "nearby", "location", "places", "status", "time"])) {
					die(print json_encode(array("return" => false, "reason" => "345")));
				}else if (!in_array($listOptions['sort'], [">", ">=", "=", "<", "<="])) {
					die(print json_encode(array("return" => false, "reason" => "42134")));
				}else if (!in_array($listOptions['order'], [null, "old", "new"])) {
					die(print json_encode(array("return" => false, "reason" => "sdfsa")));
				}else if (!in_array($listOptions['method'], ["auto", "action", "new"])) {
					die(print json_encode(array("return" => false, "reason" => "yrtyr")));
				}else if ($listOptions['logs'] == null || !preg_match("/^([0-9]+)$/", $listOptions['logs'])) {
					die(print json_encode(array("return" => false, "reason" => "rwfsdf")));
				}
				...
				if (in_array($listOptions['type'], ["time", "timeline"])) {
					$listOptions['time'] = isset($ObjRequest['time']) && is_array($ObjRequest['time']) && count($ObjRequest['time']) > 0 ? $ObjRequest['time'] : null;
					if ($listOptions['time'] == null) {
						die(print json_encode(array("return" => false, "reason" => "rtertwe")));
					}
				}else if (in_array($listOptions['type'], ["tag", "hashtag"])) {
					$listOptions['hashtag'] = isset($ObjRequest['hashtag']) && is_array($ObjRequest['hashtag']) && count($ObjRequest['hashtag']) > 0 ? $ObjRequest['hashtag'] : null;
					if ($listOptions['hashtag'] == null) {
						die(print json_encode(array("return" => false, "reason" => "dfgsd")));
					}
				}else if (in_array($listOptions['type'], ["near", "nearby", "location", "places"])) {
					$listOptions['location'] = isset($ObjRequest['location']) && !is_bool($ObjRequest['location']) ? $ObjRequest['location'] : null;
					if ($listOptions['location'] == null) {
						die(print json_encode(array("return" => false, "reason" => "dfgsd")));
					}
				}
				$listOptions['guy'] = array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id']);
				$getList = $_feed->status_list($listOptions);
				if (isset($getList['return'], $getList['count'], $getList['data']) && $getList['return'] == true) {
					if (in_array($action, ["newcheck"])) {
						die(print json_encode(array("return" => true, "count" => $getList['count'], "x" => $getList)));
					}
					$listStatusArr = array();
					...
					if (isset($statusFetch['return'], $statusFetch['count'], $statusFetch['data']) && $statusFetch['return'] == true) {
						...
						die(print json_encode(array("return" => false, "reason" => $statusFetch['reason'])));
					}else {
						die(print json_encode(array("return" => false, "reason" => "")));
					}
				}else if (isset($getList['return'], $getList['reason']) && $getList['return'] == false) {
					die(print json_encode(array("return" => false, "reason" => $getList['reason'])));
				}else {
					die(print json_encode(array("return" => false, "reason" => $getList)));
				}
			}else if (in_array($action, ["block", "unblock"])) {
				$thingsArr = isset($ObjRequest['things']) && is_array($ObjRequest['things']) && count($ObjRequest['things']) > 0 ? $ObjRequest['things'] : null;
				if ($thingsArr == null) {
					die(print json_encode(array("return" => false, "reason" => "das")));
				}
				$dataArr = array();
				foreach ($thingsArr as $i => $thingsThis) {
					...
					if (!in_array($thingsThis['type'], ["status", "comment", "user", "page", "group"])) {
						continue;
					}
					if ($action == "block") {
						$actionBlock = "add";
					...
					$useBlockOptions = array("guy" => $g_user['mode'], "action" => $actionBlock, "things" => $thingsThis);
					$useBlockAction = $_feed->status_block($useBlockOptions);
					if (isset($useBlockAction, $useBlockAction['return']) && $useBlockAction['return'] == true) {
						$dataArr[] = array_merge(array("things" => $thingsThis,"return" => true));
					}else if (isset($useBlockAction, $useBlockAction['return'], $useBlockAction['reason']) && $useBlockAction['return'] == false) {
						$dataArr[] = array_merge(array("things" => $thingsThis,"return" => false, "reason" => "ssdfg".$useBlockAction['reason']));
					}else {
						$dataArr[] = array_merge(array("things" => $thingsThis,"return" => false, "reason" => "dsfxs"));
					}
				}
			...
			if ($action == "add") {
				...
				if (isset($addFeedComment['return'], $addFeedComment['data'], $addFeedComment['data']['id']) && $addFeedComment['return'] == true) {
					...
					if ($insideType == "status") {
						$getCommentOptions['id'][$addFeedComment['data']['id']] = array("reply" => true);
					}else {
						$getCommentOptions['id'][$addFeedComment['data']['id']] = array("reply" => false);
					}
					...
					die(print json_encode(array("return" => true, "data" => $commentDataArr)));
				...
				foreach ($optionsArr as $key => $optionsArrThis) {
					...
					if (isset($optionsArrThis['limit']) && is_numeric($optionsArrThis['limit'])) $commentLimit = $optionsArrThis['limit']; else $commentLimit = 5;
					...
					$getCommentListOptions = array(
						"guy" => $g_user['mode'], 
						"inside" => [array("type" => $insideType, "id" => $insideId)], 
						"sort" => $commentSort, 
						"id" => $commentId, 
						"order" => "new", 
						"limit" => $commentLimit
					);
					$getCommentList = $_feed->comment_list($getCommentListOptions);
					if (isset($getCommentList, $getCommentList['return'], $getCommentList['data']) && $getCommentList['return'] == true) {
						foreach ($getCommentList['data'] as $commentId) {
							if ($insideType == "status") {
								$listArr[$commentId] = array("reply" => true);
							}else {
								$listArr[$commentId] = array("reply" => false);
							}
						}
					}
				}
				if (isset($listArr) && count($listArr) > 0) {
					...
					$getCommentOptions['id'] = $listArr;
					$getCommentFetch = $_feed->comment_fetch($getCommentOptions);
					if (isset($getCommentFetch['return'], $getCommentFetch['data']) && $getCommentFetch['return'] == true) {
						$dataArr = array_merge($dataArr, $getCommentFetch['data']);
					}
				}
				$countArr = count($dataArr);
				die(print json_encode(array("return" => true, "count" => $countArr, "data" => $dataArr)));
		...
		if ($type == "thumbnail") {
			...
			if ($action == "search") {
				...
				$requestPlacesOptions = array("guy" => $g_user['mode'], "action" => "search", "keywords" => $keywords);
			...
			if ($manage == "guy") {
				if ($action == "search") {
					...
					if ($searchQuery == true && mysqli_num_rows($searchQuery) > 0) {
						while ($searchFetch = mysqli_fetch_assoc($searchQuery)) {
							$getUserAction = $_user->profile(array("action" => "get", "rows" => ['id', 'username', 'fullname', 'avatar.small', 'cover.small'], "label" => "id", "value" => $searchFetch['id']));
					...
					if (isset($getMessagesTyping['return'], $getMessagesTyping['data']) && $getMessagesTyping['return'] == true) {
						foreach ($getMessagesTyping['data']['typing'] as $key => $getMessagesTypingDataThis) {
							...
							if (in_array($getMessagesTypingDataThis['guy']['type'], ["user", "users"])) {
								$getGuyData = $_user->profile(array("action" => "get", "rows" => ['fullname', 'username', 'link', 'avatar.small'], "label" => "id", "value" => $getMessagesTypingDataThis['guy']['id']));
								if (isset($getGuyData['return'], $getGuyData['data'][0]) && $getGuyData['return'] == true) {
						...
						foreach ($getMessagesViews['data'] as $thisType => $getMessagesViewsThis) {
							foreach ($getMessagesViewsThis as $key => $getMessagesViewsDataThis) {
								...
								if (in_array($getMessagesViewsDataThis['guy']['type'], ["user", "users"])) {
									$getGuyData = $_user->profile(array("action" => "get", "rows" => ['fullname', 'username', 'link', 'avatar.small'], "label" => "id", "value" => $getMessagesViewsDataThis['guy']['id']));
									if (isset($getGuyData['return'], $getGuyData['data'][0]) && $getGuyData['return'] == true) {
		...
		$siteUrl = isset($ObjRequest['url']) && is_string($ObjRequest['url']) ? $ObjRequest['url'] : null;
		if ($siteUrl == null) {
			die(print json_encode(array("return" => false, "direct" => true, "reason" => "")));
		}
		$regexUrl = $_parameter->get('regex_sites_giccos_domain');
		if (!preg_match($regexUrl, $siteUrl, $matchUrl)) {
			die(print json_encode(array("return" => false, "direct" => true, "reason" => "")));
		}
		/*
		if ($_tool->siteDie($siteUrl) == true) {
			return array("return" => false, "direct" => true, "reason" => "3");
		}
		*/
		$htmlPage = $_tool->curl($siteUrl, 0, array("cookie" => true));
		if (isset($htmlPage['return'], $htmlPage['data']) && $htmlPage['return'] == true) {
			...
			$pageDom->load($htmlPage);
			$dataArr = $pageMain = array();
			$pageMain['html'] = $pageMain['title'] = $pageMain['box'] = $pageMain['include'] = null;
			if (isset($pageDom->find("html", 0)->innertext)) {
				$pageMainHtmlDom = $pageDom->find("html", 0);
				$pageMain['html'] = array();
				if (isset($pageMainHtmlDom->lang)) {
					$pageMain['html']['lang'] = $pageMainHtmlDom->lang;
				}
				if (isset($pageMainHtmlDom->id)) {
					$pageMain['html']['id'] = $pageMainHtmlDom->id;
				}
				if (isset($pageMainHtmlDom->class)) {
					$pageMain['html']['class'] = $pageMainHtmlDom->class;
				}
			}
			if (isset($pageDom->find("title", 0)->innertext)) {
				$pageMain['title'] = ($pageDom->find("title", 0)->innertext);
			}
			if (isset($pageDom->find("#gBox", 0)->innertext)) {
				$pageMain['box'] .= ($pageDom->find("#gBox", 0)->innertext);
				$pageMain['box'] = preg_replace("#<script(.*?)>(.*?)</script>#is", "", $pageMain['box']);
				$pageMain['box'] = preg_replace("#<style(.*?)>(.*?)</style>#is", "", $pageMain['box']);
			}
			if (isset($pageMain['box'])) {
				if (isset($pageDom->find("body", 0)->find("#gBox", 0)->innertext)) {
					$pageMain['include'] .= ($pageDom->find("body", 0)->find("#gBox", 0)->innertext);
				}
				if (isset($pageDom->find("body", 0)->find("#gInclude", 0)->innertext)) {
					$pageMain['include'] .= ($pageDom->find("body", 0)->find("#gInclude", 0)->innertext);
				}
				$pageMainIncludeDom = new DOMDocument();
				libxml_use_internal_errors(true);
				$includeScriptArr = $includeCssArr = array();
				$pageMainIncludeDom->loadHTML($pageMain['include']);
				$includeScript = $pageMainIncludeDom->getElementsByTagName('script');
				if ($includeScript) {
					foreach ($includeScript as $scriptThis) {
						$includeScriptArr[] = array(
							"code" => $pageMainIncludeDom->saveHTML($scriptThis),
							"src" => $scriptThis->getAttribute('src'),
							"text" => preg_replace("/^([\s\n\r]+)$/", "", strip_tags($pageMainIncludeDom->saveHTML($scriptThis)))
						);
					}
				}
				$includeCssLink = $pageMainIncludeDom->getElementsByTagName('link');
				if ($includeCssLink) {
					foreach ($includeCssLink as $scriptThis) {
						$includeCssArr[] = array(
							"code" => $pageMainIncludeDom->saveHTML($scriptThis),
							"src" => $scriptThis->getAttribute('href'),
							"text" => preg_replace("/^([\s\n\r]+)$/", "", strip_tags($pageMainIncludeDom->saveHTML($scriptThis)))
						);
					}
					$includeCssStyle = $pageMainIncludeDom->getElementsByTagName('style');
					foreach ($includeCssStyle as $scriptThis) {
						$includeCssArr[] = array(
							"code" => $pageMainIncludeDom->saveHTML($scriptThis),
							"src" => $scriptThis->getAttribute('href'),
							"text" => preg_replace("/^([\s\n\r]+)$/", "", strip_tags($pageMainIncludeDom->saveHTML($scriptThis)))
				...
				$pageMain['include'] = array(
					"script" => $includeScriptArr,
					"css" => $includeCssArr
				);
			}
			$pageMain['url'] = $siteUrl;
			$dataArr = $pageMain;
			die(print json_encode(array("return" => true, "direct" => false, "data" => $dataArr)));
		}else {
			die(print json_encode(array("return" => false, "direct" => true, "reason" => "xs")));
		}
		...
		die(print json_encode(array("return" => false, "reason" => "", "x" => $_REQUEST, "s" => $_SERVER)));
		die(header("HTTP/1.1 320 Not Found"));
	}
}else {
	die(print json_encode(array("return" => false, "reason" => "", "x" => $_REQUEST, "a" => $_SERVER)));
	die(header("HTTP/1.1 454 Not Found"));
}
?>