	...
	function places ($object = array()) {
		...
		if ($action == "search") {
			...
			while ($getPlacesFetch = mysqli_fetch_assoc($getPlacesQuery)) {
				...
				if (!isset($getPlacesFetch['country']) && $getPlacesFetch['address']) {
					$addressExplode = explode(",", $getPlacesFetch['address']);
					$getPlacesFetch['country'] = $addressExplode[count($addressExplode) - 1];
				}
