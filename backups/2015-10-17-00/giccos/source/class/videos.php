...
Class videos {
	...
	function thumbnail ($object = array()) {
	...
	function cache ($object = array()) {
		...
		if (in_array($action, ["add"])) {
			...
			$infoArr = $_tool->readFileInfo($fileArr['tmp'], 'video');
			if (!$infoArr) {
				return array("return" => false, "reason" => "1");
			}
			...
			$clientToken = $_client->client(true);
			$getFileExpiresRequest = "SELECT * FROM `videos_cache` WHERE (`client` = '".$clientToken."' OR `user.id` = '".$userId."' OR (`author.id` = '".$guyId."' AND `author.type` = '".$guyType."')) AND `close` != '0' AND `close` <= '".time()."';";
			...
			$getFileOtherRequest = "SELECT * FROM `videos_cache` WHERE `close` != '0' AND `close` <= '".time()."' LIMIT 10;";
			...
			if (!isset($fileArr['tmp'])) {
				...
				$fileArr['info'] = $_tool->readFileInfo($fileArr['tmp'], 'video');
	...
	function createThumbnail ($object = array()) {
		$_storage = $this->class['_storage'];
		$_tool = $this->class['_tool'];
		$_parameter = $this->class['_parameter'];
		$_client = $this->class['_client'];
	...
	function createPreview ($object = array()) {
		$_storage = $this->class['_storage'];
		$_tool = $this->class['_tool'];
		$_parameter = $this->class['_parameter'];
		$_client = $this->class['_client'];
...
*/
	...
	function fetch ($object = array()) {
		return $this->info($object);
	}
	function info ($object = array()) {
		$_tool = $this->class['_tool'];
		$_parameter = $this->class['_parameter'];
		...
		if (isset($object['guy'], $object['guy']['id']) && (is_numeric($object['guy']['id']) || is_string($object['guy']['id']))) $guyId = $_tool->convertDatabaseString($object['guy']['id']); else $guyId = null;
		if ($guyType == null || $guyId == null) {
			return array("return" => false, "reason" => "xs1");
		}
		...
		if (!$getFileQuery) {
			...
			while ($videoThis = mysqli_fetch_assoc($getFileQuery)) {
				...
				if (isset($videoThis['display'])) {
					$videoThis['link'] = $_tool->links('videos/watch/'.$videoThis['display']);
