...
Class storage {
	...
	function upload ($object = array()) {
		...
		if (!in_array($format, ["image", "audio", "video"])) {
			...
			$move = isset($object['move']) && is_string($object['move']) ? $object['move'] : null;
			$folder = isset($object['folder']) && is_string($object['folder']) ? $object['folder'] : null;
			$port = isset($object['port']) && is_string($object['port']) ? $object['port'] : null;
			$name = isset($object['name']) && is_string($object['name']) ? $object['name'] : null;
			$mime = isset($object['mime']) && is_string($object['mime']) ? $object['mime'] : null;
			$compression = isset($object['compression']) && is_bool($object['compression']) ? $object['compression'] : true;
			...
			$storage['name'] = $_tool->convertDatabaseString($name);
			...
			$storage['source'] = $_client->root().$storage['dir'].'/'.$storage['nameraw'];
