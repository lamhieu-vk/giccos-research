...
# Set permission.
$_authorization = 0;
if ((isset($g_client['token']['client'], $_SERVER['HTTP_CLIENT']) && $_SERVER['HTTP_CLIENT'] = $g_client['token']['client']) || (isset($g_client['token']['robot'], $_SERVER['HTTP_ROBOT']) && $_SERVER['HTTP_ROBOT'] = $g_client['token']['robot'])) {
	$_authorization = 1;
}
# Run page.
	...
	if (isset($_uri[0], $g_client['token']['source']['url']) && is_string($_uri[0]) && $_uri[0] == $g_client['token']['source']['url']) {
		...
		if (isset($g_user['mode']) && (!isset($_uri[1]) || (isset($_uri[1]) && is_string($_uri[1]) && in_array($_uri[1], ["", "general"])))) {
			...
			if (isset($_uri[2]) && is_string($_uri[2])) {
				$_uri[2] = urldecode($_uri[2]);
				$feed_['options']['selected'] = explode("&", $_uri[2]);
				foreach ($feed_['options']['selected'] as $key => $valueSelected) {
					if (preg_match("/^([a-zA-Z0-9]{32})$/", $valueSelected)) {
						$feed_['options']['selected'][$key] = array("label" => "display", "value" => $valueSelected);
						$valueSelected = urlencode($valueSelected);
						if ($redirectPath == null) {
							$redirectPath .= $valueSelected;
						}else {
							$redirectPath .= "&".$valueSelected;
						}
					}else if (preg_match("/^([0-9]{1,11})$/", $valueSelected)) {
						$feed_['options']['selected'][$key] = array("label" => "id", "value" => $valueSelected);
						$valueSelected = urlencode($valueSelected);
						if ($redirectPath == null) {
							$redirectPath .= $valueSelected;
						}else {
			...
			if (isset($_uri[2]) && is_string($_uri[2])) {
				...
				foreach ($feed_['options']['time'] as $key => $valueTime) {
					$valueSelectedArr = explode("-", $valueTime);
					...
					if (count($feed_['options']['time'][$key]) == 0) {
						...
						if (count($feed_['options']['time'][$key]) == 1) {
							$valueTime = $feed_['options']['time'][$key][0];
						}else if (count($feed_['options']['time'][$key]) == 2) {
							$valueTime = $feed_['options']['time'][$key][0]."-".$feed_['options']['time'][$key][1];
						}
						$valueTime = urlencode($valueTime);
						if ($redirectPath == null) {
							$redirectPath .= $valueTime;
						}else {
							$redirectPath .= "&".$valueTime;
						}
					}
		...
		}else if (isset($_uri[1], $g_user['mode']) && is_string($_uri[1]) && in_array($_uri[1], ["tag", "hashtag"])) {
			$feed_['options'] = array(
				"type" => "hashtag"
			);
			if (isset($_uri[3]) && is_string($_uri[3]) && in_array(strtolower($_uri[3]), ["auto", "new", "action"])) {
			...
			if (isset($_uri[2]) && is_string($_uri[2])) {
				$_uri[2] = urldecode($_uri[2]);
				$feed_['options']['hashtag'] = explode("&", $_uri[2]);
				foreach ($feed_['options']['hashtag'] as $key => $valueHashtag) {
					if (!is_string($valueHashtag)) {
						unset($feed_['options']['hashtag'][$key]);
						continue;
					}
					$valueHashtag = urlencode($valueHashtag);
					if ($redirectPath == null) {
						$redirectPath .= $valueHashtag;
					}else {
						$redirectPath .= "&".$valueHashtag;
					}
				}
			}else {
				$feed_['options']['hashtag'] = null;
			}
			$redirectPath = $_tool->links($_uri[0].'/'.$_uri[1].'/'.$redirectPath);
			if ($redirectPath != $_client->fullurl()) {
				die(header("Location: ".$redirectPath));
		...
		}else if (isset($_uri[1], $g_user['mode']) && is_string($_uri[1]) && in_array($_uri[1], ["near", "nearby", "place", "places", "location",])) {
			$feed_['options'] = array(
				"type" => "location"
			);
			if (isset($_uri[2]) && is_string($_uri[2]) && in_array(strtolower($_uri[2]), ["auto", "new", "action"])) {
			...
			if (isset($_uri[2]) && is_string($_uri[2])) {
				$_uri[2] = urldecode($_uri[2]);
				$feed_['options']['location'] = explode("&", $_uri[2]);
				foreach ($feed_['options']['location'] as $key => $valueLocation) {
					if (!is_string($valueLocation) && !is_numeric($valueLocation)) {
						print $valueLocation;
						unset($feed_['options']['location'][$key]);
						continue;
					}
					$valueLocation = urlencode($valueLocation);
					if ($redirectPath == null) {
						$redirectPath .= $valueLocation;
					}else {
						$redirectPath .= "&".$valueLocation;
					}
				}
				//$feed_['options']['location'] = $_uri[2];
			}else {
				$feed_['options']['location'] = null;
			}
			$redirectPath = $_tool->links($_uri[0].'/'.$_uri[1].'/'.$redirectPath);
			if ($redirectPath != $_client->fullurl()) {
				die(header("Location: ".$redirectPath));
			}
			die(require_once("source/ports/feed.location.php"));
		}else {
		...
		*/
		...
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			if (is_string($_uri[2]) && preg_match("/^(static)/i", $_uri[2])) {
				...
				$dataString = ($_tool->hash('decode', $dataString, $_parameter->get('hash_photos_qr')));
		...
		if (isset($_uri[1]) && is_string($_uri[1]) && in_array($_uri[1], [null, ""])) {
			...
			$fileName = isset($_uri[2]) && is_string($_uri[2]) ? addslashes($_uri[2]) : null;
			if ($fileName == null || !preg_match("/^([a-zA-Z0-9]{8,40})$/", $fileName)) {
				die(header("HTTP/1.1 503 Service Unavailable"));
			}
			$isPreview = isset($_uri[3]) && $_uri[3] == "preview" ? true : false;
			$isThumbnail = isset($_uri[3]) && $_uri[3] == "thumbnail" ? true : false;
			if ($isPreview === true) {
				$readFile = array("type" => "other", "path" => 'storage\video\drive\1\x\0.png', "mime" => 'image/png', "expire" => $_parameter->get('http_info_expire_static'));
				die(require_once ("source/ports/templates/readfile.php"));
				//.
				$filePreviewRequest = "SELECT `path` FROM `videos_info` WHERE `videos.id` IN (SELECT `id` FROM `videos_info` WHERE `display` = '".$fileName."' OR `file.hd` = '".$fileName."' OR `file.vga` = '".$fileName."' OR `file.qvga` = '".$fileName."') LIMIT 1;";
				$filePreviewQuery = mysqli_query($_db->port('beta'), $filePreviewRequest);
				if (!$filePreviewQuery) {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}else {
					if (mysqli_num_rows($filePreviewQuery) == 0) {
						die(header("HTTP/1.1 503 Service Unavailable"));
					}else {
						$filePreview = mysqli_fetch_assoc($filePreviewQuery);
						$filePreview['path'] = $filePreview['path.preview'];
						$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $filePreview['path'], "limit" => "LIMIT 1"));
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['data'], $getMediaFile['data'][0]) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							...
							$fileUrl = $getMediaFile['data'][0]['source'];
							if (!$_tool->siteDie($fileUrl) || file_exists($fileUrl)) {
								$readFile = array("type" => "other", "path" => $fileUrl, "mime" => $fileMime, "expire" => $_parameter->get('http_info_expire_static'));
								die(require_once ("source/ports/templates/readfile.php"));
							}else {
								die(header("HTTP/1.1 503 Service Unavailable"));
							}
				...
				$fileThumbnailRequest = "SELECT `path.thumbnail` FROM `videos_info` WHERE `display` = '".$fileName."' OR `file.hd` = '".$fileName."' OR `file.vga` = '".$fileName."' OR `file.qvga` = '".$fileName."' LIMIT 1;";
				$fileThumbnailQuery = mysqli_query($_db->port('beta'), $fileThumbnailRequest);
				if (!$fileThumbnailQuery) {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}else {
					if (mysqli_num_rows($fileThumbnailQuery) == 0) {
						die(header("HTTP/1.1 503 Service Unavailable"));
					}else {
						$fileThumbnail = mysqli_fetch_assoc($fileThumbnailQuery);
						$fileThumbnail['path'] = $fileThumbnail['path.thumbnail'];
						$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $fileThumbnail['path'], "limit" => "LIMIT 1"));
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['data'], $getMediaFile['data'][0]) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							$fileMime = $getMediaFile['data'][0]['mime'];
							$fileUrl = $getMediaFile['data'][0]['source'];
							if (!$_tool->siteDie($fileUrl) || file_exists($fileUrl)) {
			...
			$fileMediaRequest = "SELECT `id`, `file.hd`, `file.vga`, `file.qvga`, `path.hd`, `path.vga`, `path.qvga` FROM `videos_info` WHERE `file.hd` = '".$fileName."' OR `file.vga` = '".$fileName."' OR `file.qvga` = '".$fileName."' LIMIT 1;";
			$fileMediaQuery = mysqli_query($_db->port('beta'), $fileMediaRequest);
			if (!$fileMediaQuery) {
				//.
			}else {
				if (mysqli_num_rows($fileMediaQuery) == 0) {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}else {
					$fileMedia = mysqli_fetch_assoc($fileMediaQuery);
					if ($fileMedia['file.hd'] == $fileName) {
						$fileMedia['path'] = $fileMedia['path.hd'];
					}else if ($fileMedia['file.vga'] == $fileName) {
						$fileMedia['path'] = $fileMedia['path.vga'];
					}else if ($fileMedia['file.qvga'] == $fileName) {
						$fileMedia['path'] = $fileMedia['path.qvga'];
					}
					// check private.
					$getMediaFile = $_storage->get(array("format" => "video", "label" => "display", "value" => $fileMedia['path'], "limit" => "LIMIT 1"));
					if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['data'], $getMediaFile['data'][0]) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
						$fileMime = $getMediaFile['data'][0]['mime'];
						$fileUrl = $getMediaFile['data'][0]['source'];
						if (!$_tool->siteDie($fileUrl) || file_exists($fileUrl)) {
							...
							$readFile = array("type" => "media", "path" => $fileUrl, "mime" => $fileMime, "expire" => $_parameter->get('http_info_expire_static'));
							die(require_once ("source/ports/templates/readfile.php"));
						}else {
							die(header("HTTP/1.1 503 Service Unavailable"));
						}
			...
			$isThumbnail = isset($_uri[3]) && $_uri[3] == "thumbnail" ? true : false;
			if ($isThumbnail === true) {
				$fileThumbnailRequest = "SELECT `thumbnail` FROM `videos_cache` WHERE `name` = '".$fileName."' AND `author.type` = '".$g_user['mode']['type']."' AND `author.id` = '".$g_user['mode']['id']."' LIMIT 1;";
				$fileThumbnailQuery = mysqli_query($_db->port('beta'), $fileThumbnailRequest);
				if (!$fileThumbnailQuery) {
					die(header("HTTP/1.1 503 Service Unavailable"));
				}else {
			...
			$fileMediaRequest = "SELECT `path` FROM `videos_cache` WHERE `name` = '".$fileName."' AND `author.type` = '".$g_user['mode']['type']."' AND `author.id` = '".$g_user['mode']['id']."' LIMIT 1;";
			$fileMediaQuery = mysqli_query($_db->port('beta'), $fileMediaRequest);
			if (!$fileMediaQuery) {
				//.
			}else {
		...
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "words") {
			...
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'], $_authorization) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host") && $_authorization >= 1) {
				if ($subinfo == "audio") {
					//.
				}else if ($subinfo == "define") {
		...
		if (isset($_uri[1]) && is_string($_uri[1]) && $_uri[1] == "raw") {
			...
			$locationPlaces = isset($_uri[2]) && is_string($_uri[2]) ? urldecode($_uri[2]) : null;
			$subinfo = isset($_uri[3]) && is_string($_uri[3]) ? urldecode($_uri[3]) : null;
			if ($locationPlaces == null || $subinfo == null) {
				die(require_once("source/ports/oops.php"));
			}
			if (isset($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'], $_authorization) && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host") && $_authorization >= 1) {
				if ($subinfo == "info") {
					/*
					$mapsPlacesOptions = array("action" => "get");
					$mapsPlacesOptions['guy'] = array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id']);
					if (is_numeric($locationPlaces)) {
						$mapsPlacesOptions['label'] = "id";
					}else if (is_string($locationPlaces)) {
						if (preg_match('/^(([0-9\.]+)\,([0-9\.]+))$/', $locationPlaces)) {
							$mapsPlacesOptions['label'] = "location";
						}else {
							$mapsPlacesOptions['label'] = "address";
						}
					}
					$mapsPlacesOptions['value'] = $locationPlaces;
					$mapsPlacesOptions['limit'] = "LIMIT 1";
					*/
					if (is_numeric($locationPlaces)) {
						$mapsPlacesOptions = array("action" => "get");
						$mapsPlacesOptions['guy'] = array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id']);
						...
						$mapsPlacesOptions = array("action" => "search");
						$mapsPlacesOptions['guy'] = array("type" => $g_user['mode']['type'], "id" => $g_user['mode']['id']);
						$mapsPlacesOptions['keywords'] = $locationPlaces;
					}
					$mapsPlacesOptions['limit'] = "LIMIT 1";
					$mapsPlaces = $_maps->places($mapsPlacesOptions);
					if (isset($mapsPlaces, $mapsPlaces['return'], $mapsPlaces['data']) && $mapsPlaces['return'] == true) {
						if (isset($mapsPlaces['data'][0])) {
							$mapsPlacesInfo = $mapsPlaces['data'][0];
							if (isset($mapsPlacesInfo['token'])) unset($mapsPlacesInfo['token']);
							$fileContent = json_encode($mapsPlacesInfo);
							$readFile = array("type" => "other", "content" => $fileContent, "mime" => 'text/html', "expire" => $_parameter->get('http_cache_expire_static'));
							die(require_once ("source/ports/templates/readfile.php"));
						}else {
							die(header("HTTP/1.1 503 Service Unavailable"));
