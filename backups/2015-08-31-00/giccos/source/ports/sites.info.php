								...
								<div class="rows table">
									<div class="value"> 
										...
										while ($countWeekdays <= 7) {
											...
											$statsView['this_week'][strtolower($_tool->convertWeekday(abs($wdt), true))] = 0;
											$statsView['last_week'][strtolower($_tool->convertWeekday(abs($wdt), true))] = 0;
											$countWeekdays++;
											$wdt--;
										}
										$statsView['this_week'] = array_reverse($statsView['this_week']);
										$statsView['last_week'] = array_reverse($statsView['last_week']);
										foreach ($siteInfoArr['stats']['views'] as $statsTypeTime => $statsThisTime) {
											if ($statsTypeTime == "this_week") {
												...
												$statsView['this_week'][$_language->text('today', 'strtolower')] = $statsView['this_week'][strtolower($_tool->convertWeekday(date("N", time()), true))];
												unset($statsView['this_week'][strtolower($_tool->convertWeekday(date("N", time()), true))]);
										...
										var datasetOptions = {
											labels: [<?php $isPrinted = 0; foreach ($statsView['this_week'] as $weekdaysLabel => $weekdaysValue) { if ($isPrinted == 0) { print '"'.$weekdaysLabel.'"'; } else { print ', "'.$weekdaysLabel.'"'; } $isPrinted++; } ?>],
										    datasets: [
										        {
