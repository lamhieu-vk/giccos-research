					...
					thisLink.attr('direct-disabled', 'true').bind('click', function (e) {
						...
						var getDisplayContent = function () {
							if (thisLink.attr('handing-link') != "true") {
								popupNotification ({type: 'notification', title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('content_from_site_loading', 'ucfirst'); ?>'});
										...
										if (isset(data['return'], data['data']) && data['return'] === true) {
											if (isset(data['data']['embed']) && typeof data['data']['embed'] == "object" && countArr(data['data']['embed']) > 0) {
												if (isset(data['data']['embed']['code']) && typeof data['data']['embed']['code'] == "string") {
													...
													popupNotification ({type: 'notification', title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('pushed_content_from_site', 'ucfirst'); ?>', global: true, audio: true});
												}
											}
										}else {
											//.
...
var status_getCodeMedia = function (object) {
	...
	if (object['media']['type'] == "link") {
		...
		for (var i in object['media']['data']) {
			...
			var infoShowObj = {"type": "link", "id": value['id'], "display": value['display'], "host": value['host'], "url": value['url'], "thumbnail": value['thumbnail'], "title": value['title'], "description": value['description'], "share": value['share']};
