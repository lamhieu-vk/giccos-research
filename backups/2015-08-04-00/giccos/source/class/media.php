...
Class media {
	...
	function replay ($format, $object = null) {
		...
		if ($format == "music" && is_array($object) && count($object) > 0) {
			...
			if ($action == "add") {
				...
				if (isset($getSql) && $getSql != null) {
					...
					if ($getNum == 0) {
						...
						return array("return" => true, "count" => $getNum, "data" => $getData);
					}
				}else {
					return array("return" => false, "reason" => "");
				}
				...
				if (isset($object['guy'], $object['guy']['type'], $object['guy']['id'])) {
					if (isset($object['guy']['type']) && (is_numeric($object['guy']['type']) || is_string($object['guy']['type']))) $guyType = $object['guy']['type']; else $guyType = null;
					if (isset($object['guy']['id']) && (is_numeric($object['guy']['id']) || is_string($object['guy']['id']))) $guyId = $object['guy']['id']; else $guyId = null;
					if ($guyType == null || $guyId == null) {
						return array("return" => false, "reason" => "");
				...
				$cleanRequest = "DELETE FROM `music_replay` WHERE `expires` <= '{$time}' AND `guy.type` = '{$guy['type']}' AND `guy.id` = '{$guy['id']}'";
				$cleanQuery = mysqli_query($db, $cleanRequest);
				if (!$cleanQuery) {
					return array("return" => false, "reason" => "");
				}else {
					$numRand = rand(10,100);
			...
			if ($action == "add") {
				...
				$cleanRequest = "DELETE FROM `videos_replay` WHERE `expires` <= '{$time}' AND `guy.type` = '{$guy['type']}' AND `guy.id` = '{$guy['id']}'";
				$cleanQuery = mysqli_query($db, $cleanRequest);
				if (!$cleanQuery) {
					return array("return" => false, "reason" => "");
				}else {
	...
	function exists ($format, $object = null) {
		...
		if ($format == "photos") {
			...
			if ($num == 0) {
				return array("return" => true, "exists" => false, "count" => 0, "id" => null);
			}else {
				$idFile = array();
				while ($fetchFile = mysqli_fetch_assoc($numCheck)) {
					$idFile[] = $fetchFile['id'];
				}
				return array("return" => true, "exists" => true, "count" => $num, "id" => $idFile);
			}
	...
	function data ($format, $object = null) {
		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "get") {
				...
				if (isset($object['label']) && (is_numeric($object['label']) || is_string($object['label']))) $label = $object['label']; else $label = null;
				if (isset($object['value']) && (is_numeric($object['value']) || is_string($object['value']))) $value = $object['value']; else $value = null;
				...
				$query = mysqli_query($db,  "SELECT {$rows} FROM `photos_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				...
				$query = mysqli_query($db, "SELECT * FROM `photos_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						$deleteInfo = mysqli_query($db, "DELETE FROM `photos_info` WHERE `id` = '{$fetch['id']}' {$push}");
						if (!$deleteViews || $deleteInfo) {
							$stats[] = array("id" => $fetch['id'], "error" => false);
						}else {
							$stats[] = array("id" => $fetch['id'], "error" => true);
						}
					}
					return array("return" => true, "stats" => $stats);
				}
			}
			...
			if ($action == "get") {
				...
				$query = mysqli_query($db, "SELECT {$rows} FROM `music_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
				...
				$query = mysqli_query($db, "SELECT * FROM `music_info` WHERE `id` = '{$id}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						$deleteInfo = mysqli_query($db,  "DELETE FROM `music_info` WHERE `id` = '{$fetch['id']}' {$push}");
						if (!$deleteViews || $deleteInfo) {
							$stats[] = array("id" => $fetch['id'], "error" => false);
						}else {
							$stats[] = array("id" => $fetch['id'], "error" => true);
			...
			if ($action == "get") {
				...
				$query = mysqli_query($db,  "SELECT {$rows} FROM `videos_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
				...
				if ($private == true) {
					if (isset($object['author']) && is_array($object['author'])) $author = $object['author']; else $author = null;
					if (!isset($author) || !isset($author['type']) || !isset($author['id'])) {
						return array("return" => false, "reason" => "");
					}
					$push = "AND `author.type` = '{$author['type']}' AND `author.id` = '{$author['id']}'";
				...
				$query = mysqli_query($db, "SELECT * FROM `videos_info` WHERE `{$label}` = '{$value}' {$push} {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						...
						if ($fetch['path.sd'] != '0') {
							$_storage->delete(array("format" => "video", "label" => "display", "value" => $fetch['path.sd']));
						}
						if ($fetch['path.thumbnail'] != '0') {
							$_storage->delete(array("format" => "image", "label" => "token", "value" => $fetch['path.thumbnail']));
						}
						$deleteViews = mysqli_query($db, "DELETE FROM `videos_views` WHERE `videos.id` = '{$fetch['id']}'");
						$deleteInfo = mysqli_query($db, "DELETE FROM `videos_info` WHERE `id` = '{$fetch['id']}' {$push}");
						if (!$deleteViews || $deleteInfo) {
							$stats[] = array("id" => $fetch['id'], "error" => false);
						}else {
							$stats[] = array("id" => $fetch['id'], "error" => true);
		...
		if ($format == "photos" && is_array($object) && count($object) > 0) {
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					...
					$secret = hash('crc32', '('.$modeType.'_'.$modeId.')::'.md5_file($file['tmp_name']));
					$nameraw = $file['name'];
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $nameraw != null && $mime != null) {
						if (isset($file['error']) && $file['error'] != 0) {
							return array("return" => false, "reason" => "");
						}else if (!isset($file['tmp_name']) || $file['tmp_name'] == null) {
							return array("return" => false, "reason" => "");
						}
						$fileUpload = $_storage->upload(array("format" => "image", "move" => "copy", "file" => $file['tmp_name'], "mime" => $file['type'], "name" => $file['name'], "strip" => false));
						...
						$insertQuery = "INSERT INTO `photos_cache` (`id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `copy`, `mime`, `size`, `path`, `user.id`, `author.id`, `author.type`) VALUES (null, '{$_tool->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$copy}', '{$mime}', '{$size}', '{$path}', '{$userId}', '{$author['id']}', '{$author['type']}')";
				...
				if (mysqli_num_rows($original_query) > 0 && mysqli_num_rows($copy_query) == 0) {
					...
					if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
						$original['tmp'] = $getMediaFile['file'][0]['source'];
					}else {
						return array("return" => false, "reason" => "");
					}
					...
					if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
						$copy['tmp'] = $getMediaFile['file'][0]['source'];
					}else {
						return array("return" => false, "reason" => "");
					}
					return array(
				...
				$request = "SELECT {$rows} FROM `photos_cache` WHERE `{$label}` = '{$value}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' {$limit}";
				$query = mysqli_query($db, $request);
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => $request);
				}else {
					$mediaData = array();
					while ($imgFetch = mysqli_fetch_assoc($query)) {
						$mediaDataThis = $imgFetch;
						$getMediaFile = $_storage->get(array("format" => "image", "label" => "display", "value" => $imgFetch['path'], "rows" => "`token`, `path`", "limit" => "LIMIT 1"));
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							$mediaDataThis['tmp'] = $getMediaFile['file'][0]['source'];
						}else {
							continue;
						}
						$mediaData[] = $mediaDataThis;
					}
					return array("return" => true, "data" => $mediaData);
				}
				...
				if ($name != null) {
					$set = $object['set'];
					$author = $modeArr;
					$imgCopy_query = mysqli_query($db,"SELECT `copy`, `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$name}' AND `author.id` = '{$modeId}' AND `author.type` = '{$modeType}' AND `user.id` = '{$userId}' AND `copy` != '0'");
					if (mysqli_num_rows($imgCopy_query) > 0) {
						$imgCopy = mysqli_fetch_assoc($imgCopy_query);
						$imgOriginal_query = mysqli_query($db, "SELECT `path`, `token`, `secret`, `name`, `nameraw` FROM `photos_cache` WHERE `name` = '{$imgCopy['copy']}' AND `author.id` = '{$modeId}' AND `author.type` = '{$modeType}' AND `user.id` = '{$userId}'");
				...
				if ($secret != null && $name != null && ($verify == true || $verify == false)) {
					if (!$verify) {
						$pushSql = " AND `close` != '0'";
					}else {
						$pushSql = "";
					}
					$sql = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE ((`secret` = '{$secret}' AND `name` = '{$name}') OR (`copy` = '{$name}')) AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}' AND `user.id` = '{$userId}' {$pushSql}";
					...
					while ($fetch = mysqli_fetch_assoc($query)) {
						$next = 0;
						if (count($f_removed) > 0) {
							foreach ($f_removed as $key => $f_removed_this) {
								if (isset($f_removed_this['secret'], $fetch['secret'])) {
									//.
								}else {
									unset($f_removed[$key]);
									continue;
								}
								if ($f_removed_this['secret'] == $fetch['secret'] && $f_removed_this['name'] != $fetch['name']) {
									$next++;
									break;
								}
							}
						...
						if ($next == 0) {
							$f_removed = array("secret" => $fetch['secret'], "name" => $fetch['name']);
						}else {
							continue;
						}
						$f_name[] = $fetch['name'];
						$f_secret[] = $fetch['secret'];
						...
						if (isset($delMediaFile['return']) && $delMediaFile['return'] == true) {
							...
							$photosLikeThisQuery = mysqli_query($db, "SELECT `id` FROM `photos_cache` WHERE `secret` = '{$fetch['secret']}' AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}' AND `user.id` = '{$userId}'");
							if ($photosLikeThisQuery) $photosLikeThisNum = mysqli_num_rows($photosLikeThisQuery); else $photosLikeThisNum = 0;
							$photosFacesQuery = mysqli_query($db, "SELECT `id`, `photos`, `faces` FROM `photos_faces` WHERE `photos` = '{$fetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}' AND `user.id` = '{$userId}'");
					...
					if (isset($f_name, $_SESSION["cache"]['photos_editor_handling']) && is_array($f_name) && count($f_name) > 0 && is_array($_SESSION["cache"]['photos_editor_handling']) && count($_SESSION["cache"]['photos_editor_handling']) > 0) {
						$cacheEditorHandling = $_SESSION["cache"]['photos_editor_handling'];
						if (is_array($cacheEditorHandling) && $cacheEditorHandling != null) {
							...
							$_SESSION["cache"]['photos_editor_handling'] = $cacheEditorHandling;
						}
					}
					if (isset($f_secret, $_SESSION["cache"]['photos_faces']) && is_array($f_secret) && count($f_secret) > 0 && is_array($_SESSION["cache"]['photos_faces']) && count($_SESSION["cache"]['photos_faces']) > 0) {
						$cacheFaces = $_SESSION["cache"]['photos_faces'];
						foreach ($cacheFaces as $key => $cacheFaceThis) {
							if (isset($cacheFaceThis['photos'], $cacheFaceThis['photos.local'], $cacheFaceThis['user.id'], $cacheFaceThis['author.type'], $cacheFaceThis['author.id']) && in_array($cacheFaceThis['photos'], $f_secret) && $cacheFaceThis['photos.local'] == "cache" && $cacheFaceThis['user.id'] == $userId && $cacheFaceThis['author.type'] == $modeType && $cacheFaceThis['author.id'] == $modeId) {
								unset($cacheFaces[$key]);
							}else {
								continue;
							}
						}
						$_SESSION["cache"]['photos_faces'] = $cacheFaces;
					}
					if (isset($f_secret, $_SESSION["cache"]['photos_faces_handling']) && is_array($f_secret) && count($f_secret) > 0 && is_array($_SESSION["cache"]['photos_faces_handling']) && count($_SESSION["cache"]['photos_faces_handling']) > 0) {
						$cacheFacesHandling = $_SESSION["cache"]['photos_faces_handling'];
						foreach ($cacheFacesHandling as $key => $cacheFacesHandlingThis) {
							if (isset($cacheFacesHandlingThis['photos'], $cacheFacesHandlingThis['photos.local'], $cacheFacesHandlingThis['user.id'], $cacheFacesHandlingThis['author.type'], $cacheFacesHandlingThis['author.id']) && in_array($cacheFacesHandlingThis['photos'], $f_secret) && $cacheFacesHandlingThis['photos.local'] == "cache" && $cacheFacesHandlingThis['user.id'] == $userId && $cacheFacesHandlingThis['author.type'] == $modeType && $cacheFacesHandlingThis['author.id'] == $modeId) {
								unset($cacheFacesHandling[$key]);
							}else {
								continue;
							}
						}
						$_SESSION["cache"]['photos_faces_handling'] = $cacheFacesHandling;
					}
					return array("return" => true);
				}else {
					return array("return" => false, "reason" => "");
				...
				$time = time();
				$request = "SELECT `id`, `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE (`client` = '{$_client->client(true)}' OR (`author.id` = '{$modeId}' AND `author.type` = '{$modeType}')) AND `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}'";
				$query = mysqli_query($db, $request);
				$f_removed = $f_name = $f_secret = $f_data = array();
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_data[] = $fetch;
				}
				$randLimit = rand($_parameter->get('cacheClean_by_user_limit_rand_min'), $_parameter->get('cacheClean_by_user_limit_rand_max'));
				$request = "SELECT `name`, `path`, `token`, `secret` FROM `photos_cache` WHERE `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}' LIMIT {$randLimit}";
				$query = mysqli_query($db, $request);
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_data[] = $fetch;
				}
				foreach ($f_data as $key => $fetch) {
					$next = 0;
					if (count($f_removed) > 0) {
						foreach ($f_removed as $key => $f_removed_this) {
							if ($f_removed_this['secret'] == $fetch['secret'] && $f_removed_this['name'] != $fetch['name']) {
				...
				if ($name != null && $secret != null && is_array($modeArr)) {
					$client = $_client->client(true);
					$query = mysqli_query($db, "SELECT * FROM `photos_cache` WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}' AND `user.id` = '{$userId}'");
					if ($query) $num = mysqli_num_rows($query); else $num = 0;
					if ($num == 0) {
						return array("return" => false, "reason" => "");
					}else {
						...
						$time = time();
						$close = $time + $_parameter->get('media_cache_time_close');
						$fileQuery = "UPDATE `photos_cache` SET `close` = '{$close}' WHERE `secret` = '{$secret}' AND `name`= '{$name}' AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}'";
						...
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							$file['tmp'] = $getMediaFile['file'][0]['source'];
						...
						if (in_array($file['ext'], $_parameter->get('photos_allow_format', true))) {
							...
							while ($fileFetch = mysqli_fetch_assoc($getFileQuery)) {
								...
								mysqli_query($db, "UPDATE `photos_faces` SET `photos` = '{$file['display']}', `photos.local` = 'drive' WHERE `photos` = '{$fileFetch['secret']}' AND `photos.local` = 'cache' AND `author.type` = '{$modeType}' AND `author.id` = '{$modeId}' AND `user.id` = '{$userId}'");
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					...
					$size = $file['size'];
					// $thumbnail = $_tool->links('photos/raw/static/music-cover-notfound.jpg');
					if (isset($file['error']) && $file['error'] != 0) {
						return array("return" => false, "reason" => "");
					}else if (!isset($file['tmp_name']) || $file['tmp_name'] == null) {
						return array("return" => false, "reason" => "");
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $mime != null && $tmp != null && $path != null) {
						$insertSql =  "INSERT INTO `music_cache` (`id`, `user.id`, `author.type`, `author.id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `copy`, `mime`, `size`, `duration`, `path`) VALUES (null, '{$userId}', '{$author['type']}', '{$author['id']}', '{$_tool->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$copy}', '{$mime}', '{$size}', '{$duration}', '{$path}')";
				...
				$query = mysqli_query($db, "SELECT {$rows} FROM `music_cache` WHERE `{$label}` = '{$value}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
					$mediaData = array();
					while ($musikFetch = mysqli_fetch_assoc($query)) {
						$musikDataThis = $musikFetch;
						$getMediaFile = $_storage->get(array("format" => "audio", "label" => "display", "value" => $musikFetch['path'], "rows" => "`token`, `path`", "limit" => "LIMIT 1"));
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							$musikDataThis['tmp'] = $getMediaFile['file'][0]['source'];
						}else {
							continue;
						}
						$mediaData[] = $musikDataThis;
					}
					return array("return" => true, "data" => $mediaData);
				}
			}else if ($action == "replace") {
				...
				if (isset($object['name']) && is_string($object['name'])) $key = addslashes($object['name']); else $key = null;
				if (isset($object['verify']) && is_bool($object['verify'])) $verify = $object['verify']; else $verify = true;
				if ($key != null && ($verify == true || $verify == false)) {
					...
					$selectRequest = "SELECT `id`, `name`, `path`, `token` FROM `music_cache` WHERE `name` = '{$key}' OR `copy` = '{$key}' {$pushSql}";
					...
					if (isset($f_name, $_SESSION["cache"]['musicHandling']) && count($f_name) > 0) {
						...
						if (is_array($s_cache) && $s_cache != null) {
							...
							$_SESSION["cache"]['photos_editor_handling'] = $s_cache;
						}
					}
					return array("return" => true);
				}else {
				...
				$client = $_client->client(true);
				$author = $modeArr;
				$query = mysqli_query($db, "SELECT `id`, `name`, `path`, `token` FROM `music_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}')) AND `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}'");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "audio",
				...
				$randLimit = rand($_parameter->get('cacheClean_by_user_limit_rand_min'), $_parameter->get('cacheClean_by_user_limit_rand_max'));
				$cacheServerQuery = mysqli_query($db, "SELECT `name`, `path`, `token` FROM `music_cache` WHERE `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}' LIMIT {$randLimit}");
				...
				if ($name != null && $secret != null && is_array($author)) {
					...
					if (mysqli_num_rows($query) == 0) {
						...
						if (in_array($file['ext'], $_parameter->get('music_allow_format', true))) {
							...
							while ($fileFetch = mysqli_fetch_assoc($file_query)) {
			...
			if ($action == "add") {
				if (isset($object['file']) && !empty($object['file']) && is_array($object['file'])) {
					...
					if ($time != null && $close != null && $client != null && $token != null && $secret != null && $name != null && $mime != null && $path != null) {
						$sql = "INSERT INTO `videos_cache` (`id`, `user.id`, `author.type`, `author.id`, `time`, `close`, `client`, `token`, `secret`, `name`, `nameraw`, `thumbnail`, `thumbnail.path`, `copy`, `mime`, `size`, `duration`, `path`) VALUES (null, '{$userId}', '{$author['type']}', '{$author['id']}', '{$_tool->timeNow()}', '{$close}', '{$client}', '{$token}', '{$secret}', '{$name}', '{$nameraw}', '{$thumbnail}', null, '{$copy}', '{$mime}', '{$size}', '{$duration}', '{$path}')";
						$insert = mysqli_query($db, $sql);
						if (!$insert) {
							return array("return" => false, "reason" => "");
						}else {
				...
				if (mysqli_num_rows($original_query) > 0 && mysqli_num_rows($copy_query) == 0) {
					...
					if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
						$original['thumbnail.path'] = $getMediaFile['file'][0]['source'];
					}else {
						return array("return" => false, "reason" => "");
					}
					$copy['ext'] = $original['ext'] = strtolower(pathinfo($original['name'], PATHINFO_EXTENSION));
				...
				$query = mysqli_query($db, "SELECT {$rows} FROM `videos_cache` WHERE `{$label}` = '{$value}' AND `author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}' {$limit}");
				if ($query) $num = mysqli_num_rows($query); else $num = 0;
				if ($num == 0) {
					return array("return" => false, "reason" => "");
				}else {
					$vioData = array();
					while ($vioFetch = mysqli_fetch_assoc($query)) {
						$vioDataThis = $vioFetch;
						$getMediaFile = $_storage->get(array("format" => "video", "label" => "display", "value" => $vioFetch['path'], "rows" => "`token`, `path`", "limit" => "LIMIT 1"));
						if (isset($getMediaFile['return'], $getMediaFile['count'], $getMediaFile['file']) && $getMediaFile['return'] == true && $getMediaFile['count'] > 0) {
							$vioDataThis['tmp'] = $getMediaFile['file'][0]['source'];
						}else {
							continue;
						}
						$vioData[] = $vioDataThis;
					}
					return array("return" => true, "data" => $vioData);
				}
			}else if ($action == "replace") {
				if (isset($object['name']) && $object['name'] != null) {
					$set = $object['set'];
				...
				if ($key != null && ($verify == true || $verify == false)) {
					...
					$selectRequest = "SELECT `id`, `name`, `path`, `token` FROM `videos_cache` WHERE `name` = '{$key}' OR `copy` = '{$key}' {$pushSql}";
					$selectQuery = mysqli_query($db, $selectRequest);
					while ($fetch = mysqli_fetch_assoc($selectQuery)) {
						$f_name[] = $fetch['name'];
						$delMediaFileOpts = array(
				...
				$client = $_client->client(true);
				$author = $modeArr;
				$query = mysqli_query($db, "SELECT `id`, `name`, `path`, `token` FROM `videos_cache` WHERE (`client` = '{$client}' OR (`author.id` = '{$author['id']}' AND `author.type` = '{$author['type']}')) AND `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}'");
				while ($fetch = mysqli_fetch_assoc($query)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "video",
				...
				$randLimit = rand($_parameter->get('cacheClean_by_user_limit_rand_min'), $_parameter->get('cacheClean_by_user_limit_rand_max'));
				$cacheServerQuery = mysqli_query($db, "SELECT `name`, `path`, `token` FROM `videos_cache` WHERE `close` != '0' AND `close` - '{$time}' >= '{$_parameter->get('media_cache_time_close')}' LIMIT {$randLimit}");
				while ($fetch = mysqli_fetch_assoc($cacheServerQuery)) {
					$f_name[] = $fetch['name'];
					$delMediaFileOpts = array(
						"format" => "video",
					...
					if (mysqli_num_rows($query) == 0) {
						...
						if (in_array($file['ext'], $_parameter->get('videos_allow_format', true))) {
							...
							if (!$insert) {
								...
								while ($fileFetch = mysqli_fetch_assoc($file_query)) {
									$_storage->delete(array("format" => "image", "label" => "display", "value" => $fileFetch['thumbnail']));
