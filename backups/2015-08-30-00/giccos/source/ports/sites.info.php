...
if (isset($siteDisplay) && is_string($siteDisplay) && preg_match("/^([a-zA-Z0-9]{30,40})$/", $siteDisplay)) {
	$siteInfoArr = array();
	$existsSite = $_sites->info(array("guy" => $g_user['mode'], "action" => "exists", "label" => "display", "value" => $siteDisplay));
	if (isset($existsSite, $existsSite['return'], $existsSite['exists']) && $existsSite['return'] == true) {
		if ($existsSite['exists'] == true) {
			//. get information
			$getInfo = $_sites->info(array("guy" => $g_user['mode'], "action" => "get", "label" => "display", "value" => $siteDisplay, "limit" => "LIMIT 1"));
			...
			//. get certificate
			if (is_scalar($siteInfoArr['certificate']) && preg_match("/^([a-zA-Z0-9]{30,40})$/", $siteInfoArr['certificate'])) {
			...
			//. get scan
			if (is_scalar($siteInfoArr['scan']) && preg_match("/^([a-zA-Z0-9]{30,40})$/", $siteInfoArr['scan'])) {
				...
				<div id="centerBox">
					...
					<div id="infoPage" class="boxGrid">
							...
							<div class="content">
								<div class="rows table">
									<div class="value"> 
										<?php
										for ($i = 1; $i <= 7; $i++) {
											$statsView['this_week'][strtolower($_tool->convertWeekday($i, true))] = 0;
											$statsView['last_week'][strtolower($_tool->convertWeekday($i, true))] = 0;
										}
										foreach ($siteInfoArr['stats']['views'] as $statsTypeTime => $statsThisTime) {
											if ($statsTypeTime == "this_week") {
												foreach ($statsThisTime as $statusLi => $thisView) {
													$weekdaysThis = strtolower($_tool->convertWeekday(date("N", $thisView['time']), true));
													if (!isset($statsView['this_week'][$weekdaysThis]) || $statsView['this_week'][$weekdaysThis] == null) {
														$statsView['this_week'][$weekdaysThis] = 0;
													}
													$statsView['this_week'][$weekdaysThis]++;
												}
												$statsView['this_week'][$_language->text('today', 'strtolower')] = $statsView['this_week'][$weekdaysThis];
												unset($statsView['this_week'][$weekdaysThis]);
											}else if ($statsTypeTime == "last_week") {
												foreach ($statsThisTime as $statusLi => $thisView) {
													$weekdaysThis = strtolower($_tool->convertWeekday(date("N", $thisView['time']), true));
													if (!isset($statsView['last_week'][$weekdaysThis]) || $statsView['last_week'][$weekdaysThis] == null) {
														$statsView['last_week'][$weekdaysThis] = 0;
													}
													$statsView['last_week'][$weekdaysThis]++;
												}
											}
										}
										?>
										...
										var datasetOptions = {
											labels: [<?php $isPrinted = 0; foreach ($statsView['last_week'] as $weekdaysLabel => $weekdaysValue) { if ($isPrinted == 0) { print '"'.$weekdaysLabel.'"'; } else { print ', "'.$weekdaysLabel.'"'; } $isPrinted++; } ?>],
										    datasets: [
										        {
										            ...
										            data: [<?php $isPrinted = 0; foreach ($statsView['last_week'] as $viewsCount) { if ($isPrinted == 0) { print $viewsCount; } else { print ", ".$viewsCount; } $isPrinted++; } ?>]
										        },
										        {
										            ...
										            pointHighlightStroke: "rgba(7, 197, 190, 1)",
										             data: [<?php $isPrinted = 0; foreach ($statsView['this_week'] as $viewsCount) { if ($isPrinted == 0) { print $viewsCount; } else { print ", ".$viewsCount; } $isPrinted++; } ?>]
										        }
										    ]
										};
										dataCache.add("sites_info_stats_dataset", datasetOptions);
										</script>
									</div>
								</div>
							</div>
						...
						<div class="comment box">
							<div class="title">
								<span><?php print $_language->text('comment(s)', 'ucfirst'); ?></span>
							</div>
							<div class="content">
								<br>
							</div>
