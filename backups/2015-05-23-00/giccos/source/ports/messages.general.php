					...
					<div id="messagesData" class="boxGrid">
						...
						<div class="content">
							...
							if ($messagesIdCount == 0) {
								...
								//.
								$codeLinesTab = null;
								$messagesDataLastReuqest = "SELECT DISTINCT `tab` FROM `messages_data` WHERE `tab` IN (SELECT `tab` FROM `messages_members` WHERE `guy.type` = '{$g_user['mode']['type']}' AND `guy.id` = '{$g_user['mode']['id']}') ORDER BY `id` DESC";
								...
								while ($messagesDataLastFetch = mysqli_fetch_assoc($messagesDataLastQuery)) {
									$getMessagesTab = $_messages->tab(array("action" => "get", "label" => "id", "value" => $messagesDataLastFetch['tab'], "limit" => "LIMIT 1"));
									if (isset($getMessagesTab['return'], $getMessagesTab['tab'][0]) && $getMessagesTab['return'] == true) {
										...
										if (isset($getMessagesMembers['return'], $getMessagesMembers['data']) && $getMessagesMembers['return']) {
											...
											foreach ($getMessagesMembers['data'] as $key => $getMessagesMembersThis) {
												...
												if (in_array($getMessagesMembersThis['guy.type'], ["user", "users"])) {
													$getMemberThis = $_user->getInfo(array("rows" => "`avatar.small`", "label" => "id", "value" => $getMessagesMembersThis['guy.id']));
											...
											if (isset($getMessagesContent['return'], $getMessagesContent['data'][0]) && $getMessagesContent['return'] == true) {
												$codeLinesContent = "
												<div class='text'>
													<span>".$_tool->parseContent($getMessagesContent['data'][0]['content']['text'])."</span>
												</div>
										...
										$codeLinesTab .=  "
										<div class='lines tab' display='{$messagesTabInfo['display']}' tab='{$messagesTabInfo['json']}' direct='".json_encode(array("link" => $_tool->links('/messages/private/'.$messagesTabInfo['display']), "by" => "dblclick", "target" => "_blank"))."'>
											<div class='header'>
												<div class='name'>
