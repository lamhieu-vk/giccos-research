...
function PhotosTagPopup (object) {
	...
	if (tagbarActive == true && tagbar != '') {
		...
		if (object['data']['faces'] !== undefined) {
	...
	function pushTagFaces (faces) {
		for (var i in faces) {
			...
			if (thisFace['guy.type'] === null || thisFace['guy.id'] === null || thisFace['guy.name'] === null) {
				...
				typeNameThis.html("<span class='tag' tag='info' tag-type='"+thisFace['guy.type']+"' tag-id='"+thisFace['guy.id']+"'><a tag-name>"+thisFace['guy.name']+"</a><i tag-close></i></span>");
				typeNameThis.find("[tag-close]").bind('click', function(){
					...
					typeNameThis.children("[input]").AutoComplete({'path': 'info', 'from': 'users', 'output': true, 'multiple': false, callback: callbackFunc});
				});
			}
		...
		var actionWhenClick = function() {
			...
			if (thisTag.attr('handling-typing') != "true") {
				if (typeNameThis.length == 0) {
					...
					thisNameType.children("[input]").AutoComplete({'path': 'info', 'from': 'users', 'output': true, 'multiple': false, callback: callbackFunc});
					setTimeout(function(){
                ...
                if (Object.keys(file).length > 0) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            console.log(data);
                            if (isset(data['return']) && data['return'] === true) {
                                if (isset(data['data']) && data['data'] !== null && typeof data['data'] == "object" && Object.keys(data['data']).length > 0) {
                                    for (var i in data['data']) {
                                        var thisData = data['data'][i];
                                        if (isset(thisdata['return']) && thisdata['return'] === true) {
                                            upload.find("[music-preview][file]").each(function() {
...
function editor_mediaPreview () {
	...
	if(editor.attr('type') == "photos"){
		...
		form.ajaxForm({
			...
			success: function(data) {
			    console.log(data);
			    if (isset(data['return']) && data['return'] === true && data['data'] !== null && Object.keys(data['data']).length > 0) {
			    	var objects = {
			    		'format': 'photos', 
			    		'type': 'preview', 
			    		'data': data['data']
...
function editor_upload (object) {
	...
	if (editor.attr('handling-preview') == "true") {
		popupNotification ({type: 2, title: '<?php print $_language->text("error", "ucfirst"); ?>', description: transferError('ERROR#FEEDS_013')+'.', from: '<?php print $_language->text("system", "ucfirst"); ?>', timeout: '<?php print $_parameter->get("notifyPopup_timeout"); ?>'});
		var error = true;
	}else if (object['data']['type'] == "status" && object['data']['content'] === null) {
		popupNotification ({type: 2, title: '<?php print $_language->text("error", "ucfirst"); ?>', description: transferError('ERROR#FEEDS_010')+'.', from: '<?php print $_language->text("system", "ucfirst"); ?>', timeout: '<?php print $_parameter->get("notifyPopup_timeout"); ?>'});
		var error = true;
	}else if (object['data']['type'] == "music" && object['data']['media']['info']['value'] === false) {
		popupNotification ({type: 2, title: '<?php print $_language->text("error", "ucfirst"); ?>', description: transferError('ERROR#FEEDS_011')+'.', from: '<?php print $_language->text("system", "ucfirst"); ?>', timeout: '<?php print $_parameter->get("notifyPopup_timeout"); ?>'});
		var error = true;
