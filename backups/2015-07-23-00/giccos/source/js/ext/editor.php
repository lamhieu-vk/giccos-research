		...
		if (statusType != boxMediaOtherOld && (statusType == "status" || statusType == "photos" || statusType == "music" || statusType == "videos" || statusType == "cited")) {
			...
			if (statusType == "status") {
				...
				textarea.donetyping(function() {
					...
					var regexpURL = new RegExp(<?php print $_parameter->get('regex_sites_url'); ?>);
...
var editor_upload = function (object) {
	...
	if (error !== true && editor.attr('handing-upload') !== "true") {
		...
		$.ajax({
			...
			success: function(data) {
				if (isset(data['return']) && data['return'] === true) {
					popupNotification ({type: 1, title: '<?php print $_language->text('notification', 'ucfirst'); ?>', description: '<?php print $_language->text('status_has_been_posted','ucfirst'); ?>. <br /> <?php print $_language->text('pull_up_for_updates','ucfirst'); ?>.'});
					editor_resetForm();
