...
Class messages {
	...
	function tab ($object) {
		...
		if ($action == "add" || $action == "create") {
			...
			if ($getTabRequest == true && mysqli_num_rows($getTabQuery) > 0) {
				...
				return array("return" => true, "tab" => $tabArr);
			}else if ($getTabRequest == true && mysqli_num_rows($getTabQuery) == 0) {
				return array("return" => true, "tab" => array());
			}else {
				return array("return" => false, "reason" => "");
			}
	...
	function members ($object) {
		...
		if ($action == "add") {
			...
			$getMembersRequest = "SELECT {$membersRows} FROM `messages_members` WHERE `{$membersLabel}` = '{$membersValue}' {$limit}";
	...
	function data ($object) {
		...
		if ($action == "check") {
			...
			if ($dataRows == null || $dataSort == null || $dataLabel == null || $dataValue == null) {
				return array("return" => false, "reason" => "");
			}
			$getDataRequest = "SELECT * FROM `messages_members` WHERE `{$dataLabel}` {$dataSort} '{$dataValue}' {$dataOrder} {$dataLimit}";
			...
			if (!$getDataQuery) {
				...
				if ($getDataCount == 0) {
					...
					while ($getDataFetch = mysqli_fetch_assoc($getDataQuery)) {
						...
						$getDataFetch['content']['text'] = $getDataFetch['content']['original'] = $getDataFetchContent;
						...
						if ($getDataFetch['author']['type'] != $authorType || $getDataFetch['author']['id'] != $authorId) {
							$getMembers = $this->members(array("action" => "get", "rows" => "`translate`", "label" => "tab", "value" => $tabId, "author" => array("type" => $getDataFetch['author']['type'], "id" => $getDataFetch['author']['id'])));
							if (isset($getMembers['return'], $getMembers['data'][0]['translate']) && $getMembers['return'] == true && $getMembers['data'][0]['translate'] != "0") {
								$authorLanguage = $getMembers['data'][0]['translate'];
							}else {
								$authorLanguage = 0;
							}
							...
							if (isset($authorLanguage) && $authorLanguage != "0" && $userLanguage != null && $authorLanguage != $userLanguage) {
								if ($getDataFetch['content'] != null && is_string($getDataFetch['content'])) {
									$translateContent = $_language->translate($userLanguage, $getDataFetch['content'], $authorLanguage);
									if (isset($translateContent['return']) && $translateContent['return'] == true) {
										$getDataFetch['content']['text'] = $translateAction['data'];
