...
Class user {
	...
	function setup () {
		...
		if (!$this->ready) {
			$_SESSION["user"] = array("login" => false, "id" => 0, "mode" => array("type" => "guy", "id" => $this->class['_client']->client(true)));
		}
	}
	function relogin () {
		if (isset($_COOKIE["AccRemember"])) {
			...
			if ($num > 0) {
				...
				return $login = $this->login(array("type" => 2, "user" => array("id" => $id), "remember" => true, "auto" => true));
	...
	function mode ($type, $id) {
		if ($this->ready == true) {
			if (isset($type, $id) && in_array($type, array("user"))) {
		...
		if (is_array($user)) {
			...
			if (!preg_match($regexUsername, $user["username"])) {
				...
				if ($username_num > 0) {
					...
					$secret = md5($time."_".rand()."_".hash('crc32', rand()));
	...
	function info ($type, $object) {
		if (isset($this->ready) && $this->ready == true) {
			...
			if ($type == "update") {
				if (isset($object['rows'])) $rows = $object['rows']; else $rows = null;
				if (isset($object['value'])) $value = $object['value']; else $value = null;
				$update = mysqli_query($this->class['_db']->port('beta'),"UPDATE `users` SET `{$rows}` = '{$value}' WHERE `id` = '{$this->user['id']}'");
				if (!$update) {
					return array("return" => false, "reason" => "");
				}else {
					return array("return" => true);
	...
	function suggest ($object) {
		...
		if ($this->ready == true && $id > 0 && $type != null) {
			...
			if ($type == "guys" || $type == "users") {
				...
				$getOther = true;
				$guy = mysqli_fetch_assoc(mysqli_query($db,"SELECT `id`, `units.temperature`, `live`, `country` FROM `users` WHERE `id` = '{$id}'"));
