...
Class client {
	...
	function location ($object = array()) {
		...
		//.
		$userId = isset($object, $object['user'], $object['user']['id']) && is_numeric($object['user']['id']) ? intval($object['user']['id']) : null;
		$guyType = isset($object, $object['guy'], $object['guy']['type']) ? $_tool->convertDatabaseString($object['guy']['type']) : null;
		$guyId = isset($object, $object['guy'], $object['guy']['id']) ? $_tool->convertDatabaseString($object['guy']['id']) : null;
		if ($userId == null || $guyType == null || $guyId == null) {
			return array("return" => false, "reason" => "sd");
		}
		//.
		$clientLocation = array();
		if (isset($_SESSION["client"], $_SESSION["client"]['location'])) {
			$clientLocation = $_SESSION["client"]['location'];
		}
		$statusPlacesRequest = "SELECT `time`, `value` FROM `status_places` WHERE `time` + '".$_parameter->get('client_location_timeout')."' > '".time()."' AND `inside.type` = 'status' AND `inside.id` IN (SELECT `id` FROM `status` WHERE `user.id` = '".$userId."') ORDER BY `id` DESC LIMIT 1;";
		$statusPlacesQuery = mysqli_query($db, $statusPlacesRequest);
		...
		$getUserProfile = $_user->profile(array("action" => "get", "rows" => ["live", "country"], "label" => "id", "value" => $userId, "limit" => "LIMIT 1"));
		if (isset($getUserProfile['return'], $getUserProfile['data'], $getUserProfile['data'][0]) && $getUserProfile['return'] == true) {
			if (isset($getUserProfile['data'][0]['live']) && is_numeric($getUserProfile['data'][0]['live'])) {
				$userProfile['live'] = $getUserProfile['data'][0]['live'];
			}
		}
		if ((isset($userProfile['live']) && is_string($userProfile['live'])) && (!isset($placesLocation) || $placesLocation == null)) {
			$placesLocation = $userProfile['live'];
		}
		if (count($clientLocation) == 0) {
			if (isset($placesLocation) && (is_string($placesLocation) || is_numeric($placesLocation))) {
				if (is_numeric($placesLocation)) {
					$getPlaces = $_maps->places(array("guy" => array("type" => $guyType, "id" => $guyId), "action" => "get", "label" => "id", "value" => $placesLocation, "limit" => "LIMIT 1"));
					if (isset($getPlaces, $getPlaces['return'], $getPlaces['data'], $getPlaces['data'][0]) && $getPlaces['return'] == true) {
						$dataMapsPlaces = $getPlaces['data'][0];
						$clientLocation = array();
						//. $clientLocation['expires'] = time() + $_parameter->get('client_location_timeout');
						if (isset($dataMapsPlaces['name'])) {
							$clientLocation['name'] = $dataMapsPlaces['name'];
						}
						if (isset($dataMapsPlaces['address'])) {
							$clientLocation['address'] = $dataMapsPlaces['address'];
						}
						if (isset($dataMapsPlaces['country'])) {
							$clientLocation['country'] = $dataMapsPlaces['country'];
						}
						if (isset($dataMapsPlaces['coordinates'])) {
							$clientLocation['coordinates'] = $dataMapsPlaces['coordinates'];
						}
					}
					...
					//. $clientLocation['expires'] = time() + $_parameter->get('client_location_timeout');
					if (isset($dataMapsPlaces['address'])) {
						$clientLocation['address'] = $dataMapsPlaces;
					}
				}
			}
		}
		if (!isset($clientLocation['address']) || !isset($clientLocation['country'])) {
			$getIP = $_ip->info(array("action" => "check", "ip" => $this->ip()));
			if (isset($getIP, $getIP['return'], $getIP['data']) && $getIP['return'] == true) {
				...
				if (isset($dataIpPlaces['city']) || isset($dataIpPlaces['region']) || isset($dataIpPlaces['country'])) {
					$clientLocation['address'] = null;
					if (isset($dataIpPlaces['city'])) {
						$clientLocation['address'] .= $dataIpPlaces['city'];
					}
					if (isset($dataIpPlaces['region'])) {
						if ($clientLocation['address'] != null) {
							$clientLocation['address'] .= ", ".$dataIpPlaces['region'];
						}else {
							$clientLocation['address'] .= $dataIpPlaces['region'];
						}
					}
					if (isset($dataIpPlaces['country'])) {
						if ($clientLocation['address'] != null) {
							$clientLocation['address'] .= ", ".$dataIpPlaces['country'];
						}else {
							$clientLocation['address'] .= $dataIpPlaces['country'];
						}
					}
				...
				if (isset($dataIpPlaces['country'])) {
					$clientLocation['country'] = $dataIpPlaces['country'];
				}
				if (isset($dataIpPlaces['latitude'], $dataIpPlaces['longitude'])) {
					$clientLocation['coordinates'] = $dataIpPlaces['latitude'].",".$dataIpPlaces['longitude'];
				}
			}
		}
		$clientLocation['reset'] = false;
		return array("return" => true, "data" => $clientLocation);
	}
	...
	function browser () { 
		...
		if (preg_match("/(iphone|ipod|ipad)/i", $u_agent) && in_array($device, ["mobile", "table"])) {
			...
			$platform = "Windows";
			if (preg_match("/NT 10.0/i", $u_agent)) { 
				$platform .= " 10"; 
			}else if (preg_match("/NT 6.2/i", $u_agent)) { 
				$platform .= " 8"; 
	...
	function logsClient ($object) {
		...
		if ($action == "add") {
			...
			$getTokenRequest = "SELECT `id`, `updated` FROM `logs_client` WHERE `token` = '".$clientToken."';";
			...
			if (!$getTokenQuery) {
				...
				if (mysqli_num_rows($getTokenQuery) == 0) {
					...
					$addRequest = "INSERT INTO `logs_client` (`id`, `time`, `updated`, `token`, `ip`, `device`, `platform`, `browser.name`, `browser.kernel`, `browser.version`, `agent`) VALUES (NULL, '{$time}', '{$updated}', '{$clientToken}', '{$ip}', '{$device}', '{$platform}', '{$browserName}', '{$browserKernel}', '{$browserVersion}', '{$agent}')";
					mysqli_query($db, $addRequest);
				}else {
					$tokenFetch = mysqli_fetch_assoc($getTokenQuery);
					if (isset($tokenFetch['updated']) && $tokenFetch['updated'] + 86400 <= time()) {
						$getBrowser = $this->browser();
						$updated = time();
						$ip = $getBrowser['ip'];
						$platform = $getBrowser['platform'];
						...
						$agent = $getBrowser['agent'];
						$device = $getBrowser['device'];
						$updateRequest = "UPDATE `logs_client` SET `updated` = '".$updated."', `ip` = '".$ip."', `device` = '".$device."', `platform` = '".$platform."', `browser.name` = '".$browserName."', `browser.kernel` = '".$browserKernel."', `browser.version` = '".$browserVersion."', `agent` = '".$agent."' WHERE `token` = '".$clientToken."';";
	...
	function token ($reset = false) {
		if ($reset == true || !isset($_SESSION["client"]['token'])) {
			$sID = $_COOKIE['gSID'];
			//.
			$token['client'] = hash('md5', 'client::'.$sID);
			$token['robot'] = hash('md5', 'robot::'.$sID);
			$token['ajax'] = hash('md5', 'ajax::'.$sID);
			//.
			$token['source']['url'] = hash('crc32', hash('md5', $this->client().'::source::url'));
			$token['source']['css'] = hash('crc32', hash('md5', $this->client().'::source::css'));
			$token['source']['js'] = hash('crc32', hash('md5', $this->client().'::source::js'));
			$token['source']['xml'] = hash('crc32', hash('md5', $this->client().'::source::xml'));
			$token['source']['ajax'] = hash('crc32', hash('md5', $this->client().'::source::ajax'));
			$token['source']['font'] = hash('crc32', hash('md5', $this->client().'::source::font'));
			$token['source']['file'] = hash('crc32', hash('md5', $this->client().'::source::file'));
			/*
			$token['source']['url'] = hash('crc32', hash('md5', 'source::url::'.$sID));
			$token['source']['css'] = hash('crc32', hash('md5', 'source::css::'.$sID));
			$token['source']['js'] = hash('crc32', hash('md5', 'source::js::'.$sID));
			$token['source']['xml'] = hash('crc32', hash('md5', 'source::xml::'.$sID));
			$token['source']['ajax'] = hash('crc32', hash('md5', 'source::ajax::'.$sID));
			$token['source']['font'] = hash('crc32', hash('md5', 'source::font::'.$sID));
			$token['source']['file'] = hash('crc32', hash('md5', 'source::file::'.$sID));
			*/
			//.
			$token['maps']['places'] = hash('crc32', hash('md5', 'maps::places::'.$sID));
			//.
			$token['action']['key']['cookie'] = hash('crc32', hash('md5', 'action::key::cookie::'.$sID));
			$token['action']['key']['obj'] = hash('crc32', hash('md5', 'action::key::obj::'.$sID));
			$token['action']['accounts'] = hash('crc32', hash('md5', 'action::accounts::'.$sID));
			$token['action']['user'] = hash('crc32', hash('md5', 'action::user::'.$sID));
			$token['action']['feed'] = hash('crc32', hash('md5', 'action::feed::'.$sID));
			$token['action']['sites'] = hash('crc32', hash('md5', 'action::sites::'.$sID));
			$token['action']['maps'] = hash('crc32', hash('md5', 'action::maps::'.$sID));
			$token['action']['photos'] = hash('crc32', hash('md5', 'action::photos::'.$sID));
			$token['action']['music'] = hash('crc32', hash('md5', 'action::music::'.$sID));
			$token['action']['videos'] = hash('crc32', hash('md5', 'action::videos::'.$sID));
			$token['action']['cited'] = hash('crc32', hash('md5', 'action::cited::'.$sID));
			$token['action']['messages'] = hash('crc32', hash('md5', 'action::messages::'.$sID));
			$token['action']['explorer'] = hash('crc32', hash('md5', 'action::explorer::'.$sID));
			$token['action']['ajaxify'] = hash('crc32', hash('md5', 'action::ajaxify::'.$sID));
			$token['action']['wall'] = hash('crc32', hash('md5', 'action::wall::'.$sID));
			//.
			$token['cache']['browser'] = hash('crc32', hash('md5', 'cache::browser::'.$sID));
			$token['cache']['photos'] = hash('crc32', hash('md5', 'cache::photos::'.$sID));
			$token['cache']['music'] = hash('crc32', hash('md5', 'cache::music::'.$sID));
			$token['cache']['videos'] = hash('crc32', hash('md5', 'cache::videos::'.$sID));
			$token['cache']['cited'] = hash('crc32', hash('md5', 'cache::cited::'.$sID));
			//.
