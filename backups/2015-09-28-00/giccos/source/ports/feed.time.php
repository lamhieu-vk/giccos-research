...
if (isset($feed_['options'], $feed_['options']['time'])) {
	if (1 + 1 == 2) {
		$timelineDistanceArr = $feed_['options']['time'];
		foreach ($timelineDistanceArr as $key => $timelineDistanceThis) {
			$distanceText = null;
			if (isset($timelineDistanceThis[0], $timelineDistanceThis[1])) {
				$distanceText = $_tool->agoDatetime($timelineDistanceThis[0], 'ago').' - '.$_tool->agoDatetime($timelineDistanceThis[1], 'ago');
			}else if (isset($timelineDistanceThis[0])) {
				$distanceText = $_tool->agoDatetime($timelineDistanceThis[0], 'ago').' - '.$_language->text('now', 'strtolower');
			}else {
				unset($timelineDistanceArr[$key]);
				continue;
			}
			$timelineText = !isset($timelineText) || $timelineText == null ? $distanceText : $timelineText.' & '.$distanceText;
		}
	}
}
...
if (isset($feed_['options'], $feed_['options']['time'])) {
	$statusListOptions['time'] = $feed_['options']['time'];
}
if (1 + 1 == 2) {
...
<div class='info inFeeds boxsub boxGrid' class='thisTotal'>
	<div class='title'> <span>".$_language->text('news_feed', 'ucfirst').": ".$_language->text('time', 'strtolower')."</span> </div>
	<div class='content'> <span>".$_language->text('status_found_based_time', 'ucfirst').". <br> ".$_language->text('timeline', 'ucfirst').": ".$timelineText."</span> </div>
</div>
";
?>
<!DOCTYPE html>
<html lang="<?php print $g_client['language']['code']; ?>" id="giccos" class="feed time">
	<head>
		<title><?php print $timelineText.' :: '.$_language->text('time', 'ucwords').' :: '.$_language->text('pages_feed.title', 'ucwords'); ?></title>
