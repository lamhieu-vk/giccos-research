...
var editor_tagPlaces = function (selector) {
	...
	function requestFind (input) {
		...
		if (_dataCache.exists(placesCacheName) == true) {
			var placesDataCache = _dataCache.get(placesCacheName);
			...
			success: function(data) {
				if (isset(data['return']) && data['return'] === true) {
					...
					if (isset(data['data'])) {
						...
						boxSuggest(input, boxSuggestOptions);
						_dataCache.add(placesCacheName, placesFoundedArr);
					}
					setTimeout(function() {
						input.removeAttr('val');
					}, 1000);
	...
	if (editor.attr('type') == "photos") {
		...
		var uploadMediaPreview = function () {
			...
			var requestUploadFunc = function (files) {
				...
				var requestUpload = _g['ajaxrequest']({
					'type': 'POST',
					'url': '<?php print $_tool->links("source/ajax/action.ajax"); ?>',
					'data': data,
					'done': function (x, s) {
				...
				_g['ajaxrequest'].statechange(requestUpload, function () {
					// console.log(_g['ajaxrequest'].response(requestUpload));
				});
				/*
				var requestUpload = new XMLHttpRequest();
				requestUpload.addEventListener("load", function () {
					console.log(this);
				});
				requestUpload.open("POST", "<?php print $_tool->links("source/ajax/action.ajax"); ?>");
				requestUpload.setRequestHeader("X-Requested-With", "xx");
				requestUpload.send(data);
				*/
				/*
				var requestUpload = $.ajax({
					// url: urlRequest,
					// url: "<?php print $_tool->links("source/ajax/action.ajax"); ?>",
					url: "83f1cf7f/7fb17803/action.ajax",
					type: "POST",
					data: data,
					dataType: "json",
					processData: false,
							...
							for (var i in data['data']) {
								if (isset(data['data'][i]['return']) && data['data'][i]['return'] === false) {
									...
									if (isset(data['data'][i]['secret']) && typeof data['data'][i]['secret'] === "string") {
										...
										setTimeout(function () {
											upload.find("[photos-uploading='"+fileSecret+"']").fadeOut('fast', function(){
											 	$(this).remove();
											});
										}, 1000);
									}
		...
		if (statusType != boxMediaOtherOld && (statusType == "status" || statusType == "photos" || statusType == "music" || statusType == "videos" || statusType == "cited")) {
			...
			if (statusType == "status") {
					...
					if (1 + 1 == 2) {
						...
						uploadPreview.children(".demo[tag]").find(".info > .header > .action .close").bind('click', function() {
							$(this).parents(".demo[tag]").fadeOut(function() {
								...
								if (_dataCache.exists('sites-notuse-url') === true) {
									_dataCache.push('sites-notuse-url', url);
								}else {
									_dataCache.add('sites-notuse-url', [url]);
								}
							});
						});
						if (_dataCache.exists('sites-info-url') === true) {
							var infoList = _dataCache.get('sites-info-url');
							var isHas = 0;
							...
							if (isHas === 0) {
								...
								newObj[url] = linkObj;
								linkObj = newObj;
								_dataCache.push('sites-info-url', linkObj);
							}
						}else {
							newObj = {};
							newObj[url] = linkObj;
							linkObj = newObj;
							_dataCache.add('sites-info-url', linkObj);
						}
					}
				};
				var linkScanInfo = function (url) {
					...
					var infoUrlList = _dataCache.get('sites-scan-url');
					...
					$.ajax({
						...
						success: function(data) {
							if (isset(data) && isset(data['return']) && data['return'] === true) {
								console.log('xs');
								if (_dataCache.exists('sites-scan-url') === true) {
									var infoList = _dataCache.get('sites-scan-url');
									var isHas = 0;
									infoListLoop: for (i in infoList) {
										if (i == url) {
											isHas++;
									...
									if (isHas === 0) {
										linkObj = [];
										linkObj[url] = true;
										_dataCache.push('sites-scan-url', linkObj);
									}
								}else {
									linkObj = [];
									linkObj[url] = true;
									_dataCache.add('sites-scan-url', linkObj);
								}
				...
				var linkGetInfo = function (url) {
					...
					if (_dataCache.exists('sites-notuse-url') === true) {
						var notuseUrlList = _dataCache.get('sites-notuse-url');
					}else {
						var notuseUrlList = {};
					}
					...
					if (_dataCache.exists('sites-info-url') === true) {
						var infoUrlList = _dataCache.get('sites-info-url');
					}else {
						var infoUrlList = {};
					}
					if (isset(infoUrlList) && typeof infoUrlList == "object") {
