...
_g['ajaxrequest'] = function (object, callback) {
	...
	eventLoop: for (var eventType in requestEvent) {
		...
		request.ajaxrequest.event(request, eventType, requestEvent[eventType]);
	}
	_g['ajaxrequest'].statechange(request, function () {
		requestReady = _g['ajaxrequest'].ready(request);
		if (requestReady.state === 4) {
			requestResponse = _g['ajaxrequest'].response(request);
			requestDone(_g['ajaxrequest'].responseparse('json', requestResponse['data']), requestResponse);
			return request;
		}
	});
	request.onprogress = requestProgress;
	request.onerror = requestError;
	request.onabort = requestAbort;
	request.onload = requestLoad;
	request.onloadstart = requestBefore;
	request.onloadend = requestAfter;
	request.open(requestType, requestURL, requestAsync);
	hasContentType = 0;
	for (var label in requestHeaders) {
		if (toLowerCase(label) == toLowerCase("Content-Type")) {
			hasContentType++;
		}
	}
	if (hasContentType === 0) {
	...
	headersLoop: for (var headerType in requestHeaders) {
		request.setRequestHeader(headerType, requestHeaders[headerType]);
	}
	...
	if (new RegExp("<?php print $_tool->links("source/ajax/action.ajax"); ?>".replace(<?php print $_parameter->get('regex_sites_giccos_domain'); ?>, ""), "ig").test(requestURL) === true) {
		request.setRequestHeader("Token", "<?php print $g_client['token']['ajax']; ?>");
		...
		request.setRequestHeader("x-Randkey", requestRandkey);
	}
	for (var label in requestData) {
		if (inArray(label, ["port", "token", "type", "action"])) {
			ObjData[i] = requestData[label];
			delete requestData[label];
		}else if (label == "obj" || label == "ObjRequest") {
			ObjData[ObjVerify] = requestData[label];
			delete requestData[i];
		}
	}
	if (typeof requestData === "object" && Object.keys(requestData).length > 0) {
		requestDataCache['requestVerify'] = requestData;
		requestData = requestDataCache;
		delete requestDataCache;
	}
	if (typeof requestData['token'] === "undefined" && typeof ObjData['port'] !== "undefined") {
		<?php 
			...
			foreach ($g_client['token']['action'] as $key => $tokenActionThis) {
				if (!in_array($key, ["key"])) {
					if ($listReplaceToken == null) {
						$listReplaceToken .= "if (ObjData['port'] == '".$key."') { requestData['token'] = '{$tokenActionThis}';  }";
					}else {
						$listReplaceToken .= "else if (ObjData['port'] == '".$key."') { requestData['token'] = '{$tokenActionThis}';  }";
					}
				}
			}
