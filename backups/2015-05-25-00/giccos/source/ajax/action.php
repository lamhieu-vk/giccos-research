...
if (isset($g_client['token']['ajax'], $_SERVER['HTTP_TOKEN'], $_SERVER['HTTP_X_REQUESTED_WITH'], $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) && $_SERVER['HTTP_TOKEN'] == $g_client['token']['ajax'] && $_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest" && $_tool->valueCheck("referer", $_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_HOST'] == $_tool->links("::host")) {
	...
	}else if (($port == "user" || $port == "user") && $token == $g_client['token']['action']['user']) {
		if ($type == "reload") {
			...
			if ($path == "info") {
				if ($from == "user" || $from == "friends" || $from == "all") {
					...
					if ($from == "user") {
						$searchRequest = "SELECT `id` FROM `user` WHERE `private.search` >= '{$_parameter->get('user_private.search_agree')}' AND ({$search_query_sql_regex})";
					}else if ($from == "friends") {
						$searchRequest = "SELECT `id` FROM `user` WHERE `private.tag` >= '{$_parameter->get('user_private.tag_agree')}' AND `id` IN (SELECT `guy.id` FROM `friends` WHERE `user.id` = '{$g_user['id']}') AND ({$search_query_sql_regex})";
					}else {
						die(print json_encode(array("return" => false, "reason" => "")));
					}
							...
							if (isset($feed_['content']['require'][$i][0], $feed_['content']['require'][$i][1])) {
								...
								for ($x = 0; $x < count($feed_['content']['require'][$i][1]); $x++) {
									...
									if (preg_match("/^(\)/", $feed_['content']['require'][$i][1][$x]) == true) {
										...
										$cache_hashtag_['author']['id'] = mysqli_fetch_assoc(mysqli_query($_db->port('beta'),"SELECT `id` FROM `user` WHERE `username` = '{$cache_hashtag_['username']}'"))['id'];
		...
		if ($type == "chatbox") {
			...
			if ($manage == "guy") {
				if ($action == "search") {
					...
					if ($from == "all") {
						$searchRequest = "SELECT `id` FROM `user` WHERE `private.search` >= '{$_parameter->get('user_private.search_agree')}' AND ({$searchQueryRegexThis})";
					}else if ($from == "friends") {
						$searchRequest = "SELECT `id` FROM `user` WHERE `private.tag` >= '{$_parameter->get('user_private.tag_agree')}' AND `id` IN (SELECT `guy.id` FROM `friends` WHERE `user.id` = '{$g_user['id']}') AND ({$searchQueryRegexThis})";
					}else {
						die(print json_encode(array("return" => false, "reason" => "")));
					}
					$guyArr = array();
				...
				if ($action == "add") {
					...
					if (isset($ObjRequest['rows']) && is_array($ObjRequest['rows']) && count($ObjRequest['rows']) > 0) $tabRows = $ObjRequest['rows']; else $tabRows = null;
					...
					$tabRowsRaw = null;
					foreach ($tabRows as $key => $tabRowsThis) {
						if ($tabRowsRaw == null) {
							$tabRowsRaw .= "`{$tabRowsThis}`";
						}else {
							$tabRowsRaw .= ", `{$tabRowsThis}`";
						}
					}
					...
					if (isset($getMessagesTab['return'], $getMessagesTab['count'], $getMessagesTab['tab']) && $getMessagesTab['return'] == true) {
						foreach ($getMessagesTab['tab'] as $key => $value) {
							$getMessagesTab['tab'][$key][$tabLabel] = $tabValue;
						}
						die(print json_encode(array("return" => true, "count" => $getMessagesTab['count'], "tab" => $getMessagesTab['tab'])));
				...
				if ($action == "add") {
					...
					$addMessagesViews = $_messages->data($addMessagesViewsOptions);
				...
				if ($wall_id > 0 && $rows != null) {
					if (in_array($rows, ["all_friends", "mutual_friends", "live", "country"])) {
						...
						if ($rows == "all_friends") {
							...
							$getFriendsRequest = "SELECT `id`, `guy.id` FROM `friends` WHERE `user.id` = '{$wall_id}' AND `guy.id` IN (SELECT `id` FROM `user` WHERE `live` = '{$g_user['live']}') AND `id` {$order} '{$friends_id}' ORDER BY `id` DESC";
						}else if ($rows == "country") {
							if (!isset($g_user['country']) || $g_user['country'] == null || $g_user['country'] == "0") {
								print json_encode(array("return" => false, "reason" => "")); die();
							}
							$getFriendsRequest = "SELECT `id`, `guy.id` FROM `friends` WHERE `user.id` = '{$wall_id}' AND `guy.id` IN (SELECT `id` FROM `user` WHERE `country` = '{$g_user['country']}') AND `id` {$order} '{$friends_id}' ORDER BY `id` DESC";
						}else {
							print json_encode(array("return" => false, "reason" => "")); die();
						}
