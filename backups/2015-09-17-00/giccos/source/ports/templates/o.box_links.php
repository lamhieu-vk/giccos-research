<div id="gboxLinks">
	...
	<div class="box lines boxGrid">
		...
		<div class="content">
			<ul class="list">
				<?php 
				print " <li class='rows null'><span>{$_language->text('you_have_not_joined_any_groups', 'strtolower')}</span></li> ";
				?>
			</ul>
		</div>
	</div>
		...
		<div class="content">
			<ul class="list">
				...
				if ($boxLinksHashtagQuery == true && mysqli_num_rows($boxLinksHashtagQuery) > 0) {
					...
					print "
					<li class='rows null'><span>{$_language->text('please_use_hashtag_on_status', 'strtolower')}</span></li>
					";
				}
				?>
			</ul>
