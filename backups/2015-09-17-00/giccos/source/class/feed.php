...
Class feed {
	...
	function status_list ($object) {
		...
		if ($guyType == "user") {
			...
			$listQuery['private'] = "AND ((`status`.`author.type` = '".$guyType."' AND `status`.`author.id` = '".$guyId."') OR (`status`.`id` IN (SELECT `inside.id` FROM `status_usertag` WHERE `inside.type` = 'status' AND `guy.type` = '".$guyType."' AND `guy.id` = '".$guyId."')) OR (`status`.`private.view` = '4'))";
		}
		...
		if ($type == "general") {
			...
			foreach ($requireArr as $keywordThis => $requireThis) {
				...
				if (!isset($listQuery['tag']) || !is_string($listQuery['tag'])) {
					$listQuery['tag'] = "(`status`.`id` IN (SELECT `inside.id` FROM `status_hashtag` WHERE `inside.type` = 'status' AND `hashtag` = '".$_tool->convertDatabaseString($keywordThis)."') ".$requireTagThis.")";
				}else {
					$listQuery['tag'] .= " OR (`status`.`id` IN (SELECT `inside.id` FROM `status_hashtag` WHERE `inside.type` = 'status' AND `hashtag` = '".$_tool->convertDatabaseString($keywordThis)."') ".$requireTagThis.")";
				}
			}
			if (!isset($listQuery['tag']) || !is_string($listQuery['tag'])) {
				return array("return" => false, "reason" => "2");
