...
Class videos {
	...
	function cache ($object) {
		...
		if (in_array($action, ["add"])) {
			...
			$fileArr['token'] = hash('crc32', '('.$guyType.'_'.$guyId.')::'.md5_file($file['tmp']));
			...
			if (isset($clientToken, $fileArr['token'], $fileArr['secret'], $fileArr['nameraw'], $fileArr['name'], $fileArr['mime'], $fileArr['size'])) {
				...
				if (!$addFileQuery) {
					...
					if (isset($fileUpload['return'], $fileUpload['data']) && $fileUpload['return'] == true) {
						...
						mysqli_query($db, "UPDATE `videos_cache` SET `thumbnail.path` = '".$fileUpload['data']['display']."' WHERE `id` = '".$idInserted."' ;");
						$thumbnailPosition = intval($fileArr['duration'] / 10);
						exec("ffmpeg -y -itsoffset -{$thumbnailPosition} -i ".$_client->root().$tmp." -vcodec mjpeg -vframes 1 -an -f rawvideo -s 1280x720 ".$_client->root().$thumbnailPath." 2>&1");
