...
Class mailer {
	...
	function id ($object) {
		...
		if ($action == "add") {
			...
			if ($mailerType == null) {
				return array("return" => false, "reason" => "");
			}else if (!in_array($mailerType, ["support", "mailer"])) {
				return array("return" => false, "reason" => "");
			}
			if ($mailerHost != null) {
		...
		if (isset($getMailer, $getMailer['return'], $getMailer['data']) && $getMailer['return'] == true) {
			...
			if ($mailLog == true) {
				...
				if (isset($addLog, $addLog['return'], $addLog['data'], $addLog['data']['token']) && $addLog['return'] == true) {
					$mailContent = str_replace("{_mailerLogToken_}", $addLog['data']['token'], $mailContent);
	...
	function logs ($object) {
		...
		if ($action == "add") {
			...
			if (isset($existsMailer, $existsMailer['return'], $existsMailer['exists']) && $existsMailer['return'] == true) {
				...
				$mailerContent = $_tool->hash('encode', $mailerContent, $mailerToken.'::'.$_parameter->get('hash_mailer_logs'));
