...
wrapperLoad.add = function (object) {
	...
	if (isset(object['close']) && object['close'] === true) {
		var codeClose = "<div class='close'></div>";
	}else {
		var codeClose = "";
	}
	var codeWrapper = "<div id='wrapperLoading' randkey='"+randkey+"'> "+codeClose+" <div class='loading'></div> </div>";
	$("body").css({'overflow-x': 'hidden', 'overflow-y': 'hidden'}).append(codeWrapper);
	...
	if (isset(object['openCallback']) && typeof object['openCallback'] === "function") {
		codeWrapper['openCallback'] = object['openCallback'];
	}
	if (isset(object['closeCallback']) && typeof object['closeCallback'] === "function") {
		codeWrapper['closeCallback'] = object['closeCallback'];
	}
	...
	if (isset(object['randkey']) && typeof object['randkey'] === "string") {
		...
		if (thisWrapper.find(".close").lengh === 0) {
			...
			thisWrapper.fadeOut(function() {
				...
				console.log(dataCaching);
				var oldOverflowY = isset(dataCaching) && isset(dataCaching['style']) && isset(dataCaching['style']['body-overflow-y']) ? dataCaching['style']['body-overflowy'] : "auto";
				var oldOverflowX = isset(dataCaching) && isset(dataCaching['style']) && isset(dataCaching['style']['body-overflow-x']) ? dataCaching['style']['body-overflowx'] : "auto";
				$("body").css({'overflow-y': oldOverflowY, 'overflow-x': oldOverflowX});
...
var loadFileSource = function (type, cg) {
	...
	if (isset(context)) {
		...
		if (type == "css") {
			fileCode = "<style file-randkey='"+fileRandkey+"' file-src='undefined' file-type='css' file-include='false' type='text/css'></style>";
		}else if (type == "js") {
			fileCode = "<script file-randkey='"+fileRandkey+"' file-src='undefined' file-type='js' file-include='false' type='text/javascript'></script>";
		}else {
			return false;
		}
		sourceBox.append(fileCode);
		if (type == "js") {
			window.onload = eval(context);
		}
		thisFile = $("[file-randkey='"+fileRandkey+"']");
		console.log(context);
		thisFile.html(context).removeAttr('file-randkey');
		return true;
		*/
	}
	...
	if (isset(src)) {
		...
		if (type == "css") {
			fileCode = "<style file-randkey='"+fileRandkey+"' file-src='"+src+"' file-type='css' file-include='true' type='text/css'></style>";
		}else if (type == "js") {
			fileCode = "<script file-randkey='"+fileRandkey+"' file-src='"+src+"' file-type='js' file-include='true' type='text/javascript'></script>";
		}else {
			return false;
		}
		sourceBox.append(fileCode);
		thisFile = $("[file-src='"+src+"'][file-type='"+type+"']");
		...
		if (isset(getFileRequest.readyState) && getFileRequest.readyState == 1) {
			thisFile.removeAttr('file-randkey');
			return true;
		}else {
			thisFile.removeAttr('file-randkey');
			return false;
...
ajaxify.reload = function (obj, cb) {
	...
	var beginReloadBoxFunc = function (value) {
		if (isset(value) && value === true) {
			...
			ajaxify.loadwrapper(true, endReloadBoxFunc);
		}
	}
...
var callbackAllFunc = function () {
	...
	//.
	setTimeout(function() {
		ajaxify.request('http://localhost/giccos/feed');
	}, 1000);
	//.
	setTimeout(function() {
		setupTimestamp();
