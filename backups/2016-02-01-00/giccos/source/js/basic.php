	...
	parse: {
		...
		datenow: {
			timestamp: function (unix) {
				var unix = _is.bool(unix) && _is.string(unix) ? unix : false;
				var dateNow = unix == true ? Math.floor(Date.now()) : Math.floor(Date.now() / 1000);
				return dateNow;
			}
		},
		datetext: function (type, timestamp) {
			...
			var dateTime = new Date(timestamp),
				dateSecond = Number(dateTime.getSeconds()),
				dateSecond = String(dateSecond < 10 ? '0' + dateSecond : dateSecond),
				dateMinute = Number(dateTime.getMinutes()),
				dateMinute = String(dateMinute < 10 ? '0' + dateMinute : dateMinute),
				dateHour = Number(dateTime.getHours()),
				dateHour = String(dateHour < 10 ? '0' + dateHour : dateHour),
				dateDay = Number(dateTime.getDate()),
				dateDay = String(dateDay < 10 ? '0' + dateDay : dateDay),
				dateWeek = Number(dateTime.getDay()),
				dateWeek = String(dateWeek + 1),
				dateMonth = Number(dateTime.getMonth()),
				dateMonth = String(dateMonth + 1),
				dateYear = String(dateTime.getFullYear());
			//.
			var date = type,
				date = date.replace(/s/g, dateSecond),
				date = date.replace(/i/g, dateMinute),
				date = date.replace(/H/g, dateHour),
				date = date.replace(/h/g, Number(dateHour) % 12),
				date = date.replace(/G/g, Number(dateHour) < 10 ? dateHour.replace("0", "") : dateHour),
				date = date.replace(/g/g, Number(Number(dateHour) % 12) < 10 ? String(Number(dateHour) % 12).replace("0", "") : dateHour),
				date = date.replace(/d/g, dateDay),
				date = date.replace(/j/g, Number(dateDay) < 10 ? dateDay.replace("0", "") : dateDay),
				date = date.replace(/m/g, dateMonth),
				date = date.replace(/n/g, Number(dateMonth) < 10 ? dateMonth.replace("0", "") : dateMonth),
				date = date.replace(/Y/g, dateYear),
				date = date.replace(/y/g, Number(dateYear) % 100);
			return date;
		},
...
_g.ajaxrequest = {
	use: function (object, callback) {
		...
		if (requestCache == true) {
			...
			_g.ajaxrequest.cache.exists(requestCacheKey, function (isCached) {
				if (isCached) {
					_g.ajaxrequest.cache.remove(requestCacheKey);
				}
			});
	...
	cache: { // local
		...
		get: function (key, callback) {
			...
			if (!_is.exists(key)) {
				...
				var ajaxRequestCached = {};
				return _cache.get('client_ajaxrequest_cached', function (isCached) {				
					if (isCached) {
						_cache.get('client_ajaxrequest_cached', function (dataCached) {
							var ajaxRequestCached = _is.array(dataCached) ? dataCached : {};
		...
		remove: function (key, callback) {
			...
			if (!_is.exists(key)) {
				...
				var fnCacheRemove = function (arrCached, callback) {
					var callback = _is.function(callback) ? callback : function () {};
					if (_is.exists(arrCached, arrCached[key])) {
						delete arrCached[key];
						return _cache.set('client_ajaxrequest_cached', arrCached, function (isRemoved) {
							callback(isRemoved);
							return isRemoved;
						});
					}else {
				...
				return _cache.get('client_ajaxrequest_cached', function (isCached) {				
					if (isCached) {
						_cache.get('client_ajaxrequest_cached', function (dataCached) {
							var ajaxRequestCached = _is.array(dataCached) ? dataCached : {};
							return fnCacheRemove(ajaxRequestCached, callback);
						});
					}else {
						return fnCacheRemove(ajaxRequestCached, callback);
					}
		...
		exists: function (key, callback) {
			...
			if (!_is.exists(key)) {
				...
				var ajaxRequestCached = {};
				return _cache.exists('client_ajaxrequest_cached', function (isCached) {				
					if (isCached) {
						_cache.get('client_ajaxrequest_cached', function (dataCached) {
							var ajaxRequestCached = _is.array(dataCached) ? dataCached : {};
	...
	bind: {
		add: function (popupElem, bindType, callback) {
			...
			_selector.cache.get(popupElem, 'bind', function (returnObj) {
				var returnObj = _is.array(returnObj) && _is.array(returnObj[0]) ? returnObj[0] : {};
				returnObj[_array.count(returnObj)] = {'type': bindType, 'func': callback};
				_selector.cache.set(popupElem, 'bind', returnObj);
			});
			return true;
		},
		remove: function (popupElem, bindType, callback) {
			var popupElem = _is.selector(popupElem) ? popupElem : null,
				bindType = _is.string(bindType) ? bindType : null,
				callback = _is.function(callback) ? callback : null;
			if (_selector.count(popupElem) == 0) {
					...
					for (var bindId in returnObj) {
						if (_is.exists(returnObj[bindId]['type']) && returnObj[bindId]['type'] == "close") {
							if (_is.exists(returnObj[bindId]['func'])) {
								returnObj[bindId]['func'](popupElem);
							}
						}
					}
				});
		...
		card: {
			codeparse: function (object, options, callback) {
				...
				var streamCardKey = _string.random(10),
					...
					streamCardSize = _is.exists(object['size']) ? Number(object['size']) : null,
					...
					for (var labelOptionsType in optionsArr) {
						...
						if (optionsType == "author") {
							if (_array.exists(labelOptionsType, ['link'])) {
								...
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><div class='icon'></div><div class='text'>block all</div></div>");
								...
								liListOptionsElem.bind('click', function (e) {
									...
									_g.image.stream.action.block.add({'guy': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}}, function (returnObj) {
											...
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													_notification.bind.add(notificationElem, 'close', function () {
														_g.image.stream.card.remove({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
														_g.image.stream.comment.remove({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
													});
													notificationElem.find(".undo").bind('click', function () {
														...
														_g.image.stream.card.show({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
														_g.image.stream.comment.show({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
														streamCardElem.slideDown(function () {
															_g.image.stream.action.block.remove({'guy': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}}, function (returnObj) {
																if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
													...
													_g.image.stream.card.hide({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
													_g.image.stream.comment.hide({'author': {'type': streamCardAuthor['type'], 'id': streamCardAuthor['id']}});
												});
											}
							...
							if (_array.exists(labelOptionsType, ['link'])) {
								...
								liListOptionsElem.bind('click', function (e) {
									...
									_g.image.stream.action.hide.add({'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
										liListOptionsElem.removeAttr('handling-request');
										if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													notificationElem.find(".undo").bind('click', function () {
														...
														_notification.close(notificationElem);
														streamCardElem.slideDown(function () {
															_g.image.stream.action.hide.remove({'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
																if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
																	if (_is.exists(returnObj['removed']) && returnObj['removed']) {
																		_notification.add({
																			classname: "image-action-preview",
								...
								liListOptionsElem.bind('click', function (e) {
									...
									_g.image.stream.action.delete.add({'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
										liListOptionsElem.removeAttr('handling-request');
										if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													notificationElem.find(".undo").bind('click', function () {
														...
														_notification.close(notificationElem);
														streamCardElem.slideDown(function () {
															_g.image.stream.action.delete.remove({'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
																if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
																	if (_is.exists(returnObj['removed']) && returnObj['removed']) {
																		_notification.add({
																			classname: "image-action-preview",
				...
				cardAuthorElem.children(".options").children(".nav").bind('click', function (e) {
					var navOptionsElem = $(this);
					if (navOptionsElem.next(".list").attr('options-popup-showing') == "true") {
						return;
					}
					navOptionsElem.next(".list").stop(true, false).fadeIn(500);
				}, 150);
				cardAuthorElem.children(".options").children(".nav").bind('dblclick', function (e) {
					var navOptionsElem = $(this);
					navOptionsElem.next(".list").stop(true, false).fadeOut(250).attr('options-popup-showing', 'true');
					_popup.add({
						classname: "stream-options-tab",
						title: "options for image",
						content: "<div></div>",
						close: true
					}, function (popupElem) {
						popupElem.find(".main.stream-options-tab").html(navOptionsElem.next(".list").children().clone(true));
						popupElem.find(".main.stream-options-tab").find(".box > .main > .li").bind('click', function () {
							_popup.close(popupElem);
						});
						_popup.bind.add(popupElem, 'close', function () {
							navOptionsElem.next(".list").removeAttr('options-popup-showing');
						});
					});
				});
				//_g.image.stream.action.comment.add({content: inputTypeCommentValue}, {'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
				...
				cardMediaElem.children(".image").children(".preview").bind('click', function () {
					var optionsViewerImageObj = {
						"navbar": {
							"information": {
								"float": "right",
								"title": "show more information this image",
								"sidebar": {
									"type": "click",
									"title": "information",
									"content": ""
						...
						"data": [
							{"source": (_is.array(streamCardFile['large']) && _is.string(streamCardFile['large']['url']) ? streamCardFile['large']['url'] : (_is.array(streamCardFile['thumbnail']) && _is.string(streamCardFile['thumbnail']['url']) ? streamCardFile['thumbnail']['url'] : null))}
						]
					};
					if (_is.string(streamCardDescription)) {
						optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row description clearfix'><div class='context'><span class='text large'>"+(streamCardDescription)+"</span></div></div>";
					}
					if (_is.exists(streamCardDate) && new RegExp(/^([0-9]{9,11})$/).test(streamCardDate)) {
						optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row date clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'><span class='text large'>"+(_parse.datetip(streamCardDate))+"</span>"+((_is.exists(streamCardEdited) && new RegExp(/^([0-9]{9,11})$/).test(streamCardEdited)) ? "<div class='row edited sub clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'><span class='text small'>"+(_parse.datetime(streamCardEdited))+"</span></div></div>" : "")+"</div></div>";
					}
					if (_is.string(streamCardNameraw)) {
						optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row image clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'><span class='text large'>"+(streamCardNameraw)+"</span>";
						if (_is.exists(streamCardSize)) {
							optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row size sub clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'><span class='text small'>"+(_parse.bytes(streamCardSize))+"</span></div></div>";
						}
						if (_is.array(streamCardTag) && _array.count(streamCardTag) > 0) {
							optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row tag sub clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'>";
							for (var tagLabel in streamCardTag) {
								var tagValue = streamCardTag[tagLabel];
								optionsViewerImageObj['navbar']['information']['sidebar']['content'] += tagLabel == 0 ? ("<a class='text small' href='#' target='_blank'>" + tagValue + "</a>") : (" <a class='text small' href='#' target='_blank'>" + tagValue + "</a>");
							}
							optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "</div></div>";
						}
						if (_is.array(streamCardColor) && _array.count(streamCardColor) > 0) {
							optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "<div class='row color sub clearfix'><div class='conicon'><div class='icon'></div></div><div class='context'>";
							for (var colorLabel in streamCardColor) {
								var colorValue = streamCardColor[colorLabel];
								optionsViewerImageObj['navbar']['information']['sidebar']['content'] += colorLabel == 0 ? ("<a class='text small' color-code='" + colorValue + "'>" + colorValue + "</a>") : (" <a class='text small' color-code='" + colorValue + "'>" + colorValue + "</a>");
							}
							optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "</div></div>";
						}
						optionsViewerImageObj['navbar']['information']['sidebar']['content'] += "</div></div>";
					}
					_viewer.image.add(optionsViewerImageObj, function (viewerImageElem) {
						viewerImageElem.find(".information > .sidebar > .body .row > .context > [color-code]").each(function () {
							var colorTextElem = $(this);
							colorTextElem.css('cursor', 'pointer');
							_tooltip.set(colorTextElem, {arrow: "s", action: "hover", content: "<div style='height: 20px; width: 40px; border-radius: 2px; background: " + (colorTextElem.attr('color-code')) + ";'></div>" });
						});
					});
				});
				...
				if (_is.exists(streamCardFavorite) && _is.exists(streamCardFavorite['allow']) && streamCardFavorite['allow']) {
					...
					_g.image.stream.action.favorite(cardActionElem.children("[action-favorite-button]").children(".icon"), 'click', {'inside': {'type': 'id', 'id': streamCardId}}, true, function (actionSuccess, favoriteValue) {
						if (actionSuccess) {
							_g.image.stream.action.stats({'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
						...
						cardCommentElem.children(".type").children(".row").find(".box.main > .body > .textarea > .input").bind('keyup', function (e) {
							...
							if ((e.keyCode ? e.keyCode : e.which) == 13) {
								...
								if (new RegExp(/^([\s\n]+)$/).test(inputTypeCommentElem.val())) {
									...
									_g.image.stream.action.comment.add({content: inputTypeCommentValue}, {'inside': {'type': 'id', 'id': streamCardId}}, function (returnObj) {
				...
				_selector.cache.set(streamCardElem, 'streamobject', object);
			...
			hide: function (attrObj, callback) {
				var attrObj = _is.array(attrObj) ? attrObj : {},
					callback = _is.function(callback) ? callback : function () {};
				var liSelectedElem = [];
				var liStreamAll = $(".stream-slice[stream-slice-float]").children(".stream-li");
				liStreamAll.each(function () {
					var liStreamElem = $(this);
					_selector.cache.get(liStreamElem, 'streamobject', function (returnObj) {
						var returnObj = _is.array(returnObj) && _is.array(returnObj[0]) ? returnObj[0] : {};
						var isLike = 0;
						if (_is.array(attrObj['author']) && _is.array(returnObj['author'])) {
							var likeAuthor = 0;
							console.log(returnObj);
							if (_is.exists(attrObj['author']['type']) && attrObj['author']['type'] == returnObj['author']['type']) {
								likeAuthor++;
							}
							if (_is.exists(attrObj['author']['type']) && attrObj['author']['id'] == returnObj['author']['id']) {
								likeAuthor++;
							}
							if (likeAuthor == _array.count(attrObj['author'])) {
								isLike++;
							}
						}
						if (isLike == _array.count(attrObj)) {
							liStreamElem.stop(true, false).slideUp();
							liSelectedElem[_array.count(liSelectedElem)] = liStreamElem;
						}
					});
				});
				callback(true, liSelectedElem);
				return true;
			},
			show: function (attrObj, callback) {
				var attrObj = _is.array(attrObj) ? attrObj : {},
					callback = _is.function(callback) ? callback : function () {};
				var liSelectedElem = [];
				var liStreamAll = $(".stream-slice[stream-slice-float]").children(".stream-li");
				liStreamAll.each(function () {
					...
					_selector.cache.get(liStreamElem, 'streamobject', function (returnObj) {
						...
						if (isLike == _array.count(attrObj)) {
							liStreamElem.stop(true, false).slideDown();
							liSelectedElem[_array.count(liSelectedElem)] = liStreamElem;
						}
					});
				});
			...
			remove: function (attrObj, callback) {
				var attrObj = _is.array(attrObj) ? attrObj : {},
					callback = _is.function(callback) ? callback : function () {};
				var liSelectedElem = [];
				var liStreamAll = $(".stream-slice[stream-slice-float]").children(".stream-li");
		...
		action: {
			stats: function (cardObj, callback) {
				...
				var fnStatsRequest = function (object, callback) {
					var object = _is.array(object) ? object : {},
						callback = _is.function(callback) ? callback : function () {};
					var insideType = _is.array(object['inside']) && _is.string(object['inside']['type']) ? object['inside']['type'] : null,
							insideId = _is.array(object['inside']) && _is.number(object['inside']['id']) ? object['inside']['id'] : null;
					var data = {
						"port": "image", 
						"type": "stats",
						"inside": {
				...
				fnStatsRequest({'inside': cardObj['inside']}, callback);
			},
			delete: {
				add: function (cardObj, callback) {
					...
					fnAddDeleteRequest({'inside': cardObj['inside']}, callback);
				},
				remove: function (cardObj, callback) {
					...
					fnRemoveDeleteRequest({'inside': cardObj['inside']}, callback);
				}
			},				
			hide: {
				add: function (cardObj, callback) {
					...
					fnAddHideRequest({'inside': cardObj['inside']}, callback);
				},
				remove: function (cardObj, callback) {
					var cardObj = _is.array(cardObj) ? cardObj : {},
						callback = _is.function(callback) ? callback : function () {};
					...
					fnRemoveHideRequest({'inside': cardObj['inside']}, callback);
				}
			},
			block: {
				add: function (cardObj, callback) {
					...
					var fnAddBlockRequest = function (object, callback) {
						var object = _is.array(object) ? object : {},
							callback = _is.function(callback) ? callback : function () {};
						var guyType = _is.array(object['guy']) && _is.string(object['guy']['type']) ? object['guy']['type'] : null,
							guyId = _is.array(object['guy']) && _is.number(object['guy']['id']) ? object['guy']['id'] : null;
						var data = {
							"port": "image", 
							"type": "block",
							"action": "add",
					...
					fnAddBlockRequest({'guy': cardObj['guy']}, callback);
				},
				remove: function (cardObj, callback) {
					var cardObj = _is.array(cardObj) ? cardObj : {},
						callback = _is.function(callback) ? callback : function () {};
					...
					fnRemoveBlockRequest({'guy': cardObj['guy']}, callback);
				}
			},
			comment: {
				add: function (dataObj, cardObj, callback) {
					...
					fnCommentRequest({'inside': cardObj['inside'], 'content': dataObj['content']}, callback);
				},
			...
			favorite: function (buttonElem, bindAction, cardObj, isToggle, callback) {
				...
				if (_selector.count(buttonElem) > 0) {
					...
					var fnFavoriteAction = function (event) {
						...
						objFavoriteRequest['inside'] = cardObj['inside'];
		...
		comment: {
			codeparse: function (object, options, callback) {
				...
				var commentCardKey = _string.random(10),
					...
					commentCardOptions = _is.array(object['options']) ? object['options'] : null,
				...
				// cardExtraElem.children(".options").html("<div class='nav'><div class='icon'></div></div><div class='list'></div>");
				cardMainElem.children(".options").html("<div class='nav'><div class='icon'></div></div><div class='list'></div>");
				console.log(commentCardOptions);
				_array.sort.default(commentCardOptions);
				for (var optionsType in commentCardOptions) {
					var optionsArr = commentCardOptions[optionsType];
					var optionsArr = _array.sort.default(optionsArr);
					var listOptionsElem = cardMainElem.children(".options").children(".list");
					if (optionsType == "author") {
						var listOptionsName = "author";
					}else if (optionsType == "comment") {
						var listOptionsName = "comment";
					}else {
						var listOptionsName = '';
					}
					if (_selector.count(listOptionsElem.children("."+optionsType)) == 0) listOptionsElem.append("<div class='box "+optionsType+"'><div class='navbar'><span class='text'>"+listOptionsName+"</span></div><div class='main'></div></div>");
					...
					for (var labelOptionsType in optionsArr) {
						...
						if (optionsType == "author") {
							if (_array.exists(labelOptionsType, ['link'])) {
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><a href='"+(_is.exists(commentCardAuthor['link']) ? commentCardAuthor['link'] : '')+"' target='_blank'><div class='icon'></div><div class='text'>to author's page</div></a></div>");
							}else if (_array.exists(labelOptionsType, ['block'])) {
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><div class='icon'></div><div class='text'>block all</div></div>");
								var liListOptionsElem = thisListOptionsElem.children(".main").children(".li."+labelOptionsType);
								liListOptionsElem.bind('click', function (e) {
									...
									_g.image.stream.action.block.add({'guy': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}}, function (returnObj) {
										liListOptionsElem.removeAttr('handling-request');
										if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													_notification.bind.add(notificationElem, 'close', function () {
														_g.image.stream.card.remove({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
														_g.image.stream.comment.remove({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
													});
													notificationElem.find(".undo").bind('click', function () {
														_notification.bind.remove(notificationElem, 'close');
														_notification.close(notificationElem);
														_g.image.stream.card.show({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
														_g.image.stream.comment.show({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
														_g.image.stream.action.block.remove({'guy': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}}, function (returnObj) {
															if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
																if (_is.exists(returnObj['removed']) && returnObj['removed']) {
																	_notification.add({
																		classname: "image-action-preview",
													...
													_g.image.stream.card.hide({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
													_g.image.stream.comment.hide({'author': {'type': commentCardAuthor['type'], 'id': commentCardAuthor['id']}});
												});
											}
										}else if (_is.exists(returnObj) && _is.exists(returnObj['return']) && _is.exists(returnObj['reason']) && !returnObj['return']) {
											//.
							...
							if (_array.exists(labelOptionsType, ['link'])) {
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><a href='"+(_is.exists(commentCardLink) ? commentCardLink : '')+"' target='_blank'><div class='icon'></div><div class='text'>to comment's page</div></a></div>");
							}else if (_array.exists(labelOptionsType, ['hide'])) {
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><div class='icon'></div><div class='text'>hide this comment</div></div>");
								var liListOptionsElem = thisListOptionsElem.children(".main").children(".li."+labelOptionsType);
								liListOptionsElem.bind('click', function (e) {
									_document.eventstop(e);
									if (liListOptionsElem.attr('handling-request') == "true") {
									...
									_g.image.stream.action.hide.add({'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
										liListOptionsElem.removeAttr('handling-request');
										if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													content: "you was hided comment successful, <a class='undo text'>you want undo?</a>",
													timeout: 6000,
													close: true
												}, function (notificationElem) {
													_notification.bind.add(notificationElem, 'close', function () {
														commentCardElem.remove();
													});
													notificationElem.find(".undo").bind('click', function () {
														_notification.bind.remove(notificationElem, 'close');
														_notification.close(notificationElem);
														commentCardElem.slideDown(function () {
															_g.image.stream.action.hide.remove({'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
																if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
																	if (_is.exists(returnObj['removed']) && returnObj['removed']) {
																		_notification.add({
																			classname: "image-action-preview",
																			...
																			content: "you was removed from list comment hide successful!",
																			timeout: 3000,
																			close: true
																		}, function (notificationElem) {
																			notificationElem.bind('click', function () {
								...
								thisListOptionsElem.children(".main").append("<div class='li "+labelOptionsType+"'><div class='icon'></div><div class='text'>delete this comment</div></div>");
								var liListOptionsElem = thisListOptionsElem.children(".main").children(".li."+labelOptionsType);
								liListOptionsElem.bind('click', function (e) {
									_document.eventstop(e);
									if (liListOptionsElem.attr('handling-request') == "true") {
									...
									_g.image.stream.action.delete.add({'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
										liListOptionsElem.removeAttr('handling-request');
										if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
											if (_is.exists(returnObj['added']) && returnObj['added']) {
												_notification.add({
													...
													content: "you was deleted this comment, <a class='undo text'>you want undo?</a>",
													timeout: 6000,
													close: true
												}, function (notificationElem) {
													_notification.bind.add(notificationElem, 'close', function () {
													...
													notificationElem.find(".undo").bind('click', function () {
														...
														_notification.close(notificationElem);
														commentCardElem.slideDown(function () {
															_g.image.stream.action.delete.remove({'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
																if (_is.exists(returnObj) && _is.exists(returnObj['return']) && returnObj['return']) {
																	if (_is.exists(returnObj['removed']) && returnObj['removed']) {
																		_notification.add({
																			classname: "image-action-preview",
																			...
																			content: "you was removed this comment from deleted list!",
																			timeout: 3000,
																			close: true
																		}, function (notificationElem) {
																			notificationElem.bind('click', function () {
				...
				cardMainElem.children(".options").children(".list").hide().bind('mouseleave', function (e) {
					$(this).fadeOut(250);
				});
				cardMainElem.children(".options").children(".nav").bind('click', function (e) {
					var navOptionsElem = $(this);
					if (navOptionsElem.next(".list").attr('options-popup-showing') == "true") {
						return;
					}else {
						if (navOptionsElem.parents(".row").is(':last-child')) {
							navOptionsElem.dblclick();
							return;
						}
					}
					navOptionsElem.next(".list").stop(true, false).fadeIn(500);
				}, 150);
				cardMainElem.children(".options").children(".nav").bind('dblclick', function (e) {
					var navOptionsElem = $(this);
					navOptionsElem.next(".list").stop(true, false).fadeOut(250).attr('options-popup-showing', 'true');
					_popup.add({
						classname: "stream-options-tab",
						title: "options for comment",
						content: "<div></div>",
						close: true
					}, function (popupElem) {
						popupElem.find(".main.stream-options-tab").html(navOptionsElem.next(".list").children().clone(true));
				...
				if (_is.exists(commentCardFavorite) && _is.exists(commentCardFavorite['allow']) && commentCardFavorite['allow']) {
					...
					_g.image.stream.action.favorite(cardMainElem.find(".footer > .action > .button.favorite > .icon"), 'click', {'inside': {'type': 'comment', 'id': commentCardId}}, true, function (actionSuccess, favoriteValue) {
						if (actionSuccess) {
							_g.image.stream.action.stats({'inside': {'type': 'comment', 'id': commentCardId}}, function (returnObj) {
								if (_is.array(returnObj) && _is.bool(returnObj['return']) && _is.array(returnObj['data']) && returnObj['return']) {
				...
				_selector.cache.set(commentCardElem, 'streamobject', object);
				callback(commentCardElem, commentCardId);
			...
			hide: function (attrObj, callback) {
				var attrObj = _is.array(attrObj) ? attrObj : {},
					callback = _is.function(callback) ? callback : function () {};
				var liSelectedElem = [];
				var liStreamAll = $(".stream-slice[stream-slice-float]").children(".stream-li").find(".box.comment > .tab .row");
				liStreamAll.each(function () {
					var liStreamElem = $(this);
					_selector.cache.get(liStreamElem, 'streamobject', function (returnObj) {
						var returnObj = _is.array(returnObj) && _is.array(returnObj[0]) ? returnObj[0] : {};
...
_g.viewer.image = {
	wrapper: {
		add: function () {
			if (!_global.exists("#g-viewer-image")) {
				_global.add("<div id='g-viewer-image'><div class='content'></div></div>");
			}
		}
	},
	add: function (object, callback) {
		var object = _is.array(object) ? object : {},
			callback = _is.function(callback) ? callback : function () {};
		if (!_is.exists(object)) {
			callback(false);
			return false;
		}
		_g.viewer.image.wrapper.add();
		var viewerImageElem = $("#g-viewer-image");
		var viewerTabKey = _string.random(10),
			viewerNavbar = _is.array(object['navbar']) ? object['navbar'] : {},
			viewerTabData = _is.array(object['data']) ? object['data'] : {},
			viewerOptions = _is.array(object['options']) ? object['options'] : {},
			viewerTabCode = "<div class='li'></div>";
		$("body").addClass('scroll-disable');
		var viewerTabElem = $(viewerTabCode);
		viewerImageElem.children(".content").append(viewerTabElem);
		viewerTabElem.attr('viewer-image-key', viewerTabKey).hide().fadeIn();
		viewerTabElem.append("<div class='navbar'></div>");
		viewerTabElem.append("<div class='main'></div>");
		viewerTabElem.append("<div class='options'></div>");
		var navbarViewerElem = viewerTabElem.children(".navbar"),
			mainViewerElem = viewerTabElem.children(".main"),
			optionsViewerElem = viewerTabElem.children(".options");
		viewerNavbar['close'] = {'float': 'left', 'title': 'close image viewer'};
		navbarLoop: for (var navbarId in viewerNavbar) {
			var windowElem = $(window),
				navbarViewerObj = viewerNavbar[navbarId];
			var buttonNavbarElem = $("<div class='button'><div class='icon'></div></div>");
			buttonNavbarElem.addClass(navbarId);
			if (_is.string(navbarViewerObj['float'])) {
				buttonNavbarElem.addClass(navbarViewerObj['float']);
			}
			if (_is.exists(navbarViewerObj['title'])) {
				_tooltip.set(buttonNavbarElem.children(".icon"), {arrow: (buttonNavbarElem.hasClass('left') ? 'e' : 'w'), action: "hover", content: navbarViewerObj['title']});
			}
			if (_is.array(navbarViewerObj['sidebar'])) {
				if (!_is.string(navbarViewerObj['sidebar']['type']) || !_is.string(navbarViewerObj['sidebar']['content'])) {
					//.
				}else {
					var sidebarButtonType = navbarViewerObj['sidebar']['type'],
						sidebarButtonTitle = navbarViewerObj['sidebar']['title'],
						sidebarButtonContent = navbarViewerObj['sidebar']['content'],
						sidebarButtonCode = "<div></div>",
						sidebarButtonElem = $(sidebarButtonCode);
					sidebarButtonElem.addClass('sidebar').append("<div class='header'><div class='title nowrap'><span class='text'>"+(sidebarButtonTitle)+"</span></div><div class='action'><div class='close'></div></div></div>").append("<div class='body'>"+(sidebarButtonContent)+"</div>");
					var headerSidebarElem = sidebarButtonElem.children(".header"),
						bodySidebarElem = sidebarButtonElem.children(".body"),
						footerSidebarElem = sidebarButtonElem.children(".footer");
					headerSidebarElem.children(".action").children(".close").bind('click', function () {
						mainViewerElem.css({
							'width': (viewerTabElem.width()) + 'px' // 'width': '100%'
						});
						sidebarButtonElem.fadeOut().css({
							'right': (0 - sidebarButtonElem.width()) + 'px'
						});
						buttonNavbarElem.removeAttr('button-sidebar-showing');
					});
					buttonNavbarElem.append(sidebarButtonElem).children(".icon").bind(sidebarButtonType, function () {
						if (buttonNavbarElem.attr('button-sidebar-showing') == "true") {
							mainViewerElem.css({
								'width': (viewerTabElem.width()) + 'px' // 'width': '100%'
							});
							sidebarButtonElem.fadeOut().css({
							...
							mainViewerElem.css({
								'width': (viewerTabElem.width() - sidebarButtonElem.width() - 20) + 'px' // 'width': 'calc(100% - 250px)' | 'width': 'calc(100% - '+sidebarButtonElem.width()+'px)'
							});
							sidebarButtonElem.fadeIn().css({
								'right': '0'
							});
							buttonNavbarElem.attr('button-sidebar-showing', 'true');
						}
					});
					setTimeout(function () {
						sidebarButtonElem.hide().css({
							'right': (0 - sidebarButtonElem.width()) + 'px'
						});
					}, 0);
				}
			...
			if (navbarId == "close") {
				buttonNavbarElem.children(".icon").bind('click', function () {
					_g.viewer.image.close(viewerTabElem);
				});
				$(document).bind('keyup', function (e) {
					if ((e.keyCode ? e.keyCode : e.which) == 27) _g.viewer.image.close(viewerTabElem);
				});
			}
			navbarViewerElem.append(buttonNavbarElem);
		}
		dataLoop: for (var dataId in viewerTabData) {
			var dataViewerObj = viewerTabData[dataId];
			var imageMainElem = $("<div class='image'><img class='img align' src='"+(_is.exists(dataViewerObj['source']) ? dataViewerObj['source'] : null)+"'></div>");
			imageMainElem.append("<div class='zoom'><div class='z-number row'><span class='text'>-</span></div><div class='z-out button row'><div class='icon'></div></div><div class='z-in button row'><div class='icon'></div></div></div>");
			imageMainElem.children(".img").bind('click', function (ev) {
				// console.log(ev);
			});
			imageMainElem.children(".img").bind('dblclick', function () {
				var windowElem = $(window),
					imgViewerElem = $(this);
				var imgViewerScale = Number(_is.exists(imgViewerElem.attr('img-scale')) ? imgViewerElem.attr('img-scale') : 1),
					imgViewerHeight = _is.exists(imgViewerElem.attr('img-dimensions-height')) ? imgViewerElem.attr('img-dimensions-height') : imgViewerElem.height(),
					imgViewerWidth = _is.exists(imgViewerElem.attr('img-dimensions-width')) ? imgViewerElem.attr('img-dimensions-width') : imgViewerElem.width();
				imgViewerElem.toggleClass('enlarge');
				if (imgViewerElem.hasClass('enlarge')) {
					imgViewerElem.fadeOut().draggable('enable').width(imgViewerWidth * imgViewerScale).height(imgViewerHeight * imgViewerScale).css({
						'top': (mainViewerElem.height() - imgViewerElem.height()) / 2,
						'right': 'auto',
						'bottom': 'auto',
						'left': (mainViewerElem.width() - imgViewerElem.width()) / 2
					}).fadeIn();
					imgViewerElem.next(".zoom").fadeIn();
					_cache.get('user_guide_checked', function (dataCached) {
						var dataCached = _is.array(dataCached) ? dataCached : {};
						if (!_is.exists(dataCached['viewer_image_dragimage'])) {
							dataCached['viewer_image_dragimage'] = _parse.datenow.timestamp();
							_cache.set('user_guide_checked', dataCached);
							_notification.add({
								type: "tips",
								title: {
									text: "viewer image - tips"
								},
								content: "drag to move image",
								timeout: 3000
							});
						}
					});
				}else {
					imgViewerElem.fadeOut().draggable('disable').width('').height('').css({
						'top': '',
						'right': '',
						'bottom': '',
						'left': ''
					}).fadeIn();
					imgViewerElem.next(".zoom").fadeOut();
				}
			});
			imageMainElem.children(".img").on('load', function () {
				var imgViewerElem = $(this),
					imgViewerObj = new Image();
				imgViewerObj.src = imageMainElem.children(".img").attr('src');
				imgViewerElem.attr('img-scale', (_is.number(dataViewerObj['scale']) ? dataViewerObj['scale'] : 1));
				imgViewerElem.next(".zoom").children(".z-number").children(".text").text((_is.number(dataViewerObj['scale']) ? dataViewerObj['scale'] : 1));
				imgViewerElem.attr('img-dimensions-width', imgViewerObj.width);
				imgViewerElem.attr('img-dimensions-height', imgViewerObj.height);
			});
			imageMainElem.children(".img").draggable({
				drag: function (ev, ui) {
					var windowElem = $(window),
						imgViewerElem = $(this);
					imgViewerElem.css('position', 'absolute');
					ui.position.top = 1 * (ui.position.top < 0 && Math.abs(ui.position.top) + 50 > imgViewerElem.height() ? -(imgViewerElem.height() - 50) : (ui.position.top > 0 && Math.abs(ui.position.top) + 50 > mainViewerElem.height() ? (mainViewerElem.height() - 50) : ui.position.top));
					ui.position.left = 1 * (ui.position.left < 0 && Math.abs(ui.position.left) + 50 > imgViewerElem.width() ? -(imgViewerElem.width() - 50) : (ui.position.left > 0 && Math.abs(ui.position.left) + 50 > mainViewerElem.width() ? (mainViewerElem.width() - 50) : ui.position.left));
				}
			}).draggable('disable').css({'position': 'absolute'}).bind('mouseover', function () {
				_cache.get('user_guide_checked', function (dataCached) {
					var dataCached = _is.array(dataCached) ? dataCached : {};
					if (!_is.exists(dataCached['viewer_image_clickzoom'])) {
						dataCached['viewer_image_clickzoom'] = _parse.datenow.timestamp();
						_cache.set('user_guide_checked', dataCached);
						_notification.add({
							type: "tips",
							title: {
							...
							content: "double click on image to enlarge",
							timeout: 3000
						});
					}
				});
			});
			imageMainElem.children(".zoom").hide().hide(function () {
				var zoomViewerElem = $(this),
					imgViewerElem = zoomViewerElem.prev(".img");
				var fnZoomImg = function () {
					var imgViewerScale = Number(_is.exists(imgViewerElem.attr('img-scale')) ? imgViewerElem.attr('img-scale') : 1),
						imgViewerHeight = _is.exists(imgViewerElem.attr('img-dimensions-height')) ? imgViewerElem.attr('img-dimensions-height') : imgViewerElem.height(),
						imgViewerWidth = _is.exists(imgViewerElem.attr('img-dimensions-width')) ? imgViewerElem.attr('img-dimensions-width') : imgViewerElem.width();
					if (imgViewerElem.hasClass('enlarge')) {
						imgViewerElem.stop(true, false).fadeOut().width(imgViewerWidth * imgViewerScale).height(imgViewerHeight * imgViewerScale).css({
							'top': (mainViewerElem.height() - imgViewerHeight * imgViewerScale) / 2,
							'right': 'auto',
							'bottom': 'auto',
							'left': (mainViewerElem.width() - imgViewerWidth * imgViewerScale) / 2
						}).fadeIn();
					}
					return true;
				};
				zoomViewerElem.children(".z-number").children(".text").text(_is.exists(imgViewerElem.attr('img-scale')) ? imgViewerElem.attr('img-scale') : 1);
				zoomViewerElem.children(".z-in").bind('click', function () {
					var inZoomElem = $(this);
					var zoomViewerValue = +0.25,
						zoomViewerScale = Number(_is.exists(imgViewerElem.attr('img-scale')) ? imgViewerElem.attr('img-scale') : 1);
					if ((zoomViewerScale + zoomViewerValue) > 5) {
						if (inZoomElem.attr('notify-zin-limited') == "true") return false;
						_notification.add({
							type: "image",
							title: {
								text: "image can't zoom-in agian"
							},
							content: "can't zoom-in this image too large, limited at x5",
							timeout: 3000
						});
						inZoomElem.attr('notify-zin-limited', 'true');
						return false;
					}
					imgViewerElem.attr('img-scale', zoomViewerScale + zoomViewerValue);
					zoomViewerElem.children(".z-number").children(".text").stop(true, false).fadeOut().text(zoomViewerScale + zoomViewerValue).fadeIn();
					fnZoomImg();
					return true;
				});
				zoomViewerElem.children(".z-out").bind('click', function () {
					var inZoomElem = $(this);
					var zoomViewerValue = -0.25,
						zoomViewerScale = Number(_is.exists(imgViewerElem.attr('img-scale')) ? imgViewerElem.attr('img-scale') : 1);
					if ((zoomViewerScale + zoomViewerValue) < 0.25) {
						if (inZoomElem.attr('notify-zout-limited') == "true") return false;
						_notification.add({
							type: "image",
							title: {
								text: "image can't zoom-out agian"
							},
							content: "can't zoom-out this image too small, limited at x0.25",
							timeout: 3000
						});
						inZoomElem.attr('notify-zout-limited', 'true');
						return false;
					}
					imgViewerElem.attr('img-scale', zoomViewerScale + zoomViewerValue);
					zoomViewerElem.children(".z-number").children(".text").stop(true, false).fadeOut().text(zoomViewerScale + zoomViewerValue).fadeIn();
			...
			_selector.cache.set(imageMainElem, 'viewerobject', dataViewerObj);
			mainViewerElem.append(imageMainElem);
			_ajaxify.each();
			callback(viewerTabElem);
			return viewerTabElem;
		}
	},
	close: function (viewerTabElem, callback) {
		var viewerTabElem = _is.selector(viewerTabElem) ? viewerTabElem : {},
			callback = _is.function(callback) ? callback : function () {};
		//.
		if (_selector.count(viewerTabElem) > 0) {
			viewerTabElem.fadeOut(500, function () {
				_selector.cache.get(viewerTabElem, 'bind', function (returnObj) {
					var returnObj = _is.array(returnObj) && _is.array(returnObj[0]) ? returnObj[0] : {};
					for (var bindId in returnObj) {
						if (_is.exists(returnObj[bindId]['type']) && returnObj[bindId]['type'] == "close") {
							if (_is.exists(returnObj[bindId]['func'])) {
								returnObj[bindId]['func'](viewerTabElem);
							}
						}
					}
				});
				viewerTabElem.remove();
				$("body").removeClass('scroll-disable');
				callback({'return': true, 'exists': true, 'removed': true});
			});
		}else {
...
window.onload = function () {
	...
	_source.add({
		'type': 'js',
		'content': '<?php print $_tool->links("source/js/template/jqueryui.js"); ?>',
		'cache': false
	});
	_source.add({
		'type': 'css',
		'content': '<?php print $_tool->links("source/css/template/jqueryui.css"); ?>',
		'cache': false
	});
};