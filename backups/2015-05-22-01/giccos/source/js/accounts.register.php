var actionBoxRegister = function () {
	...
	var dateBirthdayType = function () {
		var registerBox = $("#centerBox > #registerBox"),
			...
			inputDays = linesBirthday.find(".input.days"),
			inputMonths = linesBirthday.find(".input.months"),
			inputYears = linesBirthday.find(".input.years");
		var numberTyping = function () {
			var input = $(this),
				value = input.val();
			if (input.hasClass("days") || input.hasClass("months")) {
				var regexNumber = new RegExp(/([0-9]{1,2})/i);
				var matchs = value.match(regexNumber);
				if (isset(matchs) && matchs !== null) {
					if ((input.hasClass("days") && matchs[0] >= <?php print $_parameter->get('select_time_days_min_limit'); ?> && matchs[0] <= <?php print $_parameter->get('select_time_days_max_limit'); ?>) || (input.hasClass("months") && matchs[0] >= <?php print $_parameter->get('select_time_months_min_limit'); ?> && matchs[0] <= <?php print $_parameter->get('select_time_months_max_limit'); ?>)) {
						input.val(matchs[0]);
					}else {
						input.val('');
					}
			...
			}else if (input.hasClass("years")) {
				var regexNumber = new RegExp(/([0-9]{1,4})/i);
				var matchs = value.match(regexNumber);
				if (isset(matchs) && matchs !== null) {
					if (matchs[0].length == 4 && matchs[0] > <?php print $_parameter->get('select_time_years_min_limit'); ?> && matchs[0] < <?php print $_parameter->get('select_time_years_max_limit'); ?>) {
						input.val(matchs[0]);
		...
		inputDays.donetyping(numberTyping, 250);
		inputMonths.donetyping(numberTyping, 250);
		inputYears.donetyping(numberTyping, 250);
	};
	dateBirthdayType();
};
var registerAccounts = function () {
	...
	var actionRegisterAccounts = function () {
		var registerBox = $("#centerBox > #registerBox"),
			...
			inputBirthdayDays = registerBox.find(".input.birthday.days"),
			inputBirthdayMonths = registerBox.find(".input.birthday.months"),
			inputBirthdayYears = registerBox.find(".input.birthday.years"),
		...
		function daysRegex (string) {
			var pattern = new RegExp(<?php print $_parameter->get('regex_date_days'); ?>);
			return pattern.test(string);
		};
		function monthsRegex (string) {
			var pattern = new RegExp(<?php print $_parameter->get('regex_date_months'); ?>);
			return pattern.test(string);
		};
		function yearsRegex (string) {
			var pattern = new RegExp(<?php print $_parameter->get('regex_date_years'); ?>);
			return pattern.test(string);
		};
		...
		if (!usernameRegex(inputUsername.val())) {
			...
			popupNotification ({type: 2, title: '<?php print $_language->text('error', 'ucfirst'); ?>', description: "<?php print $_language->text('name_is_invalid', 'ucfirst'); ?>, <?php print $_language->text('please_recheck', 'strtolower'); ?>.", timeout: '<?php print $_parameter->get('notifyPopup_timeout'); ?>'});
		}else if (!daysRegex(inputBirthdayDays.val())) {
			...
			var accountProfile = {};
				...
				accountProfile['birthday']['days'] = inputBirthdayDays.val();
				accountProfile['birthday']['months'] = inputBirthdayMonths.val();
				accountProfile['birthday']['years'] = inputBirthdayYears.val();
