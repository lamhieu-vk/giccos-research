...
Class feed {
	...
	function status_block ($object) {
		...
		if ($action == "add") {
			$thingsType = isset($object['things'], $object['things']['type']) && is_string($object['things']['type']) ? $_tool->convertDatabaseString($object['things']['type']) : null;
			...
			}else if (in_array($thingsType, ["status", "user", "page", "group"])) {
	...
	function status_follow ($object) {
		...
		if ($action == "add") {
			...
			$countFollowRequest = "SELECT `id` FROM `status_follow` WHERE `inside.type` = '".$insideType."' AND `inside.id` = '".$insideId."' AND `guy.type` = '".$guyType."' AND `guy.id` = '".$guyId."' LIMIT 1;";
