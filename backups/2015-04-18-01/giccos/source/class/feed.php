		...
		if (isset($object) && is_array($object)) {
			...
			if ($type == 1 || $type == "status") {
						...
						if ($object['media']['scrapbook']['value'] == true) {
							if (isset($object['media']['scrapbook']['data']['name']['token']) && $object['media']['scrapbook']['data']['name']['token'] != null) {
								...
								if (mysqli_num_rows($scrapbook_query) > 0) {
									...
									mysqli_query($db, "INSERT INTO `photos_scrapbook` (`id`, `author.type`, `author.id`, `token`, `display`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$scrapbook_token}', '{$scrapbook_display}', '{$time}', '{$scrapbook_name}', '{$scrapbook_places['type']}', '{$scrapbook_places['value']}', '{$scrapbook_description}')");
					...
					if (isset($object['media']['info']['data']['album'])) {
						...
						if (is_array($object_album) && isset($object_album['token']) && $object_album['token'] != null) {
							...
							if (mysqli_num_rows($album_query) > 0) {
								...
								mysqli_query($db, "INSERT INTO `music_album` (`id`, `author.type`, `author.id`, `token`, `display`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$album_token}', '{$album_display}', '{$time}', '{$album_name}', '{$album_places['type']}', '{$album_places['id']}', '{$album_description}')");
					...
					if (isset($object['media']['info']['data']['playlist'])) {
						...
						if (is_array($object_playlist) && isset($object_playlist['token']) && $object_playlist['token'] != null) {
							...
							if (mysqli_num_rows($playlist_query) > 0) {
								...
								mysqli_query($db,"INSERT INTO `videos_playlist` (`id`, `author.type`, `author.id`, `token`, `display`, `time`, `name`, `places.type`, `places.value`, `description`) VALUES (null, '{$author['type']}', '{$author['id']}', '{$playlist_token}', '{$playlist_display}', '{$time}', '{$playlist_name}', '{$playlist_places['type']}', '{$playlist_places['id']}', '{$playlist_description}')");
				...
				if (isset($object['media']['info']['data']) && is_array($object['media']['info']['data'])) {
					...
					mysqli_query($db, "INSERT INTO `cited` (`id`, `token`, `display`, `author.type`, `author.id`, `time`, `author`, `quote`, `photos.id`) VALUES (null, '{$cited_token}', '{$cited_display}', '{$author['type']}', '{$author['id']}', '{$time}', '{$media_data['author']}', '{$media_data['quote']}', '{$media_data['photos.id']}')");
	...
	function status_get ($object) {
		...
		if (mysqli_num_rows($query) == 0) {
			...
			while ($status = mysqli_fetch_assoc($query)) {
				...
				$info['link'] = $this->class['_tool']->links('feed/status/').$info['display'];
				...
				if ($getMedia == true && $info['type'] == "status" && $status['link'] == 1) {
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								...
								$content['media']['data'][] = array(
									"display" => $media_fetch['data']['display'],
									...
									"share" => array(
										"link" => $this->class['_tool']->links('link/embed/').$media_fetch['data']['display']
									)
								);
							}
						}
					...
					$media_sql = "SELECT * FROM `status` WHERE `id` = (SELECT DISTINCT `status.id` FROM `status_share` WHERE `this.id` = '{$status['id']}' LIMIT 1) LIMIT 1";
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								...
								$content['media']['data'][] = array(
									...
									"link" => $media_link,
									"share" => array(
										"link" => $this->class['_tool']->links('photos/embed/').$media_fetch['data']['display']
									)
								);
							}
						}
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								...
								$content['media']['data'][] = array(
									...
									"share" => array(
										"link" => $this->class['_tool']->links('music/embed/').$media_fetch['data']['display']
									)
								);
							}else {
								//.
						...
						while ($media_data = mysqli_fetch_assoc($media_query)) {
							...
							if (isset($media_fetch['return']) && $media_fetch['return'] == true) {
								...
								$content['media']['data'][] = array(
									...
									"link" => $media_fetch['data']['link'],
									"share" => array(
										"link" => $this->class['_tool']->links('videos/embed/').$media_fetch['data']['display']
									)
								);
							}
						}
					...
					if (mysqli_num_rows($media_query) == 0) {
						...
						$content['media']['data'][] = array(
							...
							"share" => array(
								"link" => $this->class['_tool']->links('cited/embed/').$media_fetch['data']['display']
							)
						);
					}
				}else {
